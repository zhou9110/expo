...
// Prior to ABI50_0_0React Native 0.61, it contained `ABI50_0_0RCTAssetsLibraryRequestHandler` that handles loading data from URLs
...
// Because it's still a unimodule and we need to export bare ABI50_0_0React Native module, we should make sure ABI50_0_0React Native is installed.
#if __has_include(<ABI50_0_0React/ABI50_0_0RCTImageURLLoader.h>)
...
@implementation ABI50_0_0EXMediaLibraryImageLoader

ABI50_0_0RCT_EXPORT_MODULE()

#pragma mark - ABI50_0_0RCTImageURLLoader
...
- (ABI50_0_0RCTImageLoaderCancellationBlock)loadImageForURL:(NSURL *)imageURL
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                 ...
                                 completionHandler:(ABI50_0_0RCTImageLoaderCompletionBlock)completionHandler
{
  ...
  if (!imageURL) {
    completionHandler(ABI50_0_0RCTErrorWithMessage(@"Cannot load a photo library asset with no URL"), nil);
    return ^{};
  ...
  if (results.count == 0) {
    ...
    completionHandler(ABI50_0_0RCTErrorWithMessage(errorText), nil);
    return ^{};
  }
  ...
  if (resizeMode == ABI50_0_0RCTResizeModeContain) {
