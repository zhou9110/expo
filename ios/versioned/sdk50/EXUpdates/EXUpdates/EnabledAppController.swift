...
public class EnabledAppController: UpdatesStateChangeDelegate, InternalAppControllerInterface, StartupProcedureDelegate {
  private static let ErrorDomain = "ABI50_0_0EXUpdatesAppController"
  private static let ABI50_0_0EXUpdatesEventName = "Expo.nativeUpdatesEvent"
  private static let ABI50_0_0EXUpdatesStateChangeEventName = "Expo.nativeUpdatesStateChangeEvent"
  ...
  internal func sendLegacyUpdateEventToBridge(_ eventType: String, body: [String: Any]) {
    ...
    sendEventToBridge(EnabledAppController.ABI50_0_0EXUpdatesEventName, eventType, body: body)
  }
  ...
  internal func sendUpdateStateChangeEventToBridge(_ eventType: UpdatesStateEventType, body: [String: Any?]) {
    ...
    sendEventToBridge(EnabledAppController.ABI50_0_0EXUpdatesStateChangeEventName, "\(eventType)", body: body)
  }
  ...
  private func sendEventToBridge(_ eventName: String, _ eventType: String, body: [String: Any?]) {
    ...
    guard let bridge = bridge else {
      ...
      logger.warn(message: "ABI50_0_0EXUpdates: Could not emit event: name = \(eventName), type = \(eventType). Event will be emitted when the bridge is available", code: .jsRuntimeError)
      return
    }
    logger.debug(message: "sendEventToBridge: \(eventName), \(mutableBody)")
    bridge.enqueueJSCall("ABI50_0_0RCTDeviceEventEmitter.emit", args: [eventName, mutableBody])
  }
