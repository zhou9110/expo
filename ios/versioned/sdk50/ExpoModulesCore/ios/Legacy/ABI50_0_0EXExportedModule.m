...
#define ABI50_0_0EXPAND_AND_QUOTE(str) QUOTE(str)

#define ABI50_0_0EX_IS_METHOD_EXPORTED(methodName) \
[methodName hasPrefix:@ABI50_0_0EXPAND_AND_QUOTE(ABI50_0_0EX_EXPORTED_METHODS_PREFIX)]

static const NSString *noNameExceptionName = @"No custom +(const NSString *)exportedModuleName implementation.";
static const NSString *noNameExceptionReasonFormat = @"You've subclassed an ABI50_0_0EXExportedModule in %@, but didn't override the +(const NSString *)exportedModuleName method. Override this method and return a name for your exported module.";
...
@interface ABI50_0_0EXExportedModule ()
...
@implementation ABI50_0_0EXExportedModule
  ...
  while (klass) {
    ...
    @try {
      for(int i = 0; i < methodsCount; i++) {
        ...
        if (ABI50_0_0EX_IS_METHOD_EXPORTED(methodName)) {
          IMP imp = method_getImplementation(method);
          const ABI50_0_0EXMethodInfo *info = ((const ABI50_0_0EXMethodInfo *(*)(id, SEL))imp)(klass, methodSelector);
...
- (void)callExportedMethod:(NSString *)methodName withArguments:(NSArray *)arguments resolver:(ABI50_0_0EXPromiseResolveBlock)resolve rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
  ...
  if (methodSignature == nil) {
    ...
    // it means that it has been exported with ABI50_0_0EX_IMPORT_METHOD and if we cannot find method signature
