...
#import "ABI50_0_0RCTInterpolationAnimatedNode.h"
...
typedef NS_ENUM(NSInteger, ABI50_0_0RCTInterpolationOutputType) {
  ABI50_0_0RCTInterpolationOutputNumber,
  ABI50_0_0RCTInterpolationOutputColor,
  ABI50_0_0RCTInterpolationOutputString,
};
...
NSString *ABI50_0_0RCTInterpolateString(
    NSString *pattern,
    CGFloat inputValue,
...
{
  NSUInteger rangeIndex = ABI50_0_0RCTFindIndexOfNearestValue(inputValue, inputRange);
  ...
  for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
    CGFloat val = ABI50_0_0RCTInterpolateValue(
        inputValue,
...
@implementation ABI50_0_0RCTInterpolationAnimatedNode {
  __weak ABI50_0_0RCTValueAnimatedNode *_parentNode;
  NSArray<NSNumber *> *_inputRange;
  NSArray *_outputRange;
  NSString *_extrapolateLeft;
  NSString *_extrapolateRight;
  ABI50_0_0RCTInterpolationOutputType _outputType;
...
- (instancetype)initWithTag:(NSNumber *)tag config:(NSDictionary<NSString *, id> *)config
{
  if ((self = [super initWithTag:tag config:config])) {
    ...
    if ([config[@"outputType"] isEqual:@"color"]) {
      _outputType = ABI50_0_0RCTInterpolationOutputColor;
    } else if ([outputRangeConfig[0] isKindOfClass:[NSString class]]) {
      _outputType = ABI50_0_0RCTInterpolationOutputString;
      _outputPattern = outputRangeConfig[0];
    } else {
      _outputType = ABI50_0_0RCTInterpolationOutputNumber;
    }

    NSMutableArray *outputRange = [NSMutableArray arrayWithCapacity:outputRangeConfig.count];
    for (id value in outputRangeConfig) {
      switch (_outputType) {
        case ABI50_0_0RCTInterpolationOutputColor: {
          UIColor *color = [ABI50_0_0RCTConvert UIColor:value];
          [outputRange addObject:color ? color : [UIColor whiteColor]];
          break;
        }
        case ABI50_0_0RCTInterpolationOutputString:
          [outputRange addObject:outputFromStringPattern(value)];
          break;
        case ABI50_0_0RCTInterpolationOutputNumber:
...
- (void)onAttachedToNode:(ABI50_0_0RCTAnimatedNode *)parent
{
  [super onAttachedToNode:parent];
  if ([parent isKindOfClass:[ABI50_0_0RCTValueAnimatedNode class]]) {
    _parentNode = (ABI50_0_0RCTValueAnimatedNode *)parent;
  }
}

- (void)onDetachedFromNode:(ABI50_0_0RCTAnimatedNode *)parent
{
...
- (void)performUpdate
{
  ...
  switch (_outputType) {
    case ABI50_0_0RCTInterpolationOutputColor:
      _outputvalue = @(ABI50_0_0RCTInterpolateColorInRange(inputValue, _inputRange, _outputRange));
      break;
    case ABI50_0_0RCTInterpolationOutputString:
      _outputvalue = ABI50_0_0RCTInterpolateString(
          _outputPattern, inputValue, _inputRange, _outputRange, _extrapolateLeft, _extrapolateRight);
      break;
    case ABI50_0_0RCTInterpolationOutputNumber:
      self.value =
          ABI50_0_0RCTInterpolateValueInRange(inputValue, _inputRange, _outputRange, _extrapolateLeft, _extrapolateRight);
      break;
  }
}
