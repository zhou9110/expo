...
@interface ABI50_0_0RNCPicker() <UIPickerViewDataSource, UIPickerViewDelegate>
@end

@implementation ABI50_0_0RNCPicker
...
{
  return [ABI50_0_0RCTConvert NSString:_items[row][@"label"]];
}
           ...
           reusingView:(UIView *)view
{
  if (!view) {
      ...
      ABI50_0_0RNCPickerLabel* label = [[ABI50_0_0RNCPickerLabel alloc] initWithFrame:(CGRect) {
          CGPointZero,
          {
              rowWidth,
              rowHeight,
  ...
  if (@available(iOS 14.0, *)) {
      if (_selectionColor) {
          pickerView.subviews[1].backgroundColor = [ABI50_0_0RCTConvert UIColor:@(_selectionColor)];
      }
  }
    
  ABI50_0_0RNCPickerLabel* label = view.subviews[0];
  label.font = _font;

  label.textColor = [ABI50_0_0RCTConvert UIColor:_items[row][@"textColor"]] ?: _color;
      ...
      didSelectRow:(NSInteger)row inComponent:(__unused NSInteger)component
{
  ...
  if (_onChange && _items.count > (NSUInteger)row) {
    _onChange(@{
      @"newIndex": @(row),
      @"newValue": ABI50_0_0RCTNullIfNil(_items[row][@"value"]),
    });
  }
}
