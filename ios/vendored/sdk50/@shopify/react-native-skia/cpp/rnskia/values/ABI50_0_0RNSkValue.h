...
#include "ABI50_0_0RNSkAnimation.h"
 ...
 */
class ABI50_0_0RNSkValue : public ABI50_0_0RNSkReadonlyValue {
public:
  ABI50_0_0RNSkValue(std::shared_ptr<ABI50_0_0RNSkPlatformContext> platformContext,
            jsi::Runtime &runtime, const jsi::Value *arguments, size_t count)
      : ABI50_0_0RNSkReadonlyValue(platformContext) {
    if (count == 1) {
      update(runtime, arguments[0]);
    }
  }

  ~ABI50_0_0RNSkValue() { unsubscribe(); }
  ...
  JSI_PROPERTY_SET(animation) {
    ...
    // Verify input
    if (value.isObject() &&
        value.asObject(runtime).isHostObject<ABI50_0_0RNSkAnimation>(runtime)) {
      auto animation =
          value.asObject(runtime).getHostObject<ABI50_0_0RNSkAnimation>(runtime);
  ...
  JSI_EXPORT_PROPERTY_SETTERS(JSI_EXPORT_PROP_SET(ABI50_0_0RNSkValue, current),
                              JSI_EXPORT_PROP_SET(ABI50_0_0RNSkValue, animation))

  JSI_EXPORT_PROPERTY_GETTERS(JSI_EXPORT_PROP_GET(ABI50_0_0RNSkReadonlyValue,
                                                  __typename__),
                              JSI_EXPORT_PROP_GET(ABI50_0_0RNSkValue, current),
                              JSI_EXPORT_PROP_GET(ABI50_0_0RNSkValue, animation))

  JSI_EXPORT_FUNCTIONS(JSI_EXPORT_FUNC(ABI50_0_0RNSkValue, addListener),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkReadonlyValue, dispose))

private:
  void subscribe(std::shared_ptr<ABI50_0_0RNSkAnimation> animation) {
    if (animation != nullptr) {
      ...
      _unsubscribe =
          std::make_shared<std::function<void()>>(_animation->addListener(
              [weakSelf = weak_from_this()](jsi::Runtime &runtime) {
                ...
                if (self) {
                  auto selfAsThis = std::dynamic_pointer_cast<ABI50_0_0RNSkValue>(self);
  ...
  std::shared_ptr<ABI50_0_0RNSkAnimation> _animation;
