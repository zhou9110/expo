...
class ABI50_0_0RNSkTimingInfo {
public:
  explicit ABI50_0_0RNSkTimingInfo(const std::string &name) : _name(std::move(name)) {
    reset();
  }

  ~ABI50_0_0RNSkTimingInfo() {}
  ...
  void stopTiming() {
    ...
    if (_didSkip) {
      _didSkip = false;
      ABI50_0_0RNSkLogger::logToConsole("%s: Skipped frame. Previous frame time: %lldms",
