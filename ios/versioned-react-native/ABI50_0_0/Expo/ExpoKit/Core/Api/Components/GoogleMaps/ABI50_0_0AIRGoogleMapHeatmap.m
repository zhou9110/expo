//
//  ABI50_0_0AIRGoogleMapHeatmap.m
//
...
@implementation ABI50_0_0AIRGoogleMapHeatmap
...
- (void)setPoints:(NSArray<NSDictionary *> *)points
{
    ...
    for (int i = 0; i < points.count; i++) {
        CLLocationCoordinate2D coord = [ABI50_0_0RCTConvert CLLocationCoordinate2D:points[i]];
        float intensity = 1.0;
        if (points[i][@"weight"] != nil) {
            intensity = [ABI50_0_0RCTConvert float:points[i][@"weight"]];
        }
...
{
    NSArray<UIColor *> *colors = [ABI50_0_0RCTConvert UIColorArray:gradient[@"colors"]];
    NSArray<NSNumber *> *colorStartPoints = [ABI50_0_0RCTConvert NSNumberArray:gradient[@"startPoints"]];
    NSUInteger colorMapSize = [ABI50_0_0RCTConvert NSUInteger:gradient[@"colorMapSize"]];
