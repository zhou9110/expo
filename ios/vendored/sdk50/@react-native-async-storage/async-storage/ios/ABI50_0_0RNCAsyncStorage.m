...
#import "ABI50_0_0RNCAsyncStorage.h"
...
//static NSString *const ABI50_0_0RCTStorageDirectory = @"ABI50_0_0RCTAsyncLocalStorage_V1";
static NSString *const ABI50_0_0RCTStorageDirectory = @"ABI50_0_0RCTAsyncLocalStorage";
static NSString *const ABI50_0_0RCTOldStorageDirectory = @"ABI50_0_0RNCAsyncLocalStorage_V1";
static NSString *const ABI50_0_0RCTExpoStorageDirectory = @"ABI50_0_0RCTAsyncLocalStorage";
static NSString *const ABI50_0_0RCTManifestFileName = @"manifest.json";
static const NSUInteger ABI50_0_0RCTInlineValueThreshold = 1024;

#pragma mark - Static helper functions

static NSDictionary *ABI50_0_0RCTErrorForKey(NSString *key)
{
    if (![key isKindOfClass:[NSString class]]) {
        return ABI50_0_0RCTMakeAndLogError(@"Invalid key - must be a string.  Key: ", key, @{@"key": key});
    } else if (key.length < 1) {
        return ABI50_0_0RCTMakeAndLogError(
...
static BOOL ABI50_0_0RCTAsyncStorageSetExcludedFromBackup(NSString *path, NSNumber *isExcluded)
{
...
static void ABI50_0_0RCTAppendError(NSDictionary *error, NSMutableArray<NSDictionary *> **errors)
{
    if (error && errors) {
        if (!*errors) {
...
static NSArray<NSDictionary *> *ABI50_0_0RCTMakeErrors(NSArray<id<NSObject>> *results)
{
    NSMutableArray<NSDictionary *> *errors;
    for (id object in results) {
        if ([object isKindOfClass:[NSError class]]) {
            NSError *error = (NSError *)object;
            NSDictionary *keyError = ABI50_0_0RCTMakeError(error.localizedDescription, error, nil);
            ABI50_0_0RCTAppendError(keyError, &errors);
        }
    }
    return errors;
}

static NSString *ABI50_0_0RCTReadFile(NSString *filePath, NSString *key, NSDictionary **errorOut)
{
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        ...
        NSString *entryString = [NSString stringWithContentsOfFile:filePath
                                                      usedEncoding:&encoding
                                                             error:&error];
        NSDictionary *extraData = @{@"key": ABI50_0_0RCTNullIfNil(key)};

        if (error) {
            if (errorOut) {
                *errorOut = ABI50_0_0RCTMakeError(@"Failed to read storage file.", error, extraData);
            }
            return nil;
        }
        ...
        if (encoding != NSUTF8StringEncoding) {
            if (errorOut) {
                *errorOut =
                    ABI50_0_0RCTMakeError(@"Incorrect encoding of storage file: ", @(encoding), extraData);
            }
            return nil;
        }
        return entryString;
...
// Please use `ABI50_0_0RCTCreateStorageDirectoryPath` instead
static NSString *ABI50_0_0RCTCreateStorageDirectoryPath_deprecated(NSString *storageDir)
{
...
static NSString *ABI50_0_0RCTCreateStorageDirectoryPath(NSString *storageDir)
{
    NSString *storageDirectoryPath = @"";
...
static NSString *ABI50_0_0RCTCreateManifestFilePath(NSString *storageDirectory)
{
    return [storageDirectory stringByAppendingPathComponent:ABI50_0_0RCTManifestFileName];
}

// Only merges objects - all other types are just clobbered (including arrays)
static BOOL ABI50_0_0RCTMergeRecursive(NSMutableDictionary *destination, NSDictionary *source)
{
    BOOL modified = NO;
    for (NSString *key in source) {
        ...
        if ([sourceValue isKindOfClass:[NSDictionary class]]) {
            if ([destinationValue isKindOfClass:[NSDictionary class]]) {
                ...
                if (ABI50_0_0RCTMergeRecursive(destinationValue, sourceValue)) {
...
static BOOL ABI50_0_0RCTHasCreatedStorageDirectory = NO;

// NOTE(nikki93): We replace with scoped implementations of:
//   ABI50_0_0RCTGetStorageDirectory()
//   ABI50_0_0RCTGetManifestFilePath()
//   ABI50_0_0RCTGetMethodQueue()
//   ABI50_0_0RCTGetCache()
//   ABI50_0_0RCTDeleteStorageDirectory()

#define ABI50_0_0RCTGetStorageDirectory() _storageDirectory
#define ABI50_0_0RCTGetManifestFilePath() _manifestFilePath
#define ABI50_0_0RCTGetMethodQueue() self.methodQueue
#define ABI50_0_0RCTGetCache() self.cache

static NSDictionary *ABI50_0_0RCTDeleteStorageDirectory(NSString *storageDirectory)
{
  NSError *error;
  [[NSFileManager defaultManager] removeItemAtPath:storageDirectory error:&error];
  return error ? ABI50_0_0RCTMakeError(@"Failed to delete storage directory.", error, nil) : nil;
}
#define ABI50_0_0RCTDeleteStorageDirectory() ABI50_0_0RCTDeleteStorageDirectory(_storageDirectory)

static NSDate *ABI50_0_0RCTManifestModificationDate(NSString *manifestFilePath)
{
 ...
 */
static void ABI50_0_0RCTStorageDirectoryMigrationLogError(NSString *reason, NSError *error)
{
    ABI50_0_0RCTLogWarn(@"%@: %@", reason, error ? error.description : @"");
}

static void ABI50_0_0RCTStorageDirectoryCleanupOld(NSString *oldDirectoryPath)
{
    NSError *error;
    if (![[NSFileManager defaultManager] removeItemAtPath:oldDirectoryPath error:&error]) {
        ABI50_0_0RCTStorageDirectoryMigrationLogError(
...
static void ABI50_0_0RCTStorageDirectoryMigrate(NSString *oldDirectoryPath,
...
/**
 * Determine which of ABI50_0_0RCTOldStorageDirectory or ABI50_0_0RCTExpoStorageDirectory needs to migrated.
 ...
 */
static NSString *ABI50_0_0RCTGetStoragePathForMigration()
{
    BOOL isDir;
    NSString *oldStoragePath = ABI50_0_0RCTCreateStorageDirectoryPath_deprecated(ABI50_0_0RCTOldStorageDirectory);
    NSString *expoStoragePath = ABI50_0_0RCTCreateStorageDirectoryPath_deprecated(ABI50_0_0RCTExpoStorageDirectory);
    ...
    if (oldStorageDirectoryExists && expoStorageDirectoryExists) {
        ...
        if ([ABI50_0_0RCTManifestModificationDate(ABI50_0_0RCTCreateManifestFilePath(oldStoragePath))
                compare:ABI50_0_0RCTManifestModificationDate(ABI50_0_0RCTCreateManifestFilePath(expoStoragePath))] ==
            NSOrderedDescending) {
            ABI50_0_0RCTStorageDirectoryCleanupOld(expoStoragePath);
            return oldStoragePath;
        } else {
            ABI50_0_0RCTStorageDirectoryCleanupOld(oldStoragePath);
 ...
 */
static void
ABI50_0_0RCTStorageDirectoryMigrationCheck(NSString *fromStorageDirectory,
                                  ...
                                  BOOL shouldCleanupOldDirectoryAndOverwriteNewDirectory)
{
    ...
    if ([fileManager fileExistsAtPath:fromStorageDirectory isDirectory:&isDir] && isDir) {
        ...
        if ([fileManager fileExistsAtPath:toStorageDirectory]) {
            ...
            if ([ABI50_0_0RCTManifestModificationDate(ABI50_0_0RCTCreateManifestFilePath(toStorageDirectory))
                    compare:ABI50_0_0RCTManifestModificationDate(
                                ABI50_0_0RCTCreateManifestFilePath(fromStorageDirectory))] == 1) {
                ...
                if (shouldCleanupOldDirectoryAndOverwriteNewDirectory) {
                    ABI50_0_0RCTStorageDirectoryCleanupOld(fromStorageDirectory);
                }
                ...
                if (![fileManager removeItemAtPath:toStorageDirectory error:&error]) {
                    ...
                    ABI50_0_0RCTStorageDirectoryMigrate(fromStorageDirectory,
                                               toStorageDirectory,
...
#pragma mark - ABI50_0_0RNCAsyncStorage

@interface ABI50_0_0RNCAsyncStorage ()
...
@implementation ABI50_0_0RNCAsyncStorage {
    BOOL _haveSetup;
...
+ (NSString *)moduleName { return @"ABI50_0_0RCTAsyncLocalStorage"; }
- (instancetype)initWithStorageDirectory:(NSString *)storageDirectory
{
  if ((self = [super init])) {
    _storageDirectory = storageDirectory;
    _manifestFilePath = [ABI50_0_0RCTGetStorageDirectory() stringByAppendingPathComponent:ABI50_0_0RCTManifestFileName];
  }
  return self;
}
...
- (instancetype)init
{
  assert(false);
    ...
    NSString *oldStoragePath = ABI50_0_0RCTGetStoragePathForMigration();
    if (oldStoragePath != nil) {
        // Migrate our deprecated path "Documents/.../ABI50_0_0RNCAsyncLocalStorage_V1" or
        // "Documents/.../ABI50_0_0RCTAsyncLocalStorage" to "Documents/.../ABI50_0_0RCTAsyncLocalStorage_V1"
        ABI50_0_0RCTStorageDirectoryMigrationCheck(
            oldStoragePath, ABI50_0_0RCTCreateStorageDirectoryPath_deprecated(ABI50_0_0RCTStorageDirectory), YES);
    }

    // Migrate what's in "Documents/.../ABI50_0_0RCTAsyncLocalStorage_V1" to
    // "Application Support/[bundleID]/ABI50_0_0RCTAsyncLocalStorage_V1"
    ABI50_0_0RCTStorageDirectoryMigrationCheck(ABI50_0_0RCTCreateStorageDirectoryPath_deprecated(ABI50_0_0RCTStorageDirectory),
                                      ABI50_0_0RCTCreateStorageDirectoryPath(ABI50_0_0RCTStorageDirectory),
                                      NO);
...
- (void)clearAllData
{
    dispatch_async(ABI50_0_0RCTGetMethodQueue(), ^{
      [self->_manifest removeAllObjects];
      [ABI50_0_0RCTGetCache() removeAllObjects];
      ABI50_0_0RCTDeleteStorageDirectory();
    });
}
...
{
    NSString *safeFileName = ABI50_0_0RCTMD5Hash(key);
    return [ABI50_0_0RCTGetStorageDirectory() stringByAppendingPathComponent:safeFileName];
}

- (NSDictionary *)_ensureSetup
{
    ABI50_0_0RCTAssertThread(ABI50_0_0RCTGetMethodQueue(), @"Must be executed on storage thread");
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:ABI50_0_0RCTGetStorageDirectory()
                              withIntermediateDirectories:YES
                                               attributes:nil
                                                    error:&error];
    if (error) {
      return ABI50_0_0RCTMakeError(@"Failed to create storage directory.", error, nil);
    }

    if (!_haveSetup) {
        // iCloud backup exclusion
        NSNumber *isExcludedFromBackup =
            [[NSBundle mainBundle] objectForInfoDictionaryKey:@"ABI50_0_0RCTAsyncStorageExcludeFromBackup"];
        ...
        // ABI50_0_0RCTAsyncStorageSetExcludedFromBackup(ABI50_0_0RCTCreateStorageDirectoryPath(ABI50_0_0RCTStorageDirectory),
        //                                      isExcludedFromBackup);

        NSDictionary *errorOut = nil;
        // NOTE(kudo): Keep data in Documents rather than Application Support for backward compatibility
        // NSString *serialized = ABI50_0_0RCTReadFile(ABI50_0_0RCTCreateStorageDirectoryPath(ABI50_0_0RCTGetManifestFilePath())
        NSString *serialized = ABI50_0_0RCTReadFile(ABI50_0_0RCTGetManifestFilePath(),
                                           ABI50_0_0RCTManifestFileName,
                                           &errorOut);
        if (!serialized) {
            if (errorOut) {
                ...
                ABI50_0_0RCTLogTrace(@"Manifest does not exist - creating a new one.\n\n%@", errorOut);
                _manifest = [NSMutableDictionary new];
            }
        } else {
            _manifest = ABI50_0_0RCTJSONParseMutable(serialized, &error);
            if (!_manifest) {
                ABI50_0_0RCTLogError(@"Failed to parse manifest - creating a new one.\n\n%@", error);
                _manifest = [NSMutableDictionary new];
            }
        }
        _haveSetup = YES;
...
{
    NSError *error;
    NSString *serialized = ABI50_0_0RCTJSONStringify(_manifest, &error);
    // NOTE(kudo): Keep data in Documents rather than Application Support for backward compatibility
    // [serialized writeToFile:ABI50_0_0RCTCreateStorageDirectoryPath(ABI50_0_0RCTGetManifestFilePath())
    [serialized writeToFile:ABI50_0_0RCTGetManifestFilePath()
                 atomically:YES
                   encoding:NSUTF8StringEncoding
                      error:&error];
    NSDictionary *errorOut;
    if (error) {
        errorOut = ABI50_0_0RCTMakeError(@"Failed to write manifest file.", error, nil);
        ABI50_0_0RCTAppendError(errorOut, errors);
    }
    return errorOut;
}
...
{
    NSDictionary *errorOut = ABI50_0_0RCTErrorForKey(key);
    if (errorOut) {
        return errorOut;
    }
    NSString *value = [self _getValueForKey:key errorOut:&errorOut];
    [result addObject:@[key, ABI50_0_0RCTNullIfNil(value)]];  // Insert null if missing or failure.
    return errorOut;
}
...
- (NSString *)_getValueForKey:(NSString *)key errorOut:(NSDictionary **)errorOut
{
    ...
    if (value == (id)kCFNull) {
        value = [ABI50_0_0RCTGetCache() objectForKey:key];
        if (!value) {
            NSString *filePath = [self _filePathForKey:key];
            value = ABI50_0_0RCTReadFile(filePath, key, errorOut);
            if (value) {
                [ABI50_0_0RCTGetCache() setObject:value forKey:key cost:value.length];
            } else {
...
- (NSDictionary *)_writeEntry:(NSArray<NSString *> *)entry changedManifest:(BOOL *)changedManifest
{
    ...
    [ABI50_0_0RCTGetCache() setObject:value forKey:key cost:value.length];
    if (error) {
        errorOut = ABI50_0_0RCTMakeError(@"Failed to write value.", error, @{@"key": key});
           ...
           getter:(NSString * (^)(NSUInteger i, NSString *key, NSDictionary **errorOut))getValue
{
    ...
    for (NSUInteger i = 0; i < keys.count; ++i) {
        ...
        [result addObject:@[key, ABI50_0_0RCTNullIfNil(value)]];
        ABI50_0_0RCTAppendError(keyError, &errors);
    }
    callback(@[ABI50_0_0RCTNullIfNil(errors), result]);
}
...
// clang-format off
ABI50_0_0RCT_EXPORT_METHOD(multiGet:(NSArray<NSString *> *)keys
                  callback:(ABI50_0_0RCTResponseSenderBlock)callback)
// clang-format on
{
                        ...
                        callback:callback
                          getter:^NSString *(NSUInteger i, NSString *key, NSDictionary **errorOut) {
                            ...
                            if ([valueOrError isKindOfClass:[NSError class]]) {
                                ...
                                NSDictionary *extraData = @{@"key": ABI50_0_0RCTNullIfNil(key)};
                                *errorOut =
                                    ABI50_0_0RCTMakeError(error.localizedDescription, error, extraData);
                                return nil;
                            } else {
...
// clang-format off
ABI50_0_0RCT_EXPORT_METHOD(multiSet:(NSArray<NSArray<NSString *> *> *)kvPairs
                  callback:(ABI50_0_0RCTResponseSenderBlock)callback)
// clang-format on
{
    if (self.delegate != nil) {
        ...
        [self.delegate setValues:values
                      ...
                      completion:^(NSArray<id<NSObject>> *results) {
                        NSArray<NSDictionary *> *errors = ABI50_0_0RCTMakeErrors(results);
                        callback(@[ABI50_0_0RCTNullIfNil(errors)]);
                      }];
...
// clang-format off
ABI50_0_0RCT_EXPORT_METHOD(multiMerge:(NSArray<NSArray<NSString *> *> *)kvPairs
                    callback:(ABI50_0_0RCTResponseSenderBlock)callback)
// clang-format on
{
    if (self.delegate != nil) {
    ...
    for (__strong NSArray<NSString *> *entry in kvPairs) {
        ...
        if (!keyError) {
            if (value) {
                NSError *jsonError;
                NSMutableDictionary *mergedVal = ABI50_0_0RCTJSONParseMutable(value, &jsonError);
                if (ABI50_0_0RCTMergeRecursive(mergedVal, ABI50_0_0RCTJSONParse(entry[1], &jsonError))) {
                    entry = @[entry[0], ABI50_0_0RCTNullIfNil(ABI50_0_0RCTJSONStringify(mergedVal, NULL))];
                }
                if (jsonError) {
                    keyError = ABI50_0_0RCTJSErrorFromNSError(jsonError);
                }
            }
            if (!keyError) {
...
// clang-format off
ABI50_0_0RCT_EXPORT_METHOD(multiRemove:(NSArray<NSString *> *)keys
                     callback:(ABI50_0_0RCTResponseSenderBlock)callback)
// clang-format on
{
    if (self.delegate != nil) {
    ...
    for (NSString *key in keys) {
        NSDictionary *keyError = ABI50_0_0RCTErrorForKey(key);
        if (!keyError) {
            if (_manifest[key] == (id)kCFNull) {
                NSString *filePath = [self _filePathForKey:key];
                [[NSFileManager defaultManager] removeItemAtPath:filePath error:nil];
...
// clang-format off
ABI50_0_0RCT_EXPORT_METHOD(clear:(ABI50_0_0RCTResponseSenderBlock)callback)
// clang-format on
{
    if (self.delegate != nil) {
        [self.delegate removeAllValues:^(NSError *error) {
          ...
          if (error != nil) {
              result = ABI50_0_0RCTMakeError(error.localizedDescription, error, nil);
          }
          callback(@[ABI50_0_0RCTNullIfNil(result)]);
        }];
        return;
    }
    ...
    [_manifest removeAllObjects];
    [ABI50_0_0RCTGetCache() removeAllObjects];
    NSDictionary *error = ABI50_0_0RCTDeleteStorageDirectory();
    callback(@[ABI50_0_0RCTNullIfNil(error)]);
}

// clang-format off
ABI50_0_0RCT_EXPORT_METHOD(getAllKeys:(ABI50_0_0RCTResponseSenderBlock)callback)
// clang-format on
{
    if (self.delegate != nil) {
