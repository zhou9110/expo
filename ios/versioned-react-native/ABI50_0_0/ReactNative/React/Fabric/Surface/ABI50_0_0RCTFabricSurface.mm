...
#import "ABI50_0_0RCTFabricSurface.h"
...
#import <ABI50_0_0React/ABI50_0_0RCTSurfaceRootView.h>
...
@implementation ABI50_0_0RCTFabricSurface {
  ...
  ABI50_0_0RCTSurfaceTouchHandler *_Nullable _touchHandler;
}

@synthesize delegate = _delegate;

- (instancetype)initWithSurfacePresenter:(ABI50_0_0RCTSurfacePresenter *)surfacePresenter
                       ...
                       initialProperties:(NSDictionary *)initialProperties
{
  if (self = [super init]) {
    ...
    _surfaceHandler =
        SurfaceHandler{ABI50_0_0RCTStringFromNSString(moduleName), (SurfaceId)[ABI50_0_0RCTAllocateRootViewTag() integerValue]};
    _surfaceHandler->setProps(convertIdToFollyDynamic(initialProperties));

    [_surfacePresenter registerSurface:self];

    [self setMinimumSize:CGSizeZero maximumSize:ABI50_0_0RCTViewportSize()];
...
- (void)resetWithSurfacePresenter:(ABI50_0_0RCTSurfacePresenter *)surfacePresenter
{
  _view = nil;
  _surfacePresenter = surfacePresenter;
  [_surfacePresenter registerSurface:self];
...
- (NSString *)moduleName
{
  return ABI50_0_0RCTNSStringFromString(_surfaceHandler->getModuleName());
}
...
- (ABI50_0_0RCTSurfaceView *)view
{
  ...
  if (!_view) {
    _view = [[ABI50_0_0RCTSurfaceView alloc] initWithSurface:(ABI50_0_0RCTSurface *)self];
...
{
  return _surfaceHandler->getStatus() == SurfaceHandler::Status::Running ? ABI50_0_0RCTSurfaceStageRunning
                                                                         : ABI50_0_0RCTSurfaceStagePreparing;
}

- (void)_propagateStageChange
{
  ABI50_0_0RCTSurfaceStage stage = self.stage;

  // Notifying the `delegate`
  id<ABI50_0_0RCTSurfaceDelegate> delegate = self.delegate;
  if ([delegate respondsToSelector:@selector(surface:didChangeStage:)]) {
    [delegate surface:(ABI50_0_0RCTSurface *)self didChangeStage:stage];
  }
}
...
- (void)_updateLayoutContext
{
  ...
  layoutConstraints.layoutDirection = ABI50_0_0RCTLayoutDirection([[ABI50_0_0RCTI18nUtil sharedInstance] isRTL]);

  auto layoutContext = _surfaceHandler->getLayoutContext();

  layoutContext.pointScaleFactor = ABI50_0_0RCTScreenScale();
  layoutContext.swapLeftAndRightInRTL =
      [[ABI50_0_0RCTI18nUtil sharedInstance] isRTL] && [[ABI50_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL];
  layoutContext.fontSizeMultiplier = ABI50_0_0RCTFontSizeMultiplier();
...
- (void)setMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize viewportOffset:(CGPoint)viewportOffset
{
  ...
  layoutConstraints.minimumSize = ABI50_0_0RCTSizeFromCGSize(minimumSize);
  layoutConstraints.maximumSize = ABI50_0_0RCTSizeFromCGSize(maximumSize);

  if (!isnan(viewportOffset.x) && !isnan(viewportOffset.y)) {
    layoutContext.viewportOffset = ABI50_0_0RCTPointFromCGPoint(viewportOffset);
  }
...
- (CGSize)sizeThatFitsMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  return ABI50_0_0RCTCGSizeFromSize(_surfaceHandler->measure(layoutConstraints, layoutContext));
}

- (CGSize)minimumSize
{
  return ABI50_0_0RCTCGSizeFromSize(_surfaceHandler->getLayoutConstraints().minimumSize);
}

- (CGSize)maximumSize
{
  return ABI50_0_0RCTCGSizeFromSize(_surfaceHandler->getLayoutConstraints().maximumSize);
}

- (CGPoint)viewportOffset
{
  return ABI50_0_0RCTCGPointFromPoint(_surfaceHandler->getLayoutContext().viewportOffset);
}
