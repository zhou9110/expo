...
#import <ABI50_0_0React/ABI50_0_0RCTImageUtils.h>
...
BOOL ABI50_0_0RCTImageLoadingPerfInstrumentationEnabled(void)
{
  return imagePerfInstrumentationEnabled;
}

void ABI50_0_0RCTEnableImageLoadingPerfInstrumentation(BOOL enabled)
{
  imagePerfInstrumentationEnabled = enabled;
}

static NSInteger ABI50_0_0RCTImageBytesForImage(UIImage *image)
{
...
@interface ABI50_0_0RCTImageLoader () <ABI50_0_0NativeImageLoaderIOSSpec, ABI50_0_0RCTImageLoaderWithAttributionProtocol>

@end

@implementation UIImage (ABI50_0_0React)

- (NSInteger)ABI50_0_0ReactDecodedImageBytes
{
  NSNumber *imageBytes = objc_getAssociatedObject(self, _cmd);
  if (!imageBytes) {
    imageBytes = @(ABI50_0_0RCTImageBytesForImage(self));
  }
  return [imageBytes integerValue];
}

- (void)setABI50_0_0ReactDecodedImageBytes:(NSInteger)bytes
{
  objc_setAssociatedObject(self, @selector(ABI50_0_0ReactDecodedImageBytes), @(bytes), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

@end

@implementation ABI50_0_0RCTImageLoader {
  NSArray<id<ABI50_0_0RCTImageURLLoader>> * (^_loadersProvider)(ABI50_0_0RCTModuleRegistry *);
  NSArray<id<ABI50_0_0RCTImageDataDecoder>> * (^_decodersProvider)(ABI50_0_0RCTModuleRegistry *);
  NSArray<id<ABI50_0_0RCTImageURLLoader>> *_loaders;
  NSArray<id<ABI50_0_0RCTImageDataDecoder>> *_decoders;
  NSOperationQueue *_imageDecodeQueue;
  dispatch_queue_t _URLRequestQueue;
  id<ABI50_0_0RCTImageCache> _imageCache;
  ...
  __weak id<ABI50_0_0RCTImageRedirectProtocol> _redirectDelegate;
}
...
- (instancetype)initWithRedirectDelegate:(id<ABI50_0_0RCTImageRedirectProtocol>)redirectDelegate
                        ...
                        decodersProvider:(NSArray<id<ABI50_0_0RCTImageDataDecoder>> * (^)(ABI50_0_0RCTModuleRegistry *))getHandlers
{
...
- (void)setUp
{
  ...
  _URLRequestQueue = dispatch_queue_create("com.facebook.ABI50_0_0React.ImageLoaderURLRequestQueue", DISPATCH_QUEUE_SERIAL);
}
...
#pragma mark - ABI50_0_0RCTImageLoaderProtocol 1/3

- (id<ABI50_0_0RCTImageCache>)imageCache
{
  if (!_imageCache) {
    // set up with default cache
    _imageCache = [ABI50_0_0RCTImageCache new];
  }
  return _imageCache;
}

- (void)setImageCache:(id<ABI50_0_0RCTImageCache>)cache
{
  if (_imageCache) {
    ABI50_0_0RCTLogWarn(@"ABI50_0_0RCTImageCache was already set and has now been overridden.");
  }
  _imageCache = cache;
}

- (id<ABI50_0_0RCTImageURLLoader>)imageURLLoaderForURL:(NSURL *)URL
{
  ...
  if (!_loaders) {
    ...
    if (!_loaders) {
      ...
      if (_loadersProvider) {
        ...
        ABI50_0_0RCTAssert(_bridge, @"Trying to find ABI50_0_0RCTImageURLLoaders and bridge not set.");
        _loaders = [_bridge modulesConformingToProtocol:@protocol(ABI50_0_0RCTImageURLLoader)];
      }

      _loaders =
          [_loaders sortedArrayUsingComparator:^NSComparisonResult(id<ABI50_0_0RCTImageURLLoader> a, id<ABI50_0_0RCTImageURLLoader> b) {
  ...
  if (ABI50_0_0RCT_DEBUG) {
    ...
    id<ABI50_0_0RCTImageURLLoader> previousLoader = nil;
    for (id<ABI50_0_0RCTImageURLLoader> loader in _loaders) {
        ...
        if (previousLoader) {
          if (priority == previousPriority) {
            ABI50_0_0RCTLogError(
                @"The ABI50_0_0RCTImageURLLoaders %@ and %@ both reported that"
...
- (id<ABI50_0_0RCTImageDataDecoder>)imageDataDecoderForData:(NSData *)data
{
  if (!_maxConcurrentLoadingTasks) {
    [self setUp];
  }
  ...
  if (!_decoders) {
    ...
    if (_decodersProvider) {
      ...
      ABI50_0_0RCTAssert(_bridge, @"Trying to find ABI50_0_0RCTImageDataDecoders and bridge not set.");
      _decoders = [_bridge modulesConformingToProtocol:@protocol(ABI50_0_0RCTImageDataDecoder)];
    }

    _decoders = [_decoders
        sortedArrayUsingComparator:^NSComparisonResult(id<ABI50_0_0RCTImageDataDecoder> a, id<ABI50_0_0RCTImageDataDecoder> b) {
  ...
  if (ABI50_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI50_0_0RCTImageDataDecoder> previousDecoder = nil;
    for (id<ABI50_0_0RCTImageDataDecoder> decoder in _decoders) {
      ...
      if ([decoder canDecodeImageData:data]) {
        if (previousDecoder) {
          if (priority == previousPriority) {
            ABI50_0_0RCTLogError(
                @"The ABI50_0_0RCTImageDataDecoders %@ and %@ both reported that"
...
static UIImage *ABI50_0_0RCTResizeImageIfNeeded(UIImage *image, CGSize size, CGFloat scale, ABI50_0_0RCTResizeMode resizeMode)
{
  if (CGSizeEqualToSize(size, CGSizeZero) || CGSizeEqualToSize(image.size, CGSizeZero) ||
      CGSizeEqualToSize(image.size, size)) {
    return image;
  }
  CGRect targetSize = ABI50_0_0RCTTargetRect(image.size, size, scale, resizeMode);
  CGAffineTransform transform = ABI50_0_0RCTTransformFromTargetRect(image.size, targetSize);
  image = ABI50_0_0RCTTransformImage(image, size, scale, transform);
  return image;
}

#pragma mark - ABI50_0_0RCTImageLoaderProtocol 2/3

- (nullable ABI50_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                           callback:(ABI50_0_0RCTImageLoaderCompletionBlock)callback
{
  return [self loadImageWithURLRequest:imageURLRequest priority:ABI50_0_0RCTImageLoaderPriorityImmediate callback:callback];
}
                                                           ...
                                                           callback:(ABI50_0_0RCTImageLoaderCompletionBlock)callback
{
  return [self loadImageWithURLRequest:imageURLRequest
                                 ...
                                 scale:1
                               clipped:YES
                            resizeMode:ABI50_0_0RCTResizeModeStretch
                              priority:priority
                         progressBlock:nil
...
- (nullable ABI50_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                   ...
                                                   partialLoadBlock:(ABI50_0_0RCTImageLoaderPartialLoadBlock)partialLoadBlock
                                                    completionBlock:(ABI50_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  return [self loadImageWithURLRequest:imageURLRequest
                                  size:size
                                 scale:scale
                            ...
                            resizeMode:resizeMode
                              priority:ABI50_0_0RCTImageLoaderPriorityImmediate
...
- (nullable ABI50_0_0RCTImageLoaderCancellationBlock)loadImageWithURLRequest:(NSURLRequest *)imageURLRequest
                                                   ...
                                                   partialLoadBlock:(ABI50_0_0RCTImageLoaderPartialLoadBlock)partialLoadBlock
                                                    completionBlock:(ABI50_0_0RCTImageLoaderCompletionBlock)completionBlock
{
  ABI50_0_0RCTImageURLLoaderRequest *request = [self loadImageWithURLRequest:imageURLRequest
      size:size
      scale:scale
      clipped:clipped
      resizeMode:resizeMode
...
- (void)dequeueTasks
{
  dispatch_async(_URLRequestQueue, ^{
    ...
    for (ABI50_0_0RCTNetworkTask *task in self->_pendingTasks.reverseObjectEnumerator) {
      switch (task.status) {
        case ABI50_0_0RCTNetworkTaskFinished:
          if (!tasksToRemove) {
          ...
          self->_activeTasks--;
          break;
        case ABI50_0_0RCTNetworkTaskPending:
          break;
        case ABI50_0_0RCTNetworkTaskInProgress:
          // Check task isn't "stuck"
          if (task.requestToken == nil) {
            ABI50_0_0RCTLogWarn(@"Task orphaned for request %@", task.request);
            if (!tasksToRemove) {
              tasksToRemove = [NSMutableArray new];
            }
            [tasksToRemove addObject:task];
    ...
    // Start queued tasks
    for (ABI50_0_0RCTNetworkTask *task in self->_pendingTasks) {
      if (MAX(self->_activeTasks, self->_scheduledDecodes) >= self->_maxConcurrentLoadingTasks) {
        break;
      }
      if (task.status == ABI50_0_0RCTNetworkTaskPending) {
        [task start];
        self->_activeTasks++;
      }
    }
 ...
 */
- (ABI50_0_0RCTImageURLLoaderRequest *)_loadImageOrDataWithURLRequest:(NSURLRequest *)request
                                                        size:(CGSize)size
                                                       scale:(CGFloat)scale
                                                  resizeMode:(ABI50_0_0RCTResizeMode)resizeMode
                                                    priority:(ABI50_0_0RCTImageLoaderPriority)priority
                                                 attribution:(const ImageURLLoaderAttribution &)attribution
                                               progressBlock:(ABI50_0_0RCTImageLoaderProgressBlock)progressHandler
                                            partialLoadBlock:(ABI50_0_0RCTImageLoaderPartialLoadBlock)partialLoadHandler
...
{
  {
    ...
    [NSURLProtocol setProperty:@"ABI50_0_0RCTImageLoader" forKey:@"trackingName" inRequest:mutableRequest];
  ...
  id<ABI50_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:request.URL];
  ...
  void (^completionHandler)(NSError *, id, id, NSURLResponse *) =
      ^(NSError *error, id imageOrData, id imageMetadata, NSURLResponse *response) {
        ...
        if (ABI50_0_0RCTIsMainQueue() && ![imageOrData isKindOfClass:[UIImage class]]) {
  ...
  if (loadHandler && !requiresScheduling) {
    if ([loadHandler conformsToProtocol:@protocol(ABI50_0_0RCTImageURLLoaderWithAttribution)]) {
      return [(id<ABI50_0_0RCTImageURLLoaderWithAttribution>)loadHandler
    ...
    ABI50_0_0RCTImageLoaderCancellationBlock cb = [loadHandler loadImageForURL:request.URL
                                                                 size:size
                                                                scale:scale
                                                           resizeMode:resizeMode
                                                      progressHandler:progressHandler
    ...
    return [[ABI50_0_0RCTImageURLLoaderRequest alloc] initWithRequestId:nil imageURL:request.URL cancellationBlock:cb];
  }
  ...
  __weak ABI50_0_0RCTImageLoader *weakSelf = self;
  dispatch_async(_URLRequestQueue, ^{
    ...
    if (loadHandler) {
      ...
      if ([loadHandler conformsToProtocol:@protocol(ABI50_0_0RCTImageURLLoaderWithAttribution)]) {
        ABI50_0_0RCTImageURLLoaderRequest *loaderRequest = [(id<ABI50_0_0RCTImageURLLoaderWithAttribution>)loadHandler
               loadImageForURL:request.URL
                          size:size
                         scale:scale
                    resizeMode:resizeMode
  ...
  return [[ABI50_0_0RCTImageURLLoaderRequest alloc] initWithRequestId:requestId
...
- (ABI50_0_0RCTImageLoaderCancellationBlock)_loadURLRequest:(NSURLRequest *)request
                                     progressBlock:(ABI50_0_0RCTImageLoaderProgressBlock)progressHandler
                                   completionBlock:(void (^)(NSError *error, id imageOrData, NSURLResponse *response))
                                                       completionHandler
{
  ABI50_0_0RCTNetworking *networking = [_moduleRegistry moduleForName:"Networking"];
  if (ABI50_0_0RCT_DEBUG && !networking) {
    ABI50_0_0RCTLogError(
        @"No suitable image URL loader found for %@. You may need to "
         " import the ABI50_0_0RCTNetwork library in order to load images.",
        request.URL.absoluteString);
    return NULL;
  }

  // Check if networking module can load image
  if (ABI50_0_0RCT_DEBUG && ![networking canHandleRequest:request]) {
    ABI50_0_0RCTLogError(@"No suitable image URL loader found for %@", request.URL.absoluteString);
    return NULL;
  }

  // Use networking module to load image
  ABI50_0_0RCTURLRequestCompletionBlock processResponse = ^(NSURLResponse *response, NSData *data, NSError *error) {
    // Check for system errors
    if (error) {
      completionHandler(error, nil, response);
      return;
    } else if (!response) {
      completionHandler(ABI50_0_0RCTErrorWithMessage(@"Response metadata error"), nil, response);
      return;
    } else if (!data) {
      completionHandler(ABI50_0_0RCTErrorWithMessage(@"Unknown image download error"), nil, response);
      return;
    }
  ...
  __block ABI50_0_0RCTNetworkTask *task =
                         ...
                         completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                           ...
                           if (error || !response || !data) {
                             ...
                             if (error) {
                               ...
                               someError = ABI50_0_0RCTErrorWithMessage(@"Response metadata error");
                             } else {
                               someError = ABI50_0_0RCTErrorWithMessage(@"Unknown image download error");
                             }
...
#pragma mark - ABI50_0_0RCTImageLoaderWithAttributionProtocol
                                      ...
                                      completionBlock:(ABI50_0_0RCTImageLoaderCompletionBlockWithMetadata)completionBlock
{
  ...
  void (^completionHandler)(NSError *, id, id, BOOL, NSURLResponse *) =
      ^(NSError *error, id imageOrData, id imageMetadata, BOOL cacheResult, NSURLResponse *response) {
        ...
        ABI50_0_0RCTImageLoaderCompletionBlock decodeCompletionHandler = ^(NSError *error_, UIImage *image) {
  ...
  ABI50_0_0RCTImageURLLoaderRequest *loaderRequest = [self _loadImageOrDataWithURLRequest:imageURLRequest
                                                                            size:size
                                                                           scale:scale
                                                                      resizeMode:resizeMode
                                                                        priority:priority
  ...
  return [[ABI50_0_0RCTImageURLLoaderRequest alloc] initWithRequestId:loaderRequest.requestId
...
{
  id<ABI50_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:url];
  if ([loadHandler respondsToSelector:@selector(shouldEnablePerfLogging)]) {
    return [(id<ABI50_0_0RCTImageURLLoaderWithAttribution>)loadHandler shouldEnablePerfLogging];
  }
  return NO;
}
...
- (void)trackURLImageVisibilityForRequest:(ABI50_0_0RCTImageURLLoaderRequest *)loaderRequest imageView:(UIView *)imageView
{
  ...
  id<ABI50_0_0RCTImageURLLoader> loadHandler = [self imageURLLoaderForURL:loaderRequest.imageURL];
  if ([loadHandler respondsToSelector:@selector(trackURLImageVisibilityForRequest:imageView:)]) {
    [(id<ABI50_0_0RCTImageURLLoaderWithAttribution>)loadHandler trackURLImageVisibilityForRequest:loaderRequest
                                                                               imageView:imageView];
  }
}
...
- (void)trackURLImageRequestDidDestroy:(ABI50_0_0RCTImageURLLoaderRequest *)loaderRequest
{
  ...
  if ([loadHandler respondsToSelector:@selector(trackURLImageRequestDidDestroy:)]) {
    [(id<ABI50_0_0RCTImageURLLoaderWithAttribution>)loadHandler trackURLImageRequestDidDestroy:loaderRequest];
  }
}
...
- (void)trackURLImageDidDestroy:(ABI50_0_0RCTImageURLLoaderRequest *)loaderRequest
{
  ...
  if ([loadHandler respondsToSelector:@selector(trackURLImageDidDestroy:)]) {
    [(id<ABI50_0_0RCTImageURLLoaderWithAttribution>)loadHandler trackURLImageDidDestroy:loaderRequest];
  }
}

#pragma mark - ABI50_0_0RCTImageLoaderProtocol 3/3

- (ABI50_0_0RCTImageLoaderCancellationBlock)decodeImageData:(NSData *)data
                                              size:(CGSize)size
                                             scale:(CGFloat)scale
                                           clipped:(BOOL)clipped
                                        resizeMode:(ABI50_0_0RCTResizeMode)resizeMode
...
{
  if (data.length == 0) {
    completionBlock(ABI50_0_0RCTErrorWithMessage(@"No image data"), nil);
    return ^{
    };
  }
  ...
  void (^completionHandler)(NSError *, UIImage *) = ^(NSError *error, UIImage *image) {
    if (ABI50_0_0RCTIsMainQueue()) {
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          completionBlock(error, clipped ? ABI50_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
        }
      });
    } else if (!std::atomic_load(cancelled.get())) {
      completionBlock(error, clipped ? ABI50_0_0RCTResizeImageIfNeeded(image, size, scale, resizeMode) : image);
  ...
  id<ABI50_0_0RCTImageDataDecoder> imageDecoder = [self imageDataDecoderForData:data];
  if (imageDecoder) {
    ...
    dispatch_block_t decodeBlock = ^{
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!std::atomic_load(cancelled.get())) {
          ...
          UIImage *image = ABI50_0_0RCTDecodeImageWithData(data, size, scale, resizeMode);

#if ABI50_0_0RCT_DEV
          CGSize imagePixelSize = ABI50_0_0RCTSizeInPixels(image.size, image.scale);
          CGSize screenPixelSize = ABI50_0_0RCTSizeInPixels(ABI50_0_0RCTScreenSize(), ABI50_0_0RCTScreenScale());
          ...
          if (image) {
            ...
            NSError *finalError = ABI50_0_0RCTErrorWithMessage(errorMessage);
...
- (ABI50_0_0RCTImageLoaderCancellationBlock)getImageSizeForURLRequest:(NSURLRequest *)imageURLRequest
  ...
  void (^completion)(NSError *, id, id, BOOL, NSURLResponse *) =
      ^(NSError *error, id imageOrData, id imageMetadata, BOOL cacheResult, NSURLResponse *response) {
        ...
        if ([imageOrData isKindOfClass:[NSData class]]) {
          NSDictionary *meta = ABI50_0_0RCTGetImageMetadata(imageOrData);
...
- (NSDictionary *)getImageCacheStatus:(NSArray *)requests
{
  ...
  for (id request in requests) {
    NSURLRequest *urlRequest = [ABI50_0_0RCTConvert NSURLRequest:request];
    if (urlRequest) {
...
#pragma mark - ABI50_0_0RCTURLRequestHandler
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  // Even better may be to implement this on the ABI50_0_0RCTImageURLLoader that would try to load it,
  // but we'd have to run the logic both in ABI50_0_0RCTPhotoLibraryImageLoader and
  // ABI50_0_0RCTAssetsLibraryRequestHandler. Once we drop iOS7 though, we'd drop
  // ABI50_0_0RCTAssetsLibraryRequestHandler and can move it there.
  ...
  for (id<ABI50_0_0RCTImageURLLoader> loader in _loaders) {
    // Don't use ABI50_0_0RCTImageURLLoader protocol for modules that already conform to
    // ABI50_0_0RCTURLRequestHandler as it's inefficient to decode an image and then
    // convert it back into data
    if (![loader conformsToProtocol:@protocol(ABI50_0_0RCTURLRequestHandler)] && [loader canLoadImageURL:requestURL]) {
      return YES;
    }
  }
...
- (id)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI50_0_0RCTURLRequestDelegate>)delegate
{
  __block ABI50_0_0RCTImageLoaderCancellationBlock requestToken;
  requestToken = [self loadImageWithURLRequest:request
                                      callback:^(NSError *error, UIImage *image) {
                                        ...
                                        if (ABI50_0_0RCTImageHasAlpha(image.CGImage)) {
                                          mimeType = @"image/png";
...
{
  if (requestToken) {
    ((ABI50_0_0RCTImageLoaderCancellationBlock)requestToken)();
  }
}
...
{
  return std::make_shared<ABI50_0_0facebook::ABI50_0_0React::NativeImageLoaderIOSSpecJSI>(params);
}
...
{
  NSURLRequest *request = [ABI50_0_0RCTConvert NSURLRequest:uri];
...
ABI50_0_0RCT_EXPORT_METHOD(getSizeWithHeaders
                  ...
                  : (ABI50_0_0RCTPromiseRejectBlock)reject)
{
  NSURL *URL = [ABI50_0_0RCTConvert NSURL:uri];
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:URL];
  [headers enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
    [request addValue:[ABI50_0_0RCTConvert NSString:value] forHTTPHeaderField:key];
  }];
  [self getImageSizeForURLRequest:request
                            block:^(NSError *error, CGSize size) {
                              if (error) {
...
ABI50_0_0RCT_EXPORT_METHOD(prefetchImageWithMetadata
 ...
 */
@implementation ABI50_0_0RCTBridge (ABI50_0_0RCTImageLoader)

- (ABI50_0_0RCTImageLoader *)imageLoader
{
  ABI50_0_0RCTLogWarn(
      @"Calling bridge.imageLoader is deprecated and will not work in newer versions of ABI50_0_0RN. Please update to the "
       "moduleForClass API or turboModuleRegistry API.");
  return [self moduleForClass:[ABI50_0_0RCTImageLoader class]];
}

@end

@implementation ABI50_0_0RCTBridgeProxy (ABI50_0_0RCTImageLoader)
...
Class ABI50_0_0RCTImageLoaderCls(void)
{
  return ABI50_0_0RCTImageLoader.class;
}
