...
@interface ABI50_0_0EXFacebook (ExportedMethods)
...
- (void)logOutAsync:(ABI50_0_0EXPromiseResolveBlock)resolve
           rejecter:(ABI50_0_0EXPromiseRejectBlock)reject;

- (void)getAuthenticationCredentialAsync:(ABI50_0_0EXPromiseResolveBlock)resolve
                   rejecter:(ABI50_0_0EXPromiseRejectBlock)reject;
...
@interface ABI50_0_0EXScopedFacebook ()
...
// Expo Go-only ABI50_0_0EXFacebook module, which ensures that Facebook SDK configurations
// of different projects don't collide.

@implementation ABI50_0_0EXScopedFacebook : ABI50_0_0EXFacebook
...
- (instancetype)initWithScopeKey:(NSString *)scopeKey manifest:(ABI50_0_0EXManifestsManifest *)manifest
{
  if (self = [super init]) {
    ...
    if (hasPreviouslySetAutoInitEnabled || manifestDefinesAutoInitEnabled) {
      ...
      // the mechanism behind ABI50_0_0EXFacebookAppDelegate.
      // Check for FacebookAppId in case this is a custom client build
      if (scopedFacebookAppId) {
        [FBSDKApplicationDelegate initializeSDK:nil];
        _isInitialized = YES;
        if (manifestFacebookAppId) {
          ABI50_0_0EXLogInfo(@"Overriding Facebook App ID with Expo Go's. To test your own Facebook App ID, you'll need to build a standalone app. Refer to our documentation for more info- https://docs.expo.io/versions/latest/sdk/facebook/");
        }
      } else {
        ABI50_0_0EXLogWarn(@"FacebookAutoInit is enabled, but no FacebookAppId has been provided. Facebook SDK initialization aborted.");
      }
    }
  }
  return self;
...
- (void)setAutoInitEnabled:(BOOL)enabled resolver:(ABI50_0_0EXPromiseResolveBlock)resolve rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
  if (enabled) {
...
- (void)logInWithReadPermissionsWithConfig:(NSDictionary *)config resolver:(ABI50_0_0EXPromiseResolveBlock)resolve rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
...
- (void)getAuthenticationCredentialAsync:(ABI50_0_0EXPromiseResolveBlock)resolve rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
  // If the developer didn't initialize the SDK, let them know.
  if (!_isInitialized) {
    reject(@"ERR_FACEBOOK_UNINITIALIZED", @"Facebook SDK has not been initialized yet.", nil);
...
- (void)logOutAsync:(ABI50_0_0EXPromiseResolveBlock)resolve rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
  // If the developer didn't initialize the SDK, let them know.
  if (!_isInitialized) {
    reject(@"ERR_FACEBOOK_UNINITIALIZED", @"Facebook SDK has not been initialized yet.", nil);
...
- (void)setModuleRegistry:(ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  id<ABI50_0_0EXAppLifecycleService> appLifecycleService = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI50_0_0EXAppLifecycleService)];
