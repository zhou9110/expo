...
#import "ABI50_0_0RCTBridgeModuleDecorator.h"
#import "ABI50_0_0RCTConstants.h"
#import "ABI50_0_0RCTInitializing.h"
...
@implementation ABI50_0_0RCTModuleData {
  NSDictionary<NSString *, id> *_constantsToExport;
  NSString *_queueName;
  __weak ABI50_0_0RCTBridge *_bridge;
  ABI50_0_0RCTBridgeModuleProvider _moduleProvider;
    ...
    if (_requiresMainQueueSetup) {
      ...
      ABI50_0_0RCTLogWarn(
          @"Module %@ requires main queue setup since it overrides `%s` but doesn't implement "
           "`requiresMainQueueSetup`. In a future release ABI50_0_0React Native will default to initializing all native modules "
...
- (instancetype)initWithModuleClass:(Class)moduleClass
            ...
            viewRegistry_DEPRECATED:(ABI50_0_0RCTViewRegistry *)viewRegistry_DEPRECATED
                  ...
                  callableJSModules:(ABI50_0_0RCTCallableJSModules *)callableJSModules
{
  return [self initWithModuleClass:moduleClass
                    moduleProvider:^id<ABI50_0_0RCTBridgeModule> {
...
- (instancetype)initWithModuleClass:(Class)moduleClass
                     moduleProvider:(ABI50_0_0RCTBridgeModuleProvider)moduleProvider
                             bridge:(ABI50_0_0RCTBridge *)bridge
                     moduleRegistry:(ABI50_0_0RCTModuleRegistry *)moduleRegistry
            viewRegistry_DEPRECATED:(ABI50_0_0RCTViewRegistry *)viewRegistry_DEPRECATED
                      bundleManager:(ABI50_0_0RCTBundleManager *)bundleManager
...
- (instancetype)initWithModuleInstance:(id<ABI50_0_0RCTBridgeModule>)instance
                                bridge:(ABI50_0_0RCTBridge *)bridge
                        moduleRegistry:(ABI50_0_0RCTModuleRegistry *)moduleRegistry
               viewRegistry_DEPRECATED:(ABI50_0_0RCTViewRegistry *)viewRegistry_DEPRECATED
                         bundleManager:(ABI50_0_0RCTBundleManager *)bundleManager
...
- (void)setUpInstanceAndBridge:(int32_t)requestId
{
  ...
  ABI50_0_0RCT_PROFILE_BEGIN_EVENT(
      ...
      @"[ABI50_0_0RCTModuleData setUpInstanceAndBridge]",
      @{@"moduleClass" : NSStringFromClass(_moduleClass)});
  {
    ...
    if (shouldSetup) {
      if (!_instance) {
        if (ABI50_0_0RCT_DEBUG && _requiresMainQueueSetup) {
          ABI50_0_0RCTAssertMainQueue();
        }
        ABI50_0_0RCT_PROFILE_BEGIN_EVENT(ABI50_0_0RCTProfileTagAlways, @"[ABI50_0_0RCTModuleData setUpInstanceAndBridge] Create module", nil);
      ...
      if (_instance && ABI50_0_0RCTProfileIsProfiling()) {
        ABI50_0_0RCTProfileHookInstance(_instance);
      }
    }
    ...
    if (shouldSetup) {
      ...
      ABI50_0_0RCTBridgeModuleDecorator *moduleDecorator =
          [[ABI50_0_0RCTBridgeModuleDecorator alloc] initWithViewRegistry:_viewRegistry_DEPRECATED
...
- (void)setBridgeForInstance
{
  if ([_instance respondsToSelector:@selector(bridge)] && _instance.bridge != _bridge) {
    ABI50_0_0RCT_PROFILE_BEGIN_EVENT(ABI50_0_0RCTProfileTagAlways, @"[ABI50_0_0RCTModuleData setBridgeForInstance]", nil);
    @try {
...
- (void)_initializeModule
{
  if (!_isInitialized && [_instance respondsToSelector:@selector(initialize)]) {
    _isInitialized = YES;
    [(id<ABI50_0_0RCTInitializing>)_instance initialize];
  }
}
...
- (void)finishSetupForInstance
{
  if (!_setupComplete && _instance) {
    ABI50_0_0RCT_PROFILE_BEGIN_EVENT(ABI50_0_0RCTProfileTagAlways, @"[ABI50_0_0RCTModuleData finishSetupForInstance]", nil);
    _setupComplete = YES;
    [_bridge registerModuleForFrameUpdates:_instance withModuleData:self];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:ABI50_0_0RCTDidInitializeModuleNotification
                      object:_bridge
                    userInfo:@{@"module" : _instance, @"bridge" : ABI50_0_0RCTNullIfNil(_bridge.parentBridge)}];
    ABI50_0_0RCT_PROFILE_END_EVENT(ABI50_0_0RCTProfileTagAlways, @"");
  }
}
...
- (void)setUpMethodQueue
{
  if (_instance && !_methodQueue && _bridge.valid) {
    ABI50_0_0RCT_PROFILE_BEGIN_EVENT(ABI50_0_0RCTProfileTagAlways, @"[ABI50_0_0RCTModuleData setUpMethodQueue]", nil);
    ...
    if (!_methodQueue && _bridge.valid) {
      ...
      _queueName = [NSString stringWithFormat:@"com.facebook.ABI50_0_0React.%@Queue", self.name];
...
- (void)calculateMethods
{
  ...
  NSMutableArray<id<ABI50_0_0RCTBridgeMethod>> *moduleMethods = [NSMutableArray new];
  NSMutableDictionary<NSString *, id<ABI50_0_0RCTBridgeMethod>> *moduleMethodsByName = [NSMutableDictionary new];
  ...
  while (cls && cls != [NSObject class] && cls != [NSProxy class]) {
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
      ...
      if ([NSStringFromSelector(selector) hasPrefix:@"__rct_export__"]) {
        ...
        auto exportedMethod = ((const ABI50_0_0RCTMethodInfo *(*)(id, SEL))imp)(_moduleClass, selector);
        id<ABI50_0_0RCTBridgeMethod> moduleMethod = [[ABI50_0_0RCTModuleMethod alloc] initWithExportedMethod:exportedMethod
...
- (id<ABI50_0_0RCTBridgeModule>)instance
{
  ...
  if (!_setupComplete) {
    ABI50_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI50_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI50_0_0RCTModuleData instanceForClass:%@]", _moduleClass]), nil);
    if (_requiresMainQueueSetup) {
      ...
      ABI50_0_0RCT_PROFILE_BEGIN_EVENT(ABI50_0_0RCTProfileTagAlways, @"[ABI50_0_0RCTModuleData instance] main thread setup", nil);

      if (!ABI50_0_0RCTIsMainQueue()) {
        ABI50_0_0RCTLogWarn(@"ABI50_0_0RCTBridge required dispatch_sync to load %@. This may lead to deadlocks", _moduleClass);
      }
...
- (NSString *)name
{
  return ABI50_0_0RCTBridgeModuleNameForClass(_moduleClass);
}

- (NSArray<id<ABI50_0_0RCTBridgeMethod>> *)methods
{
  [self calculateMethods];
  return _methods;
}

- (NSDictionary<NSString *, id<ABI50_0_0RCTBridgeMethod>> *)methodsByName
{
  [self calculateMethods];
...
- (void)gatherConstantsAndSignalJSRequireEnding:(BOOL)startMarkers
{
  ...
  if (_hasConstantsToExport && !_constantsToExport) {
    ABI50_0_0RCT_PROFILE_BEGIN_EVENT(
        ABI50_0_0RCTProfileTagAlways, ([NSString stringWithFormat:@"[ABI50_0_0RCTModuleData gatherConstants] %@", _moduleClass]), nil);
    (void)[self instance];
    ...
    if (startMarkers) {
       ...
       *  - This delegates to ABI50_0_0RCTNativeModule::getConstants(), which calls ABI50_0_0RCTModuleData gatherConstants().
    ...
    if (_requiresMainQueueSetup) {
      if (!ABI50_0_0RCTIsMainQueue()) {
        ABI50_0_0RCTLogWarn(@"Required dispatch_sync to load constants for %@. This may lead to deadlocks", _moduleClass);
      }
...
- (dispatch_queue_t)methodQueue
{
  if (_bridge.valid) {
    ...
    ABI50_0_0RCTAssert(_methodQueue != nullptr, @"Module %@ has no methodQueue (instance: %@)", self, instance);
  }
  return _methodQueue;
}
