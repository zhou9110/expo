...
CompactValue Node::computeEdgeValueForRow(
  ...
  if (!edges[rowEdge].isUndefined()) {
    ...
    return edges[edge];
  } else if (!edges[ABI50_0_0YGEdgeHorizontal].isUndefined()) {
    return edges[ABI50_0_0YGEdgeHorizontal];
  } else if (!edges[ABI50_0_0YGEdgeAll].isUndefined()) {
    return edges[ABI50_0_0YGEdgeAll];
  } else {
    return defaultValue;
  }
}
...
CompactValue Node::computeEdgeValueForColumn(
  ...
  if (!edges[edge].isUndefined()) {
    return edges[edge];
  } else if (!edges[ABI50_0_0YGEdgeVertical].isUndefined()) {
    return edges[ABI50_0_0YGEdgeVertical];
  } else if (!edges[ABI50_0_0YGEdgeAll].isUndefined()) {
    return edges[ABI50_0_0YGEdgeAll];
  } else {
    return defaultValue;
...
CompactValue Node::computeRowGap(
    ...
    CompactValue defaultValue) {
  if (!gutters[ABI50_0_0YGGutterRow].isUndefined()) {
    return gutters[ABI50_0_0YGGutterRow];
  } else if (!gutters[ABI50_0_0YGGutterAll].isUndefined()) {
    return gutters[ABI50_0_0YGGutterAll];
  } else {
    return defaultValue;
  }
}
...
CompactValue Node::computeColumnGap(
    const Style::Gutters& gutters,
    CompactValue defaultValue) {
  if (!gutters[ABI50_0_0YGGutterColumn].isUndefined()) {
    return gutters[ABI50_0_0YGGutterColumn];
  } else if (!gutters[ABI50_0_0YGGutterAll].isUndefined()) {
    return gutters[ABI50_0_0YGGutterAll];
  } else {
    return defaultValue;
...
void Node::setMeasureFunc(ABI50_0_0YGMeasureFunc measureFunc) {
...
void Node::setLayoutMargin(float margin, ABI50_0_0YGEdge edge) {
  assertFatal(
      edge < layout_.margin.size(), "Edge must be top/left/bottom/right");
  layout_.margin[edge] = margin;
}

void Node::setLayoutBorder(float border, ABI50_0_0YGEdge edge) {
  assertFatal(
      edge < layout_.border.size(), "Edge must be top/left/bottom/right");
  layout_.border[edge] = border;
}

void Node::setLayoutPadding(float padding, ABI50_0_0YGEdge edge) {
  assertFatal(
...
void Node::setLayoutPosition(float position, ABI50_0_0YGEdge edge) {
  assertFatal(
...
void Node::setLayoutDimension(float dimensionValue, ABI50_0_0YGDimension dimension) {
...
ABI50_0_0YGValue Node::marginLeadingValue(const FlexDirection axis) const {
  if (isRow(axis) && !style_.margin()[ABI50_0_0YGEdgeStart].isUndefined()) {
    return style_.margin()[ABI50_0_0YGEdgeStart];
  } else {
    return style_.margin()[leadingEdge(axis)];
  }
}

ABI50_0_0YGValue Node::marginTrailingValue(const FlexDirection axis) const {
  if (isRow(axis) && !style_.margin()[ABI50_0_0YGEdgeEnd].isUndefined()) {
    return style_.margin()[ABI50_0_0YGEdgeEnd];
  } else {
    return style_.margin()[trailingEdge(axis)];
  }
}

ABI50_0_0YGValue Node::resolveFlexBasisPtr() const {
  ABI50_0_0YGValue flexBasis = style_.flexBasis();
  if (flexBasis.unit != ABI50_0_0YGUnitAuto && flexBasis.unit != ABI50_0_0YGUnitUndefined) {
    return flexBasis;
  }
  if (!style_.flex().isUndefined() && style_.flex().unwrap() > 0.0f) {
    return config_->useWebDefaults() ? ABI50_0_0YGValueAuto : ABI50_0_0YGValueZero;
  }
  return ABI50_0_0YGValueAuto;
}

void Node::resolveDimension() {
  using namespace yoga;
  const Style& style = getStyle();
  for (auto dim : {ABI50_0_0YGDimensionWidth, ABI50_0_0YGDimensionHeight}) {
...
float Node::getLeadingBorder(const FlexDirection axis) const {
  ABI50_0_0YGValue leadingBorder = isRow(axis)
      ? computeEdgeValueForRow(
            style_.border(),
            ABI50_0_0YGEdgeStart,
            leadingEdge(axis),
...
float Node::getTrailingBorder(const FlexDirection axis) const {
  ABI50_0_0YGValue trailingBorder = isRow(axis)
      ? computeEdgeValueForRow(
            style_.border(),
            ABI50_0_0YGEdgeEnd,
            trailingEdge(axis),
