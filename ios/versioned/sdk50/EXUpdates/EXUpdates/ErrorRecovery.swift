...
@objc(ABI50_0_0EXUpdatesRemoteLoadStatus)
 ...
 */
@objc(ABI50_0_0EXUpdatesErrorRecovery)
@objcMembers
public final class ErrorRecovery: NSObject {
  ...
  private var previousFatalErrorHandler: ABI50_0_0RCTFatalHandler?
  private var previousFatalExceptionHandler: ABI50_0_0RCTFatalExceptionHandler?
  ...
  private func crash() {
    ...
    if let initialError = initialError as? NSError {
      // format these keys similar to RN -- ABI50_0_0RCTFatal in ABI50_0_0RCTAssert.m
      name = NSExceptionName(rawValue: "\(ABI50_0_0RCTFatalExceptionName): \(initialError.localizedDescription)")
      reason = ABI50_0_0RCTFormatError(initialError.localizedDescription, (initialError.userInfo[ABI50_0_0RCTJSStackTraceKey] as? [[String: Any]]), 175)
      userInfo = initialError.userInfo
      userInfo["ABI50_0_0RCTUntruncatedMessageKey"] = ABI50_0_0RCTFormatError(
        initialError.localizedDescription,
        (initialError.userInfo[ABI50_0_0RCTJSStackTraceKey] as? [[String: Any]]),
        0
      )
    } else if let initialError = initialError as? NSException {
    ...
    userInfo["ABI50_0_0EXUpdatesLaterEncounteredErrors"] = encounteredErrors
  ...
  private func registerObservers() {
    NotificationCenter.default.addObserver(
      ...
      name: NSNotification.Name.ABI50_0_0RCTJavaScriptDidFailToLoad,
      object: nil
    )
    NotificationCenter.default.addObserver(
      self,
      selector: #selector(handleContentDidAppear),
      name: NSNotification.Name.ABI50_0_0RCTContentDidAppear,
      object: nil
    )
  }

  private func unregisterObservers() {
    NotificationCenter.default.removeObserver(self, name: NSNotification.Name.ABI50_0_0RCTJavaScriptDidFailToLoad, object: nil)
    NotificationCenter.default.removeObserver(self, name: NSNotification.Name.ABI50_0_0RCTContentDidAppear, object: nil)
  }
  ...
  private func setRCTErrorHandlers() {
    ...
    previousFatalErrorHandler = ABI50_0_0RCTGetFatalHandler()
    previousFatalExceptionHandler = ABI50_0_0RCTGetFatalExceptionHandler()

    ABI50_0_0RCTSetFatalHandler { error in
      self.handle(error: error! as NSError)
    }
    ABI50_0_0RCTSetFatalExceptionHandler { exception in
      self.handle(exception: exception!)
    }
  }

  private func unsetRCTErrorHandlers() {
    ABI50_0_0RCTSetFatalHandler(previousFatalErrorHandler)
    ABI50_0_0RCTSetFatalExceptionHandler(previousFatalExceptionHandler)
