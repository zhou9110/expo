...
#include <ABI50_0_0cxxreact/ABI50_0_0JsArgumentHelpers.h>
...
using namespace ABI50_0_0facebook::xplat;
using dynamic = folly::dynamic;

#define ABI50_0_0EXPECT_JSAE(statement, exstr)                                         \
  do {                                                                        \
    try {                                                                     \
      statement;                                                              \
      FAIL() << "Expected JsArgumentException(" << (exstr) << ") not thrown"; \
    } catch (const JsArgumentException& ex) {                                 \
      ABI50_0_0EXPECT_EQ(ex.what(), std::string(exstr));                               \
    }                                                                         \
...
TEST(JsArgumentHelpersTest, args) {
  ...
  ABI50_0_0EXPECT_EQ(jsArgAsBool(args, 0), aBool);
  ABI50_0_0EXPECT_EQ(jsArgAsInt(args, 1), anInt);
  ABI50_0_0EXPECT_EQ(jsArgAsDouble(args, 2), aDouble);
  ABI50_0_0EXPECT_EQ(jsArgAsString(args, 3), aString);
  ABI50_0_0EXPECT_EQ(jsArgAsArray(args, 4), anArray);
  ABI50_0_0EXPECT_EQ(jsArgAsObject(args, 5), anObject);

  // const args
  const folly::dynamic& cargs = args;
  const folly::dynamic& a4 = jsArgAsArray(cargs, 4);
  ABI50_0_0EXPECT_EQ(a4, anArray);
  ABI50_0_0EXPECT_EQ(jsArgAsObject(cargs, 5), anObject);

  // helpers returning dynamic should return same object without copying
  ABI50_0_0EXPECT_EQ(&jsArgAsArray(args, 4), &(args[4]));
  ABI50_0_0EXPECT_EQ(&jsArgAsArray(cargs, 4), &(args[4]));
  ...
  ABI50_0_0EXPECT_EQ(jsArgN(args, 3, &folly::dynamic::getString), aString);
  ABI50_0_0EXPECT_EQ(jsArg(args[3], &folly::dynamic::getString), aString);

  // conversions
  ABI50_0_0EXPECT_EQ(jsArgAsDouble(args, 1), anInt * 1.0);
  ABI50_0_0EXPECT_EQ(jsArgAsString(args, 1), aNumericString);
  ABI50_0_0EXPECT_EQ(jsArgAsInt(args, 6), anInt);

  // Test exception messages.

  // out_of_range
  ABI50_0_0EXPECT_JSAE(
      jsArgAsBool(args, 7),
  ...
  try {
    ...
    ABI50_0_0EXPECT_GT(exwhat.size(), exhead.size());
    ABI50_0_0EXPECT_GT(exwhat.size(), extail.size());

    ABI50_0_0EXPECT_TRUE(std::equal(exhead.cbegin(), exhead.cend(), exwhat.cbegin()))
        << "JsArgumentException('" << exwhat << "') does not begin with '"
        << exhead << "'";
    ABI50_0_0EXPECT_TRUE(std::equal(extail.crbegin(), extail.crend(), exwhat.crbegin()))
