...
public struct UpdatesModuleConstants {
  ...
  /**
   ...
   can be used by `expo-asset` or a similar module to override ABI50_0_0React Native's asset resolution and
...
@objc(ABI50_0_0EXUpdatesAppControllerInterface)
public protocol AppControllerInterface {
  /**
   The ABI50_0_0RCTBridge for which ABI50_0_0EXUpdates is providing the JS bundle and assets.
  ...
  /**
   Delegate which will be notified when ABI50_0_0EXUpdates has an update ready to launch and
   `launchAssetUrl` is nonnull.
   */
  @objc weak var delegate: AppControllerDelegate? { get set }

  /**
   The URL on disk to source asset for the ABI50_0_0RCTBridge.
   Will be null until the AppController delegate method is called.
   This should be provided in the `sourceURLForBridge:` method of ABI50_0_0RCTBridgeDelegate.
   */
...
@objc(ABI50_0_0EXUpdatesAppControllerDelegate)
 ...
 *
 * This class also optionally holds a reference to the app's [ABI50_0_0ReactNativeHost], which allows
 * expo-updates to reload JS and send events through the bridge.
 */
@objc(ABI50_0_0EXUpdatesAppController)
@objcMembers
