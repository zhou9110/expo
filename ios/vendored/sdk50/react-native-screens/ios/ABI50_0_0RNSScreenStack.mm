...
#import <ABI50_0_0React/ABI50_0_0RCTMountingTransactionObserving.h>
...
#import "ABI50_0_0RNSScreenStackAnimator.h"
...
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
    ,
    ABI50_0_0RCTMountingTransactionObserving
#endif
    >
...
@implementation ABI50_0_0RNSNavigationController
...
- (void)viewDidLayoutSubviews
{
  ...
  if ([self.topViewController isKindOfClass:[ABI50_0_0RNSScreen class]]) {
    ABI50_0_0RNSScreen *screenController = (ABI50_0_0RNSScreen *)self.topViewController;
    ...
    // If ABI50_0_0RNSScreen includes a navigation controller of type ABI50_0_0RNSNavigationController, it should not calculate
...
#if !TARGET_OS_TV
@interface ABI50_0_0RNSScreenEdgeGestureRecognizer : UIScreenEdgePanGestureRecognizer
@end

@implementation ABI50_0_0RNSScreenEdgeGestureRecognizer
@end

@interface ABI50_0_0RNSPanGestureRecognizer : UIPanGestureRecognizer
@end

@implementation ABI50_0_0RNSPanGestureRecognizer
@end
#endif

@implementation ABI50_0_0RNSScreenStackView {
  UINavigationController *_controller;
  NSMutableArray<ABI50_0_0RNSScreenView *> *_ABI50_0_0ReactSubviews;
  BOOL _invalidated;
  BOOL _isFullWidthSwiping;
  UIPercentDrivenInteractiveTransition *_interactionController;
  BOOL _hasLayout;
  __weak ABI50_0_0RNSScreenStackManager *_manager;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI50_0_0React::ABI50_0_0RNSScreenStackProps>();
    _props = defaultProps;
...
- (instancetype)initWithManager:(ABI50_0_0RNSScreenStackManager *)manager
{
  if (self = [super init]) {
    _hasLayout = NO;
    _invalidated = NO;
...
- (void)initCommonProps
{
  ...
  _controller = [ABI50_0_0RNSNavigationController new];
...
- (BOOL)shouldCancelDismissFromView:(ABI50_0_0RNSScreenView *)fromView toView:(ABI50_0_0RNSScreenView *)toView
{
  int fromIndex = (int)[_ABI50_0_0ReactSubviews indexOfObject:fromView];
  int toIndex = (int)[_ABI50_0_0ReactSubviews indexOfObject:toView];
  for (int i = fromIndex; i > toIndex; i--) {
    if (_ABI50_0_0ReactSubviews[i].preventNativeDismiss) {
      return YES;
      break;
    }
  }
...
- (void)emitOnFinishTransitioningEvent
{
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const ABI50_0_0React::ABI50_0_0RNSScreenStackEventEmitter>(_eventEmitter)
        ->onFinishTransitioning(ABI50_0_0React::ABI50_0_0RNSScreenStackEventEmitter::OnFinishTransitioning{});
  }
#else
                    ...
                    animated:(BOOL)animated
{
  ...
  for (UIView *subview in view.ABI50_0_0ReactSubviews) {
    if ([subview isKindOfClass:[ABI50_0_0RNSScreenStackHeaderConfig class]]) {
      config = (ABI50_0_0RNSScreenStackHeaderConfig *)subview;
      break;
    }
  }
  [ABI50_0_0RNSScreenStackHeaderConfig willShowViewController:viewController animated:animated withConfig:config];
}
...
- (void)presentationControllerDidDismiss:(UIPresentationController *)presentationController
{
  ...
  if ([presentationController.presentedViewController isKindOfClass:[ABI50_0_0RNSScreen class]]) {
    ...
    if (self.onFinishTransitioning) {
      ...
      // finish event to arrive later than onDismiss (see ABI50_0_0RNSScreen#notifyDismiss)
...
- (void)setModalViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  for (NSUInteger i = changeRootIndex; i < controllers.count; i++) {
    if ([_presentedModals containsObject:controllers[i]]) {
      ABI50_0_0RCTAssert(false, @"Modally presented controllers are being reshuffled, this is not allowed");
    }
  }

  __weak ABI50_0_0RNSScreenStackView *weakSelf = self;
  ...
  void (^finish)(void) = ^{
    ...
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
        ...
        BOOL shouldAnimate = lastModal && [next isKindOfClass:[ABI50_0_0RNSScreen class]] &&
            ((ABI50_0_0RNSScreen *)next).screenView.stackAnimation != ABI50_0_0RNSScreenStackAnimationNone;
  ...
  if (changeRootController.presentedViewController != nil &&
    ...
    BOOL shouldAnimate = changeRootIndex == controllers.count &&
        [changeRootController.presentedViewController isKindOfClass:[ABI50_0_0RNSScreen class]] &&
        ((ABI50_0_0RNSScreen *)changeRootController.presentedViewController).screenView.stackAnimation !=
...
- (void)setPushViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  BOOL firstTimePush = ![previousTop isKindOfClass:[ABI50_0_0RNSScreen class]];
  ...
  if (firstTimePush) {
    ...
    if (![controllers containsObject:previousTop]) {
      ...
      if (![_controller.viewControllers containsObject:top] &&
          ((ABI50_0_0RNSScreenView *)top.view).replaceAnimation == ABI50_0_0RNSScreenReplaceAnimationPush) {
        // setting new controllers with animation does `push` animation by default
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
        auto screenController = (ABI50_0_0RNSScreen *)top;
  ...
  for (ABI50_0_0RNSScreenView *screen in _ABI50_0_0ReactSubviews) {
    if (!screen.dismissed && screen.controller != nil) {
      if (pushControllers.count == 0) {
        ...
        if (screen.stackPresentation == ABI50_0_0RNSScreenStackPresentationPush) {
          [pushControllers addObject:screen.controller];
        } else {
...
{
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
  if ([_controller.visibleViewController isKindOfClass:[ABI50_0_0RNSScreen class]]) {
    [(ABI50_0_0RNSScreen *)_controller.visibleViewController resetViewToScreen];
  }
#else
  dispatch_async(dispatch_get_main_queue(), ^{
    [self invalidate];
...
{
  ABI50_0_0RNSScreenView *screen;
  if (operation == UINavigationControllerOperationPush) {
    screen = ((ABI50_0_0RNSScreen *)toVC).screenView;
  } else if (operation == UINavigationControllerOperationPop) {
    screen = ((ABI50_0_0RNSScreen *)fromVC).screenView;
  }
  BOOL shouldCancelDismiss = [self shouldCancelDismissFromView:(ABI50_0_0RNSScreenView *)fromVC.view
                                                        toView:(ABI50_0_0RNSScreenView *)toVC.view];
  if (screen != nil &&
      ...
      ((operation == UINavigationControllerOperationPop && shouldCancelDismiss) || _isFullWidthSwiping ||
       [ABI50_0_0RNSScreenStackAnimator isCustomAnimation:screen.stackAnimation])) {
    return [[ABI50_0_0RNSScreenStackAnimator alloc] initWithOperation:operation];
  }
  return nil;
}
...
- (void)cancelTouchesInParent
{
  ...
  if (parent != nil) {
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
    ABI50_0_0RCTSurfaceTouchHandler *touchHandler = [parent performSelector:@selector(touchHandler)];
#else
    ABI50_0_0RCTTouchHandler *touchHandler = [parent performSelector:@selector(touchHandler)];
#endif
...
{
  ABI50_0_0RNSScreenView *topScreen = _ABI50_0_0ReactSubviews.lastObject;

#if TARGET_OS_TV
  [self cancelTouchesInParent];
  return YES;
#else
  // ABI50_0_0RNSPanGestureRecognizer will receive events iff topScreen.fullScreenSwipeEnabled == YES;
  // Events are filtered in gestureRecognizer:shouldReceivePressOrTouchEvent: method
  if ([gestureRecognizer isKindOfClass:[ABI50_0_0RNSPanGestureRecognizer class]]) {
  ...
  // Now we're dealing with ABI50_0_0RNSScreenEdgeGestureRecognizer (or _UIParallaxTransitionPanGestureRecognizer)
  if (topScreen.customAnimationOnSwipe && [ABI50_0_0RNSScreenStackAnimator isCustomAnimation:topScreen.stackAnimation]) {
    if ([gestureRecognizer isKindOfClass:[ABI50_0_0RNSScreenEdgeGestureRecognizer class]]) {
      ...
      BOOL isCorrectEdge = (_controller.view.semanticContentAttribute == UISemanticContentAttributeForceRightToLeft &&
                            ((ABI50_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeRight) ||
          (_controller.view.semanticContentAttribute != UISemanticContentAttributeForceRightToLeft &&
           ((ABI50_0_0RNSScreenEdgeGestureRecognizer *)gestureRecognizer).edges == UIRectEdgeLeft);
      if (isCorrectEdge) {
        [self cancelTouchesInParent];
        return YES;
      }
...
- (void)setupGestureHandlers
{
  ...
  ABI50_0_0RNSScreenEdgeGestureRecognizer *leftEdgeSwipeGestureRecognizer =
      [[ABI50_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI50_0_0RNSScreenEdgeGestureRecognizer *rightEdgeSwipeGestureRecognizer =
      [[ABI50_0_0RNSScreenEdgeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipe:)];
  ...
  ABI50_0_0RNSPanGestureRecognizer *panRecognizer = [[ABI50_0_0RNSPanGestureRecognizer alloc] initWithTarget:self
...
- (void)handleSwipe:(UIPanGestureRecognizer *)gestureRecognizer
{
  ...
  float velocity;
  float distance;
  if (topScreen.swipeDirection == ABI50_0_0RNSScreenSwipeDirectionVertical) {
...
{
  ABI50_0_0RNSScreenView *fromView = [_controller.transitionCoordinator viewForKey:UITransitionContextFromViewKey];
  ABI50_0_0RNSScreenView *toView = [_controller.transitionCoordinator viewForKey:UITransitionContextToViewKey];
  // we can intercept clicking back button here, we check ABI50_0_0ReactSuperview since this method also fires when
  // navigating back from JS
  if (_interactionController == nil && fromView.ABI50_0_0ReactSuperview) {
    ...
    if (shouldCancelDismiss) {
      ...
      dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        int fromIndex = (int)[self->_ABI50_0_0ReactSubviews indexOfObject:fromView];
        int toIndex = (int)[self->_ABI50_0_0ReactSubviews indexOfObject:toView];
...
- (BOOL)isInGestureResponseDistance:(UIGestureRecognizer *)gestureRecognizer topScreen:(ABI50_0_0RNSScreenView *)topScreen
{
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  if (CGRectContainsPoint(_controller.navigationBar.frame, point)) {
    ...
    UIView *headerConfig = [[_ABI50_0_0ReactSubviews.lastObject ABI50_0_0ReactSubviews] firstObject];
    if ([headerConfig isKindOfClass:[ABI50_0_0RNSScreenStackHeaderConfig class]]) {
...
// ABI50_0_0RNSScreenStackView is a UIGestureRecognizerDelegate for three types of gesture recognizers:
// ABI50_0_0RNSPanGestureRecognizer, ABI50_0_0RNSScreenEdgeGestureRecognizer, _UIParallaxTransitionPanGestureRecognizer
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceivePressOrTouchEvent:(NSObject *)event
{
  ...
  if (![topScreen isKindOfClass:[ABI50_0_0RNSScreenView class]] || !topScreen.gestureEnabled ||
      _controller.viewControllers.count < 2 || [topScreen isModal]) {
    return NO;
  }

  // We want to pass events to ABI50_0_0RNSPanGestureRecognizer iff full screen swipe is enabled.
  if ([gestureRecognizer isKindOfClass:[ABI50_0_0RNSPanGestureRecognizer class]]) {
    return topScreen.fullScreenSwipeEnabled;
  }

  // ABI50_0_0RNSScreenEdgeGestureRecognizer || _UIParallaxTransitionPanGestureRecognizer
  return YES;
}
    ...
    shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
  if ([gestureRecognizer isKindOfClass:[ABI50_0_0RNSPanGestureRecognizer class]] &&
    ...
    ABI50_0_0RNSPanGestureRecognizer *panGestureRecognizer = (ABI50_0_0RNSPanGestureRecognizer *)gestureRecognizer;
...
{
  if (![subview isKindOfClass:[ABI50_0_0RNSScreenView class]]) {
    ABI50_0_0RCTLogError(@"ScreenStack only accepts children of type Screen");
    return;
  }
...
- (void)mountChildComponentView:(UIView<ABI50_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  ...
  [_ABI50_0_0ReactSubviews insertObject:(ABI50_0_0RNSScreenView *)childComponentView atIndex:index];
  ((ABI50_0_0RNSScreenView *)childComponentView).ABI50_0_0ReactSuperview = self;
  dispatch_async(dispatch_get_main_queue(), ^{
    [self maybeAddToParentAndUpdateContainer];
  });
}
...
{
  ABI50_0_0RNSScreenView *screenChildComponent = (ABI50_0_0RNSScreenView *)childComponentView;
  ...
  ABI50_0_0RCTAssert(
      screenChildComponent.ABI50_0_0ReactSuperview == self,
  ...
  ABI50_0_0RCTAssert(
      ...
      screenChildComponent,
      @(index),
      @([_ABI50_0_0ReactSubviews indexOfObject:screenChildComponent]),
      @([[_ABI50_0_0ReactSubviews objectAtIndex:index] tag]));
  screenChildComponent.ABI50_0_0ReactSuperview = nil;
  [_ABI50_0_0ReactSubviews removeObject:screenChildComponent];
...
- (void)mountingTransactionWillMount:(ABI50_0_0React::MountingTransaction const &)transaction
                withSurfaceTelemetry:(ABI50_0_0React::SurfaceTelemetry const &)surfaceTelemetry
{
  for (auto &mutation : transaction.getMutations()) {
    if (mutation.type == ABI50_0_0React::ShadowViewMutation::Type::Remove && mutation.parentShadowView.componentName != nil &&
        strcmp(mutation.parentShadowView.componentName, "ABI50_0_0RNSScreenStack") == 0) {
      [self takeSnapshot];
      return;
    }
  }
...
{
  return ABI50_0_0React::concreteComponentDescriptorProvider<ABI50_0_0React::ABI50_0_0RNSScreenStackComponentDescriptor>();
}
#else
...
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
Class<ABI50_0_0RCTComponentViewProtocol> ABI50_0_0RNSScreenStackCls(void)
{
  return ABI50_0_0RNSScreenStackView.class;
}
#endif

@implementation ABI50_0_0RNSScreenStackManager {
  NSPointerArray *_stacks;
}

ABI50_0_0RCT_EXPORT_MODULE()

ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(onFinishTransitioning, ABI50_0_0RCTDirectEventBlock);

#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
#else
- (UIView *)view
{
  ABI50_0_0RNSScreenStackView *view = [[ABI50_0_0RNSScreenStackView alloc] initWithManager:self];
  if (!_stacks) {
...
- (void)invalidate
{
  for (ABI50_0_0RNSScreenStackView *stack in _stacks) {
