...
public:
  JsiDependencyManager(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context,
                       jsi::Runtime &runtime,
  ...
  JSI_HOST_FUNCTION(subscribeNode) {
    ...
    std::vector<
        std::pair<std::shared_ptr<ABI50_0_0RNSkReadonlyValue>, std::function<void()>>>
        unsubscribers;
    ...
    node->getPropsContainer()->enumerateMappedProps(
        [&](const PropId key, const std::vector<NodeProp *> &propMapping) {
          ...
          if (isAnimatedValue(nativeValue)) {
            ...
            auto animatedValue = std::dynamic_pointer_cast<ABI50_0_0RNSkReadonlyValue>(
  ...
  JSI_HOST_FUNCTION(update) {
    ...
    std::set<std::shared_ptr<ABI50_0_0RNSkReadonlyValue>> uniqueValues;
  ...
  */
  ...
  createCtor(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context) {
...
private:
  ...
  /**
   ...
   ABI50_0_0RNSkReadonlyValue.
   */
  bool isAnimatedValue(JsiValue &value) {
    return value.getType() == PropType::HostObject &&
           std::dynamic_pointer_cast<ABI50_0_0RNSkReadonlyValue>(
               value.getAsHostObject()) != nullptr;
  }

  /**
   Returns the ABI50_0_0RNSkReadonlyValue pointer for a value that is an Animated value
   */
  std::shared_ptr<ABI50_0_0RNSkReadonlyValue> getAnimatedValue(JsiValue &value) {
    return std::dynamic_pointer_cast<ABI50_0_0RNSkReadonlyValue>(
  ...
  std::map<JsiDomNode *,
           std::vector<std::pair<std::shared_ptr<ABI50_0_0RNSkReadonlyValue>,
                                 std::function<void()>>>>
      _subscriptions;
};
} // namespace ABI50_0_0RNSkia
