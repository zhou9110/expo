...
export function linkTo(
  ...
  href: string,
  event?: string,
  { unstable_ignoreAnchor: ignoreAnchor }: LinkToOptions = {}
) {
...
function getNavigateAction(
  ...
  while (actionStateRoute) {
    ...
    if (!ignoreAnchor && hasAnchorRoute) {
      /* 
        ...
        We can't just setup `initial = false` on every route, as React Navigation considers the `index` route the initial route.
        The `hasAnchorRoute` check ensures that we only set `initial = false` when the user has specified an anchor route.
      */
