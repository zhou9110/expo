...
@implementation ABI50_0_0RNSVGSvgView {
  NSMutableDictionary<NSString *, ABI50_0_0RNSVGNode *> *_clipPaths;
  NSMutableDictionary<NSString *, ABI50_0_0RNSVGNode *> *_templates;
  NSMutableDictionary<NSString *, ABI50_0_0RNSVGPainter *> *_painters;
  NSMutableDictionary<NSString *, ABI50_0_0RNSVGNode *> *_markers;
  NSMutableDictionary<NSString *, ABI50_0_0RNSVGNode *> *_masks;
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    rendered = false;
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
    static const auto defaultProps = std::make_shared<const ABI50_0_0RNSVGSvgViewProps>();
    _props = defaultProps;
...
{
  return concreteComponentDescriptorProvider<ABI50_0_0RNSVGSvgViewComponentDescriptor>();
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  const auto &newProps = static_cast<const ABI50_0_0RNSVGSvgViewProps &>(*props);

  self.minX = newProps.minX;
  self.minY = newProps.minY;
  self.vbWidth = newProps.vbWidth;
  self.vbHeight = newProps.vbHeight;
  self.bbWidth = [ABI50_0_0RNSVGLength lengthWithString:ABI50_0_0RCTNSStringFromString(newProps.bbWidth)];
  self.bbHeight = [ABI50_0_0RNSVGLength lengthWithString:ABI50_0_0RCTNSStringFromString(newProps.bbHeight)];
  self.align = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newProps.align);
  self.meetOrSlice = intToRNSVGVBMOS(newProps.meetOrSlice);
  if (ABI50_0_0RCTUIColorFromSharedColor(newProps.tintColor)) {
    self.tintColor = ABI50_0_0RCTUIColorFromSharedColor(newProps.tintColor);
  }
  if (ABI50_0_0RCTUIColorFromSharedColor(newProps.color)) {
    self.tintColor = ABI50_0_0RCTUIColorFromSharedColor(newProps.color);
  }
...
- (void)invalidate
{
  ABI50_0_0RNSVGPlatformView *parent = self.superview;
  if ([parent isKindOfClass:[ABI50_0_0RNSVGNode class]]) {
    if (!rendered) {
      return;
    }
    ABI50_0_0RNSVGNode *svgNode = (ABI50_0_0RNSVGNode *)parent;
    [svgNode invalidate];
    rendered = false;
    return;
  }
...
- (void)setBbWidth:(ABI50_0_0RNSVGLength *)bbWidth
{
...
- (void)setBbHeight:(ABI50_0_0RNSVGLength *)bbHeight
{
...
- (void)drawToContext:(CGContextRef)context withRect:(CGRect)rect
{
  ...
  if (self.align) {
    ...
    _viewBoxTransform = [ABI50_0_0RNSVGViewBox getTransform:tRect eRect:rect align:self.align meetOrSlice:self.meetOrSlice];
  ...
  for (ABI50_0_0RNSVGView *node in self.subviews) {
    if ([node isKindOfClass:[ABI50_0_0RNSVGNode class]]) {
      ABI50_0_0RNSVGNode *svg = (ABI50_0_0RNSVGNode *)node;
...
- (ABI50_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  for (ABI50_0_0RNSVGNode *node in [self.subviews reverseObjectEnumerator]) {
    if (![node isKindOfClass:[ABI50_0_0RNSVGNode class]]) {
      continue;
    }
    ...
    ABI50_0_0RNSVGPlatformView *hitChild = [node hitTest:transformed withEvent:event];
...
- (void)defineClipPath:(__kindof ABI50_0_0RNSVGNode *)clipPath clipPathName:(NSString *)clipPathName
{
  if (!_clipPaths) {
...
- (ABI50_0_0RNSVGNode *)getDefinedClipPath:(NSString *)clipPathName
{
  return _clipPaths ? [_clipPaths objectForKey:clipPathName] : nil;
}

- (void)defineTemplate:(ABI50_0_0RNSVGNode *)definedTemplate templateName:(NSString *)templateName
{
  if (!_templates) {
...
- (ABI50_0_0RNSVGNode *)getDefinedTemplate:(NSString *)templateName
{
  return _templates ? [_templates objectForKey:templateName] : nil;
}

- (void)definePainter:(ABI50_0_0RNSVGPainter *)painter painterName:(NSString *)painterName
{
  if (!_painters) {
...
- (ABI50_0_0RNSVGPainter *)getDefinedPainter:(NSString *)painterName;
{
  return _painters ? [_painters objectForKey:painterName] : nil;
}

- (void)defineMarker:(ABI50_0_0RNSVGNode *)marker markerName:(NSString *)markerName
{
  if (!_markers) {
...
- (ABI50_0_0RNSVGNode *)getDefinedMarker:(NSString *)markerName;
{
  return _markers ? [_markers objectForKey:markerName] : nil;
}

- (void)defineMask:(ABI50_0_0RNSVGNode *)mask maskName:(NSString *)maskName
{
  if (!_masks) {
...
- (ABI50_0_0RNSVGNode *)getDefinedMask:(NSString *)maskName;
{
...
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
Class<ABI50_0_0RCTComponentViewProtocol> ABI50_0_0RNSVGSvgViewCls(void)
{
  return ABI50_0_0RNSVGSvgView.class;
}
#endif // ABI50_0_0RCT_NEW_ARCH_ENABLED
