...
using ABI50_0_0RNSkViewInfo = struct ABI50_0_0RNSkViewInfo {
  ABI50_0_0RNSkViewInfo() { view = nullptr; }
  std::shared_ptr<ABI50_0_0RNSkView> view;
  std::unordered_map<std::string, ABI50_0_0RNJsi::JsiValueWrapper> props;
};

class ABI50_0_0RNSkJsiViewApi : public ABI50_0_0RNJsi::JsiHostObject,
                       public std::enable_shared_from_this<ABI50_0_0RNSkJsiViewApi> {
public:
  /**
  ...
  JSI_HOST_FUNCTION(setJsiProperty) {
    ...
    info->props.insert_or_assign(arguments[1].asString(runtime).utf8(runtime),
                                 ABI50_0_0RNJsi::JsiValueWrapper(runtime, arguments[2]));
  ...
  JSI_HOST_FUNCTION(registerValuesInView) {
    ...
    for (size_t i = 0; i < size; ++i) {
      auto value = values.getValueAtIndex(runtime, i)
                       .asObject(runtime)
                       .asHostObject<ABI50_0_0RNSkReadonlyValue>(runtime);
  ...
  JSI_EXPORT_FUNCTIONS(JSI_EXPORT_FUNC(ABI50_0_0RNSkJsiViewApi, setJsiProperty),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkJsiViewApi, callJsiMethod),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkJsiViewApi, registerValuesInView),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkJsiViewApi, requestRedraw),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkJsiViewApi, makeImageSnapshot))

  /**
   * Constructor
   * @param platformContext Platform context
   */
  explicit ABI50_0_0RNSkJsiViewApi(std::shared_ptr<ABI50_0_0RNSkPlatformContext> platformContext)
...
private:
   ...
   */
  ABI50_0_0RNSkViewInfo *getEnsuredViewInfo(size_t nativeId) {
    if (_viewInfos.count(nativeId) == 0) {
      ABI50_0_0RNSkViewInfo info;
      std::lock_guard<std::mutex> lock(_mutex);
  ...
  std::unordered_map<size_t, ABI50_0_0RNSkViewInfo> _viewInfos;
