...
#import "ABI50_0_0RCTFont.h"
#import "ABI50_0_0RCTAssert.h"
#import "ABI50_0_0RCTLog.h"

#import <CoreText/CoreText.h>

typedef CGFloat ABI50_0_0RCTFontWeight;
static ABI50_0_0RCTFontWeight weightOfFont(UIFont *font)
{
  ...
  for (NSString *suffix in weightSuffixes) {
    ...
    if (CFStringFind((CFStringRef)fontName, (CFStringRef)suffix, options).location != kCFNotFound) {
      return (ABI50_0_0RCTFontWeight)fontWeights[i].doubleValue;
    }
    i++;
  }

  auto traits = (__bridge_transfer NSDictionary *)CTFontCopyTraits((CTFontRef)font);
  return (ABI50_0_0RCTFontWeight)[traits[UIFontWeightTrait] doubleValue];
}
...
static ABI50_0_0RCTFontHandler defaultFontHandler;

void ABI50_0_0RCTSetDefaultFontHandler(ABI50_0_0RCTFontHandler handler)
{
  defaultFontHandler = handler;
}

BOOL ABI50_0_0RCTHasFontHandlerSet()
{
...
static NSString *FontWeightDescriptionFromUIFontWeight(UIFontWeight fontWeight)
{
  ...
  ABI50_0_0RCTAssert(NO, @"Unknown UIFontWeight passed in: %f", fontWeight);
  return @"regular";
}

static UIFont *cachedSystemFont(CGFloat size, ABI50_0_0RCTFontWeight weight)
{
  static NSCache<NSValue *, UIFont *> *fontCache = [NSCache new];

  struct __attribute__((__packed__)) CacheKey {
    CGFloat size;
    ABI50_0_0RCTFontWeight weight;
  };
...
@implementation ABI50_0_0RCTConvert (ABI50_0_0RCTFont)

+ (UIFont *)UIFont:(id)json
{
  json = [self NSDictionary:json];
  return [ABI50_0_0RCTFont updateFont:nil
                  withFamily:[ABI50_0_0RCTConvert NSString:json[@"fontFamily"]]
                        size:[ABI50_0_0RCTConvert NSNumber:json[@"fontSize"]]
                      weight:[ABI50_0_0RCTConvert NSString:json[@"fontWeight"]]
                       style:[ABI50_0_0RCTConvert NSString:json[@"fontStyle"]]
                     variant:[ABI50_0_0RCTConvert NSStringArray:json[@"fontVariant"]]
             scaleMultiplier:1];
}

ABI50_0_0RCT_ENUM_CONVERTER(
    ABI50_0_0RCTFontWeight,
    (@{
...
typedef BOOL ABI50_0_0RCTFontStyle;
ABI50_0_0RCT_ENUM_CONVERTER(
    ABI50_0_0RCTFontStyle,
    (@{
      @"normal" : @NO,
      @"italic" : @YES,
      @"oblique" : @YES,
...
typedef NSDictionary ABI50_0_0RCTFontVariantDescriptor;
+ (ABI50_0_0RCTFontVariantDescriptor *)ABI50_0_0RCTFontVariantDescriptor:(id)json
{
  ...
  ABI50_0_0RCTFontVariantDescriptor *value = mapping[json];
  if (ABI50_0_0RCT_DEBUG && !value && [json description].length > 0) {
    ABI50_0_0RCTLogInfo(
        @"Invalid ABI50_0_0RCTFontVariantDescriptor '%@'. should be one of: %@",
        json,
...
ABI50_0_0RCT_ARRAY_CONVERTER(ABI50_0_0RCTFontVariantDescriptor)

@end

@implementation ABI50_0_0RCTFont
...
+ (UIFont *)updateFont:(UIFont *)font
            withFamily:(NSString *)family
               ...
               variant:(NSArray<ABI50_0_0RCTFontVariantDescriptor *> *)variant
       scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  const ABI50_0_0RCTFontWeight defaultFontWeight = UIFontWeightRegular;
  const CGFloat defaultFontSize = 14;

  // Initialize properties to defaults
  CGFloat fontSize = defaultFontSize;
  ABI50_0_0RCTFontWeight fontWeight = defaultFontWeight;
  ...
  // Get font attributes
  fontSize = [ABI50_0_0RCTConvert CGFloat:size] ?: fontSize;
  if (scaleMultiplier > 0.0 && scaleMultiplier != 1.0) {
    fontSize = round(fontSize * scaleMultiplier);
  }
  familyName = [ABI50_0_0RCTConvert NSString:family] ?: familyName;
  isItalic = style ? [ABI50_0_0RCTConvert ABI50_0_0RCTFontStyle:style] : isItalic;
  fontWeight = weight ? [ABI50_0_0RCTConvert ABI50_0_0RCTFontWeight:weight] : fontWeight;
  ...
  if (!didFindFont && fontNamesForFamilyName(familyName).count == 0) {
    ...
    if (font) {
      ...
      ABI50_0_0RCTLogInfo(@"Unrecognized font family '%@'", familyName);
  ...
  if (variant) {
    NSArray *fontFeatures = [ABI50_0_0RCTConvert ABI50_0_0RCTFontVariantDescriptorArray:variant];
    UIFontDescriptor *fontDescriptor = [font.fontDescriptor
