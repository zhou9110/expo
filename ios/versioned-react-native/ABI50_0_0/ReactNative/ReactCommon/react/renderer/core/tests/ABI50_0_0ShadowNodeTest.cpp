...
TEST_F(ShadowNodeTest, handleShadowNodeCreation) {
  ABI50_0_0EXPECT_FALSE(nodeZ_->getSealed());
  ABI50_0_0EXPECT_STREQ(nodeZ_->getComponentName(), "Test");
  ABI50_0_0EXPECT_EQ(nodeZ_->getTag(), 18);
  ABI50_0_0EXPECT_EQ(nodeZ_->getSurfaceId(), surfaceId_);
  ABI50_0_0EXPECT_NE(nodeZ_->getEventEmitter(), nullptr);
  ABI50_0_0EXPECT_EQ(nodeZ_->getChildren().size(), 0);
}

TEST_F(ShadowNodeTest, handleSealRecusive) {
  nodeZ_->sealRecursive();
  ABI50_0_0EXPECT_TRUE(nodeZ_->getSealed());
  ABI50_0_0EXPECT_TRUE(nodeZ_->getProps()->getSealed());
}
...
TEST_F(ShadowNodeTest, handleShadowNodeSimpleCloning) {
  ...
  ABI50_0_0EXPECT_STREQ(nodeA_->getComponentName(), nodeARevision2->getComponentName());
  ABI50_0_0EXPECT_EQ(nodeA_->getTag(), nodeARevision2->getTag());
  ABI50_0_0EXPECT_EQ(nodeA_->getSurfaceId(), nodeARevision2->getSurfaceId());
  ABI50_0_0EXPECT_EQ(nodeA_->getEventEmitter(), nodeARevision2->getEventEmitter());
}

TEST_F(ShadowNodeTest, handleShadowNodeMutation) {
  auto nodeABChildren = nodeAB_->getChildren();
  ABI50_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI50_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABA_);
  ABI50_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);
  ...
  ABI50_0_0EXPECT_EQ(nodeABChildren.size(), 2);
  ABI50_0_0EXPECT_EQ(nodeABChildren.at(0), nodeABArevision2);
  ABI50_0_0EXPECT_EQ(nodeABChildren.at(1), nodeABB_);

  // Seal the entire tree.
  nodeAB_->sealRecursive();
  ABI50_0_0EXPECT_TRUE(nodeAB_->getSealed());
  ABI50_0_0EXPECT_TRUE(nodeABArevision2->getSealed());
  ABI50_0_0EXPECT_TRUE(nodeABB_->getSealed());
}
...
TEST_F(ShadowNodeTest, handleCloneFunction) {
  ...
  ABI50_0_0EXPECT_NE(nodeAB_, nodeABClone);
  ...
  ABI50_0_0EXPECT_EQ(nodeAB_->getTag(), nodeABClone->getTag());
  ABI50_0_0EXPECT_EQ(nodeAB_->getSurfaceId(), nodeABClone->getSurfaceId());
  ABI50_0_0EXPECT_EQ(nodeAB_->getProps(), nodeABClone->getProps());
}
...
TEST_F(ShadowNodeTest, handleState) {
  ...
  ABI50_0_0EXPECT_EQ(firstNode->getState(), secondNode->getState());
  ABI50_0_0EXPECT_NE(firstNode->getState(), thirdNode->getState());
  secondNode->setStateData(TestState());
  ABI50_0_0EXPECT_NE(firstNode->getState(), secondNode->getState());
