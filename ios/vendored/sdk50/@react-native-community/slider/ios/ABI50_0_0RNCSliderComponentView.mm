...
#import "ABI50_0_0RNCSliderComponentView.h"

#import <ABI50_0_0React/ABI50_0_0RCTConversions.h>

#import <react/renderer/components/ABI50_0_0RNCSlider/ComponentDescriptors.h>
#import <react/renderer/components/ABI50_0_0RNCSlider/EventEmitters.h>
#import <react/renderer/components/ABI50_0_0RNCSlider/Props.h>
#import <react/renderer/components/ABI50_0_0RNCSlider/ABI50_0_0RCTComponentViewHelpers.h>
#import <ABI50_0_0React/ABI50_0_0RCTBridge+Private.h>
#import "ABI50_0_0RCTImagePrimitivesConversions.h"
#import <ABI50_0_0React/ABI50_0_0RCTImageLoaderProtocol.h>
#import "ABI50_0_0RCTFabricComponentsPlugins.h"
#import "ABI50_0_0RNCSlider.h"

using namespace ABI50_0_0facebook::ABI50_0_0React;

@interface ABI50_0_0RNCSliderComponentView () <ABI50_0_0RCTRNCSliderViewProtocol>

@end


@implementation ABI50_0_0RNCSliderComponentView
{
    ABI50_0_0RNCSlider *slider;
    UIImage *_image;
    BOOL _isSliding;
}
...
{
    return concreteComponentDescriptorProvider<ABI50_0_0RNCSliderComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        static const auto defaultProps = std::make_shared<const ABI50_0_0RNCSliderProps>();
        _props = defaultProps;
        slider = [[ABI50_0_0RNCSlider alloc] initWithFrame:self.bounds];
...
- (void)tapHandler:(UITapGestureRecognizer *)gesture {
    if ([gesture.view class] != [ABI50_0_0RNCSlider class]) {
        return;
    }
    ABI50_0_0RNCSlider *slider = (ABI50_0_0RNCSlider *)gesture.view;
    ...
    std::dynamic_pointer_cast<const ABI50_0_0RNCSliderEventEmitter>(_eventEmitter)
    ->onRNCSliderSlidingStart(ABI50_0_0RNCSliderEventEmitter::OnRNCSliderSlidingStart{.value = static_cast<Float>(slider.lastValue)});
    
    // Trigger onValueChange to address https://github.com/react-native-community/react-native-slider/issues/212
    std::dynamic_pointer_cast<const ABI50_0_0RNCSliderEventEmitter>(_eventEmitter)
    ->onRNCSliderValueChange(ABI50_0_0RNCSliderEventEmitter::OnRNCSliderValueChange{.value = static_cast<Float>(slider.value)});
    
    std::dynamic_pointer_cast<const ABI50_0_0RNCSliderEventEmitter>(_eventEmitter)
    ->onRNCSliderSlidingComplete(ABI50_0_0RNCSliderEventEmitter::OnRNCSliderSlidingComplete{.value = static_cast<Float>(slider.value)});
}

- (void)sliderValueChanged:(ABI50_0_0RNCSlider *)sender
{
    [self ABI50_0_0RNCSendSliderEvent:sender withContinuous:YES isSlidingStart:NO];
}

- (void)sliderTouchStart:(ABI50_0_0RNCSlider *)sender
{
    [self ABI50_0_0RNCSendSliderEvent:sender withContinuous:NO isSlidingStart:YES];
    _isSliding = YES;
    sender.isSliding = YES;
}

- (void)sliderTouchEnd:(ABI50_0_0RNCSlider *)sender
{
    [self ABI50_0_0RNCSendSliderEvent:sender withContinuous:NO isSlidingStart:NO];
    sender.isSliding = NO;
    _isSliding = NO;
}

- (void)ABI50_0_0RNCSendSliderEvent:(ABI50_0_0RNCSlider *)sender withContinuous:(BOOL)continuous isSlidingStart:(BOOL)isSlidingStart
{
    ...
    if (continuous) {
        if (sender.lastValue != value)  {
            std::dynamic_pointer_cast<const ABI50_0_0RNCSliderEventEmitter>(_eventEmitter)
            ->onRNCSliderValueChange(ABI50_0_0RNCSliderEventEmitter::OnRNCSliderValueChange{.value = static_cast<Float>(value)});
        }
    } else {
        if (!isSlidingStart) {
            std::dynamic_pointer_cast<const ABI50_0_0RNCSliderEventEmitter>(_eventEmitter)
            ->onRNCSliderSlidingComplete(ABI50_0_0RNCSliderEventEmitter::OnRNCSliderSlidingComplete{.value = static_cast<Float>(value)});
        }
        if (isSlidingStart) {
            std::dynamic_pointer_cast<const ABI50_0_0RNCSliderEventEmitter>(_eventEmitter)
            ->onRNCSliderSlidingStart(ABI50_0_0RNCSliderEventEmitter::OnRNCSliderSlidingStart{.value = static_cast<Float>(value)});
        }
    }
...
{
    const auto &oldScreenProps = *std::static_pointer_cast<const ABI50_0_0RNCSliderProps>(_props);
    const auto &newScreenProps = *std::static_pointer_cast<const ABI50_0_0RNCSliderProps>(props);
    ...
    if (oldScreenProps.thumbTintColor != newScreenProps.thumbTintColor) {
        slider.thumbTintColor = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.thumbTintColor);
    }
    if (oldScreenProps.minimumTrackTintColor != newScreenProps.minimumTrackTintColor) {
        slider.minimumTrackTintColor = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.minimumTrackTintColor);
    }
    if (oldScreenProps.maximumTrackTintColor != newScreenProps.maximumTrackTintColor) {
        slider.maximumTrackTintColor = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.maximumTrackTintColor);
    }
...
- (void)loadImageFromImageSource:(ImageSource)source completionBlock:(ABI50_0_0RNCLoadImageCompletionBlock)completionBlock failureBlock:(ABI50_0_0RNCLoadImageFailureBlock)failureBlock
{
    NSString *uri = [[NSString alloc] initWithUTF8String:source.uri.c_str()];
    if ((BOOL)uri.length) {
        [[[ABI50_0_0RCTBridge currentBridge] moduleForName:@"ImageLoader"]
        loadImageWithURLRequest:NSURLRequestFromImageSource(source)
        size:CGSizeMake(source.size.width, source.size.height)
        scale:source.scale
        clipped:NO
        resizeMode:ABI50_0_0RCTResizeModeCover
        progressBlock:nil
...
Class<ABI50_0_0RCTComponentViewProtocol> ABI50_0_0RNCSliderCls(void)
{
    return ABI50_0_0RNCSliderComponentView.class;
}
