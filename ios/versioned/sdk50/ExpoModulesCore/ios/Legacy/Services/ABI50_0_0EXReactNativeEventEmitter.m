...
@interface ABI50_0_0EXReactNativeEventEmitter ()

@property (nonatomic, assign) int listenersCount;
@property (nonatomic, weak) ABI50_0_0EXModuleRegistry *exModuleRegistry;
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSNumber *> *modulesListenersCounts;

@end

@implementation ABI50_0_0EXReactNativeEventEmitter

ABI50_0_0RCT_EXPORT_MODULE(ABI50_0_0EXReactNativeEventEmitter)
...
{
  return @[@protocol(ABI50_0_0EXEventEmitterService)];
}
...
- (NSArray<NSString *> *)supportedEvents
{
  ...
  for (ABI50_0_0EXExportedModule *exportedModule in [_exModuleRegistry getAllExportedModules]) {
    if ([exportedModule conformsToProtocol:@protocol(ABI50_0_0EXEventEmitter)]) {
      id<ABI50_0_0EXEventEmitter> eventEmitter = (id<ABI50_0_0EXEventEmitter>)exportedModule;
...
ABI50_0_0RCT_EXPORT_METHOD(addProxiedListener:(NSString *)moduleName eventName:(NSString *)eventName)
{
  ...
  if (ABI50_0_0RCT_DEBUG && module == nil) {
    ABI50_0_0EXLogError(@"Module for name `%@` has not been found.", moduleName);
    return;
  } else if (ABI50_0_0RCT_DEBUG && ![module conformsToProtocol:@protocol(ABI50_0_0EXEventEmitter)]) {
    ABI50_0_0EXLogError(@"Module `%@` is not an ABI50_0_0EXEventEmitter, thus it cannot be subscribed to.", moduleName);
    return;
  }

  // Validate eventEmitter
  id<ABI50_0_0EXEventEmitter> eventEmitter = (id<ABI50_0_0EXEventEmitter>)module;

  if (ABI50_0_0RCT_DEBUG && ![[eventEmitter supportedEvents] containsObject:eventName]) {
    ABI50_0_0EXLogError(@"`%@` is not a supported event type for %@. Supported events are: `%@`",
...
ABI50_0_0RCT_EXPORT_METHOD(removeProxiedListeners:(NSString *)moduleName count:(double)count)
{
  ...
  if (newModuleListenersCount == 0) {
    ...
    ABI50_0_0EXLogError(@"Attempted to remove more `%@` listeners than added", moduleName);
  ...
  if (_listenersCount - count < 0) {
    ABI50_0_0EXLogError(@"Attempted to remove more proxied event emitter listeners than added");
    _listenersCount = 0;
  } else {
...
- (void)setModuleRegistry:(ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  // See https://github.com/facebook/react-native/blob/2c2b83171603b47e5eec61eea55139f760dba090/ABI50_0_0React/Base/ABI50_0_0RCTModuleData.mm#L274-L289.
  if ([moduleRegistry isKindOfClass:[ABI50_0_0EXModuleRegistry class]]) {
