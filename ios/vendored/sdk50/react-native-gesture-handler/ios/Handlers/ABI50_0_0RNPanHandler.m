...
#import "ABI50_0_0RNPanHandler.h"

#import <UIKit/UIGestureRecognizerSubclass.h>

@interface ABI50_0_0RNBetterPanGestureRecognizer : UIPanGestureRecognizer
...
@implementation ABI50_0_0RNBetterPanGestureRecognizer {
...
- (void)activateAfterLongPress
{
  ...
  // Send event in ACTIVE state because UIGestureRecognizerStateBegan is mapped to ABI50_0_0RNGestureHandlerStateBegan
  [_gestureHandler handleGesture:self inState:ABI50_0_0RNGestureHandlerStateActive];
}
...
@implementation ABI50_0_0RNPanGestureHandler

- (instancetype)initWithTag:(NSNumber *)tag
{
  if ((self = [super initWithTag:tag])) {
    _recognizer = [[ABI50_0_0RNBetterPanGestureRecognizer alloc] initWithGestureHandler:self];
  }
  return self;
}
...
{
  [super resetConfig];
  ABI50_0_0RNBetterPanGestureRecognizer *recognizer = (ABI50_0_0RNBetterPanGestureRecognizer *)_recognizer;
...
- (void)configure:(NSDictionary *)config
{
...
#if !TARGET_OS_TV && __IPHONE_OS_VERSION_MAX_ALLOWED >= 130400
  if (@available(iOS 13.4, *)) {
    bool enableTrackpadTwoFingerGesture = [ABI50_0_0RCTConvert BOOL:config[@"enableTrackpadTwoFingerGesture"]];
  ...
  if (prop != nil) {
    CGFloat dist = [ABI50_0_0RCTConvert CGFloat:prop];
    recognizer.minDistSq = dist * dist;
  }

  prop = config[@"minVelocity"];
  if (prop != nil) {
    CGFloat velocity = [ABI50_0_0RCTConvert CGFloat:prop];
    recognizer.minVelocitySq = velocity * velocity;
  }

  prop = config[@"activateAfterLongPress"];
  if (prop != nil) {
    recognizer.activateAfterLongPress = [ABI50_0_0RCTConvert CGFloat:prop] / 1000.0;
...
{
  ABI50_0_0RNGestureHandlerState savedState = _lastState;
...
- (ABI50_0_0RNGestureHandlerEventExtraData *)eventExtraData:(UIPanGestureRecognizer *)recognizer
{
  return [ABI50_0_0RNGestureHandlerEventExtraData forPan:[recognizer locationInView:recognizer.view]
