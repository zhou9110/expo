...
@interface ABI50_0_0EXScopedNotificationCategoriesModule ()

@property (nonatomic, strong) NSString *scopeKey;

@end

@implementation ABI50_0_0EXScopedNotificationCategoriesModule
...
- (void)getNotificationCategoriesAsyncWithResolver:(ABI50_0_0EXPromiseResolveBlock)resolve reject:(ABI50_0_0EXPromiseRejectBlock)reject
{
  [[UNUserNotificationCenter currentNotificationCenter] getNotificationCategoriesWithCompletionHandler:^(NSSet<UNNotificationCategory *> *categories) {
    ...
    for (UNNotificationCategory *category in categories) {
      if ([ABI50_0_0EXScopedNotificationsUtils isId:category.identifier scopedByExperience:self->_scopeKey]) {
...
{
  NSString *scopedCategoryIdentifier = [ABI50_0_0EXScopedNotificationsUtils scopedIdentifierFromId:categoryId
...
- (NSMutableDictionary *)serializeCategory:(UNNotificationCategory *)category
{
  ...
  serializedCategory[@"identifier"] = [ABI50_0_0EXScopedNotificationsUtils getScopeAndIdentifierFromScopedIdentifier:serializedCategory[@"identifier"]].identifier;
                                                                         ...
                                                                         isInExpoGo:(BOOL)isInExpoGo __deprecated_msg("To be removed once SDK 43 is phased out")
{
  if (isInExpoGo) {
    ...
    [ABI50_0_0EXScopedNotificationCategoryMigrator migrateLegacyScopedCategoryIdentifiersForProjectWithScopeKey:scopeKey];
  } else {
    // Used to prefix with "experienceId-" even in standalone apps in SDKs <= 40, so we need to unscope those
    [ABI50_0_0EXScopedNotificationCategoryMigrator unscopeLegacyCategoryIdentifiersForProjectWithScopeKey:scopeKey];
  }
}
