...
#import "ABI50_0_0RCTEventDispatcher.h"
...
#import <ABI50_0_0React/ABI50_0_0RCTComponentEvent.h>
...
static NSNumber *ABI50_0_0RCTGetEventID(NSNumber *viewTag, NSString *eventName, uint16_t coalescingKey)
{
  return @(viewTag.intValue | (((uint64_t)eventName.hash & 0xFFFF) << 32) | (((uint64_t)coalescingKey) << 48));
}

static uint16_t ABI50_0_0RCTUniqueCoalescingKeyGenerator = 0;

@interface ABI50_0_0RCTEventDispatcher () <ABI50_0_0RCTTurboModule>
@end

@implementation ABI50_0_0RCTEventDispatcher {
  ...
  NSMutableDictionary<NSNumber *, id<ABI50_0_0RCTEvent>> *_events;
  ...
  NSHashTable<id<ABI50_0_0RCTEventDispatcherObserver>> *_observers;
...
{
  [_callableJSModules invokeModule:@"ABI50_0_0RCTViewEventEmitter" method:@"emit" withArgs:@[ name, ABI50_0_0RCTNullIfNil(ABI50_0_0ReactTag) ]];
}

- (void)sendAppEventWithName:(NSString *)name body:(id)body
{
  [_callableJSModules invokeModule:@"ABI50_0_0RCTNativeAppEventEmitter"
                            method:@"emit"
...
{
  [_callableJSModules invokeModule:@"ABI50_0_0RCTDeviceEventEmitter" method:@"emit" withArgs:body ? @[ name, body ] : @[ name ]];
}
                   ...
                   eventCount:(NSInteger)eventCount
{
  ...
  ABI50_0_0RCTComponentEvent *event = [[ABI50_0_0RCTComponentEvent alloc] initWithName:events[type] viewTag:ABI50_0_0ReactTag body:body];
...
- (void)notifyObserversOfEvent:(id<ABI50_0_0RCTEvent>)event
{
  ...
  for (id<ABI50_0_0RCTEventDispatcherObserver> observer in _observers) {
...
- (void)sendEvent:(id<ABI50_0_0RCTEvent>)event
{
  ...
  if (event.canCoalesce) {
    eventID = ABI50_0_0RCTGetEventID(event.viewTag, event.eventName, event.coalescingKey);
    id<ABI50_0_0RCTEvent> previousEvent = _events[eventID];
    if (previousEvent) {
    ...
    id<ABI50_0_0RCTEvent> previousEvent = _events[eventID];
    eventID = ABI50_0_0RCTGetEventID(event.viewTag, event.eventName, ABI50_0_0RCTUniqueCoalescingKeyGenerator++);
    ABI50_0_0RCTAssert(
        previousEvent == nil,
...
Class ABI50_0_0RCTEventDispatcherCls(void)
{
  return ABI50_0_0RCTEventDispatcher.class;
}
