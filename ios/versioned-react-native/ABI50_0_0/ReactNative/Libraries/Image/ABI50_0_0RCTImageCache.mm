...
static NSUInteger ABI50_0_0RCTMaxCacheableDecodedImageSizeInBytes = 2 * 1024 * 1024;
static NSUInteger ABI50_0_0RCTImageCacheTotalCostLimit = 20 * 1024 * 1024;

void ABI50_0_0RCTSetImageCacheLimits(NSUInteger maxCacheableDecodedImageSizeInBytes, NSUInteger imageCacheTotalCostLimit)
{
  ABI50_0_0RCTMaxCacheableDecodedImageSizeInBytes = maxCacheableDecodedImageSizeInBytes;
  ABI50_0_0RCTImageCacheTotalCostLimit = imageCacheTotalCostLimit;
}

static NSString *ABI50_0_0RCTCacheKeyForImage(NSString *imageTag, CGSize size, CGFloat scale, ABI50_0_0RCTResizeMode resizeMode)
{
  return
      [NSString stringWithFormat:@"%@|%g|%g|%g|%lld", imageTag, size.width, size.height, scale, (long long)resizeMode];
}

@implementation ABI50_0_0RCTImageCache {
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _decodedImageCache.totalCostLimit = ABI50_0_0RCTImageCacheTotalCostLimit;
...
- (void)addImageToCache:(UIImage *)image forKey:(NSString *)cacheKey
{
  ...
  NSInteger bytes = image.ABI50_0_0ReactDecodedImageBytes;
  if (bytes <= ABI50_0_0RCTMaxCacheableDecodedImageSizeInBytes) {
...
{
  NSString *cacheKey = ABI50_0_0RCTCacheKeyForImage(url, size, scale, resizeMode);
  @synchronized(_cacheStaleTimes) {
