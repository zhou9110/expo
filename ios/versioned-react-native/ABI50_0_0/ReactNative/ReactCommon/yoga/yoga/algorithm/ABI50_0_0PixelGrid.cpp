...
void roundLayoutResultsToPixelGrid(
  ...
  const double nodeLeft = node->getLayout().position[ABI50_0_0YGEdgeLeft];
  const double nodeTop = node->getLayout().position[ABI50_0_0YGEdgeTop];

  const double nodeWidth = node->getLayout().dimension(ABI50_0_0YGDimensionWidth);
  const double nodeHeight = node->getLayout().dimension(ABI50_0_0YGDimensionHeight);
