...
TEST(ShadowNodeFamilyTest, sealObjectCorrectly) {
  ...
  ABI50_0_0EXPECT_EQ(ancestors1.size(), 0);

  // Positive case:
  auto ancestors2 = shadowNodeAAA->getFamily().getAncestors(*shadowNodeA);
  ABI50_0_0EXPECT_EQ(ancestors2.size(), 2);
  ABI50_0_0EXPECT_EQ(&ancestors2[0].first.get(), shadowNodeA.get());
  ABI50_0_0EXPECT_EQ(&ancestors2[1].first.get(), shadowNodeAA.get());
}
