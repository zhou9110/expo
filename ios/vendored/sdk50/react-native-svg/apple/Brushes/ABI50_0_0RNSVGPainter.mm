...
@implementation ABI50_0_0RNSVGPainter {
  NSArray<ABI50_0_0RNSVGLength *> *_points;
  NSArray<NSNumber *> *_colors;
  ABI50_0_0RNSVGBrushType _type;
...
- (instancetype)initWithPointsArray:(NSArray<ABI50_0_0RNSVGLength *> *)pointsArray
{
  if ((self = [super init])) {
    _points = pointsArray;
  }
...
- (void)setPattern:(ABI50_0_0RNSVGPattern *)pattern
{
...
void ABI50_0_0PatternFunction(void *info, CGContextRef context)
{
  ABI50_0_0RNSVGPainter *_painter = (__bridge ABI50_0_0RNSVGPainter *)info;
  ABI50_0_0RNSVGPattern *_pattern = [_painter pattern];
  ...
  if (vbWidth > 0 && vbHeight > 0) {
    ...
    CGAffineTransform _viewBoxTransform = [ABI50_0_0RNSVGViewBox getTransform:vbRect
                                                               eRect:rect
...
- (CGFloat)getVal:(ABI50_0_0RNSVGLength *)length relative:(CGFloat)relative
{
  ABI50_0_0RNSVGLengthUnitType unit = [length unit];
  CGFloat val = [ABI50_0_0RNSVGPropHelper fromRelative:length relative:relative];
...
- (void)paintPattern:(CGContextRef)context bounds:(CGRect)bounds
{
  ...
  const CGPatternCallbacks callbacks = {0, &ABI50_0_0PatternFunction, NULL};
...
{
  if ([_colors count] == 0) {
    ABI50_0_0RCTLogWarn(@"No stops in gradient");
    return;
  }
  CGGradientRef gradient = CGGradientRetain([ABI50_0_0RCTConvert ABI50_0_0RNSVGCGGradient:_colors]);
