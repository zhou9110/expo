...
#import <ABI50_0_0React/ABI50_0_0RCTImageBlurUtils.h>
 ...
 * at `idealSize`.
 */
static BOOL ABI50_0_0RCTShouldReloadImageForSizeChange(CGSize currentSize, CGSize idealSize)
{
...
/**
 * See ABI50_0_0RCTConvert (ImageSource). We want to send down the source as a similar
 * JSON parameter.
 */
static NSDictionary *onLoadParamsForSource(ABI50_0_0RCTImageSource *source)
{
  NSDictionary *dict = @{
...
@interface ABI50_0_0RCTImageView ()

@property (nonatomic, copy) ABI50_0_0RCTDirectEventBlock onLoadStart;
@property (nonatomic, copy) ABI50_0_0RCTDirectEventBlock onProgress;
@property (nonatomic, copy) ABI50_0_0RCTDirectEventBlock onError;
@property (nonatomic, copy) ABI50_0_0RCTDirectEventBlock onPartialLoad;
@property (nonatomic, copy) ABI50_0_0RCTDirectEventBlock onLoad;
@property (nonatomic, copy) ABI50_0_0RCTDirectEventBlock onLoadEnd;

@end

@implementation ABI50_0_0RCTImageView {
  ...
  __weak id<ABI50_0_0RCTImageLoaderWithAttributionProtocol> _imageLoader;

  // The image source that's currently displayed
  ABI50_0_0RCTImageSource *_imageSource;

  // The image source that's being loaded from the network
  ABI50_0_0RCTImageSource *_pendingImageSource;
  ...
  ABI50_0_0RCTImageURLLoaderRequest *_loaderRequest;
}
...
- (instancetype)initWithBridge:(ABI50_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _imageView = [ABI50_0_0RCTUIImageViewAnimated new];
...
- (void)updateWithImage:(UIImage *)image
{
  ...
  if (_resizeMode == ABI50_0_0RCTResizeModeRepeat) {
...
- (void)setImageSources:(NSArray<ABI50_0_0RCTImageSource *> *)imageSources
{
...
- (void)setResizeMode:(ABI50_0_0RCTResizeMode)resizeMode
{
...
- (ABI50_0_0RCTImageSource *)imageSourceForSize:(CGSize)size
{
  ...
  const CGFloat scale = ABI50_0_0RCTScreenScale();
  const CGFloat targetImagePixels = size.width * size.height * scale * scale;

  ABI50_0_0RCTImageSource *bestSource = nil;
  CGFloat bestFit = CGFLOAT_MAX;
  for (ABI50_0_0RCTImageSource *source in _imageSources) {
...
- (BOOL)shouldChangeImageSource
{
  ...
  ABI50_0_0RCTImageSource *desiredImageSource = [self imageSourceForSize:self.frame.size];
...
- (void)reloadImage
{
  ...
  ABI50_0_0RCTImageSource *source = [self imageSourceForSize:self.frame.size];
  ...
  if (source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    ABI50_0_0RCTImageLoaderProgressBlock progressHandler = nil;
    if (self.onProgress) {
      ABI50_0_0RCTDirectEventBlock onProgress = self.onProgress;
    ...
    __weak ABI50_0_0RCTImageView *weakSelf = self;
    ABI50_0_0RCTImageLoaderPartialLoadBlock partialLoadHandler = ^(UIImage *image) {
      [weakSelf imageLoaderLoadedImage:image error:nil forImageSource:source partial:YES];
    };

    CGSize imageSize = self.bounds.size;
    CGFloat imageScale = ABI50_0_0RCTScreenScale();
    if (!UIEdgeInsetsEqualToEdgeInsets(_capInsets, UIEdgeInsetsZero)) {
    ...
    ABI50_0_0RCTImageLoaderCompletionBlockWithMetadata completionHandler = ^(NSError *error, UIImage *loadedImage, id metadata) {
    ...
    ABI50_0_0RCTImageURLLoaderRequest *loaderRequest =
        [_imageLoader loadImageWithURLRequest:source.request
                                  ...
                                  attribution:{.nativeViewTag = [self.ABI50_0_0ReactTag intValue],
...
- (void)imageLoaderLoadedImage:(UIImage *)loadedImage
                         error:(NSError *)error
                forImageSource:(ABI50_0_0RCTImageSource *)source
                       partial:(BOOL)isPartialLoad
{
  ...
  void (^setImageBlock)(UIImage *) = ^(UIImage *image) {
    ABI50_0_0RCTImageView *strongSelf = weakSelf;
    if (!strongSelf) {
      return;
    }
    if (!isPartialLoad) {
    ...
    if (isPartialLoad) {
      ...
      if (strongSelf->_onLoad) {
        ABI50_0_0RCTImageSource *sourceLoaded = [source imageSourceWithSize:image.size scale:source.scale];
  ...
  if (_blurRadius > __FLT_EPSILON__) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      UIImage *blurredImage = ABI50_0_0RCTBlurredImageWithRadius(loadedImage, blurRadius);
      ABI50_0_0RCTExecuteOnMainQueue(^{
...
- (void)ABI50_0_0ReactSetFrame:(CGRect)frame
{
  ...
  if ([self shouldChangeImageSource]) {
    ...
    CGSize idealSize =
        ABI50_0_0RCTTargetSize(imageSize, imageScale, frame.size, ABI50_0_0RCTScreenScale(), (ABI50_0_0RCTResizeMode)self.contentMode, YES);

    // Don't reload if the current image or target image size is close enough
    if (!ABI50_0_0RCTShouldReloadImageForSizeChange(imageSize, idealSize) ||
        !ABI50_0_0RCTShouldReloadImageForSizeChange(_targetSize, idealSize)) {
      return;
    }
