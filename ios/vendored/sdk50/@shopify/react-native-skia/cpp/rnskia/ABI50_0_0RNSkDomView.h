...
class ABI50_0_0RNSkDomRenderer : public ABI50_0_0RNSkRenderer,
                        public std::enable_shared_from_this<ABI50_0_0RNSkDomRenderer> {
public:
  ABI50_0_0RNSkDomRenderer(std::function<void()> requestRedraw,
                  std::shared_ptr<ABI50_0_0RNSkPlatformContext> context);

  ~ABI50_0_0RNSkDomRenderer();
  ...
  void updateTouches(std::vector<ABI50_0_0RNSkTouchInfo> &touches);
...
private:
  ...
  ABI50_0_0RNSkTimingInfo _renderTimingInfo;

  std::mutex _touchMutex;
  std::vector<std::vector<ABI50_0_0RNSkTouchInfo>> _currentTouches;
  std::vector<std::vector<ABI50_0_0RNSkTouchInfo>> _touchesCache;
  std::mutex _rootLock;
};

class ABI50_0_0RNSkDomView : public ABI50_0_0RNSkView {
public:
  /**
   * Constructor
   */
  ABI50_0_0RNSkDomView(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context,
              std::shared_ptr<ABI50_0_0RNSkCanvasProvider> canvasProvider)
      : ABI50_0_0RNSkView(context, canvasProvider,
                 std::make_shared<ABI50_0_0RNSkDomRenderer>(
                     std::bind(&ABI50_0_0RNSkView::requestRedraw, this), context)) {}

  void updateTouchState(std::vector<ABI50_0_0RNSkTouchInfo> &touches) override {
    std::static_pointer_cast<ABI50_0_0RNSkDomRenderer>(getRenderer())
