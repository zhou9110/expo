...
@implementation ABI50_0_0RCTWrapperShadowView {
  __weak ABI50_0_0RCTBridge *_bridge;
  ABI50_0_0RCTWrapperMeasureBlock _measureBlock;
...
- (instancetype)initWithBridge:(ABI50_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    _bridge = bridge;
    ABI50_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI50_0_0RCTWrapperShadowViewMeasure);
  }

  return self;
}

static ABI50_0_0YGSize ABI50_0_0RCTWrapperShadowViewMeasure(
    ...
    ABI50_0_0YGMeasureMode heightMode)
{
  ...
  ABI50_0_0RCTWrapperShadowView *shadowView = (__bridge ABI50_0_0RCTWrapperShadowView *)ABI50_0_0YGNodeGetContext(node);
  CGSize size = [shadowView measureWithMinimumSize:minimumSize maximumSize:maximumSize];

  return (ABI50_0_0YGSize){ABI50_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width), ABI50_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height)};
}
...
- (CGSize)measureWithMinimumSize:(CGSize)minimumSize maximumSize:(CGSize)maximumSize
{
  ...
  if (!_measureBlock) {
    ...
    __block ABI50_0_0RCTWrapperMeasureBlock measureBlock;
    NSNumber *ABI50_0_0ReactTag = self.ABI50_0_0ReactTag;

    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);

    dispatch_async(dispatch_get_main_queue(), ^{
      ABI50_0_0RCTUIManager *uiManager = bridge.uiManager;
      ABI50_0_0RCTWrapperView *view = (ABI50_0_0RCTWrapperView *)[uiManager viewForABI50_0_0ReactTag:ABI50_0_0ReactTag];
    ...
    if (dispatch_semaphore_wait(semaphore, timeout)) {
      ABI50_0_0RCTLogError(@"Unable to retrieve `measureBlock` for view (%@) because the main thread is busy.", self);
    }
  ...
  if (dispatch_semaphore_wait(semaphore, timeout)) {
    ABI50_0_0RCTLogError(@"Unable to compute layout for view (%@) because the main thread is busy.", self);
  }
