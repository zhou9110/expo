...
#ifdef ABI50_0_0HAVE_GOOGLE_MAPS_UTILS
...
#define REQUIRES_GOOGLE_MAPS_UTILS(feature) do { \
 [NSException raise:@"ABI50_0_0ReactNativeMapsDependencyMissing" \
...
@interface ABI50_0_0AIRGoogleMap () <GMSIndoorDisplayDelegate>
...
@implementation ABI50_0_0AIRGoogleMap
{
...
- (void)insertABI50_0_0ReactSubview:(id<ABI50_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
  ...
  if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapMarker class]]) {
    ABI50_0_0AIRGoogleMapMarker *marker = (ABI50_0_0AIRGoogleMapMarker*)subview;
    marker.realMarker.map = self;
    [self.markers addObject:marker];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapPolygon class]]) {
    ABI50_0_0AIRGoogleMapPolygon *polygon = (ABI50_0_0AIRGoogleMapPolygon*)subview;
    polygon.polygon.map = self;
    [self.polygons addObject:polygon];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapPolyline class]]) {
    ABI50_0_0AIRGoogleMapPolyline *polyline = (ABI50_0_0AIRGoogleMapPolyline*)subview;
    polyline.polyline.map = self;
    [self.polylines addObject:polyline];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapCircle class]]) {
    ABI50_0_0AIRGoogleMapCircle *circle = (ABI50_0_0AIRGoogleMapCircle*)subview;
    circle.circle.map = self;
    [self.circles addObject:circle];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapUrlTile class]]) {
    ABI50_0_0AIRGoogleMapUrlTile *tile = (ABI50_0_0AIRGoogleMapUrlTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapWMSTile class]]) {
    ABI50_0_0AIRGoogleMapWMSTile *tile = (ABI50_0_0AIRGoogleMapWMSTile*)subview;
    tile.tileLayer.map = self;
    [self.tiles addObject:tile];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapOverlay class]]) {
    ABI50_0_0AIRGoogleMapOverlay *overlay = (ABI50_0_0AIRGoogleMapOverlay*)subview;
    overlay.overlay.map = self;
    [self.overlays addObject:overlay];
  } else if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapHeatmap class]]){
    ABI50_0_0AIRGoogleMapHeatmap *heatmap = (ABI50_0_0AIRGoogleMapHeatmap*)subview;
    heatmap.heatmap.map = self;
    [self.heatmaps addObject:heatmap];
  } else {
    NSArray<id<ABI50_0_0RCTComponent>> *childSubviews = [subview ABI50_0_0ReactSubviews];
    for (int i = 0; i < childSubviews.count; i++) {
      [self insertABI50_0_0ReactSubview:(UIView *)childSubviews[i] atIndex:atIndex];
    }
  }
  [_ABI50_0_0ReactSubviews insertObject:(UIView *)subview atIndex:(NSUInteger) atIndex];
}
...
- (void)removeABI50_0_0ReactSubview:(id<ABI50_0_0RCTComponent>)subview {
  ...
  if ([subview isKindOfClass:[ABI50_0_0AIRGoogleMapMarker class]]) {
    ...
    NSArray<id<ABI50_0_0RCTComponent>> *childSubviews = [subview ABI50_0_0ReactSubviews];
    for (int i = 0; i < childSubviews.count; i++) {
      [self removeABI50_0_0ReactSubview:(UIView *)childSubviews[i]];
    }
  }
  [_ABI50_0_0ReactSubviews removeObject:(UIView *)subview];
}
#pragma clang diagnostic pop
...
- (void)layoutSubviews {
  ...
  if (_initialCamera != nil) {
    ...
    self.camera = [ABI50_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_initialRegion];
    _initialRegionSet = true;
  } else if (_region.span.latitudeDelta != 0.0 &&
      _region.span.longitudeDelta != 0.0) {
    self.camera = [ABI50_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:_region];
  }
}
...
- (void)setInitialRegion:(MKCoordinateRegion)initialRegion {
  ...
  if(!_initialRegionSet && _didLayoutSubviews){
    self.camera = [ABI50_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andMKCoordinateRegion:initialRegion];
    _initialRegionSet = true;
  }
}
...
- (void)setRegion:(MKCoordinateRegion)region {
  ...
  if(_didLayoutSubviews) {
    self.camera = [ABI50_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self  andMKCoordinateRegion:region];
  }
}
...
- (void)setOnMapReady:(ABI50_0_0RCTBubblingEventBlock)onMapReady {
...
- (BOOL)didTapMarker:(GMSMarker *)marker {
  ABI50_0_0AIRGMSMarker *airMarker = (ABI50_0_0AIRGMSMarker *)marker;
...
- (void)didTapPolyline:(GMSOverlay *)polyline {
  ABI50_0_0AIRGMSPolyline *airPolyline = (ABI50_0_0AIRGMSPolyline *)polyline;
...
- (void)didTapPolygon:(GMSOverlay *)polygon {
    ABI50_0_0AIRGMSPolygon *airPolygon = (ABI50_0_0AIRGMSPolygon *)polygon;
...
- (void)didChangeCameraPosition:(GMSCameraPosition *)position isGesture:(BOOL)isGesture{
  id event = @{@"continuous": @YES,
               @"region": regionAsJSON([ABI50_0_0AIRGoogleMap makeGMSCameraPositionFromMap:self andGMSCameraPosition:position]),
...
- (CGRect) frameForMarker:(ABI50_0_0AIRGoogleMapMarker*) mrkView {
...
- (NSDictionary*) getMarkersFramesWithOnlyVisible:(BOOL)onlyVisible {
    ...
    for (ABI50_0_0AIRGoogleMapMarker* mrkView in self.markers) {
...
- (ABI50_0_0AIRGoogleMapMarker*) markerAtPoint:(CGPoint)point {
    ABI50_0_0AIRGoogleMapMarker* mrk = nil;
    for (ABI50_0_0AIRGoogleMapMarker* mrkView in self.markers) {
        CGRect frame = [self frameForMarker:mrkView];
...
- (id)extendedMapGestureHandler:(UIGestureRecognizer*)gestureRecognizer {
    ...
    if (!tapTouch)
        ...
        if (isTapInsideBubble) {
            ...
            ABI50_0_0AIRGoogleMapMarker* markerView = nil;
            ABI50_0_0AIRGMSMarker* marker = nil;
            for (ABI50_0_0AIRGoogleMapMarker* mrk in self.markers) {
            ...
            UIView* realSubview = [(ABI50_0_0RCTView*)bubbleView hitTest:tapPointInBubble withEvent:nil];
            ABI50_0_0AIRGoogleMapCalloutSubview* realPressableSubview = nil;
            if (realSubview) {
                UIView* tmp = realSubview;
                while (tmp && tmp != win && tmp != bubbleView) {
                    if ([tmp respondsToSelector:@selector(onPress)]) {
                        realPressableSubview = (ABI50_0_0AIRGoogleMapCalloutSubview*) tmp;
                        break;
                    }
                    tmp = tmp.superview;
                }
            ...
            if (markerView) {
                ...
                if (isInsideCallout) {
                    ...
                    ABI50_0_0AIRGoogleMapMarker* markerAtTapPoint = [self markerAtPoint:tapPoint];
...
- (void)setKmlSrc:(NSString *)kmlUrl {
  ...
  for (GMUPlacemark *place in parser.placemarks) {
    ...
    ABI50_0_0AIRGoogleMapMarker *marker = (ABI50_0_0AIRGoogleMapMarker *)[[ABI50_0_0AIRGoogleMapMarkerManager alloc] view];
    if (!marker.bridge) {
    ...
    marker.imageSrc = [ABI50_0_0AIRGoogleMap GetIconUrl:place parser:parser];
    marker.layer.backgroundColor = [UIColor clearColor].CGColor;
    marker.layer.position = CGPointZero;

    [self insertABI50_0_0ReactSubview:(UIView *) marker atIndex:index];
