...
char *const ABI50_0_0RCTUIManagerQueueName = "com.facebook.ABI50_0_0React.ShadowQueue";

static BOOL pseudoUIManagerQueueFlag = NO;

dispatch_queue_t ABI50_0_0RCTGetUIManagerQueue(void)
{
  ...
  dispatch_once(&onceToken, ^{
    if ([NSOperation instancesRespondToSelector:@selector(qualityOfService)]) {
      ...
      shadowQueue = dispatch_queue_create(ABI50_0_0RCTUIManagerQueueName, attr);
    } else {
      shadowQueue = dispatch_queue_create(ABI50_0_0RCTUIManagerQueueName, DISPATCH_QUEUE_SERIAL);
...
BOOL ABI50_0_0RCTIsUIManagerQueue(void)
{
  static void *queueKey = &queueKey;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    dispatch_queue_set_specific(ABI50_0_0RCTGetUIManagerQueue(), queueKey, queueKey, NULL);
  });
  return dispatch_get_specific(queueKey) == queueKey;
}

BOOL ABI50_0_0RCTIsPseudoUIManagerQueue(void)
{
...
void ABI50_0_0RCTExecuteOnUIManagerQueue(dispatch_block_t block)
{
  if (ABI50_0_0RCTIsUIManagerQueue() || ABI50_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    dispatch_async(ABI50_0_0RCTGetUIManagerQueue(), ^{
      block();
    });
  }
}

void ABI50_0_0RCTUnsafeExecuteOnUIManagerQueueSync(dispatch_block_t block)
{
  if (ABI50_0_0RCTIsUIManagerQueue() || ABI50_0_0RCTIsPseudoUIManagerQueue()) {
    block();
  } else {
    if (ABI50_0_0RCTIsMainQueue()) {
      ...
      dispatch_sync(ABI50_0_0RCTGetUIManagerQueue(), ^{
        block();
      });
    }
  }
...
NSNumber *ABI50_0_0RCTAllocateRootViewTag(void)
{
