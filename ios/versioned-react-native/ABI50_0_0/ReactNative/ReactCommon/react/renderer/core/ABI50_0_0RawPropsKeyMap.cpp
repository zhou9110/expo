...
#include "ABI50_0_0RawPropsKeyMap.h"
...
RawPropsValueIndex RawPropsKeyMap::at(
  ...
  ABI50_0_0React_native_assert(length > 0);
  ABI50_0_0React_native_assert(length < kPropNameLengthHardCap);
  // 1. Find the bucket.
  auto lower = int{buckets_[length - 1]};
  auto upper = int{buckets_[length]} - 1;
  ABI50_0_0React_native_assert(lower - 1 <= upper);
