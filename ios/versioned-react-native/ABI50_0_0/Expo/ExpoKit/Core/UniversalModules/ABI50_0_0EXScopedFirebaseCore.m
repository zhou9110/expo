...
#import <ABI50_0_0EXFirebaseCore/ABI50_0_0EXFirebaseCore+FIROptions.h>
...
@implementation ABI50_0_0EXScopedFirebaseCore {
  ...
  for (NSString* name in names) {
    if (!protectedAppNames[name] && (!options || ![name isEqualToString:appName])) {
      [[FIRApp appNamed:name] deleteApp:^(BOOL success) {
        if (!success) {
          ABI50_0_0EXLogWarn(@"Failed to delete Firebase app: %@", name);
        }
      }];
    }
  }
...
+ (nullable NSDictionary *)googleServicesFileFromManifest:(ABI50_0_0EXManifestsManifest *)manifest
{
  ...
  @try {
    ...
    if (error) ABI50_0_0EXLogWarn(@"Invalid googleServicesFile: %@", error);
    return plist;
  }
  @catch(NSException* exception) {
    ABI50_0_0EXLogWarn(@"Invalid googleServicesFile: %@", exception);
    return nil;
  }
}
