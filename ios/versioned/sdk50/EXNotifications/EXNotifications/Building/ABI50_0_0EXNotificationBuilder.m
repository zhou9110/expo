...
@implementation ABI50_0_0EXNotificationBuilder

ABI50_0_0EX_REGISTER_MODULE();

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI50_0_0EXNotificationBuilder)];
}
...
- (UNNotificationAttachment *)attachmentFromRequest:(NSDictionary *)request
{
  ...
  if (error) {
    ABI50_0_0EXLogWarn(@"[expo-notifications] Could not have created a notification attachment out of request: %@. Error: %@.", [request description], [error description]);
    return nil;
  }
  return attachment;
}
