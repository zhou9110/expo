...
#import "ABI50_0_0RNSVGBezierElement.h"
...
#import "ABI50_0_0RNSVGVectorEffect.h"
#import "ABI50_0_0RNSVGViewBox.h"

@implementation ABI50_0_0RNSVGRenderable {
  NSMutableDictionary *_originProperties;
  NSArray<NSString *> *_lastMergedList;
  NSArray<NSString *> *_attributeList;
  NSArray<ABI50_0_0RNSVGLength *> *_sourceStrokeDashArray;
  CGFloat *_strokeDashArrayData;
  CGPathRef _srcHitPath;
}

static ABI50_0_0RNSVGRenderable *_contextElement;
+ (ABI50_0_0RNSVGRenderable *)contextElement
{
  return _contextElement;
}
+ (void)setContextElement:(ABI50_0_0RNSVGRenderable *)contextElement
{
...
- (id)init
{
  if (self = [super init]) {
    _fillOpacity = 1;
    _strokeOpacity = 1;
    _strokeWidth = [ABI50_0_0RNSVGLength lengthWithNumber:1];
...
- (void)setFill:(ABI50_0_0RNSVGBrush *)fill
{
  if (fill == _fill) {
    return;
  }
...
- (void)setFillRule:(ABI50_0_0RNSVGCGFCRule)fillRule
{
...
- (void)setStroke:(ABI50_0_0RNSVGBrush *)stroke
{
  if (stroke == _stroke) {
    return;
  }
...
- (void)setStrokeWidth:(ABI50_0_0RNSVGLength *)strokeWidth
{
...
- (void)setStrokeDasharray:(NSArray<ABI50_0_0RNSVGLength *> *)strokeDasharray
{
...
- (void)setVectorEffect:(ABI50_0_0RNSVGVectorEffect)vectorEffect
{
...
UInt32 ABI50_0_0saturate(CGFloat value)
{
...
- (void)renderTo:(CGContextRef)context rect:(CGRect)rect
{
  ...
  if (self.mask) {
    ...
    ABI50_0_0RNSVGMask *_maskNode = (ABI50_0_0RNSVGMask *)[self.svgView getDefinedMask:self.mask];
    ...
    for (NSUInteger i = 0; i < npixels; i++) {
      ...
      *currentPixel = ABI50_0_0saturate(luma) << 24;
      currentPixel++;
    }
...
- (void)prepareStrokeDash:(NSUInteger)count strokeDasharray:(NSArray<ABI50_0_0RNSVGLength *> *)strokeDasharray
{
...
{
  ABI50_0_0RNSVGMarker *markerStart = (ABI50_0_0RNSVGMarker *)[self.svgView getDefinedMarker:self.markerStart];
  ABI50_0_0RNSVGMarker *markerMid = (ABI50_0_0RNSVGMarker *)[self.svgView getDefinedMarker:self.markerMid];
  ABI50_0_0RNSVGMarker *markerEnd = (ABI50_0_0RNSVGMarker *)[self.svgView getDefinedMarker:self.markerEnd];
  if (markerStart || markerMid || markerEnd) {
    _contextElement = self;
    NSArray<ABI50_0_0RNSVGMarkerPosition *> *positions = [ABI50_0_0RNSVGMarkerPosition fromCGPath:path];
    ...
    for (ABI50_0_0RNSVGMarkerPosition *position in positions) {
      ABI50_0_0RNSVGMarkerType type = [position type];
      ABI50_0_0RNSVGMarker *marker;
      switch (type) {
        case kStartMarker:
          marker = markerStart;
          break;
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
  ...
  if (self.stroke) {
    ...
    NSArray<ABI50_0_0RNSVGLength *> *strokeDasharray = self.strokeDasharray;
...
- (ABI50_0_0RNSVGPlatformView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL canReceiveTouchEvents = (self.pointerEvents != ABI50_0_0RCTPointerEventsNone && ![self isHidden]);
  ...
  if (self.clipPath) {
    ABI50_0_0RNSVGClipPath *clipNode = (ABI50_0_0RNSVGClipPath *)[self.svgView getDefinedClipPath:self.clipPath];
    if ([clipNode isSimpleClipPath]) {
      ...
      ABI50_0_0RNSVGRenderable *clipGroup = (ABI50_0_0RNSVGRenderable *)clipNode;
...
- (void)mergeProperties:(__kindof ABI50_0_0RNSVGRenderable *)target
{
