...
#import <ABI50_0_0React/ABI50_0_0RCTActionSheetManager.h>
...
@interface ABI50_0_0RCTActionSheetManager () <UIActionSheetDelegate, ABI50_0_0NativeActionSheetManagerSpec>

@property (nonatomic, strong) NSMutableArray<UIAlertController *> *alertControllers;

@end

@implementation ABI50_0_0RCTActionSheetManager
                ...
                anchorViewTag:(NSNumber *)anchorViewTag
{
  ...
  if (anchorViewTag) {
    sourceView = [self.viewRegistry_DEPRECATED viewForABI50_0_0ReactTag:anchorViewTag];
  } else {
...
ABI50_0_0RCT_EXPORT_METHOD(showActionSheetWithOptions
                  : (ABI50_0_0JS::NativeActionSheetManager::SpecShowActionSheetWithOptionsOptions &)options callback
                  : (ABI50_0_0RCTResponseSenderBlock)callback)
{
  if (ABI50_0_0RCTRunningInAppExtension()) {
    ABI50_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  }

  NSString *title = options.title();
  NSString *message = options.message();
  NSArray<NSString *> *buttons = ABI50_0_0RCTConvertOptionalVecToArray(options.options(), ^id(NSString *element) {
    return element;
  });
  NSArray<NSNumber *> *disabledButtonIndices;
  NSInteger cancelButtonIndex =
      options.cancelButtonIndex() ? [ABI50_0_0RCTConvert NSInteger:@(*options.cancelButtonIndex())] : -1;
  NSArray<NSNumber *> *destructiveButtonIndices;
  if (options.disabledButtonIndices()) {
    disabledButtonIndices = ABI50_0_0RCTConvertVecToArray(*options.disabledButtonIndices(), ^id(double element) {
      return @(element);
    });
  }
  if (options.destructiveButtonIndices()) {
    destructiveButtonIndices = ABI50_0_0RCTConvertVecToArray(*options.destructiveButtonIndices(), ^id(double element) {
      return @(element);
    });
  } else {
  ...
  UIViewController *controller = ABI50_0_0RCTPresentedViewController();
  NSNumber *anchor = [ABI50_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  UIColor *tintColor = [ABI50_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];
  UIColor *cancelButtonTintColor =
      [ABI50_0_0RCTConvert UIColor:options.cancelButtonTintColor() ? @(*options.cancelButtonTintColor()) : nil];
  ...
  // the action sheet on iPad. ABI50_0_0RCTResponseSenderBlock can only be called once so
  ...
  NSString *userInterfaceStyle = [ABI50_0_0RCTConvert NSString:options.userInterfaceStyle()];
...
ABI50_0_0RCT_EXPORT_METHOD(dismissActionSheet)
{
  if (_alertControllers.count == 0) {
    ABI50_0_0RCTLogWarn(@"Unable to dismiss action sheet");
  }
...
ABI50_0_0RCT_EXPORT_METHOD(showShareActionSheetWithOptions
                  : (ABI50_0_0JS::NativeActionSheetManager::SpecShowShareActionSheetWithOptionsOptions &)options failureCallback
                  : (ABI50_0_0RCTResponseSenderBlock)failureCallback successCallback
                  : (ABI50_0_0RCTResponseSenderBlock)successCallback)
{
  if (ABI50_0_0RCTRunningInAppExtension()) {
    ABI50_0_0RCTLogError(@"Unable to show action sheet from app extension");
    return;
  ...
  NSURL *URL = [ABI50_0_0RCTConvert NSURL:options.url()];
  if (URL) {
    if ([URL.scheme.lowercaseString isEqualToString:@"data"]) {
      NSError *error;
      NSData *data = [NSData dataWithContentsOfURL:URL options:(NSDataReadingOptions)0 error:&error];
      if (!data) {
        failureCallback(@[ ABI50_0_0RCTJSErrorFromNSError(error) ]);
        return;
      }
  ...
  if (items.count == 0) {
    ABI50_0_0RCTLogError(@"No `url` or `message` to share");
    return;
  }
  ...
  NSArray *excludedActivityTypes =
      ABI50_0_0RCTConvertOptionalVecToArray(options.excludedActivityTypes(), ^id(NSString *element) {
        return element;
      });
  ...
  shareController.completionWithItemsHandler =
      ^(NSString *activityType, BOOL completed, __unused NSArray *returnedItems, NSError *activityError) {
        if (activityError) {
          failureCallback(@[ ABI50_0_0RCTJSErrorFromNSError(activityError) ]);
        } else if (completed || activityType == nil) {
          successCallback(@[ @(completed), ABI50_0_0RCTNullIfNil(activityType) ]);
        }
      };

  NSNumber *anchorViewTag = [ABI50_0_0RCTConvert NSNumber:options.anchor() ? @(*options.anchor()) : nil];
  shareController.view.tintColor = [ABI50_0_0RCTConvert UIColor:options.tintColor() ? @(*options.tintColor()) : nil];
...
Class ABI50_0_0RCTActionSheetManagerCls(void)
{
  return ABI50_0_0RCTActionSheetManager.class;
}
