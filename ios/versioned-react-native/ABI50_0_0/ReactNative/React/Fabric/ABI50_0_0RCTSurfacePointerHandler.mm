...
#import <ABI50_0_0React/ABI50_0_0RCTReactTaggedView.h>
...
typedef NS_ENUM(NSInteger, ABI50_0_0RCTPointerEventType) {
  ABI50_0_0RCTPointerEventTypeStart,
  ABI50_0_0RCTPointerEventTypeMove,
  ABI50_0_0RCTPointerEventTypeEnd,
  ABI50_0_0RCTPointerEventTypeCancel,
};
...
struct ActivePointer {
   ...
   */
  UIView<ABI50_0_0RCTComponentViewProtocol> *initialComponentView = nil;

  /*
   * The current target component view of the pointer
   */
  UIView<ABI50_0_0RCTComponentViewProtocol> *componentView = nil;
...
{
  return [(id<ABI50_0_0RCTTouchableComponentViewProtocol>)componentView touchEventEmitterAtPoint:point];
}

static NSOrderedSet<ABI50_0_0RCTReactTaggedView *> *GetTouchableViewsInPathToRoot(UIView *componentView)
{
  NSMutableOrderedSet *results = [NSMutableOrderedSet orderedSet];
  do {
    if ([componentView respondsToSelector:@selector(touchEventEmitterAtPoint:)]) {
      [results addObject:[ABI50_0_0RCTReactTaggedView wrap:componentView]];
    }
...
static PointerEvent CreatePointerEventFromActivePointer(
    ...
    ABI50_0_0RCTPointerEventType eventType,
    UIView *rootComponentView)
{
  ...
  if (eventType == ABI50_0_0RCTPointerEventTypeCancel) {
    event.clientPoint = ABI50_0_0RCTPointFromCGPoint(CGPointZero);
    event.screenPoint =
        ABI50_0_0RCTPointFromCGPoint([rootComponentView convertPoint:CGPointZero
                                          toCoordinateSpace:rootComponentView.window.screen.coordinateSpace]);
    event.offsetPoint = ABI50_0_0RCTPointFromCGPoint([rootComponentView convertPoint:CGPointZero
                                                                     toView:activePointer.componentView]);
  } else {
    event.clientPoint = ABI50_0_0RCTPointFromCGPoint(activePointer.clientPoint);
    event.screenPoint = ABI50_0_0RCTPointFromCGPoint(activePointer.screenPoint);
    event.offsetPoint = ABI50_0_0RCTPointFromCGPoint(activePointer.offsetPoint);
  }
  ...
  if (activePointer.touchType == UITouchTypeIndirectPointer) {
    ...
    event.pressure = eventType != ABI50_0_0RCTPointerEventTypeEnd ? 0.5 : 0.0;
  }
    ...
    UIKeyModifierFlags modifierFlags)
{
  ...
  event.clientPoint = ABI50_0_0RCTPointFromCGPoint(clientLocation);
  event.screenPoint = ABI50_0_0RCTPointFromCGPoint(screenLocation);
  event.offsetPoint = ABI50_0_0RCTPointFromCGPoint(offsetLocation);
  event.width = 1.0;
  event.height = 1.0;
  event.tiltX = 0;
  event.tiltY = 0;
    ...
    UIView *rootComponentView)
{
  ...
  activePointer.force = ABI50_0_0RCTZeroIfNaN(uiTouch.force / uiTouch.maximumPossibleForce);
...
{
  NSOrderedSet<ABI50_0_0RCTReactTaggedView *> *initialViewSet =
      GetTouchableViewsInPathToRoot(activePointer.initialComponentView);
  for (ABI50_0_0RCTReactTaggedView *canidateTaggedView in initialViewSet) {
...
@interface ABI50_0_0RCTSurfacePointerHandler () <UIGestureRecognizerDelegate>
@end

@implementation ABI50_0_0RCTSurfacePointerHandler {
  ...
  NSMutableDictionary<NSNumber *, NSOrderedSet<ABI50_0_0RCTReactTaggedView *> *> *_currentlyHoveredViewsPerPointer;
...
{
  ABI50_0_0RCTAssert(self.view == nil, @"ABI50_0_0RCTSurfacePointerHandler already has an attached view.");
...
- (void)detachFromView:(UIView *)view
{
  ...
  ABI50_0_0RCTAssert(self.view == view, @"ABI50_0_0RCTSufracePointerHandler attached to another view.");
...
- (void)_updateTouches:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  for (UITouch *touch in touches) {
    ...
    ABI50_0_0RCTAssert(iterator != _activePointers.end(), @"Inconsistency between local and UIKit touch registries");
...
- (void)_dispatchActivePointers:(std::vector<ActivePointer>)activePointers eventType:(ABI50_0_0RCTPointerEventType)eventType
{
  for (const auto &activePointer : activePointers) {
    ...
    if (eventEmitter != nil) {
      switch (eventType) {
        case ABI50_0_0RCTPointerEventTypeStart: {
          eventEmitter->onPointerDown(pointerEvent);
          break;
        }
        case ABI50_0_0RCTPointerEventTypeMove: {
          eventEmitter->onPointerMove(pointerEvent);
          break;
        }
        case ABI50_0_0RCTPointerEventTypeEnd: {
        ...
        case ABI50_0_0RCTPointerEventTypeCancel: {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI50_0_0RCTPointerEventTypeStart];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI50_0_0RCTPointerEventTypeMove];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI50_0_0RCTPointerEventTypeEnd];
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
  ...
  [self _updateTouches:touches withEvent:event];
  [self _dispatchActivePointers:[self _activePointersFromTouches:touches] eventType:ABI50_0_0RCTPointerEventTypeCancel];
  [self _unregisterTouches:touches];
...
- (void)reset
{
  ...
  if (!_activePointers.empty()) {
    ...
    [self _dispatchActivePointers:activePointers eventType:ABI50_0_0RCTPointerEventTypeCancel];
 ...
 * should be fired.
 */
- (NSOrderedSet<ABI50_0_0RCTReactTaggedView *> *)handleIncomingPointerEvent:(PointerEvent)event
                                                            onView:(nullable UIView *)targetView
{
  ...
  NSOrderedSet<ABI50_0_0RCTReactTaggedView *> *currentlyHoveredViews =
  ...
  ABI50_0_0RCTReactTaggedView *targetTaggedView = [ABI50_0_0RCTReactTaggedView wrap:targetView];
  ABI50_0_0RCTReactTaggedView *prevTargetTaggedView = [currentlyHoveredViews firstObject];
  UIView *prevTargetView = prevTargetTaggedView.view;

  NSOrderedSet<ABI50_0_0RCTReactTaggedView *> *eventPathViews = GetTouchableViewsInPathToRoot(targetView);
  ...
  for (ABI50_0_0RCTReactTaggedView *taggedView in [currentlyHoveredViews reverseObjectEnumerator]) {
  ...
  for (ABI50_0_0RCTReactTaggedView *taggedView in [eventPathViews reverseObjectEnumerator]) {
    UIView *componentView = taggedView.view;
