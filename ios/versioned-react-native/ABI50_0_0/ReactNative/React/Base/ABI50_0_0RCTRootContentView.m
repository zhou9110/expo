...
@implementation ABI50_0_0RCTRootContentView

- (instancetype)initWithFrame:(CGRect)frame
                       bridge:(ABI50_0_0RCTBridge *)bridge
                     ABI50_0_0ReactTag:(NSNumber *)ABI50_0_0ReactTag
              sizeFlexibility:(ABI50_0_0RCTRootViewSizeFlexibility)sizeFlexibility
{
  if ((self = [super initWithFrame:frame])) {
    _bridge = bridge;
    self.ABI50_0_0ReactTag = ABI50_0_0ReactTag;
...
ABI50_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : (nonnull NSCoder *)aDecoder)
...
- (void)insertABI50_0_0ReactSubview:(UIView *)subview atIndex:(NSInteger)atIndex
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!self->_contentHasAppeared) {
      ...
      [[NSNotificationCenter defaultCenter] postNotificationName:ABI50_0_0RCTContentDidAppearNotification object:self.superview];
    }
  });
}
...
- (CGSize)availableSize
{
  ...
  return CGSizeMake(
      _sizeFlexibility & ABI50_0_0RCTRootViewSizeFlexibilityWidth ? INFINITY : size.width,
      _sizeFlexibility & ABI50_0_0RCTRootViewSizeFlexibilityHeight ? INFINITY : size.height);
}

- (void)updateAvailableSize
{
  if (!self.ABI50_0_0ReactTag || !_bridge.isValid) {
    return;
  }
...
- (void)invalidate
{
  if (self.userInteractionEnabled) {
    ...
    [(ABI50_0_0RCTRootView *)self.superview contentViewInvalidated];

    [_bridge enqueueJSCall:@"AppRegistry"
                    method:@"unmountApplicationComponentAtRootTag"
                      args:@[ self.ABI50_0_0ReactTag ]
                completion:NULL];
  }
}
