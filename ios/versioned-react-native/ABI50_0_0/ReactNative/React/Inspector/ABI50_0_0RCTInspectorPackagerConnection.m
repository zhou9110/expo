...
// ABI50_0_0ReactAndroid/src/main/java/com/facebook/ABI50_0_0React/devsupport/InspectorPackagerConnection.java
// please keep consistent :)

const int ABI50_0_0RECONNECT_DELAY_MS = 2000;

@implementation ABI50_0_0RCTBundleStatus
@end

@interface ABI50_0_0RCTInspectorPackagerConnection () <SRWebSocketDelegate> {
  NSURL *_url;
  NSMutableDictionary<NSString *, ABI50_0_0RCTInspectorLocalConnection *> *_inspectorConnections;
  SRWebSocket *_webSocket;
  dispatch_queue_t _jsQueue;
  BOOL _closed;
  BOOL _suppressConnectionErrors;
  ABI50_0_0RCTBundleStatusProvider _bundleStatusProvider;
}
@end

@interface ABI50_0_0RCTInspectorRemoteConnection () {
  __weak ABI50_0_0RCTInspectorPackagerConnection *_owningPackagerConnection;
  NSString *_pageId;
}
- (instancetype)initWithPackagerConnection:(ABI50_0_0RCTInspectorPackagerConnection *)owningPackagerConnection
...
@implementation ABI50_0_0RCTInspectorPackagerConnection
...
- (instancetype)initWithURL:(NSURL *)url
{
  if (self = [super init]) {
    ...
    _jsQueue = dispatch_queue_create("com.facebook.ABI50_0_0React.WebSocketExecutor", DISPATCH_QUEUE_SERIAL);
  }
  return self;
}
...
- (void)handleProxyMessage:(NSDictionary<NSString *, id> *)message
{
  ...
  if ([@"getPages" isEqualToString:event]) {
    ...
    ABI50_0_0RCTLogError(@"Unknown event: %@", event);
  }
}
...
- (void)handleConnect:(NSDictionary *)payload
{
  ...
  ABI50_0_0RCTInspectorLocalConnection *existingConnection = _inspectorConnections[pageId];
  if (existingConnection) {
    [_inspectorConnections removeObjectForKey:pageId];
    [existingConnection disconnect];
    ABI50_0_0RCTLogWarn(@"Already connected: %@", pageId);
    return;
  }

  ABI50_0_0RCTInspectorRemoteConnection *remoteConnection =
      [[ABI50_0_0RCTInspectorRemoteConnection alloc] initWithPackagerConnection:self pageId:pageId];

  ABI50_0_0RCTInspectorLocalConnection *inspectorConnection = [ABI50_0_0RCTInspector connectPage:[pageId integerValue]
...
{
  NSString *pageId = payload[@"pageId"];
  ABI50_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
...
- (void)handleWrappedEvent:(NSDictionary *)payload
{
  ...
  ABI50_0_0RCTInspectorLocalConnection *inspectorConnection = _inspectorConnections[pageId];
  if (!inspectorConnection) {
    ABI50_0_0RCTLogWarn(@"Not connected to page: %@ , failed trying to handle event: %@", pageId, wrappedEvent);
    return;
  }
...
- (NSArray *)pages
{
  NSArray<ABI50_0_0RCTInspectorPage *> *pages = [ABI50_0_0RCTInspector pages];
  NSMutableArray *array = [NSMutableArray arrayWithCapacity:pages.count];

  ABI50_0_0RCTBundleStatusProvider statusProvider = _bundleStatusProvider;
  ABI50_0_0RCTBundleStatus *bundleStatus = statusProvider == nil ? nil : statusProvider();

  for (ABI50_0_0RCTInspectorPage *page in pages) {
...
- (void)webSocket:(__unused SRWebSocket *)webSocket didReceiveMessage:(id)opaqueMessage
{
  ...
  if (![opaqueMessage isKindOfClass:[NSString class]]) {
    ABI50_0_0RCTLogWarn(@"Unrecognized inspector message, object is of type: %@", [opaqueMessage class]);
    return;
  }

  NSString *messageText = opaqueMessage;
  NSError *error = nil;
  id parsedJSON = ABI50_0_0RCTJSONParse(messageText, &error);
  if (error) {
    ABI50_0_0RCTLogWarn(@"Unrecognized inspector message, string was not valid JSON: %@", messageText);
    return;
  }
...
{
  if (_closed) {
    ABI50_0_0RCTLogError(@"Illegal state: Can't connect after having previously been closed.");
    return;
  }

  // The corresponding android code has a lot of custom config options for
  // timeouts, but our previous class, ABI50_0_0RCTSRWebSocket didn't have the same
...
{
  if (_closed) {
    ABI50_0_0RCTLogError(@"Illegal state: Can't reconnect after having previously been closed.");
    return;
  }

  if (_suppressConnectionErrors) {
    ABI50_0_0RCTLogWarn(@"Couldn't connect to packager, will silently retry");
    _suppressConnectionErrors = true;
  }

  __weak ABI50_0_0RCTInspectorPackagerConnection *weakSelf = self;
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, ABI50_0_0RECONNECT_DELAY_MS * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
    ABI50_0_0RCTInspectorPackagerConnection *strongSelf = weakSelf;
...
- (void)sendToPackager:(NSDictionary *)messageObject
{
  ...
  dispatch_async(_jsQueue, ^{
    ...
    if (strongSelf && !strongSelf->_closed) {
      NSError *error;
      NSString *messageText = ABI50_0_0RCTJSONStringify(messageObject, &error);
      if (error) {
        ABI50_0_0RCTLogWarn(@"Couldn't send event to packager: %@", error);
      } else {
...
- (void)abort:(NSString *)message withCause:(NSError *)cause
{
  ...
  if (![cause.domain isEqual:NSPOSIXErrorDomain] || cause.code != ECONNREFUSED) {
    ABI50_0_0RCTLogInfo(@"Error occurred, shutting down websocket connection: %@ %@", message, cause);
  }
...
@implementation ABI50_0_0RCTInspectorRemoteConnection
...
- (void)onDisconnect
{
  ABI50_0_0RCTInspectorPackagerConnection *owningPackagerConnectionStrong = _owningPackagerConnection;
