...
#import <ABI50_0_0EXSplashScreen/ABI50_0_0EXSplashScreenViewNativeProvider.h>
...
@interface ABI50_0_0EXSplashScreenService ()

@property (nonatomic, strong) NSMapTable<UIViewController *, ABI50_0_0EXSplashScreenViewController *> *splashScreenControllers;
/**
...
@implementation ABI50_0_0EXSplashScreenService

ABI50_0_0EX_REGISTER_SINGLETON_MODULE(SplashScreen);
...
- (void)showSplashScreenFor:(UIViewController *)viewController
                    options:(ABI50_0_0EXSplashScreenOptions)options
{
  id<ABI50_0_0EXSplashScreenViewProvider> splashScreenViewProvider = [ABI50_0_0EXSplashScreenViewNativeProvider new];
  return [self showSplashScreenFor:viewController
                           options:options
          splashScreenViewProvider:splashScreenViewProvider
                   successCallback:^{}
                   failureCallback:^(NSString *message){ ABI50_0_0EXLogWarn(@"%@", message); }];
}


- (void)showSplashScreenFor:(UIViewController *)viewController
                    options:(ABI50_0_0EXSplashScreenOptions)options
   splashScreenViewProvider:(id<ABI50_0_0EXSplashScreenViewProvider>)splashScreenViewProvider
            successCallback:(void (^)(void))successCallback
            failureCallback:(void (^)(NSString * _Nonnull))failureCallback
{
  if ((options & ABI50_0_0EXSplashScreenForceShow) == 0 && [self.splashScreenControllers objectForKey:viewController]) {
  ...
  ABI50_0_0EXSplashScreenViewController *splashScreenController = [[ABI50_0_0EXSplashScreenViewController alloc] initWithRootView:rootView
...
- (void)showSplashScreenFor:(UIViewController *)viewController
                    options:(ABI50_0_0EXSplashScreenOptions)options
     splashScreenController:(ABI50_0_0EXSplashScreenViewController *)splashScreenController
            successCallback:(void (^)(void))successCallback
            failureCallback:(void (^)(NSString * _Nonnull))failureCallback
{
  if ((options & ABI50_0_0EXSplashScreenForceShow) == 0 && [self.splashScreenControllers objectForKey:viewController]) {
...
- (void)onAppContentWillReload:(UIViewController *)viewController
{
  ...
  if (needsShow) {
    // For reloading apps, specify `ABI50_0_0EXSplashScreenForceShow` to show splash screen again
    [self showSplashScreenFor:viewController
                      options:ABI50_0_0EXSplashScreenForceShow
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
  ...
  if (rootViewController) {
    [self showSplashScreenFor:rootViewController options:ABI50_0_0EXSplashScreenDefault];
  }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
  if (object == UIApplication.sharedApplication.keyWindow && [keyPath isEqualToString:kRootViewController]) {
    ...
    if (newRootViewController != nil && newRootViewController != self.observingRootViewController) {
      [self removeRootViewControllerListener];
      [self showSplashScreenFor:newRootViewController options:ABI50_0_0EXSplashScreenDefault];
      [self addRootViewControllerListener];
    }
  }
  if (object == UIApplication.sharedApplication.keyWindow.rootViewController && [keyPath isEqualToString:kView]) {
    ...
    if (newView != nil && [newView.nextResponder isKindOfClass:[UIViewController class]]) {
      ...
      [self hideSplashScreenFor:viewController options:ABI50_0_0EXSplashScreenDefault successCallback:^(BOOL hasEffect){} failureCallback:^(NSString *message){}];
      [self.splashScreenControllers removeObjectForKey:viewController];
      [self showSplashScreenFor:viewController options:ABI50_0_0EXSplashScreenDefault];
    }
  }
}
