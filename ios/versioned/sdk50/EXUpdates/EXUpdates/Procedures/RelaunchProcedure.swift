  ...
  func run(procedureContext: ProcedureContext) {
    ...
    launcherWithDatabase.launchUpdate(withSelectionPolicy: selectionPolicy) { error, success in
      if success {
        ...
        ABI50_0_0RCTReloadCommandSetBundleURL(launcherWithDatabase.launchAssetUrl)
        ABI50_0_0RCTTriggerReloadCommandListeners(self.triggerReloadCommandListenersReason)

        // TODO(wschurman): this was moved to after the ABI50_0_0RCT calls to unify reload
