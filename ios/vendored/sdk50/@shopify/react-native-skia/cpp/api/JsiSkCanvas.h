...
#include "ABI50_0_0RNSkTypedArray.h"
...
public:
  ...
  JSI_HOST_FUNCTION(readPixels) {
    ...
    auto dest =
        count > 3
            ? ABI50_0_0RNSkTypedArray::getTypedArray(runtime, arguments[3], *info)
            : ABI50_0_0RNSkTypedArray::getTypedArray(runtime, jsi::Value::null(), *info);
    if (!dest.isObject()) {
      return jsi::Value::null();
    }
  ...
  explicit JsiSkCanvas(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context)
      : JsiSkHostObject(std::move(context)) {}

  JsiSkCanvas(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context, SkCanvas *canvas)
