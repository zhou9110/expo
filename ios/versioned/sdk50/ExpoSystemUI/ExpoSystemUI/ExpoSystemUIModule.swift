...
public class ExpoSystemUIModule: Module {
  ...
  static func getBackgroundColor() -> String? {
    ...
    ABI50_0_0EXUtilities.performSynchronously {
      ...
      if let window = UIApplication.shared.delegate?.window, let backgroundColor = window?.rootViewController?.view.backgroundColor?.cgColor {
        color = ABI50_0_0EXUtilities.hexString(with: backgroundColor)
      }
    }
    return color
  }
  ...
  static func setBackgroundColorAsync(color: Int?) {
    ABI50_0_0EXUtilities.performSynchronously {
      ...
      let backgroundColor = ABI50_0_0EXUtilities.uiColor(color)
      // Set the app-wide window, this could have future issues when running multiple ABI50_0_0React apps,
