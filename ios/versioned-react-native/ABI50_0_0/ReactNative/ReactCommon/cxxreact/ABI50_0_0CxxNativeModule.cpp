...
#include "ABI50_0_0CxxNativeModule.h"
#include "ABI50_0_0Instance.h"

#include <folly/json.h>
#include <glog/logging.h>
#include <iterator>

#include "ABI50_0_0JsArgumentHelpers.h"
#include "ABI50_0_0MessageQueueThread.h"
#include "ABI50_0_0SystraceSection.h"

#include <ABI50_0_0logger/ABI50_0_0React_native_log.h>

using ABI50_0_0facebook::xplat::module::CxxModule;
...
void CxxNativeModule::emitWarnIfWarnOnUsage(
  ...
  if (shouldWarnOnUse_) {
    std::string message = folly::to<std::string>(
        ...
        module_name,
        "\").");
    ABI50_0_0React_native_log_warn(message.c_str());
  }
}
...
std::string CxxNativeModule::getSyncMethodName(unsigned int ABI50_0_0ReactMethodId) {
  if (ABI50_0_0ReactMethodId >= methods_.size()) {
    throw std::invalid_argument(folly::to<std::string>(
        "methodId ",
        ABI50_0_0ReactMethodId,
        " out of range [0..",
        methods_.size(),
        "]"));
  }
  return methods_[ABI50_0_0ReactMethodId].name;
}
...
void CxxNativeModule::invoke(
  ...
  const auto& method = methods_[ABI50_0_0ReactMethodId];
  ...
  messageQueueThread_->runOnQueue([method,
    ...
    } catch (const ABI50_0_0facebook::xplat::JsArgumentException& ex) {
      throw;
