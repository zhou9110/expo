...
@interface ABI50_0_0EXModulesProxyConfig ()
...
@implementation ABI50_0_0EXModulesProxyConfig
...
@interface ABI50_0_0RCTBridge (RegisterAdditionalModuleClasses)
...
@interface ABI50_0_0EXNativeModulesProxy ()

@property (nonatomic, strong) NSRegularExpression *regexp;
@property (nonatomic, strong) ABI50_0_0EXModuleRegistry *exModuleRegistry;
...
@implementation ABI50_0_0EXNativeModulesProxy {
  __weak ABI50_0_0EXAppContext * _Nullable _appContext;
}

@synthesize bridge = _bridge;
@synthesize nativeModulesConfig = _nativeModulesConfig;

ABI50_0_0RCT_EXPORT_MODULE(NativeUnimoduleProxy)
...
{
  if (self = [super init]) {
    _exModuleRegistry = moduleRegistry != nil ? moduleRegistry : [[ABI50_0_0EXModuleRegistryProvider new] moduleRegistry];
...
/**
 The initializer for Expo Go to pass a custom `ABI50_0_0EXModuleRegistry`
 other than the default one from `ABI50_0_0EXModuleRegistryProvider`.
 The `ABI50_0_0EXModuleRegistry` is still owned by this class.
 */
- (instancetype)initWithCustomModuleRegistry:(nonnull ABI50_0_0EXModuleRegistry *)moduleRegistry
{
...
/**
 Convenience initializer used by ABI50_0_0React Native in the new setup, where the modules are registered automatically.
 */
- (instancetype)init
{
...
# pragma mark - ABI50_0_0React API
...
- (nonnull ABI50_0_0EXModulesProxyConfig *)nativeModulesConfig
{
  ...
  for (ABI50_0_0EXExportedModule *exportedModule in [_exModuleRegistry getAllExportedModules]) {
    @try {
  ...
  ABI50_0_0EXModulesProxyConfig *config = [[ABI50_0_0EXModulesProxyConfig alloc] initWithConstants:exportedModulesConstants
...
{
  ABI50_0_0ExpoBridgeModule* expoBridgeModule = [bridge moduleForClass:ABI50_0_0ExpoBridgeModule.class];
...
ABI50_0_0RCT_EXPORT_METHOD(callMethod:(NSString *)moduleName methodNameOrKey:(id)methodNameOrKey arguments:(NSArray *)arguments resolver:(ABI50_0_0RCTPromiseResolveBlock)resolve rejecter:(ABI50_0_0RCTPromiseRejectBlock)reject)
{
  ...
  ABI50_0_0EXExportedModule *module = [_exModuleRegistry getExportedModuleForName:moduleName];
  if (module == nil) {
...
- (void)registerExpoModulesInBridge:(ABI50_0_0RCTBridge *)bridge
{
  // Registering expo modules (excluding Swifty view managers!) in bridge is needed only when the proxy module owns
  // the registry (was autoinitialized by ABI50_0_0React Native). Otherwise they're registered by the registry adapter.
  BOOL ownsModuleRegistry = _ownsModuleRegistry && ![bridge moduleIsInitialized:[ABI50_0_0EXReactNativeEventEmitter class]];

  // An array of `ABI50_0_0RCTBridgeModule` classes to register.
  NSMutableArray<Class<ABI50_0_0RCTBridgeModule>> *additionalModuleClasses = [NSMutableArray new];
  ...
  // `registerAdditionalModuleClasses:` call below is not thread-safe if ABI50_0_0RCTUIManager is not initialized.
  // The case happens especially with reanimated which accesses `bridge.uiManager` and initialize bridge in js thread.
  // Accessing uiManager here, we try to make sure ABI50_0_0RCTUIManager is initialized.
  [bridge uiManager];
  ...
  // Get the instance of `ABI50_0_0EXReactEventEmitter` bridge module and give it access to the interop bridge.
  ABI50_0_0EXReactNativeEventEmitter *eventEmitter = [bridge moduleForClass:[ABI50_0_0EXReactNativeEventEmitter class]];
  ...
  if (ownsModuleRegistry) {
    ...
    // It calls `setModuleRegistry:` on all `ABI50_0_0EXModuleRegistryConsumer`s.
    [_exModuleRegistry initialize];
  }
}

- (void)registerAdditionalModuleClasses:(NSArray<Class> *)moduleClasses inBridge:(ABI50_0_0RCTBridge *)bridge
{
  // In remote debugging mode, i.e. executorClass is `ABI50_0_0RCTWebSocketExecutor`,
  // there is a deadlock issue in `registerAdditionalModuleClasses:` and causes app freezed.
  //   - The JS thread acquired the `ABI50_0_0RCTCxxBridge._moduleRegistryLock` lock in `ABI50_0_0RCTCxxBridge._initializeBridgeLocked`
  //      = it further goes into ABI50_0_0RCTObjcExecutor and tries to get module config from main thread
  //   - The main thread is pending in `ABI50_0_0RCTCxxBridge.registerAdditionalModuleClasses` where trying to acquire the same lock.
  ...
  // As we just workaround in `ABI50_0_0RCTWebSocketExecutor` case, the risk of `_moduleRegistryCreated` race condition should be lower.
  //
  // Learn more about the non-locked initialization:
  // https://github.com/facebook/react-native/blob/757bb75fbf837714725d7b2af62149e8e2a7ee51/ABI50_0_0React/CxxBridge/ABI50_0_0RCTCxxBridge.mm#L922-L935
  // See the `_moduleRegistryCreated` false case
  if ([NSStringFromClass([bridge executorClass]) isEqualToString:@"ABI50_0_0RCTWebSocketExecutor"]) {
...
- (Class)registerComponentData:(ViewModuleWrapper *)viewModule inBridge:(ABI50_0_0RCTBridge *)bridge
{
  // Hacky way to get a dictionary with `ABI50_0_0RCTComponentData` from UIManager.
  NSMutableDictionary<NSString *, ABI50_0_0RCTComponentData *> *componentDataByName = [bridge.uiManager valueForKey:@"_componentDataByName"];
  ...
  ABI50_0_0EXComponentData *componentData = [[ABI50_0_0EXComponentData alloc] initWithViewModule:viewModule
  ...
  [[ABI50_0_0RCTComponentViewFactory currentComponentViewFactory] registerComponentViewClass:viewClass];
#endif
 ...
 */
- (void)registerLegacyComponentData:(Class)moduleClass inBridge:(ABI50_0_0RCTBridge *)bridge
{
  // Hacky way to get a dictionary with `ABI50_0_0RCTComponentData` from UIManager.
  NSMutableDictionary<NSString *, ABI50_0_0RCTComponentData *> *componentDataByName = [bridge.uiManager valueForKey:@"_componentDataByName"];
  NSString *className = [moduleClass moduleName] ?: NSStringFromClass(moduleClass);

  if ([moduleClass isSubclassOfClass:[ABI50_0_0RCTViewManager class]] && !componentDataByName[className]) {
    ABI50_0_0RCTComponentData *componentData = [[ABI50_0_0RCTComponentData alloc] initWithManagerClass:moduleClass bridge:bridge eventDispatcher:bridge.eventDispatcher];
    componentDataByName[className] = componentData;
  }
}
