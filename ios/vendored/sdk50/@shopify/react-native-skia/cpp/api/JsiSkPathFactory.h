...
public:
  ...
  JSI_HOST_FUNCTION(MakeFromCmds) {
    ...
    for (int i = 0; i < cmdCount; i++) {
      ...
      if (cmd.size(runtime) < 1) {
        ABI50_0_0RNSkLogger::logToConsole("Invalid command found (got an empty array)");
      ...
      case MOVE: {
        if (cmd.size(runtime) < 3) {
          ABI50_0_0RNSkLogger::logToConsole("Invalid move command found");
          return jsi::Value::null();
        }
      ...
      case LINE: {
        if (cmd.size(runtime) < 3) {
          ABI50_0_0RNSkLogger::logToConsole("Invalid line command found");
          return jsi::Value::null();
        }
        auto x = cmd.getValueAtIndex(runtime, 1).asNumber();
        auto y = cmd.getValueAtIndex(runtime, 2).asNumber();
      ...
      default: {
        ABI50_0_0RNSkLogger::logToConsole("Found an unknown command");
        return jsi::Value::null();
      }
      }
    }
  ...
  explicit JsiSkPathFactory(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context)
