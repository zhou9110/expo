...
@implementation ABI50_0_0RCTModalHostView {
  __weak ABI50_0_0RCTBridge *_bridge;
  BOOL _isPresented;
  ABI50_0_0RCTModalHostViewController *_modalViewController;
  ABI50_0_0RCTTouchHandler *_touchHandler;
  UIView *_ABI50_0_0ReactSubview;
  UIInterfaceOrientation _lastKnownOrientation;
  ABI50_0_0RCTDirectEventBlock _onRequestClose;
}

ABI50_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithFrame : (CGRect)frame)
ABI50_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : coder)

- (instancetype)initWithBridge:(ABI50_0_0RCTBridge *)bridge
{
  if ((self = [super initWithFrame:CGRectZero])) {
    _bridge = bridge;
    _modalViewController = [ABI50_0_0RCTModalHostViewController new];
    UIView *containerView = [UIView new];
    containerView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _modalViewController.view = containerView;
    _touchHandler = [[ABI50_0_0RCTTouchHandler alloc] initWithBridge:bridge];
    _isPresented = NO;
...
{
  if (_ABI50_0_0ReactSubview && _isPresented) {
    [_bridge.uiManager setSize:newBounds.size forView:_ABI50_0_0ReactSubview];
    [self notifyForOrientationChange];
  }
}

- (void)setOnRequestClose:(ABI50_0_0RCTDirectEventBlock)onRequestClose
{
...
- (void)notifyForOrientationChange
{
  ...
  UIInterfaceOrientation currentOrientation = [ABI50_0_0RCTSharedApplication() statusBarOrientation];
...
{
  ABI50_0_0RCTAssert(_ABI50_0_0ReactSubview == nil, @"Modal view can only have one subview");
  [super insertABI50_0_0ReactSubview:subview atIndex:atIndex];
  [_touchHandler attachToView:subview];

  [_modalViewController.view insertSubview:subview atIndex:0];
  _ABI50_0_0ReactSubview = subview;
}

- (void)removeABI50_0_0ReactSubview:(UIView *)subview
{
  ABI50_0_0RCTAssert(subview == _ABI50_0_0ReactSubview, @"Cannot remove view other than modal view");
  // Superclass (category) removes the `subview` from actual `superview`.
  [super removeABI50_0_0ReactSubview:subview];
  [_touchHandler detachFromView:subview];
  _ABI50_0_0ReactSubview = nil;
}

- (void)didUpdateABI50_0_0ReactSubviews
{
  // Do nothing, as subview (singular) is managed by `insertABI50_0_0ReactSubview:atIndex:`
}
...
- (void)didMoveToWindow
{
  ...
  if (!self.userInteractionEnabled && ![self.superview.ABI50_0_0ReactSubviews containsObject:self]) {
    return;
  }
...
- (void)ensurePresentedOnlyIfNeeded
{
  ...
  if (shouldBePresented) {
    ABI50_0_0RCTAssert(self.ABI50_0_0ReactViewController, @"Can't present modal view controller without a presenting view controller");
