...
#import <ABI50_0_0React/ABI50_0_0RCTWebSocketExecutor.h>
...
typedef void (^ABI50_0_0RCTWSMessageCallback)(NSError *error, NSDictionary<NSString *, id> *reply);

@interface ABI50_0_0RCTWebSocketExecutor () <SRWebSocketDelegate, ABI50_0_0RCTTurboModule>

@end

@implementation ABI50_0_0RCTWebSocketExecutor {
  SRWebSocket *_socket;
  dispatch_queue_t _jsQueue;
  NSMutableDictionary<NSNumber *, ABI50_0_0RCTWSMessageCallback> *_callbacks;
...
{
  if (!_url) {
    NSInteger port = [[[_bridge bundleURL] port] integerValue] ?: ABI50_0_0RCT_METRO_PORT;
    ...
    _url = [ABI50_0_0RCTConvert NSURL:URLString];
  }
  ...
  if (![self connectToProxy]) {
    ...
    NSString *error = [NSString stringWithFormat:
                                    @"Connection to %@ timed out. Are you "
                                     ...
                                     "you have the right IP address in `ABI50_0_0RCTWebSocketExecutor.m`.",
                                    _url];
    _setupError = ABI50_0_0RCTErrorWithMessage(error);
    ABI50_0_0RCTFatal(_setupError);
    return;
  }
...
- (BOOL)prepareJSRuntime
{
  ...
  if (initError) {
    ABI50_0_0RCTLogInfo(@"Websocket runtime setup failed: %@", initError);
  }
...
{
  NSError *error = nil;
  NSDictionary<NSString *, id> *reply = ABI50_0_0RCTJSONParse(message, &error);
  NSNumber *messageID = reply[@"replyID"];
  ABI50_0_0RCTWSMessageCallback callback = _callbacks[messageID];
  if (callback) {
...
{
  dispatch_semaphore_signal(_socketOpenSemaphore);
  ABI50_0_0RCTLogInfo(@"WebSocket connection failed with error %@", error);
}

- (void)sendMessage:(NSDictionary<NSString *, id> *)message onReply:(ABI50_0_0RCTWSMessageCallback)callback
{
  ...
  dispatch_async(_jsQueue, ^{
    ...
    [self->_socket sendString:ABI50_0_0RCTJSONStringify(messageWithID, NULL) error:nil];
  });
}

- (void)executeApplicationScript:(NSData *)script
                       sourceURL:(NSURL *)URL
                      onComplete:(ABI50_0_0RCTJavaScriptCompleteBlock)onComplete
{
  // Hack: the bridge transitions out of loading state as soon as this method returns, which prevents us
  // from completely invalidating the bridge and preventing an endless barage of ABI50_0_0RCTLog.logIfNoNativeHook
  ...
  NSDictionary<NSString *, id> *message = @{
    ...
    @"url" : ABI50_0_0RCTNullIfNil(URL.absoluteString),
  ...
  [self sendMessage:message
            onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
              if (socketError) {
                ...
                onComplete(error ? ABI50_0_0RCTErrorWithMessage(error) : nil);
              }
...
- (void)flushedQueue:(ABI50_0_0RCTJavaScriptCallback)onComplete
{
...
- (void)callFunctionOnModule:(NSString *)module
                   ...
                   arguments:(NSArray *)args
                    callback:(ABI50_0_0RCTJavaScriptCallback)onComplete
{
  [self _executeJSCall:@"callFunctionReturnFlushedQueue" arguments:@[ module, method, args ] callback:onComplete];
}

- (void)invokeCallbackID:(NSNumber *)cbID arguments:(NSArray *)args callback:(ABI50_0_0RCTJavaScriptCallback)onComplete
{
  [self _executeJSCall:@"invokeCallbackAndReturnFlushedQueue" arguments:@[ cbID, args ] callback:onComplete];
}

- (void)_executeJSCall:(NSString *)method arguments:(NSArray *)arguments callback:(ABI50_0_0RCTJavaScriptCallback)onComplete
{
  ABI50_0_0RCTAssert(onComplete != nil, @"callback was missing for exec JS call");
  ...
  [self sendMessage:message
            onReply:^(NSError *socketError, NSDictionary<NSString *, id> *reply) {
              ...
              NSError *jsonError;
              id result = ABI50_0_0RCTJSONParse(reply[@"result"], &jsonError);
              NSString *error = reply[@"error"];
              onComplete(result, error ? ABI50_0_0RCTErrorWithMessage(error) : jsonError);
            }];
}

- (void)injectJSONText:(NSString *)script
    asGlobalObjectNamed:(NSString *)objectName
               callback:(ABI50_0_0RCTJavaScriptCompleteBlock)onComplete
{
  dispatch_async(_jsQueue, ^{
...
{
  ABI50_0_0RCTExecuteOnMainQueue(block);
}
...
- (void)dealloc
{
  ABI50_0_0RCTAssert(!self.valid, @"-invalidate must be called before -dealloc");
}
...
Class ABI50_0_0RCTWebSocketExecutorCls(void)
{
#if ABI50_0_0RCT_DEV
  return ABI50_0_0RCTWebSocketExecutor.class;
#else
  return nil;
#endif
}
