...
@implementation ABI50_0_0EXCalendarConverter
...
+ (NSDictionary *)serializeSource:(EKSource *)source
{
  ...
  return @{
       ...
       @"type": [ABI50_0_0EXCalendarConverter _sourceType:source.sourceType],
       @"name": ABI50_0_0EXNullIfNil(source.title)
       };
}
...
+ (NSMutableDictionary *)serializeCalendar:(EKCalendar *)calendar
{
  ...
  serializedCalendar[@"id"] = ABI50_0_0EXNullIfNil(calendar.calendarIdentifier);
  serializedCalendar[@"title"] = ABI50_0_0EXNullIfNil(calendar.title);
  serializedCalendar[@"source"] = ABI50_0_0EXNullIfNil([ABI50_0_0EXCalendarConverter serializeSource:calendar.source]);
  serializedCalendar[@"entityType"] = [ABI50_0_0EXCalendarConverter _entityType:calendar.allowedEntityTypes];
  serializedCalendar[@"color"] = calendar.CGColor ? [ABI50_0_0EXUtilities hexStringWithCGColor:calendar.CGColor] : [NSNull null];
  serializedCalendar[@"type"] = [ABI50_0_0EXCalendarConverter _calendarType:calendar.type];
  serializedCalendar[@"allowsModifications"] = @(calendar.allowsContentModifications);
  serializedCalendar[@"allowedAvailabilities"] = [ABI50_0_0EXCalendarConverter _calendarSupportedAvailabilitiesFromMask:calendar.supportedEventAvailabilities];
...
+ (NSArray *)serializeCalendars:(NSArray *)calendars
{
  ...
  for (EKCalendar *calendar in calendars) {
    [serializedCalendars addObject:[ABI50_0_0EXCalendarConverter serializeCalendar:calendar]];
  }
...
+ (NSMutableDictionary *)_serializeCalendarItem:(EKCalendarItem *)item withDateFormatter:(NSDateFormatter *)dateFormatter
{
  ...
  serializedItem[@"timeZone"] = ABI50_0_0EXNullIfNil(item.timeZone.name);
  serializedItem[@"url"] = ABI50_0_0EXNullIfNil(item.URL.absoluteString.stringByRemovingPercentEncoding);
  serializedItem[@"notes"] = ABI50_0_0EXNullIfNil(item.notes);
  serializedItem[@"alarms"] = [ABI50_0_0EXCalendarConverter _serializeAlarms:item.alarms withDateFormatter:dateFormatter];

  if (item.hasRecurrenceRules) {
    EKRecurrenceRule *rule = [item.recurrenceRules objectAtIndex:0];
    NSString *frequencyType = [ABI50_0_0EXCalendarConverter _recurrenceFrequency:[rule frequency]];
...
+ (NSArray *)serializeCalendarEvents:(NSArray *)calendarEvents
{
  ...
  for (EKEvent *event in calendarEvents) {
    ...
    [serializedCalendarEvents addObject:[ABI50_0_0EXCalendarConverter serializeCalendarEvent:event]];
  }
...
+ (NSDictionary *)serializeCalendarEvent:(EKEvent *)event
{
  ...
  NSMutableDictionary *formedCalendarEvent = [ABI50_0_0EXCalendarConverter _serializeCalendarItem:event withDateFormatter:dateFormatter];
  ...
  formedCalendarEvent[@"availability"] = [ABI50_0_0EXCalendarConverter _eventAvailability:event.availability];
  formedCalendarEvent[@"status"] = [ABI50_0_0EXCalendarConverter _eventStatus:event.status];
  formedCalendarEvent[@"organizer"] = ABI50_0_0EXNullIfNil([ABI50_0_0EXCalendarConverter _serializeAttendee:event.organizer]);
...
+ (NSArray *)serializeReminders:(NSArray<EKReminder *> *)reminders
{
  ...
  for (EKReminder *reminder in reminders) {
    [serializedReminders addObject:[ABI50_0_0EXCalendarConverter serializeReminder:reminder]];
  }
...
+ (NSDictionary *)serializeReminder:(EKReminder *)reminder
{
  ...
  NSMutableDictionary *formedReminder = [ABI50_0_0EXCalendarConverter _serializeCalendarItem:reminder withDateFormatter:dateFormatter];
...
+ (NSArray *)serializeAttendees:(NSArray<EKParticipant *> *)attendees
{
  ...
  for (EKParticipant *attendee in attendees) {
    [serializedAttendees addObject:[ABI50_0_0EXCalendarConverter _serializeAttendee:attendee]];
  }
...
+ (NSDictionary *)_serializeAttendee:(EKParticipant *)attendee
{
  ...
  if (attendee.participantRole) {
    formedAttendee[@"role"] = [ABI50_0_0EXCalendarConverter _participantRole:attendee.participantRole];
  }

  if (attendee.participantStatus) {
    formedAttendee[@"status"] = [ABI50_0_0EXCalendarConverter _participantStatus:attendee.participantStatus];
  }

  if (attendee.participantType) {
    formedAttendee[@"type"] = [ABI50_0_0EXCalendarConverter _participantType:attendee.participantType];
  }
