...
/**
 Each module that has a view manager definition needs to be wrapped by `ABI50_0_0RCTViewManager`.
 Unfortunately, we can't use just one class because ABI50_0_0React Native checks for duplicated classes.
 We're generating its subclasses in runtime as a workaround.
 */
@objc
public final class ViewModuleWrapper: ABI50_0_0RCTViewManager, DynamicModuleWrapperProtocol {
  /**
  ...
  /**
   The designated initializer that is used by ABI50_0_0React Native to create module instances.
   https://github.com/facebook/react-native/blob/540c41be9/packages/react-native/ABI50_0_0React/Views/ABI50_0_0RCTComponentData.m#L506-L507
  ...
  /**
   Dummy initializer, for use only in `ABI50_0_0EXModuleRegistryAdapter.extraModulesForModuleRegistry:`.
   */
  @objc
  ...
  public static func createViewModuleWrapperClass(module: ViewModuleWrapper) -> ViewModuleWrapper.Type? {
    ...
    return prefixedViewName.withCString { viewNamePtr in
      ...
      // ABI50_0_0React Native initializes modules with `init` without params,
...
let directEventBlockImplementation = imp_implementationWithBlock({ ["ABI50_0_0RCTDirectEventBlock"] } as @convention(block) () -> [String])
