...
@interface ABI50_0_0EXBarCodeScanner() <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
...
NSString *const ABI50_0_0EX_BARCODE_TYPES_KEY = @"barCodeTypes";

@implementation ABI50_0_0EXBarCodeScanner
...
- (void)setSettings:(NSDictionary<NSString *, id> *)settings
{
  for (NSString *key in settings) {
    if ([key isEqualToString:ABI50_0_0EX_BARCODE_TYPES_KEY]) {
      NSArray<NSString *> *value = settings[key];
      NSSet *previousTypes = [NSSet setWithArray:_settings[ABI50_0_0EX_BARCODE_TYPES_KEY]];
      ...
      if (![previousTypes isEqualToSet:newTypes]) {
        ...
        nextSettings[ABI50_0_0EX_BARCODE_TYPES_KEY] = value;
...
- (void)maybeStartBarCodeScanning
{
  ...
  if (_settings && _settings[ABI50_0_0EX_BARCODE_TYPES_KEY]) {
    requestedObjectTypes = [[NSArray alloc] initWithArray:_settings[ABI50_0_0EX_BARCODE_TYPES_KEY]];
  }
...
{
  if (!_settings || !_settings[ABI50_0_0EX_BARCODE_TYPES_KEY] || !_metadataOutput) {
    return;
  }
  ...
  for (AVMetadataObject *metadata in metadataObjects) {
    if ([metadata isKindOfClass:[AVMetadataMachineReadableCodeObject class]]) {
      ...
      for (id barcodeType in _settings[ABI50_0_0EX_BARCODE_TYPES_KEY]) {
        ...
        if (codeMetadata.stringValue && [codeMetadata.type isEqualToString:barcodeType]) {
          if (_onBarCodeScanned) {
            _onBarCodeScanned([ABI50_0_0EXBarCodeScannerUtils avMetadataCodeObjectToDicitionary:codeMetadata]);
          }
          return;
        }
      }
    ...
    if (curFrameTimeStamp - lastFrameTimeStamp > kMinMargin) {
      ...
      [self scanBarcodesFromImage:videoFrameImage withCompletion:^(ZXResult *barCodeScannerResult, NSError *error) {
        if (self->_onBarCodeScanned) {
          self->_onBarCodeScanned([ABI50_0_0EXBarCodeScannerUtils zxResultToDicitionary:barCodeScannerResult]);
        }
      }];
    }
  }
...
{
  return @{
           ABI50_0_0EX_BARCODE_TYPES_KEY: [[ABI50_0_0EXBarCodeScannerUtils validBarCodeTypes] allValues],
           };
}
