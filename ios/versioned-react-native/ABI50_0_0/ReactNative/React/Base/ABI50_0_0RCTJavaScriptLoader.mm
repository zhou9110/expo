...
#import "ABI50_0_0RCTJavaScriptLoader.h"
...
#import "ABI50_0_0RCTMultipartDataTask.h"
#import "ABI50_0_0RCTPerformanceLogger.h"
#import "ABI50_0_0RCTUtils.h"

NSString *const ABI50_0_0RCTJavaScriptLoaderErrorDomain = @"ABI50_0_0RCTJavaScriptLoaderErrorDomain";

@interface ABI50_0_0RCTSource () {
 @public
  NSURL *_url;
  NSData *_data;
  NSUInteger _length;
...
@implementation ABI50_0_0RCTSource

static ABI50_0_0RCTSource *ABI50_0_0RCTSourceCreate(NSURL *url, NSData *data, int64_t length) NS_RETURNS_RETAINED
{
  using ABI50_0_0facebook::ABI50_0_0React::ScriptTag;
  ABI50_0_0facebook::ABI50_0_0React::BundleHeader header;
  [data getBytes:&header length:sizeof(header)];

  ABI50_0_0RCTSource *source = [ABI50_0_0RCTSource new];
  source->_url = url;
  source->_data = data;
  source->_length = length;
  source->_filesChangedCount = ABI50_0_0RCTSourceFilesChangedCountNotBuiltByBundler;
  return source;
}

@end

@implementation ABI50_0_0RCTLoadingProgress
...
@implementation ABI50_0_0RCTJavaScriptLoader

ABI50_0_0RCT_NOT_IMPLEMENTED(-(instancetype)init)

+ (void)loadBundleAtURL:(NSURL *)scriptURL
             onProgress:(ABI50_0_0RCTSourceLoadProgressBlock)onProgress
             onComplete:(ABI50_0_0RCTSourceLoadBlock)onComplete
{
  int64_t sourceLength;
  NSError *error;
  NSData *data = [self attemptSynchronousLoadOfBundleAtURL:scriptURL sourceLength:&sourceLength error:&error];
  if (data) {
    onComplete(nil, ABI50_0_0RCTSourceCreate(scriptURL, data, sourceLength));
    return;
  }

  const BOOL isCannotLoadSyncError = [error.domain isEqualToString:ABI50_0_0RCTJavaScriptLoaderErrorDomain] &&
      error.code == ABI50_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously;
                                          ...
                                          error:(NSError **)error
{
  ...
  if (!scriptURL) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI50_0_0RCTJavaScriptLoaderErrorNoScriptURL
                 userInfo:@{
  ...
  if (!scriptURL.fileURL) {
    if (error) {
      *error = [NSError errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                                   code:ABI50_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
                               userInfo:@{
  ...
  if (!bundle) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI50_0_0RCTJavaScriptLoaderErrorFailedOpeningFile
                 userInfo:@{
  ...
  if (readResult != 1) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI50_0_0RCTJavaScriptLoaderErrorFailedReadingFile
                 userInfo:@{
  ...
  ABI50_0_0facebook::ABI50_0_0React::ScriptTag tag = ABI50_0_0facebook::ABI50_0_0React::parseTypeFromHeader(header);
  switch (tag) {
    case ABI50_0_0facebook::ABI50_0_0React::ScriptTag::RAMBundle:
      break;

    case ABI50_0_0facebook::ABI50_0_0React::ScriptTag::String: {
      ...
      if (error) {
        *error =
            [NSError errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                                code:ABI50_0_0RCTJavaScriptLoaderErrorCannotBeLoadedSynchronously
  ...
  if (stat(scriptURL.path.UTF8String, &statInfo) != 0) {
    if (error) {
      *error = [NSError
          errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                     code:ABI50_0_0RCTJavaScriptLoaderErrorFailedStatingFile
                 userInfo:@{
...
static void parseHeaders(NSDictionary *headers, ABI50_0_0RCTSource *source)
{
...
static void attemptAsynchronousLoadOfBundleAtURL(
    NSURL *scriptURL,
    ABI50_0_0RCTSourceLoadProgressBlock onProgress,
    ABI50_0_0RCTSourceLoadBlock onComplete)
{
  scriptURL = sanitizeURL(scriptURL);
  ...
  if (scriptURL.fileURL) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      NSError *error = nil;
      NSData *source = [NSData dataWithContentsOfFile:scriptURL.path options:NSDataReadingMappedIfSafe error:&error];
      onComplete(error, ABI50_0_0RCTSourceCreate(scriptURL, source, source.length));
    });
    return;
  }

  ABI50_0_0RCTMultipartDataTask *task = [[ABI50_0_0RCTMultipartDataTask alloc] initWithURL:scriptURL
      partHandler:^(NSInteger statusCode, NSDictionary *headers, NSData *data, NSError *error, BOOL done) {
        ...
        if (error) {
          if ([error.domain isEqualToString:NSURLErrorDomain]) {
            error = [NSError
                errorWithDomain:ABI50_0_0RCTJavaScriptLoaderErrorDomain
                           code:ABI50_0_0RCTJavaScriptLoaderErrorURLLoadFailed
                       userInfo:@{
                         NSLocalizedDescriptionKey :
                             [@"Could not connect to development server.\n\n"
                               "Ensure the following:\n"
                               "- Node server is running and available on the same network - run 'npm start' from ABI50_0_0React-native root\n"
        ...
        ABI50_0_0RCTSource *source = ABI50_0_0RCTSourceCreate(sourceURL, data, data.length);
...
static NSURL *sanitizeURL(NSURL *url)
{
  ...
  return [ABI50_0_0RCTConvert NSURL:url.absoluteString];
}

static ABI50_0_0RCTLoadingProgress *progressEventFromData(NSData *rawData)
{
  NSString *text = [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding];
  id info = ABI50_0_0RCTJSONParse(text, nil);
  if (!info || ![info isKindOfClass:[NSDictionary class]]) {
    return nil;
  }

  ABI50_0_0RCTLoadingProgress *progress = [ABI50_0_0RCTLoadingProgress new];
...
static ABI50_0_0RCTLoadingProgress *progressEventFromDownloadProgress(NSNumber *total, NSNumber *done)
{
  ABI50_0_0RCTLoadingProgress *progress = [ABI50_0_0RCTLoadingProgress new];
...
{
  NSDictionary *parsedResponse = ABI50_0_0RCTJSONParse(rawText, nil);
