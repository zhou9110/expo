...
@protocol ABI50_0_0RCTJavaScriptExecutor;

ABI50_0_0RCT_EXTERN NSArray<Class> *ABI50_0_0RCTGetModuleClasses(void);
...
// Used by ABI50_0_0RCTDevMenu
@property (nonatomic, copy) NSString *bridgeDescription;

+ (instancetype)currentBridge;
+ (void)setCurrentBridge:(ABI50_0_0RCTBridge *)bridge;

/**
 * Bridge setup code - creates an instance of ABI50_0_0RCTBachedBridge. Exposed for
 * test only
 */
- (void)setUp;
 ...
 * a background thread if the ABI50_0_0RCTBridge happens to deallocate on a background
 * thread. Therefore, we want all writes to it to be seen atomically.
 */
@property (atomic, strong) ABI50_0_0RCTBridge *batchedBridge;

/**
 * The block that creates the modules' instances to be added to the bridge.
 * Exposed for ABI50_0_0RCTCxxBridge
 */
@property (nonatomic, copy, readonly) ABI50_0_0RCTBridgeModuleListProvider moduleProvider;

/**
 * Used by ABI50_0_0RCTDevMenu to override the `hot` param of the current bundleURL.
 */
@property (nonatomic, strong, readwrite) NSURL *bundleURL;

/**
 * An object that allows one to require NativeModules/TurboModules.
 * ABI50_0_0RCTModuleRegistry is implemented in bridgeless mode and bridge mode.
 * Used by ABI50_0_0RCTRootView.
 */
@property (nonatomic, strong, readonly) ABI50_0_0RCTModuleRegistry *moduleRegistry;

@end

@interface ABI50_0_0RCTBridge (ABI50_0_0RCTCxxBridge)

/**
 * Used by ABI50_0_0RCTModuleData
 */
...
/**
 * Used by ABI50_0_0RCTModuleData to register the module for frame updates after it is
 * lazily initialized.
 */
- (void)registerModuleForFrameUpdates:(id<ABI50_0_0RCTBridgeModule>)module withModuleData:(ABI50_0_0RCTModuleData *)moduleData;

/**
 * Dispatch work to a module's queue - this is also supports the fake ABI50_0_0RCTJSThread
 * queue. Exposed for the ABI50_0_0RCTProfiler
 */
...
/**
 * Systrace profiler toggling methods exposed for the ABI50_0_0RCTDevMenu
 */
- (void)startProfiling;
...
/**
 * Hook exposed for ABI50_0_0RCTLog to send logs to JavaScript when not running in JSC
 */
...
@interface ABI50_0_0RCTBridge (Inspector)

@property (nonatomic, readonly, getter=isInspectable) BOOL inspectable;

@end

@interface ABI50_0_0RCTCxxBridge : ABI50_0_0RCTBridge

// TODO(cjhopman): this seems unsafe unless we require that it is only called on the main js queue.
@property (nonatomic, readonly) void *runtime;

- (instancetype)initWithParentBridge:(ABI50_0_0RCTBridge *)bridge NS_DESIGNATED_INITIALIZER;
