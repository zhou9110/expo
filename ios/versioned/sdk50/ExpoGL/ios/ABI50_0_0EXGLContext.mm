...
@implementation ABI50_0_0EXGLContext

- (instancetype)initWithDelegate:(id<ABI50_0_0EXGLContextDelegate>)delegate
               andModuleRegistry:(nonnull ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  if (self = [super init]) {
    ...
    _objectManager = (ABI50_0_0EXGLObjectManager *)[_moduleRegistry getExportedModuleOfClass:[ABI50_0_0EXGLObjectManager class]];
...
- (void)initialize
{
  self->_contextId = ABI50_0_0EXGLContextCreate();
...
- (void)prepare:(void(^)(BOOL))callback andEnableExperimentalWorkletSupport:(BOOL)enableExperimentalWorkletSupport
{
  ...
  id<ABI50_0_0EXUIManager> uiManager = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI50_0_0EXUIManager)];
  id<ABI50_0_0EXJavaScriptContextProvider> jsContextProvider = [_moduleRegistry getModuleImplementingProtocol:@protocol(ABI50_0_0EXJavaScriptContextProvider)];
  ...
  if (jsRuntimePtr) {
    ...
    [uiManager dispatchOnClientThread:^{
      ABI50_0_0EXGLContext *self = weakSelf;
      id<ABI50_0_0EXUIManager> uiManager = weakUIManager;
      ...
      ABI50_0_0EXGLContextSetDefaultFramebuffer(self->_contextId, [self defaultFramebuffer]);
      ABI50_0_0EXGLContextPrepare(jsRuntimePtr, self->_contextId, [self](){
        [self flush];
      });

      if (enableExperimentalWorkletSupport) {
        dispatch_sync(dispatch_get_main_queue(), ^{
          ABI50_0_0EXGLContextPrepareWorklet(self->_contextId);
        });
      }
      _isContextReady = YES;
    ...
    BLOCK_SAFE_RUN(callback, NO);
    ABI50_0_0EXLogWarn(@"ABI50_0_0EXGL: Can only run on JavaScriptCore! Do you have 'Remote Debugging' enabled in your app's Developer Menu (https://reactnative.dev/docs/debugging)? ABI50_0_0EXGL is not supported while using Remote Debugging, you will need to disable it to use ABI50_0_0EXGL.");
  }
}
...
- (void)flush
{
  ...
  [self runAsync:^{
    ABI50_0_0EXGLContextFlush(self->_contextId);
...
- (void)destroy
{
  ...
  [self runAsync:^{
    ...
    [uiManager dispatchOnClientThread:^{
      // Destroy JS binding
      ABI50_0_0EXGLContextDestroy(self->_contextId);
                         ...
                         reject:(ABI50_0_0EXPromiseRejectBlock)reject
{
  ...
  [self runAsync:^{
    ...
    if (options[@"framebuffer"] && options[@"framebuffer"][@"id"]) {
      ...
      sourceFramebuffer = ABI50_0_0EXGLContextGetObject(self.contextId, exglFramebufferId);
    } else {
    ...
    if (sourceFramebuffer == 0) {
      reject(
             @"E_GL_NO_FRAMEBUFFER",
             nil,
             ABI50_0_0EXErrorWithMessage(@"No framebuffer bound. Create and bind one to take a snapshot from it.")
             );
      return;
    }
    if (width <= 0 || height <= 0) {
      reject(
             ...
             nil,
             ABI50_0_0EXErrorWithMessage(@"Rect's width and height must be greater than 0. If you didn't set `rect` option, check if the viewport is set correctly.")
             );
      return;
    }
    ...
    // Begin image context
    CGFloat scale = [ABI50_0_0EXUtilities screenScale];
    ...
    if ([format isEqualToString:@"webp"]) {
      ABI50_0_0EXLogWarn(@"iOS doesn't support 'webp' representation, so 'takeSnapshot' won't work with that format. The image is going to be exported as 'png', but consider using a different code for iOS. Check this docs to learn how to do platform specific code (https://reactnative.dev/docs/platform-specific-code)");
