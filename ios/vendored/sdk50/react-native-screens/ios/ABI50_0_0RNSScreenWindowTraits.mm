...
@implementation ABI50_0_0RNSScreenWindowTraits
...
+ (void)assertViewControllerBasedStatusBarAppearenceSet
{
  ...
  if (!viewControllerBasedAppearence) {
    ABI50_0_0RCTLogError(@"If you want to change the appearance of status bar, you have to change \
...
#if !TARGET_OS_TV
+ (UIStatusBarStyle)statusBarStyleForRNSStatusBarStyle:(ABI50_0_0RNSStatusBarStyle)statusBarStyle
{
#if defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && defined(__IPHONE_13_0) && \
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0
  if (@available(iOS 13.0, *)) {
    switch (statusBarStyle) {
      case ABI50_0_0RNSStatusBarStyleAuto:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleLightContent
            : UIStatusBarStyleDarkContent;
      case ABI50_0_0RNSStatusBarStyleInverted:
        return [UITraitCollection.currentTraitCollection userInterfaceStyle] == UIUserInterfaceStyleDark
            ? UIStatusBarStyleDarkContent
            : UIStatusBarStyleLightContent;
      case ABI50_0_0RNSStatusBarStyleLight:
        return UIStatusBarStyleLightContent;
      case ABI50_0_0RNSStatusBarStyleDark:
  ...
  if (statusBarStyle == ABI50_0_0RNSStatusBarStyleLight) {
    return UIStatusBarStyleLightContent;
  }
...
+ (void)enforceDesiredDeviceOrientation
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIInterfaceOrientation currentDeviceOrientation =
        [ABI50_0_0RNSScreenWindowTraits interfaceOrientationFromDeviceOrientation:[[UIDevice currentDevice] orientation]];
    UIInterfaceOrientation currentInterfaceOrientation = [ABI50_0_0RNSScreenWindowTraits interfaceOrientation];
    UIInterfaceOrientation newOrientation = UIInterfaceOrientationUnknown;
    if ([ABI50_0_0RNSScreenWindowTraits maskFromOrientation:currentDeviceOrientation] & orientationMask) {
      if (!([ABI50_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
      ...
      if (!([ABI50_0_0RNSScreenWindowTraits maskFromOrientation:currentInterfaceOrientation] & orientationMask)) {
        ...
        // rotation from mask
        newOrientation = [ABI50_0_0RNSScreenWindowTraits defaultOrientationForOrientationMask:orientationMask];
      } else {
...
// method to be used in Expo for checking if ABI50_0_0RNScreens have trait set
+ (BOOL)shouldAskScreensForTrait:(ABI50_0_0RNSWindowTrait)trait
                ...
                inViewController:(UIViewController *)vc
{
  ...
  if ([lastViewController conformsToProtocol:@protocol(ABI50_0_0RNSViewControllerDelegate)]) {
    ...
    if ([lastViewController isKindOfClass:[ABI50_0_0RNSViewController class]]) {
      vc = [(ABI50_0_0RNSViewController *)lastViewController findActiveChildVC];
    } else if ([lastViewController isKindOfClass:[ABI50_0_0RNSNavigationController class]]) {
      vc = [(ABI50_0_0RNSNavigationController *)lastViewController topViewController];
    }
    return [vc isKindOfClass:[ABI50_0_0RNSScreen class]] &&
        [(ABI50_0_0RNSScreen *)vc findChildVCForConfigAndTrait:trait includingModals:includingModals] != nil;
  }
  return NO;
}
...
{
  return [ABI50_0_0RNSScreenWindowTraits shouldAskScreensForTrait:ABI50_0_0RNSWindowTraitOrientation
                                         includingModals:YES
                                        inViewController:vc];
}
