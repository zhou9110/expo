...
@protocol ABI50_0_0RCTSurfaceProtocol <NSObject>

@property (atomic, readonly) ABI50_0_0RCTSurfaceStage stage;
@property (atomic, readonly) NSString *moduleName;
@property (atomic, readwrite, weak, nullable) id<ABI50_0_0RCTSurfaceDelegate> delegate;
 ...
 * `viewportOffset` is ignored in `ABI50_0_0RCTSurface` but used in `ABI50_0_0RCTFabricSurface`.
 */
 ...
 * Those methods are a no-op for regular ABI50_0_0RCTSurface (for now), but all call sites must call them appropriately.
 */
- (void)start;
- (void)stop;
