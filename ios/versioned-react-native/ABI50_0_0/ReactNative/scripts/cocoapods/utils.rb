...
class ReactNativePodsUtils
    ...
    def self.set_node_modules_user_settings(installer, react_native_path)
        ...
        projects.each do |project|
            project.build_configurations.each do |config|
                config.build_settings["ABI50_0_0REACT_NATIVE_PATH"] = File.join("${PODS_ROOT}", "..", react_native_path)
            end
    ...
    def self.set_codegen_search_paths(target_installation_result)
        header_search_paths = ReactNativePodsUtils.create_header_search_path_for_frameworks("PODS_CONFIGURATION_BUILD_DIR", "ABI50_0_0React-Codegen", "React_Codegen", [])
            .map { |search_path| "\"#{search_path}\"" }
        ReactNativePodsUtils.update_header_paths_if_depends_on(target_installation_result, "ABI50_0_0React-Codegen", header_search_paths)
    end
