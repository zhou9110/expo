...
struct ContextManager {
  std::unordered_map<ABI50_0_0EXGLContextId, ContextState> contextMap;
  std::shared_mutex contextLookupMutex;
  ABI50_0_0EXGLContextId nextId = 1;
};
...
ABI50_0_0EXGLContextId ContextCreate() {
  // Out of ids?
  if (manager.nextId >= std::numeric_limits<ABI50_0_0EXGLContextId>::max()) {
    ABI50_0_0EXGLSysLog("Ran out of ABI50_0_0EXGLContext ids!");
    return 0;
  }

  std::unique_lock lock = getUniqueLockSafely(manager.contextLookupMutex);
  ABI50_0_0EXGLContextId ctxId = manager.nextId++;
  if (manager.contextMap.find(ctxId) != manager.contextMap.end()) {
    ABI50_0_0EXGLSysLog("Tried to reuse an ABI50_0_0EXGLContext id. This shouldn't really happen...");
    return 0;
  }
  manager.contextMap[ctxId].ctx = new ABI50_0_0EXGLContext(ctxId);
  return ctxId;
}
