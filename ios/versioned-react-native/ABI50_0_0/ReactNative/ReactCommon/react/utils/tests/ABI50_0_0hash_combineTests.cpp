...
struct hash<Person> {
  size_t operator()(const Person& person) const {
    return ABI50_0_0facebook::ABI50_0_0React::hash_combine(person.firstName, person.lastName);
  }
};
} // namespace std
...
TEST(hash_combineTests, testIntegerTemplating) {
  ...
  ABI50_0_0EXPECT_EQ(hashedValue, seed);

  ABI50_0_0EXPECT_NE(hash_combine(1), hash_combine(2));
}
...
TEST(hash_combineTests, testIntegerCombinationsHashing) {
  ...
  ABI50_0_0EXPECT_NE(hash_combine(1, 2), hash_combine(2, 1));
}
...
TEST(hash_combineTests, testContiniousIntegerHashing) {
  ...
  for (int i = 1; i <= 200; ++i) {
    ...
    hash_combine(seed, i);
    ABI50_0_0EXPECT_NE(seed, previousSeed);
  }
}

TEST(hash_combineTests, testStrings) {
  std::size_t seed = 0;
  hash_combine<std::string>(seed, "ABI50_0_0React");

  auto hashedValue = hash_combine<std::string>("ABI50_0_0React");
  ABI50_0_0EXPECT_EQ(hashedValue, seed);

  ABI50_0_0EXPECT_NE(
      hash_combine<std::string>("ABI50_0_0React"),
      hash_combine<std::string>("ABI50_0_0React native"));
}
...
TEST(hash_combineTests, testCustomTypes) {
  ...
  ABI50_0_0EXPECT_NE(hash_combine(person1), hash_combine(person2));
}
