...
public final class VideoViewModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    View(ABI50_0_0EXVideoView.self) {
      Events(
        "onStatusUpdate",
        "onLoadStart",
        "onLoad",
      ...
      Prop("status") { (view: ABI50_0_0EXVideoView, status: [String: Any]) in
        view.status = status
      }

      Prop("useNativeControls") { (view: ABI50_0_0EXVideoView, useNativeControls: Bool) in
        view.useNativeControls = useNativeControls
      }

      Prop("source") { (view: ABI50_0_0EXVideoView, source: [String: Any]) in
        view.source = source
      }

      Prop("resizeMode") { (view: ABI50_0_0EXVideoView, resizeMode: String) in
        view.nativeResizeMode = resizeMode
      }
    }
  }

  private func runBlockForView(_ tag: Int, _ block: @escaping (ABI50_0_0EXVideoView) -> Void) {
    let uiManager: ABI50_0_0EXUIManager? = appContext?.legacyModule(implementing: ABI50_0_0EXUIManager.self)

    uiManager?.executeUIBlock({ (view: Any) in
      if let view = view as? ABI50_0_0EXVideoView {
        block(view)
      }
    }, forView: tag, of: ABI50_0_0EXVideoView.self)
  }
}
