...
#import <ABI50_0_0React/ABI50_0_0RCTAutoInsetsProtocol.h>
#import "ABI50_0_0RNCWKProcessPoolManager.h"
#if !TARGET_OS_OSX
...
static NSString *const HistoryShimName = @"ABI50_0_0ReactNativeHistoryShim";
...
@interface ABI50_0_0_SwizzleHelperWK : UIView
@property (nonatomic, copy) WKWebView *webView;
@end
@implementation ABI50_0_0_SwizzleHelperWK
...
- (void)scrollWheel:(NSEvent *)theEvent {
  ABI50_0_0RNCWebViewImpl *rncWebView = (ABI50_0_0RNCWebViewImpl *)[self superview];
  ABI50_0_0RCTAssert([rncWebView isKindOfClass:[rncWebView class]], @"superview must be an ABI50_0_0RNCWebViewImpl");
...
@interface ABI50_0_0RNCWebViewImpl () <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, WKHTTPCookieStoreObserver,
#if !TARGET_OS_OSX
UIScrollViewDelegate,
#endif // !TARGET_OS_OSX
ABI50_0_0RCTAutoInsetsProtocol>

@property (nonatomic, copy) ABI50_0_0RNCWKWebView *webView;
@property (nonatomic, strong) WKUserScript *postMessageScript;
@property (nonatomic, strong) WKUserScript *atStartScript;
@property (nonatomic, strong) WKUserScript *atEndScript;
@end

@implementation ABI50_0_0RNCWebViewImpl
{
#if !TARGET_OS_OSX
  UIColor * _savedBackgroundColor;
#else
  ABI50_0_0RCTUIColor * _savedBackgroundColor;
#endif // !TARGET_OS_OSX
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
...
#else
    super.backgroundColor = [ABI50_0_0RCTUIColor clearColor];
#endif // !TARGET_OS_OSX
    _bounces = YES;
    _scrollEnabled = YES;
...
#if !TARGET_OS_OSX
    _savedStatusBarStyle = ABI50_0_0RCTSharedApplication().statusBarStyle;
    _savedStatusBarHidden = ABI50_0_0RCTSharedApplication().statusBarHidden;
#endif // !TARGET_OS_OSX
    ...
    _mediaCapturePermissionGrantType = ABI50_0_0RNCWebViewPermissionGrantType_Prompt;
#endif
  }
...
- (void)startLongPress:(UILongPressGestureRecognizer *)pressSender
{
    ...
    for(NSDictionary *menuItem in self.menuItems) {
      NSString *menuItemLabel = [ABI50_0_0RCTConvert NSString:menuItem[@"label"]];
      NSString *menuItemKey = [ABI50_0_0RCTConvert NSString:menuItem[@"key"]];
...
- (void)tappedMenuItem:(NSString *)eventType
{
  ...
  [self.webView evaluateJavaScript: @"window.getSelection().toString()" completionHandler: ^(id result, NSError *error) {
    if (error != nil) {
      ABI50_0_0RCTLogWarn(@"%@", [NSString stringWithFormat:@"Error evaluating injectedJavaScript: This is possibly due to an unsupported return type. Try adding true to the end of your injectedJavaScript string. %@", error]);
    } else {
      if (self.onCustomMenuSelection) {
        ...
        NSString *label = [ABI50_0_0RCTConvert NSString:selectedMenuItem[@"label"]];
        ...
        ABI50_0_0RCTLogWarn(@"Error evaluating onCustomMenuSelection: You must implement an `onCustomMenuSelection` callback when using custom menu items");
      }
    }
  }];
}
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  if(self.useSharedProcessPool) {
    wkWebViewConfig.processPool = [[ABI50_0_0RNCWKProcessPoolManager sharedManager] sharedProcessPoolForScopeKey:self.scopeKey];
  }
  ...
  [wkWebViewConfig.userContentController addScriptMessageHandler:[[ABI50_0_0RNCWeakScriptMessageDelegate alloc] initWithDelegate:self]
...
- (void)didMoveToWindow
{
  if (self.window != nil && _webView == nil) {
    ...
    _webView = [[ABI50_0_0RNCWKWebView alloc] initWithFrame:self.bounds configuration: wkWebViewConfig];
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  ABI50_0_0RCTUnsafeExecuteOnMainQueueSync(^{
    [ABI50_0_0RCTSharedApplication() setStatusBarStyle:self->_savedStatusBarStyle animated:YES];
  });
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  ABI50_0_0RCTUnsafeExecuteOnMainQueueSync(^{
    [ABI50_0_0RCTSharedApplication() setStatusBarHidden:self->_savedStatusBarHidden animated:YES];
    [ABI50_0_0RCTSharedApplication() setStatusBarStyle:self->_savedStatusBarStyle animated:YES];
  });
#pragma clang diagnostic pop
}
...
#else
- (void)setBackgroundColor:(ABI50_0_0RCTUIColor *)backgroundColor
#endif // !TARGET_OS_OSX
{
...
- (void)setContentInset:(UIEdgeInsets)contentInset
{
  ...
  [ABI50_0_0RCTView autoAdjustInsetsForView:self
...
- (void)visitSource
{
  ...
  NSString *html = [ABI50_0_0RCTConvert NSString:_source[@"html"]];
  if (html) {
    NSURL *baseURL = [ABI50_0_0RCTConvert NSURL:_source[@"baseUrl"]];
    if (!baseURL) {
  ...
  NSString *headerCookie = [ABI50_0_0RCTConvert NSString:_source[@"headers"][@"cookie"]];
  if(headerCookie) {
  ...
  [self syncCookiesToWebView:^{
    // Because of the way ABI50_0_0React works, as pages redirect, we actually end up
    ...
    if (request.URL.host) {
      ...
      NSURL* readAccessUrl = allowingReadAccessToURL ? [ABI50_0_0RCTConvert NSURL:allowingReadAccessToURL] : request.URL;
...
-(void)setHideKeyboardAccessoryView:(BOOL)hideKeyboardAccessoryView
{
  ...
  NSString* name = [NSString stringWithFormat:@"%@ABI50_0_0_SwizzleHelperWK", subview.class.superclass];
  ...
  if(newClass == nil)
  {
    ...
    Method method = class_getInstanceMethod([ABI50_0_0_SwizzleHelperWK class], @selector(inputAccessoryView));
...
- (void)postMessage:(NSString *)message
{
  ...
  NSString *source = [NSString
                      ...
                      ABI50_0_0RCTJSONStringify(eventInitDict, NULL)
  ];
...
- (void)layoutSubviews
{
  ...
  // Ensure webview takes the position and dimensions of ABI50_0_0RNCWebViewImpl
...
- (void)webView:(WKWebView *)webView runJavaScriptTextInputPanelWithPrompt:(NSString *)prompt defaultText:(NSString *)defaultText initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(NSString *))completionHandler{
  ...
  const NSRect ABI50_0_0RCTSingleTextFieldFrame = NSMakeRect(0.0, 0.0, 275.0, 22.0);
  NSTextField *textField = [[NSTextField alloc] initWithFrame:ABI50_0_0RCTSingleTextFieldFrame];
...
- (void)                         webView:(WKWebView *)webView
  ...
  if (_mediaCapturePermissionGrantType == ABI50_0_0RNCWebViewPermissionGrantType_GrantIfSameHost_ElsePrompt || _mediaCapturePermissionGrantType == ABI50_0_0RNCWebViewPermissionGrantType_GrantIfSameHost_ElseDeny) {
    if ([origin.host isEqualToString:webView.URL.host]) {
      decisionHandler(WKPermissionDecisionGrant);
    } else {
      WKPermissionDecision decision = _mediaCapturePermissionGrantType == ABI50_0_0RNCWebViewPermissionGrantType_GrantIfSameHost_ElsePrompt ? WKPermissionDecisionPrompt : WKPermissionDecisionDeny;
      decisionHandler(decision);
    }
  } else if (_mediaCapturePermissionGrantType == ABI50_0_0RNCWebViewPermissionGrantType_Deny) {
    decisionHandler(WKPermissionDecisionDeny);
  } else if (_mediaCapturePermissionGrantType == ABI50_0_0RNCWebViewPermissionGrantType_Grant) {
    decisionHandler(WKPermissionDecisionGrant);
  } else {
...
-(UIViewController *)topViewController{
  return ABI50_0_0RCTPresentedViewController();
}
                  ...
                  decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if (_onShouldStartLoadWithRequest) {
        NSMutableDictionary<NSString *, id> *event = [self baseEvent];
        int lockIdentifier = [[ABI50_0_0RNCWebViewDecisionManager getInstance] setDecisionHandler: ^(BOOL shouldStart){
...
{
  ABI50_0_0RCTLogWarn(@"Webview Process Terminated");
  if (_onContentProcessDidTerminate) {
    NSMutableDictionary<NSString *, id> *event = [self baseEvent];
    _onContentProcessDidTerminate(event);
  }
          ...
          thenCall: (void (^)(NSString*)) callback
{
  if (self.enableApplePay) {
    ABI50_0_0RCTLogWarn(@"Cannot run javascript when apple pay is enabled");
    return;
  }
    ...
    if (@available(iOS 11.0, *)) {
      ...
      for (NSHTTPCookie *cookie in [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookies]) {
        [script appendFormat:@"document.cookie = %@ + '=' + %@",
         ABI50_0_0RCTJSONStringify(cookie.name, NULL),
         ABI50_0_0RCTJSONStringify(cookie.value, NULL)];
        if (cookie.path) {
          [script appendFormat:@" + '; Path=' + %@", ABI50_0_0RCTJSONStringify(cookie.path, NULL)];
        }
...
- (NSURLRequest *)requestForSource:(id)json {
  NSURLRequest *request = [ABI50_0_0RCTConvert NSURLRequest:self.source];
...
@implementation ABI50_0_0RNCWeakScriptMessageDelegate
