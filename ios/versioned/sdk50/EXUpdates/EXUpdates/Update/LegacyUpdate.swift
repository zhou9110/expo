...
public final class LegacyUpdate: Update {
  ...
  public static func update(
    ...
    if manifest.isUsingDeveloperTool() {
      ...
      commitTime = ABI50_0_0RCTConvert.nsDate(commitTimeString)
    }
    ...
    let jsBundleAsset = UpdateAsset(key: bundleKey, type: EmbeddedAppLoader.ABI50_0_0EXUpdatesEmbeddedBundleFileType)
    jsBundleAsset.url = bundleUrl
    jsBundleAsset.isLaunchAsset = true
    jsBundleAsset.mainBundleFilename = EmbeddedAppLoader.ABI50_0_0EXUpdatesEmbeddedBundleFilename
  ...
  private static let ABI50_0_0EXUpdatesExpoAssetBaseUrl = "https://classic-assets.eascdn.net/~assets/"
  private static let ABI50_0_0EXUpdatesExpoIoDomain = "expo.io"
  private static let ABI50_0_0EXUpdatesExpHostDomain = "exp.host"
  private static let ABI50_0_0EXUpdatesExpoTestDomain = "expo.test"
  ...
  static func bundledAssetBaseUrl(withManifest: LegacyManifest, config: UpdatesConfig) -> URL {
    ...
    guard let host = host else {
      ...
      return URL(string: ABI50_0_0EXUpdatesExpoAssetBaseUrl)!
    }

    if host.contains(ABI50_0_0EXUpdatesExpoIoDomain) ||
      host.contains(ABI50_0_0EXUpdatesExpHostDomain) ||
      host.contains(ABI50_0_0EXUpdatesExpoTestDomain) {
      // The URL is valid and constant, so it'll never throw
      // swiftlint:disable:next force_unwrapping
      return URL(string: ABI50_0_0EXUpdatesExpoAssetBaseUrl)!
    }
