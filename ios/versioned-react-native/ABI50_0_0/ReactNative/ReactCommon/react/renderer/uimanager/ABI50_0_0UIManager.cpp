...
#include <ABI50_0_0React/renderer/uimanager/ABI50_0_0SurfaceRegistryBinding.h>
...
ShadowTree::Unique UIManager::stopSurface(SurfaceId surfaceId) const {
  ...
  if (shadowTree) {
    // We execute JavaScript/ABI50_0_0React part of the process at the very end to
...
void UIManager::unregisterCommitHook(UIManagerCommitHook& commitHook) {
  ...
  auto iterator =
      std::find(commitHooks_.begin(), commitHooks_.end(), &commitHook);
  ABI50_0_0React_native_assert(iterator != commitHooks_.end());
...
void UIManager::unregisterMountHook(UIManagerMountHook& mountHook) {
  ...
  ABI50_0_0React_native_assert(iterator != mountHooks_.end());
