...
#import "ABI50_0_0RCTAutoInsetsProtocol.h"
#import "ABI50_0_0RCTBorderCurve.h"
#import "ABI50_0_0RCTBorderDrawing.h"
#import "ABI50_0_0RCTI18nUtil.h"
#import "ABI50_0_0RCTLocalizedString.h"
#import "ABI50_0_0RCTLog.h"
#import "ABI50_0_0RCTViewUtils.h"
#import "ABI50_0_0UIView+React.h"

ABI50_0_0RCT_MOCK_DEF(ABI50_0_0RCTView, ABI50_0_0RCTContentInsets);
#define ABI50_0_0RCTContentInsets ABI50_0_0RCT_MOCK_USE(ABI50_0_0RCTView, ABI50_0_0RCTContentInsets)

const UIAccessibilityTraits ABI50_0_0SwitchAccessibilityTrait = 0x20000000000001;

@implementation UIView (ABI50_0_0RCTViewUnmounting)
...
- (void)ABI50_0_0React_remountAllSubviews
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI50_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI50_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  for (UIView *subview in self.subviews) {
    [subview ABI50_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }
}
...
- (UIView *)ABI50_0_0React_findClipView
{
  ...
  while (testView) {
    ...
    if ([testView isABI50_0_0ReactRootView]) {
      break;
    }
...
static NSString *ABI50_0_0RCTRecursiveAccessibilityLabel(UIView *view)
{
  NSMutableString *str = [NSMutableString stringWithString:@""];
  for (UIView *subview in view.subviews) {
    NSString *label = subview.accessibilityLabel;
    if (!label) {
      label = ABI50_0_0RCTRecursiveAccessibilityLabel(subview);
    }
...
@implementation ABI50_0_0RCTView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _borderCurve = ABI50_0_0RCTBorderCurveCircular;
    _borderStyle = ABI50_0_0RCTBorderStyleSolid;
...
ABI50_0_0RCT_NOT_IMPLEMENTED(-(instancetype)initWithCoder : unused)

- (void)setABI50_0_0ReactLayoutDirection:(UIUserInterfaceLayoutDirection)layoutDirection
{
  if (_ABI50_0_0ReactLayoutDirection != layoutDirection) {
    _ABI50_0_0ReactLayoutDirection = layoutDirection;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  BOOL needsHitSubview = !(_pointerEvents == ABI50_0_0RCTPointerEventsNone || _pointerEvents == ABI50_0_0RCTPointerEventsBoxOnly);
  ...
  switch (_pointerEvents) {
    ...
    case ABI50_0_0RCTPointerEventsBoxOnly:
      return hitView;
    case ABI50_0_0RCTPointerEventsBoxNone:
      return hitSubview;
    default:
      ABI50_0_0RCTLogInfo(@"Invalid pointer-events specified %lld on %@", (long long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
- (NSString *)accessibilityLabel
{
  ...
  return ABI50_0_0RCTRecursiveAccessibilityLabel(self);
}
...
- (BOOL)didActivateAccessibilityCustomAction:(UIAccessibilityCustomAction *)action
{
  ...
  if (actionObject) {
    _onAccessibilityAction(@{@"actionName" : actionObject[@"name"], @"actionTarget" : self.ABI50_0_0ReactTag});
  }
  return YES;
}
...
- (NSString *)accessibilityValue
{
  ...
  dispatch_once(&onceToken, ^{
    rolesAndStatesDescription = @{
      @"alert" : ABI50_0_0RCTLocalizedString("alert", "important, and usually time-sensitive, information"),
      @"busy" : ABI50_0_0RCTLocalizedString("busy", "an element currently being updated or modified"),
      @"checkbox" : ABI50_0_0RCTLocalizedString("checkbox", "checkable interactive control"),
      @"combobox" : ABI50_0_0RCTLocalizedString(
          "combo box",
          "input that controls another element that can pop up to help the user set the value of that input"),
      @"menu" : ABI50_0_0RCTLocalizedString("menu", "offers a list of choices to the user"),
      @"menubar" : ABI50_0_0RCTLocalizedString(
          "menu bar", "presentation of menu that usually remains visible and is usually presented horizontally"),
      @"menuitem" : ABI50_0_0RCTLocalizedString("menu item", "an option in a set of choices contained by a menu or menubar"),
      @"progressbar" :
          ABI50_0_0RCTLocalizedString("progress bar", "displays the progress status for tasks that take a long time"),
      @"radio" : ABI50_0_0RCTLocalizedString(
          "radio button",
          "a checkable input that when associated with other radio buttons, only one of which can be checked at a time"),
      @"radiogroup" : ABI50_0_0RCTLocalizedString("radio group", "a group of radio buttons"),
      @"scrollbar" : ABI50_0_0RCTLocalizedString("scroll bar", "controls the scrolling of content within a viewing area"),
      @"spinbutton" : ABI50_0_0RCTLocalizedString(
          "spin button", "defines a type of range that expects the user to select a value from among discrete choices"),
      @"switch" : ABI50_0_0RCTLocalizedString("switch", "represents the states 'on' and 'off'"),
      @"tab" : ABI50_0_0RCTLocalizedString("tab", "an interactive element inside a tablist"),
      @"tablist" : ABI50_0_0RCTLocalizedString("tab list", "container for a set of tabs"),
      @"timer" : ABI50_0_0RCTLocalizedString(
          "timer",
          "a numerical counter listing the amount of elapsed time from a starting point or the remaining time until an end point"),
      @"toolbar" : ABI50_0_0RCTLocalizedString(
          "tool bar",
          "a collection of commonly used function buttons or controls represented in a compact visual form"),
      @"checked" : ABI50_0_0RCTLocalizedString("checked", "a checkbox, radio button, or other widget which is checked"),
      @"unchecked" : ABI50_0_0RCTLocalizedString("unchecked", "a checkbox, radio button, or other widget which is unchecked"),
      @"expanded" :
          ABI50_0_0RCTLocalizedString("expanded", "a menu, dialog, accordian panel, or other widget which is expanded"),
      @"collapsed" :
          ABI50_0_0RCTLocalizedString("collapsed", "a menu, dialog, accordian panel, or other widget which is collapsed"),
      @"mixed" :
          ABI50_0_0RCTLocalizedString("mixed", "a checkbox, radio button, or other widget which is both checked and unchecked"),
    };
  });

  // Handle Switch.
  if ((self.accessibilityTraits & ABI50_0_0SwitchAccessibilityTrait) == ABI50_0_0SwitchAccessibilityTrait) {
  ...
  // Handle states which haven't already been handled in ABI50_0_0RCTViewManager.
  for (NSString *state in self.accessibilityState) {
    id val = self.accessibilityState[state];
    if (!val) {
      continue;
...
{
  if (self.ABI50_0_0ReactAccessibilityElement == self) {
...
- (BOOL)performAccessibilityAction:(NSString *)name
{
  if (_onAccessibilityAction && accessibilityActionsNameMap[name]) {
    _onAccessibilityAction(@{@"actionName" : name, @"actionTarget" : self.ABI50_0_0ReactTag});
    return YES;
  }
  return NO;
}
...
{
  return [[super description] stringByAppendingFormat:@" ABI50_0_0ReactTag: %@; frame = %@; layer = %@",
...
+ (void)autoAdjustInsetsForView:(UIView<ABI50_0_0RCTAutoInsetsProtocol> *)parentView
                   ...
                   updateOffset:(BOOL)updateOffset
{
  ...
  if (parentView.automaticallyAdjustContentInsets) {
    UIEdgeInsets autoInset = ABI50_0_0RCTContentInsets(parentView);
  ...
  if (_removeClippedSubviews) {
    for (UIView *view in self.ABI50_0_0ReactSubviews) {
      if (view.superview != self) {
        ...
        [view ABI50_0_0React_remountAllSubviews];
      }
    }
  } else {
    // If _removeClippedSubviews is false, we must already be showing all subviews
    [super ABI50_0_0React_remountAllSubviews];
  }
}
...
- (void)ABI50_0_0React_updateClippedSubviewsWithClipRect:(CGRect)clipRect relativeToView:(UIView *)clipView
{
  ...
  if (!_removeClippedSubviews) {
    ...
    return [super ABI50_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
  }

  if (self.ABI50_0_0ReactSubviews.count == 0) {
  ...
  for (UIView *view in self.ABI50_0_0ReactSubviews) {
    if (!CGSizeEqualToSize(CGRectIntersection(clipRect, view.frame).size, CGSizeZero)) {
      ...
      if (CGRectContainsRect(clipRect, view.frame)) {
        ...
        [view ABI50_0_0React_updateClippedSubviewsWithClipRect:clipRect relativeToView:clipView];
      }
...
- (void)setRemoveClippedSubviews:(BOOL)removeClippedSubviews
{
  if (!removeClippedSubviews && _removeClippedSubviews) {
    [self ABI50_0_0React_remountAllSubviews];
  }
...
- (void)updateClippedSubviews
{
  ...
  UIView *clipView = [self ABI50_0_0React_findClipView];
  if (clipView) {
    [self ABI50_0_0React_updateClippedSubviewsWithClipRect:clipView.bounds relativeToView:clipView];
  }
}
...
static CGFloat ABI50_0_0RCTDefaultIfNegativeTo(CGFloat defaultValue, CGFloat x)
{
...
- (UIEdgeInsets)bordersAsInsets
{
  ...
  const BOOL isRTL = _ABI50_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;

  if ([[ABI50_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat borderStartWidth = ABI50_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, _borderStartWidth);
    const CGFloat borderEndWidth = ABI50_0_0RCTDefaultIfNegativeTo(_borderRightWidth, _borderEndWidth);
    ...
    return (UIEdgeInsets){
        ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
        ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderLeftWidth),
        ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
        ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, directionAwareBorderRightWidth),
    };
  }
  ...
  return (UIEdgeInsets){
      ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, _borderTopWidth),
      ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, ABI50_0_0RCTDefaultIfNegativeTo(_borderLeftWidth, directionAwareBorderLeftWidth)),
      ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, _borderBottomWidth),
      ABI50_0_0RCTDefaultIfNegativeTo(borderWidth, ABI50_0_0RCTDefaultIfNegativeTo(_borderRightWidth, directionAwareBorderRightWidth)),
  };
}
...
- (ABI50_0_0RCTCornerRadii)cornerRadii
{
  ...
  const CGFloat logicalTopStartRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderStartStartRadius, _borderTopStartRadius);
  const CGFloat logicalTopEndRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderStartEndRadius, _borderTopEndRadius);
  const CGFloat logicalBottomStartRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderEndStartRadius, _borderBottomStartRadius);
  const CGFloat logicalBottomEndRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderEndEndRadius, _borderBottomEndRadius);

  if ([[ABI50_0_0RCTI18nUtil sharedInstance] doLeftAndRightSwapInRTL]) {
    const CGFloat topStartRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, logicalTopStartRadius);
    const CGFloat topEndRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, logicalTopEndRadius);
    const CGFloat bottomStartRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, logicalBottomStartRadius);
    const CGFloat bottomEndRadius = ABI50_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, logicalBottomEndRadius);
    ...
    topLeftRadius = ABI50_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopLeftRadius);
    topRightRadius = ABI50_0_0RCTDefaultIfNegativeTo(radius, directionAwareTopRightRadius);
    bottomLeftRadius = ABI50_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomLeftRadius);
    bottomRightRadius = ABI50_0_0RCTDefaultIfNegativeTo(radius, directionAwareBottomRightRadius);
  } else {
    ...
    topLeftRadius =
        ABI50_0_0RCTDefaultIfNegativeTo(radius, ABI50_0_0RCTDefaultIfNegativeTo(_borderTopLeftRadius, directionAwareTopLeftRadius));
    topRightRadius =
        ABI50_0_0RCTDefaultIfNegativeTo(radius, ABI50_0_0RCTDefaultIfNegativeTo(_borderTopRightRadius, directionAwareTopRightRadius));
    bottomLeftRadius =
        ABI50_0_0RCTDefaultIfNegativeTo(radius, ABI50_0_0RCTDefaultIfNegativeTo(_borderBottomLeftRadius, directionAwareBottomLeftRadius));
    bottomRightRadius = ABI50_0_0RCTDefaultIfNegativeTo(
        radius, ABI50_0_0RCTDefaultIfNegativeTo(_borderBottomRightRadius, directionAwareBottomRightRadius));
  }

  // Get scale factors required to prevent radii from overlapping
  const CGSize size = self.bounds.size;
  const CGFloat topScaleFactor = ABI50_0_0RCTZeroIfNaN(MIN(1, size.width / (topLeftRadius + topRightRadius)));
  const CGFloat bottomScaleFactor = ABI50_0_0RCTZeroIfNaN(MIN(1, size.width / (bottomLeftRadius + bottomRightRadius)));
  const CGFloat rightScaleFactor = ABI50_0_0RCTZeroIfNaN(MIN(1, size.height / (topRightRadius + bottomRightRadius)));
  const CGFloat leftScaleFactor = ABI50_0_0RCTZeroIfNaN(MIN(1, size.height / (topLeftRadius + bottomLeftRadius)));

  // Return scaled radii
  return (ABI50_0_0RCTCornerRadii){
...
- (ABI50_0_0RCTBorderColors)borderColorsWithTraitCollection:(UITraitCollection *)traitCollection
{
  const BOOL isRTL = _ABI50_0_0ReactLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;
  ...
  return (ABI50_0_0RCTBorderColors){
...
- (void)displayLayer:(CALayer *)layer
{
  ...
  ABI50_0_0RCTUpdateShadowPathForView(self);

  const ABI50_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
  const UIEdgeInsets borderInsets = [self bordersAsInsets];
  const ABI50_0_0RCTBorderColors borderColors = [self borderColorsWithTraitCollection:self.traitCollection];

  BOOL useIOSBorderRendering = ABI50_0_0RCTCornerRadiiAreEqual(cornerRadii) && ABI50_0_0RCTBorderInsetsAreEqual(borderInsets) &&
      ABI50_0_0RCTBorderColorsAreEqual(borderColors) && _borderStyle == ABI50_0_0RCTBorderStyleSolid &&
  ...
  UIImage *image = ABI50_0_0RCTGetBorderImage(
...
static BOOL ABI50_0_0RCTLayerHasShadow(CALayer *layer)
{
  return layer.shadowOpacity * CGColorGetAlpha(layer.shadowColor) > 0;
}

static void ABI50_0_0RCTUpdateShadowPathForView(ABI50_0_0RCTView *view)
{
  if (ABI50_0_0RCTLayerHasShadow(view.layer)) {
    if (CGColorGetAlpha(view.backgroundColor.CGColor) > 0.999) {
      // If view has a solid background color, calculate shadow path from border
      const ABI50_0_0RCTCornerRadii cornerRadii = [view cornerRadii];
      const ABI50_0_0RCTCornerInsets cornerInsets = ABI50_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero);
      CGPathRef shadowPath = ABI50_0_0RCTPathCreateWithRoundedRect(view.bounds, cornerInsets, NULL);
...
- (void)updateClippingForLayer:(CALayer *)layer
{
  ...
  if (self.clipsToBounds) {
    const ABI50_0_0RCTCornerRadii cornerRadii = [self cornerRadii];
    if (ABI50_0_0RCTCornerRadiiAreEqual(cornerRadii)) {
      cornerRadius = cornerRadii.topLeft;

    } else {
      CAShapeLayer *shapeLayer = [CAShapeLayer layer];
      CGPathRef path =
          ABI50_0_0RCTPathCreateWithRoundedRect(self.bounds, ABI50_0_0RCTGetCornerInsets(cornerRadii, UIEdgeInsetsZero), NULL);
...
#define setBorderCurve(side)                            \
  -(void)setBorder##side##Curve : (ABI50_0_0RCTBorderCurve)curve \
  {                                                     \
...
#define setBorderStyle(side)                            \
  -(void)setBorder##side##Style : (ABI50_0_0RCTBorderStyle)style \
  {                                                     \
