...
#include <ABI50_0_0yoga/algorithm/ABI50_0_0BoundAxis.h>
...
#include <ABI50_0_0yoga/algorithm/ABI50_0_0FlexLine.h>
...
static inline bool styleDefinesDimension(
  ...
  return !(
      resolvedDimension.unit == ABI50_0_0YGUnitAuto ||
      resolvedDimension.unit == ABI50_0_0YGUnitUndefined ||
      (resolvedDimension.unit == ABI50_0_0YGUnitPoint && !isUndefined &&
       resolvedDimension.value < 0.0f) ||
      (resolvedDimension.unit == ABI50_0_0YGUnitPercent && !isUndefined &&
...
static void computeFlexBasisForChild(
  ...
  if (!resolvedFlexBasis.isUndefined() && !yoga::isUndefined(mainAxisSize)) {
    ...
    child->setLayoutComputedFlexBasis(yoga::maxOrDefined(
        yoga::resolveValue(
            child->getResolvedDimension(ABI50_0_0YGDimensionWidth), ownerWidth),
        paddingAndBorder));
    ...
    child->setLayoutComputedFlexBasis(yoga::maxOrDefined(
        yoga::resolveValue(
            child->getResolvedDimension(ABI50_0_0YGDimensionHeight), ownerHeight),
        paddingAndBorder));
  } else {
    // Compute the flex basis and hypothetical main size (i.e. the clamped flex
    // basis).
    childWidth = ABI50_0_0YGUndefined;
    childHeight = ABI50_0_0YGUndefined;
...
static void layoutAbsoluteChild(
  ...
  float childWidth = ABI50_0_0YGUndefined;
  float childHeight = ABI50_0_0YGUndefined;
  ...
  if (styleDefinesDimension(child, FlexDirection::Row, width)) {
    childWidth =
        yoga::resolveValue(child->getResolvedDimension(ABI50_0_0YGDimensionWidth), width)
            .unwrap() +
        marginRow;
  } else {
    ...
    if (child->isLeadingPositionDefined(FlexDirection::Row) &&
      ...
      childWidth = node->getLayout().measuredDimension(ABI50_0_0YGDimensionWidth) -
  ...
  if (styleDefinesDimension(child, FlexDirection::Column, height)) {
    childHeight = yoga::resolveValue(
                      child->getResolvedDimension(ABI50_0_0YGDimensionHeight), height)
                      .unwrap() +
        marginColumn;
  } else {
    ...
    if (child->isLeadingPositionDefined(FlexDirection::Column) &&
      ...
      childHeight = node->getLayout().measuredDimension(ABI50_0_0YGDimensionHeight) -
  ...
  if (yoga::isUndefined(childWidth) || yoga::isUndefined(childHeight)) {
    ...
    calculateLayoutInternal(
        ...
        depth,
        generationCount);
    childWidth = child->getLayout().measuredDimension(ABI50_0_0YGDimensionWidth) +
        child->getMarginForAxis(FlexDirection::Row, width).unwrap();
    childHeight = child->getLayout().measuredDimension(ABI50_0_0YGDimensionHeight) +
        child->getMarginForAxis(FlexDirection::Column, width).unwrap();
  }
...
static void measureNodeWithMeasureFunc(
  ...
  if (widthMeasureMode == MeasureMode::Undefined) {
    availableWidth = ABI50_0_0YGUndefined;
  }
  if (heightMeasureMode == MeasureMode::Undefined) {
    availableHeight = ABI50_0_0YGUndefined;
  }

  const auto& padding = node->getLayout().padding;
  const auto& border = node->getLayout().border;
  const float paddingAndBorderAxisRow = padding[ABI50_0_0YGEdgeLeft] +
      padding[ABI50_0_0YGEdgeRight] + border[ABI50_0_0YGEdgeLeft] + border[ABI50_0_0YGEdgeRight];
  const float paddingAndBorderAxisColumn = padding[ABI50_0_0YGEdgeTop] +
      padding[ABI50_0_0YGEdgeBottom] + border[ABI50_0_0YGEdgeTop] + border[ABI50_0_0YGEdgeBottom];
  ...
  if (widthMeasureMode == MeasureMode::Exactly &&
    ...
    const ABI50_0_0YGSize measuredSize = node->measure(
...
static void measureNodeWithoutChildren(
  ...
  if (widthMeasureMode == MeasureMode::Undefined ||
      widthMeasureMode == MeasureMode::AtMost) {
    width = padding[ABI50_0_0YGEdgeLeft] + padding[ABI50_0_0YGEdgeRight] + border[ABI50_0_0YGEdgeLeft] +
        border[ABI50_0_0YGEdgeRight];
  }
  node->setLayoutMeasuredDimension(
  ...
  if (heightMeasureMode == MeasureMode::Undefined ||
      heightMeasureMode == MeasureMode::AtMost) {
    height = padding[ABI50_0_0YGEdgeTop] + padding[ABI50_0_0YGEdgeBottom] + border[ABI50_0_0YGEdgeTop] +
        border[ABI50_0_0YGEdgeBottom];
  }
  node->setLayoutMeasuredDimension(
...
static void zeroOutLayoutRecursively(yoga::Node* const node) {
  node->getLayout() = {};
  node->setLayoutDimension(0, ABI50_0_0YGDimensionWidth);
  node->setLayoutDimension(0, ABI50_0_0YGDimensionHeight);
...
static float calculateAvailableInnerDimension(
    const yoga::Node* const node,
    const ABI50_0_0YGDimension dimension,
...
static float computeFlexBasisForChildren(
  ...
  ABI50_0_0YGNodeRef singleFlexChild = nullptr;
  ...
  for (auto currentLineChild : flexLine.itemsInFlow) {
    ...
    if (!childStyle.aspectRatio().isUndefined()) {
      ...
      childCrossSize += marginCross;
        ...
        currentLineChild->marginLeadingValue(crossAxis).unit != ABI50_0_0YGUnitAuto &&
        currentLineChild->marginTrailingValue(crossAxis).unit != ABI50_0_0YGUnitAuto) {
...
static void justifyMainAxis(
  ...
  for (size_t i = startOfLineIndex; i < flexLine.endOfLineIndex; i++) {
    ...
    if (child->getStyle().positionType() != PositionType::Absolute) {
      if (child->marginLeadingValue(mainAxis).unit == ABI50_0_0YGUnitAuto) {
        numberOfAutoMarginsOnCurrentLine++;
      }
      if (child->marginTrailingValue(mainAxis).unit == ABI50_0_0YGUnitAuto) {
        numberOfAutoMarginsOnCurrentLine++;
      }
    }
  }
          ...
          if (isNodeBaselineLayout) {
            ...
            const float ascent = calculateBaseline(child) +
                child
                    ...
                    .unwrap();
            const float descent =
                child->getLayout().measuredDimension(ABI50_0_0YGDimensionHeight) +
                child
                    ->getMarginForAxis(
                        FlexDirection::Column, availableInnerWidth)
                    .unwrap() -
...
//      the node or ABI50_0_0YGUndefined if the size is not available; interpretation
...
static void calculateLayoutImpl(
  ...
  const ABI50_0_0YGEdge startEdge =
      direction == Direction::LTR ? ABI50_0_0YGEdgeLeft : ABI50_0_0YGEdgeRight;
  const ABI50_0_0YGEdge endEdge = direction == Direction::LTR ? ABI50_0_0YGEdgeRight : ABI50_0_0YGEdgeLeft;
  ...
  node->setLayoutMargin(marginColumnLeading, ABI50_0_0YGEdgeTop);
  const float marginColumnTrailing =
      node->getTrailingMargin(flexColumnDirection, ownerWidth).unwrap();
  node->setLayoutMargin(marginColumnTrailing, ABI50_0_0YGEdgeBottom);
  ...
  node->setLayoutBorder(node->getLeadingBorder(flexColumnDirection), ABI50_0_0YGEdgeTop);
  node->setLayoutBorder(
      node->getTrailingBorder(flexColumnDirection), ABI50_0_0YGEdgeBottom);
  ...
  for (; endOfLineIndex < childCount;
    ...
    if (measureModeMainDim != MeasureMode::Exactly) {
      ...
      const float minInnerWidth =
          yoga::resolveValue(style.minDimension(ABI50_0_0YGDimensionWidth), ownerWidth)
              .unwrap() -
          paddingAndBorderAxisRow;
      const float maxInnerWidth =
          yoga::resolveValue(style.maxDimension(ABI50_0_0YGDimensionWidth), ownerWidth)
              .unwrap() -
          paddingAndBorderAxisRow;
      const float minInnerHeight =
          yoga::resolveValue(style.minDimension(ABI50_0_0YGDimensionHeight), ownerHeight)
              .unwrap() -
          paddingAndBorderAxisColumn;
      const float maxInnerHeight =
          yoga::resolveValue(style.maxDimension(ABI50_0_0YGDimensionHeight), ownerHeight)
              .unwrap() -
          paddingAndBorderAxisColumn;
      ...
      for (size_t i = startOfLineIndex; i < endOfLineIndex; i++) {
        ...
        if (child->getStyle().positionType() == PositionType::Absolute) {
          ...
          if (alignItem == Align::Stretch &&
              child->marginLeadingValue(crossAxis).unit != ABI50_0_0YGUnitAuto &&
              child->marginTrailingValue(crossAxis).unit != ABI50_0_0YGUnitAuto) {
            ...
            if (child->marginLeadingValue(crossAxis).unit == ABI50_0_0YGUnitAuto &&
                child->marginTrailingValue(crossAxis).unit == ABI50_0_0YGUnitAuto) {
              leadingCrossDim +=
...
static const char* measureModeName(
  ...
  switch (mode) {
    ...
    case MeasureMode::Exactly:
      return performLayout ? "LAY_EXACTLY" : "ABI50_0_0EXACTLY";
    case MeasureMode::AtMost:
...
bool calculateLayoutInternal(
  ...
  if (!needToVisitNode && cachedResults != nullptr) {
    layout->setMeasuredDimension(
        ABI50_0_0YGDimensionWidth, cachedResults->computedWidth);
    layout->setMeasuredDimension(
        ABI50_0_0YGDimensionHeight, cachedResults->computedHeight);
    ...
    if (gPrintChanges) {
      ...
      yoga::log(
          ...
          measureModeName(widthMeasureMode, performLayout),
          measureModeName(heightMeasureMode, performLayout),
          layout->measuredDimension(ABI50_0_0YGDimensionWidth),
          layout->measuredDimension(ABI50_0_0YGDimensionHeight),
          LayoutPassReasonToString(reason));
    }
...
void calculateLayout(
  ...
  float width = ABI50_0_0YGUndefined;
  ...
  if (styleDefinesDimension(node, FlexDirection::Row, ownerWidth)) {
    ...
    widthMeasureMode = MeasureMode::Exactly;
                  ...
                  style.maxDimension(ABI50_0_0YGDimensionWidth), ownerWidth)
                  .isUndefined()) {
    width = yoga::resolveValue(style.maxDimension(ABI50_0_0YGDimensionWidth), ownerWidth)
                .unwrap();
    widthMeasureMode = MeasureMode::AtMost;
  } else {
    width = ownerWidth;
  ...
  float height = ABI50_0_0YGUndefined;
  ...
  if (styleDefinesDimension(node, FlexDirection::Column, ownerHeight)) {
    ...
    heightMeasureMode = MeasureMode::Exactly;
                  ...
                  style.maxDimension(ABI50_0_0YGDimensionHeight), ownerHeight)
                  .isUndefined()) {
    height =
        yoga::resolveValue(style.maxDimension(ABI50_0_0YGDimensionHeight), ownerHeight)
            .unwrap();
