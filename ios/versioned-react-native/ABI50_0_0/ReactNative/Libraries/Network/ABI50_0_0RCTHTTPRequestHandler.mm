...
@interface ABI50_0_0RCTHTTPRequestHandler () <NSURLSessionDataDelegate, ABI50_0_0RCTTurboModule>

@end

static NSURLSessionConfigurationProvider urlSessionConfigurationProvider;

void ABI50_0_0RCTSetCustomNSURLSessionConfigurationProvider(NSURLSessionConfigurationProvider provider)
{
  urlSessionConfigurationProvider = provider;
}

@implementation ABI50_0_0RCTHTTPRequestHandler {
...
- (BOOL)canHandleRequest:(NSURLRequest *)request
{
  ...
  dispatch_once(&onceToken, ^{
    // technically, ABI50_0_0RCTHTTPRequestHandler can handle file:// as well,
    // but it's less efficient than using ABI50_0_0RCTFileRequestHandler
    schemes = [[NSSet alloc] initWithObjects:@"http", @"https", nil];
  });
  return [schemes containsObject:request.URL.scheme.lowercaseString];
}

- (NSURLSessionDataTask *)sendRequest:(NSURLRequest *)request withDelegate:(id<ABI50_0_0RCTURLRequestDelegate>)delegate
{
  std::lock_guard<std::mutex> lock(_mutex);
  // Lazy setup
  if (!_session && [self isValid]) {
    // You can override default NSURLSession instance property allowsCellularAccess (default value YES)
    //  by providing the following key to your ABI50_0_0RN project (edit ios/project/Info.plist file in Xcode):
    // <key>ABI50_0_0ReactNetworkForceWifiOnly</key>    <true/>
    ...
    NSNumber *useWifiOnly = [infoDictionary objectForKey:@"ABI50_0_0ReactNetworkForceWifiOnly"];
    ...
    if (urlSessionConfigurationProvider) {
      ...
      // Set allowsCellularAccess to NO ONLY if key ABI50_0_0ReactNetworkForceWifiOnly exists AND its value is YES
      if (useWifiOnly) {
...
{
  id<ABI50_0_0RCTURLRequestDelegate> delegate;
  {
    std::lock_guard<std::mutex> lock(_mutex);
...
Class ABI50_0_0RCTHTTPRequestHandlerCls(void)
{
  return ABI50_0_0RCTHTTPRequestHandler.class;
}
