...
@interface ABI50_0_0RCTSurfaceHostingView ()
...
@implementation ABI50_0_0RCTSurfaceHostingView {
...
- (instancetype)initWithSurface:(id<ABI50_0_0RCTSurfaceProtocol>)surface
                sizeMeasureMode:(ABI50_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    // For backward compatibility with ABI50_0_0RCTRootView, set a color here instead of transparent (OS default).
...
- (void)layoutSubviews
{
  ...
  ABI50_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(
...
{
  if (ABI50_0_0RCTSurfaceStageIsPreparing(_stage)) {
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  ABI50_0_0RCTSurfaceMinimumSizeAndMaximumSizeFromSizeAndSizeMeasureMode(size, _sizeMeasureMode, &minimumSize, &maximumSize);
...
- (void)setStage:(ABI50_0_0RCTSurfaceStage)stage
{
  ...
  BOOL shouldInvalidateLayout = ABI50_0_0RCTSurfaceStageIsRunning(stage) != ABI50_0_0RCTSurfaceStageIsRunning(_stage) ||
      ABI50_0_0RCTSurfaceStageIsPreparing(stage) != ABI50_0_0RCTSurfaceStageIsPreparing(_stage);
...
- (void)setSizeMeasureMode:(ABI50_0_0RCTSurfaceSizeMeasureMode)sizeMeasureMode
{
...
- (void)setActivityIndicatorViewFactory:(ABI50_0_0RCTSurfaceHostingViewActivityIndicatorViewFactory)activityIndicatorViewFactory
{
...
- (void)_updateViews
{
  self.isSurfaceViewVisible = ABI50_0_0RCTSurfaceStageIsRunning(_stage);
  self.isActivityIndicatorViewVisible = ABI50_0_0RCTSurfaceStageIsPreparing(_stage);
}
...
#pragma mark - ABI50_0_0RCTSurfaceDelegate

- (void)surface:(__unused ABI50_0_0RCTSurface *)surface didChangeStage:(ABI50_0_0RCTSurfaceStage)stage
{
...
- (void)surface:(__unused ABI50_0_0RCTSurface *)surface didChangeIntrinsicSize:(__unused CGSize)intrinsicSize
{
  ABI50_0_0RCTExecuteOnMainQueue(^{
    [self _invalidateLayout];
  });
