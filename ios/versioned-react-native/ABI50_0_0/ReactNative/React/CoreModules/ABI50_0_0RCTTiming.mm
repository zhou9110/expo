...
@interface _ABI50_0_0RCTTimer : NSObject
...
@implementation _ABI50_0_0RCTTimer
...
@interface _ABI50_0_0RCTTimingProxy : NSObject

@end

// NSTimer retains its target, insert this class to break potential retain cycles
@implementation _ABI50_0_0RCTTimingProxy {
  __weak id _target;
}

+ (instancetype)proxyWithTarget:(id)target
{
  _ABI50_0_0RCTTimingProxy *proxy = [self new];
  if (proxy) {
...
@implementation ABI50_0_0RCTTiming {
  NSMutableDictionary<NSNumber *, _ABI50_0_0RCTTimer *> *_timers;
  NSTimer *_sleepTimer;
  BOOL _sendIdleEvents;
  BOOL _inBackground;
  id<ABI50_0_0RCTTimingDelegate> _timingDelegate;
}
...
- (void)setup
{
  ...
  ABI50_0_0RCTExecuteOnMainQueue(^{
    if (!self->_inBackground && [ABI50_0_0RCTSharedApplication() applicationState] == UIApplicationStateBackground) {
...
- (void)didUpdateFrame:(ABI50_0_0RCTFrameUpdate *)update
{
  NSDate *nextScheduledTarget = [NSDate distantFuture];
  NSMutableArray<_ABI50_0_0RCTTimer *> *timersToCall = [NSMutableArray new];
  NSDate *now = [NSDate date]; // compare all the timers to the same base time
  @synchronized(_timers) {
    for (_ABI50_0_0RCTTimer *timer in _timers.allValues) {
  ...
  if (timersToCall.count > 0) {
    NSArray<NSNumber *> *sortedTimers = [[timersToCall sortedArrayUsingComparator:^(_ABI50_0_0RCTTimer *a, _ABI50_0_0RCTTimer *b) {
  ...
  for (_ABI50_0_0RCTTimer *timer in timersToCall) {
    if (timer.repeats) {
      [timer reschedule];
      nextScheduledTarget = [nextScheduledTarget earlierDate:timer.target];
    } else {
  ...
  if (_inBackground) {
    ...
    // No need to call the pauseCallback as ABI50_0_0RCTDisplayLink will ask us about our paused
  ...
  @synchronized(self) {
    if (!_sleepTimer || !_sleepTimer.valid) {
      _sleepTimer = [[NSTimer alloc] initWithFireDate:sleepTarget
                                             interval:0
                                               target:[_ABI50_0_0RCTTimingProxy proxyWithTarget:self]
 ...
 */
ABI50_0_0RCT_EXPORT_METHOD(createTimer
                  ...
                  : (BOOL)repeats)
{
  ...
  NSDate *schedulingTime = [ABI50_0_0RCTConvert NSDate:[NSNumber numberWithDouble:jsSchedulingTime]];
                        ...
                        repeats:(BOOL)repeats
{
  ...
  _ABI50_0_0RCTTimer *timer = [[_ABI50_0_0RCTTimer alloc] initWithCallbackID:callbackID
                                                  interval:jsDuration
...
ABI50_0_0RCT_EXPORT_METHOD(deleteTimer : (double)timerID)
{
  @synchronized(_timers) {
...
ABI50_0_0RCT_EXPORT_METHOD(setSendIdleEvents : (BOOL)sendIdleEvents)
{
...
Class ABI50_0_0RCTTimingCls(void)
{
  return ABI50_0_0RCTTiming.class;
}
