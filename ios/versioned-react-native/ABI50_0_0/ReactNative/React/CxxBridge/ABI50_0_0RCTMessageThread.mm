...
#include "ABI50_0_0RCTMessageThread.h"
...
ABI50_0_0RCTMessageThread::ABI50_0_0RCTMessageThread(NSRunLoop *runLoop, ABI50_0_0RCTJavaScriptCompleteBlock errorBlock)
    : m_cfRunLoop([runLoop getCFRunLoop]), m_errorBlock(errorBlock), m_shutdown(false)
{
  CFRetain(m_cfRunLoop);
}

ABI50_0_0RCTMessageThread::~ABI50_0_0RCTMessageThread()
{
  ABI50_0_0RCTAssert(m_shutdown, @"ABI50_0_0RCTMessageThread: quitSynchronous() not called before destructor");
  CFRelease(m_cfRunLoop);
}

// This is analogous to dispatch_async
void ABI50_0_0RCTMessageThread::runAsync(std::function<void()> func)
{
...
void ABI50_0_0RCTMessageThread::runSync(std::function<void()> func)
{
...
void ABI50_0_0RCTMessageThread::tryFunc(const std::function<void()> &func)
{
...
void ABI50_0_0RCTMessageThread::runOnQueue(std::function<void()> &&func)
{
  if (m_shutdown) {
    return;
  }
...
void ABI50_0_0RCTMessageThread::runOnQueueSync(std::function<void()> &&func)
{
  if (m_shutdown) {
    return;
  }
...
void ABI50_0_0RCTMessageThread::quitSynchronous()
{
  m_shutdown = true;
  runSync([] {});
...
void ABI50_0_0RCTMessageThread::setRunLoop(NSRunLoop *runLoop)
{
  CFRelease(m_cfRunLoop);
