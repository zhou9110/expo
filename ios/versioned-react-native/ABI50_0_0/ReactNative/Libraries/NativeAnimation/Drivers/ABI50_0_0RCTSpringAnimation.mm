...
@interface ABI50_0_0RCTSpringAnimation ()

@property (nonatomic, strong) NSNumber *animationId;
@property (nonatomic, strong) ABI50_0_0RCTValueAnimatedNode *valueNode;
@property (nonatomic, assign) BOOL animationHasBegun;
@property (nonatomic, assign) BOOL animationHasFinished;

@end

const NSTimeInterval ABI50_0_0MAX_DELTA_TIME = 0.064;

@implementation ABI50_0_0RCTSpringAnimation {
  CGFloat _toValue;
  CGFloat _fromValue;
  ...
  ABI50_0_0RCTResponseSenderBlock _callback;
...
- (instancetype)initWithId:(NSNumber *)animationId
                   ...
                   forNode:(ABI50_0_0RCTValueAnimatedNode *)valueNode
                  callBack:(nullable ABI50_0_0RCTResponseSenderBlock)callback
{
  if ((self = [super init])) {
    _animationId = animationId;
    _lastPosition = valueNode.value;
    _valueNode = valueNode;
    _lastVelocity = [ABI50_0_0RCTConvert CGFloat:config[@"initialVelocity"]];
...
{
  NSNumber *iterations = [ABI50_0_0RCTConvert NSNumber:config[@"iterations"]] ?: @1;
  _toValue = [ABI50_0_0RCTConvert CGFloat:config[@"toValue"]];
  _overshootClamping = [ABI50_0_0RCTConvert BOOL:config[@"overshootClamping"]];
  _restDisplacementThreshold = [ABI50_0_0RCTConvert CGFloat:config[@"restDisplacementThreshold"]];
  _restSpeedThreshold = [ABI50_0_0RCTConvert CGFloat:config[@"restSpeedThreshold"]];
  _stiffness = [ABI50_0_0RCTConvert CGFloat:config[@"stiffness"]];
  _damping = [ABI50_0_0RCTConvert CGFloat:config[@"damping"]];
  _mass = [ABI50_0_0RCTConvert CGFloat:config[@"mass"]];
...
- (void)stepAnimationWithTime:(NSTimeInterval)currentTime
{
  ...
  if (_animationStartTime == -1) {
    ...
    // Handle frame drops, and only advance dt by a max of ABI50_0_0MAX_DELTA_TIME
    NSTimeInterval deltaTime = MIN(ABI50_0_0MAX_DELTA_TIME, currentTime - _animationCurrentTime);
    _t = _t + deltaTime / ABI50_0_0RCTAnimationDragCoefficient();
  }
