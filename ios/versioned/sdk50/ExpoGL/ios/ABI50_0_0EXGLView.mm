...
@implementation ABI50_0_0EXGLView
...
- (instancetype)initWithModuleRegistry:(ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  if ((self = [super init])) {
    ...
    self.contentScaleFactor = [ABI50_0_0EXUtilities screenScale];
    ...
    _glContext = [[ABI50_0_0EXGLContext alloc] initWithDelegate:self andModuleRegistry:moduleRegistry];
...
- (void)maybeCallSurfaceCreated
{
  ...
  if (_onSurfaceCreate && _glContext.isInitialized && _isLayouted) {
    ABI50_0_0EXGLContextId exglCtxId = _glContext.contextId;
...
- (void)setOnSurfaceCreate:(ABI50_0_0EXDirectEventBlock)onSurfaceCreate
{
...
- (void)resizeViewBuffersToWidth:(short)width height:(short)height
{
  ...
  [_glContext runAsync:^{
    ...
    ABI50_0_0EXGLContextSetDefaultFramebuffer(self->_glContext.contextId, self->_msaaFramebuffer);
...
- (void)draw
{
  // exglCtxId may be unset if we get here (on the UI thread) before ABI50_0_0EXGLContextCreate(...) is
  // called on the JS thread to create the ABI50_0_0EXGL context and save its id (see ABI50_0_0EXGLContext.initializeContextWithBridge method).
