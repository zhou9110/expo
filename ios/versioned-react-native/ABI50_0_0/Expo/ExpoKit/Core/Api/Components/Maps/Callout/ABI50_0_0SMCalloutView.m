...
@interface UIView (ABI50_0_0SMFrameAdditions)
...
@interface ABI50_0_0SMCalloutView ()
...
@property (nonatomic, assign) ABI50_0_0SMCalloutArrowDirection currentArrowDirection;
@property (nonatomic, assign) BOOL popupCancelled;
@end

@implementation ABI50_0_0SMCalloutView

+ (ABI50_0_0SMCalloutView *)platformCalloutView {

    // if you haven't compiled ABI50_0_0SMClassicCalloutView into your app, then we can't possibly create an instance of it!
    if (!NSClassFromString(@"ABI50_0_0SMClassicCalloutView"))
        return [ABI50_0_0SMCalloutView new];

    // ok we have both - so choose the best one based on current platform
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1)
        return [ABI50_0_0SMCalloutView new]; // iOS 7+
    else
        return [NSClassFromString(@"ABI50_0_0SMClassicCalloutView") new];
}

- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.permittedArrowDirection = ABI50_0_0SMCalloutArrowDirectionDown;
        self.presentAnimation = ABI50_0_0SMCalloutAnimationBounce;
        self.dismissAnimation = ABI50_0_0SMCalloutAnimationFade;
...
- (ABI50_0_0SMCalloutBackgroundView *)backgroundView {
...
- (ABI50_0_0SMCalloutBackgroundView *)defaultBackgroundView {
    return [ABI50_0_0SMCalloutMaskedBackgroundView new];
}
...
- (void)presentCalloutFromRect:(CGRect)rect inLayer:(CALayer *)layer ofView:(UIView *)view constrainedToLayer:(CALayer *)constrainedLayer animated:(BOOL)animated {
    ...
    ABI50_0_0SMCalloutArrowDirection bestDirection = ABI50_0_0SMCalloutArrowDirectionDown;

    // we'll point it up though if that's the only option you gave us.
    if (self.permittedArrowDirection == ABI50_0_0SMCalloutArrowDirectionUp)
        bestDirection = ABI50_0_0SMCalloutArrowDirectionUp;
    ...
    if (self.permittedArrowDirection == ABI50_0_0SMCalloutArrowDirectionAny && topSpace < self.calloutHeight && bottomSpace > topSpace)
        bestDirection = ABI50_0_0SMCalloutArrowDirectionUp;
    ...
    CGFloat anchorY = self.calloutOffset.y + (bestDirection == ABI50_0_0SMCalloutArrowDirectionDown ? CGRectGetMinY(rect) : CGRectGetMaxY(rect));
    ...
    CGPoint calloutOrigin = {
            .x = calloutX + adjustX,
            .y = bestDirection == ABI50_0_0SMCalloutArrowDirectionDown ? (anchorY - self.calloutHeight) : anchorY
    };
...
- (CAAnimation *)animationWithType:(ABI50_0_0SMCalloutAnimation)type presenting:(BOOL)presenting {
    ...
    if (type == ABI50_0_0SMCalloutAnimationBounce) {
    ...
    else if (type == ABI50_0_0SMCalloutAnimationFade) {
        CABasicAnimation *fade = [CABasicAnimation animationWithKeyPath:@"opacity"];
    ...
    else if (type == ABI50_0_0SMCalloutAnimationStretch) {
...
- (void)layoutSubviews {
    ...
    CGFloat dy = self.currentArrowDirection == ABI50_0_0SMCalloutArrowDirectionUp ? TOP_ANCHOR_MARGIN : 0;
...
// import this known "private API" from ABI50_0_0SMCalloutBackgroundView
@interface ABI50_0_0SMCalloutBackgroundView (EmbeddedImages)
...
@interface ABI50_0_0SMCalloutMaskedBackgroundView ()
...
@implementation ABI50_0_0SMCalloutMaskedBackgroundView
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        if (!blackArrowImage) {
            blackArrowImage = [ABI50_0_0SMCalloutBackgroundView embeddedImageNamed:@"CalloutArrow"];
...
@implementation ABI50_0_0SMCalloutBackgroundView
...
@implementation UIView (ABI50_0_0SMFrameAdditions)
