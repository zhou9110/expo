...
#import <ABI50_0_0React/utils/ABI50_0_0SharedFunction.h>
...
@implementation ABI50_0_0RCTImageManager {
  id<ABI50_0_0RCTImageLoaderWithAttributionProtocol> _imageLoader;
...
- (instancetype)initWithImageLoader:(id<ABI50_0_0RCTImageLoaderWithAttributionProtocol>)imageLoader
{
  if (self = [super init]) {
    ...
    _backgroundSerialQueue =
        dispatch_queue_create("com.facebook.ABI50_0_0React-native.image-manager-queue", DISPATCH_QUEUE_SERIAL);
  }
...
{
  SystraceSection s("ABI50_0_0RCTImageManager::requestImage");
   ...
   * work (such as creating an `NSURLRequest` object and some obscure logic inside `ABI50_0_0RCTImageLoader`) can take a couple
   ...
   *
   * Technically, we don't need to dispatch this to *serial* queue. The interface of `ABI50_0_0RCTImageLoader` promises to be
  ...
  dispatch_async(_backgroundSerialQueue, ^{
    ...
    ABI50_0_0RCTImageLoaderCancellationBlock cancelationBlock = loaderRequest.cancellationBlock;
