...
NSString *const ABI50_0_0RCTContentDidAppearNotification = @"ABI50_0_0RCTContentDidAppearNotification";

@interface ABI50_0_0RCTUIManager (ABI50_0_0RCTRootView)

- (NSNumber *)allocateRootTag;

@end

@implementation ABI50_0_0RCTRootView {
  ABI50_0_0RCTBridge *_bridge;
  NSString *_moduleName;
  ABI50_0_0RCTRootContentView *_contentView;
            ...
            initialProperties:(NSDictionary *)initialProperties
{
  ...
  ABI50_0_0RCTAssert(bridge, @"A bridge instance is required to create an ABI50_0_0RCTRootView");
  ABI50_0_0RCTAssert(moduleName, @"A moduleName is required to create an ABI50_0_0RCTRootView");

  ABI50_0_0RCT_PROFILE_BEGIN_EVENT(ABI50_0_0RCTProfileTagAlways, @"-[ABI50_0_0RCTRootView init]", nil);
  if (!bridge.isLoading) {
    [bridge.performanceLogger markStartForTag:ABI50_0_0RCTPLTTI];
  }
  ...
  if (self = [super initWithFrame:frame]) {
    ...
    _sizeFlexibility = ABI50_0_0RCTRootViewSizeFlexibilityNone;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hideLoadingView)
                                                 name:ABI50_0_0RCTContentDidAppearNotification
                                               object:self];
...
{
  ABI50_0_0RCTBridge *bridge = [[ABI50_0_0RCTBridge alloc] initWithBundleURL:bundleURL moduleProvider:nil launchOptions:launchOptions];
...
- (CGSize)sizeThatFits:(CGSize)size
{
  ...
  fitSize = CGSizeMake(
      _sizeFlexibility & ABI50_0_0RCTRootViewSizeFlexibilityWidth ? fitSize.width : currentSize.width,
      _sizeFlexibility & ABI50_0_0RCTRootViewSizeFlexibilityHeight ? fitSize.height : currentSize.height);
...
- (void)setMinimumSize:(CGSize)minimumSize
{
  ...
  __block NSNumber *tag = self.ABI50_0_0ReactTag;
  __weak typeof(self) weakSelf = self;
  ABI50_0_0RCTExecuteOnUIManagerQueue(^{
    __strong typeof(self) strongSelf = weakSelf;
    if (strongSelf && strongSelf->_bridge.isValid) {
      ABI50_0_0RCTRootShadowView *shadowView = (ABI50_0_0RCTRootShadowView *)[strongSelf->_bridge.uiManager shadowViewForABI50_0_0ReactTag:tag];
...
{
  ABI50_0_0RCTAssertMainQueue();
  if (!super.ABI50_0_0ReactTag) {
    /**
     * Every root view that is created must have a unique ABI50_0_0React tag.
     * Numbering of these tags goes from 1, 11, 21, 31, etc
     *
     * NOTE: Since the bridge persists, the RootViews might be reused, so the
     * ABI50_0_0React tag must be re-assigned every time a new UIManager is created.
     */
    self.ABI50_0_0ReactTag = ABI50_0_0RCTAllocateRootViewTag();
  }
  return super.ABI50_0_0ReactTag;
}

- (void)bridgeDidReload
{
  ABI50_0_0RCTAssertMainQueue();
  // Clear the ABI50_0_0ReactTag so it can be re-assigned
  self.ABI50_0_0ReactTag = nil;
}
...
- (void)javaScriptDidLoad:(NSNotification *)notification
{
  ...
  // ABI50_0_0RCTRootContentView is scoped to the right bridge
...
- (void)bundleFinishedLoading:(ABI50_0_0RCTBridge *)bridge
{
  ABI50_0_0RCTAssert(bridge != nil, @"Bridge cannot be nil");
  if (!bridge.valid) {
    return;
  }

  [_contentView removeFromSuperview];
  _contentView = [[ABI50_0_0RCTRootContentView alloc] initWithFrame:self.bounds
                                                    bridge:bridge
  ...
  if (_sizeFlexibility == ABI50_0_0RCTRootViewSizeFlexibilityNone) {
    self.intrinsicContentSize = self.bounds.size;
  }
}

- (void)runApplication:(ABI50_0_0RCTBridge *)bridge
{
  NSString *moduleName = _moduleName ?: @"";
  NSDictionary *appParameters = @{
    @"rootTag" : _contentView.ABI50_0_0ReactTag,
    @"initialProps" : _appProperties ?: @{},
  };

  ABI50_0_0RCTLogInfo(@"Running application %@ (%@)", moduleName, appParameters);
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection
{
  ...
  if (ABI50_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground) {
    return;
  }

  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI50_0_0RCTUserInterfaceStyleDidChangeNotification
                    object:self
                  userInfo:@{
                    ABI50_0_0RCTUserInterfaceStyleDidChangeNotificationTraitCollectionKey : self.traitCollection,
                  }];
}
...
@implementation ABI50_0_0RCTRootView (Deprecated)

- (CGSize)intrinsicSize
{
  ABI50_0_0RCTLogWarn(@"Calling deprecated `[-ABI50_0_0RCTRootView intrinsicSize]`.");
  return self.intrinsicContentSize;
}

- (void)cancelTouches
{
  ABI50_0_0RCTLogWarn(@"`-[ABI50_0_0RCTRootView cancelTouches]` is deprecated and will be deleted soon.");
