...
#include "ABI50_0_0Differentiator.h"
...
class TinyMap final {
 public:
  ...
  inline Iterator find(KeyT key) {
    ...
    ABI50_0_0React_native_assert(key != 0);
  ...
  inline void insert(Pair pair) {
    ABI50_0_0React_native_assert(pair.first != 0);
 ...
 * It is currently not possible for ABI50_0_0ReactJS, and therefore ABI50_0_0React Native, to
 move
 * a node *from* one parent to another without an entirely new subtree being
 * created. When we "reparent" in ABI50_0_0React Native here it is only because
 intermediate
...
static void calculateShadowViewMutationsFlattener(
  ...
  for (size_t index = 0;
    ...
    if (existsInOtherTree) {
      ABI50_0_0React_native_assert(otherTreeNodePairPtr != nullptr);
      ...
      ABI50_0_0React_native_assert(newTreeNodePair->shadowView.tag != 0);
      ABI50_0_0React_native_assert(oldTreeNodePair->shadowView.tag != 0);
      ABI50_0_0React_native_assert(
      ...
      ABI50_0_0React_native_assert(treeChildPair.otherTreePair != nullptr);
    }
    ...
    if (treeChildPair.isConcreteView) {
      if (reparentMode == ReparentMode::Flatten) {
        ...
        ABI50_0_0React_native_assert(existsInOtherTree == treeChildPair.inOtherTree());
