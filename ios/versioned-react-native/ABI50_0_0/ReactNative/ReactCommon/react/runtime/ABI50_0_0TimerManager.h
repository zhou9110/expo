...
#include "ABI50_0_0PlatformTimerRegistry.h"
...
class TimerManager {
 public:
  ...
  void callABI50_0_0ReactNativeMicrotasks(jsi::Runtime& runtime);
 ...
 private:
  std::shared_ptr<TimerHandle> createABI50_0_0ReactNativeMicrotask(
      jsi::Function&& callback,
      std::vector<jsi::Value>&& args);

  void deleteABI50_0_0ReactNativeMicrotask(
      jsi::Runtime& runtime,
  ...
  // The ABI50_0_0React Native microtask queue is used to back public APIs including
  // `queueMicrotask`, `clearImmediate`, and `setImmediate` (which is used by
  // the Promise polyfill) when the JSVM microtask mechanism is not used.
  std::vector<uint32_t> ABI50_0_0ReactNativeMicrotasksQueue_;
};
