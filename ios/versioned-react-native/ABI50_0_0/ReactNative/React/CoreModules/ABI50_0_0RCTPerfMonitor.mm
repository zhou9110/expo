...
#import <ABI50_0_0React/ABI50_0_0RCTFPSGraph.h>
...
#import <ABI50_0_0React/ABI50_0_0RCTPerformanceLoggerLabels.h>
...
#if __has_include(<ABI50_0_0React/ABI50_0_0RCTDevMenu.h>)
#import <ABI50_0_0React/ABI50_0_0RCTDevMenu.h>
#endif

static NSString *const ABI50_0_0RCTPerfMonitorCellIdentifier = @"ABI50_0_0RCTPerfMonitorCellIdentifier";

static const CGFloat ABI50_0_0RCTPerfMonitorBarHeight = 50;
static const CGFloat ABI50_0_0RCTPerfMonitorExpandHeight = 250;

typedef BOOL (*ABI50_0_0RCTJSCSetOptionType)(const char *);

NSArray<NSString *> *LabelsForABI50_0_0RCTPerformanceLoggerTags();

static BOOL ABI50_0_0RCTJSCSetOption(const char *option)
{
  return NO;
}

static vm_size_t ABI50_0_0RCTGetResidentMemorySize(void)
{
...
@interface ABI50_0_0RCTPerfMonitor : NSObject <
                                ABI50_0_0RCTBridgeModule,
                                ABI50_0_0RCTTurboModule,
                                ABI50_0_0RCTInitializing,
                                ABI50_0_0RCTInvalidating,
                                UITableViewDataSource,
                                UITableViewDelegate>

#if __has_include(<ABI50_0_0React/ABI50_0_0RCTDevMenu.h>)
@property (nonatomic, strong, readonly) ABI50_0_0RCTDevMenuItem *devMenuItem;
#endif
...
@property (nonatomic, strong, readonly) ABI50_0_0RCTFPSGraph *jsGraph;
@property (nonatomic, strong, readonly) ABI50_0_0RCTFPSGraph *uiGraph;
@property (nonatomic, strong, readonly) UILabel *jsGraphLabel;
@property (nonatomic, strong, readonly) UILabel *uiGraphLabel;

@end

@implementation ABI50_0_0RCTPerfMonitor {
#if __has_include(<ABI50_0_0React/ABI50_0_0RCTDevMenu.h>)
  ABI50_0_0RCTDevMenuItem *_devMenuItem;
#endif
  ...
  ABI50_0_0RCTFPSGraph *_uiGraph;
  ABI50_0_0RCTFPSGraph *_jsGraph;
...
{
#if __has_include(<ABI50_0_0React/ABI50_0_0RCTDevMenu.h>)
  [(ABI50_0_0RCTDevMenu *)[_moduleRegistry moduleForName:"DevMenu"] addItem:self.devMenuItem];
#endif
}
...
#if __has_include(<ABI50_0_0React/ABI50_0_0RCTDevMenu.h>)
- (ABI50_0_0RCTDevMenuItem *)devMenuItem
{
  if (!_devMenuItem) {
    __weak __typeof__(self) weakSelf = self;
    __weak ABI50_0_0RCTDevSettings *devSettings = [self->_moduleRegistry moduleForName:"DevSettings"];
    if (devSettings.isPerfMonitorShown) {
      [weakSelf show];
    }
    _devMenuItem = [ABI50_0_0RCTDevMenuItem
...
{
  if (!_container) {
    CGSize statusBarSize = ABI50_0_0RCTSharedApplication().statusBarFrame.size;
    CGFloat statusBarHeight = statusBarSize.height;
    _container = [[UIView alloc] initWithFrame:CGRectMake(10, statusBarHeight, 180, ABI50_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_memory) {
    _memory = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 44, ABI50_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_heap) {
    _heap = [[UILabel alloc] initWithFrame:CGRectMake(44, 0, 44, ABI50_0_0RCTPerfMonitorBarHeight)];
...
{
  if (!_views) {
    _views = [[UILabel alloc] initWithFrame:CGRectMake(88, 0, 44, ABI50_0_0RCTPerfMonitorBarHeight)];
...
- (ABI50_0_0RCTFPSGraph *)uiGraph
{
  if (!_uiGraph) {
    _uiGraph = [[ABI50_0_0RCTFPSGraph alloc] initWithFrame:CGRectMake(134, 14, 40, 30) color:[UIColor lightGrayColor]];
  }
  return _uiGraph;
}

- (ABI50_0_0RCTFPSGraph *)jsGraph
{
  if (!_jsGraph) {
    _jsGraph = [[ABI50_0_0RCTFPSGraph alloc] initWithFrame:CGRectMake(178, 14, 40, 30) color:[UIColor lightGrayColor]];
  }
  return _jsGraph;
}
...
- (UITableView *)metrics
{
  if (!_metrics) {
    _metrics = [[UITableView alloc] initWithFrame:CGRectMake(
                                                      0,
                                                      ABI50_0_0RCTPerfMonitorBarHeight,
                                                      self.container.frame.size.width,
                                                      self.container.frame.size.height - ABI50_0_0RCTPerfMonitorBarHeight)];
    ...
    [_metrics registerClass:[UITableViewCell class] forCellReuseIdentifier:ABI50_0_0RCTPerfMonitorCellIdentifier];
  }
...
- (void)show
{
  ...
  ABI50_0_0RCTJSCSetOption("logGC=1");
...
- (void)hide
{
  ...
  ABI50_0_0RCTJSCSetOption("logGC=0");
...
- (void)redirectLogs
{
  ...
  __weak __typeof__(self) weakSelf = self;
  _queue = dispatch_queue_create("com.facebook.ABI50_0_0React.ABI50_0_0RCTPerfMonitor", DISPATCH_QUEUE_SERIAL);
...
- (void)updateStats
{
  ...
  double mem = (double)ABI50_0_0RCTGetResidentMemorySize() / 1024 / 1024;
...
- (void)tap
{
  ...
  if (CGRectIsEmpty(_storedMonitorFrame)) {
    _storedMonitorFrame = CGRectMake(0, 20, self.container.window.frame.size.width, ABI50_0_0RCTPerfMonitorExpandHeight);
...
{
  ABI50_0_0RCTFPSGraph *graph = displayLink == _jsDisplayLink ? _jsGraph : _uiGraph;
...
- (void)loadPerformanceLoggerData
{
  ...
  ABI50_0_0RCTPerformanceLogger *performanceLogger = [_bridge performanceLogger];
  NSArray<NSNumber *> *values = [performanceLogger valuesForTags];
  for (NSString *label in LabelsForABI50_0_0RCTPerformanceLoggerTags()) {
...
{
  UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ABI50_0_0RCTPerfMonitorCellIdentifier
                                                          forIndexPath:indexPath];

  if (!cell) {
    cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                  reuseIdentifier:ABI50_0_0RCTPerfMonitorCellIdentifier];
  }
...
NSArray<NSString *> *LabelsForABI50_0_0RCTPerformanceLoggerTags()
{
  ...
  for (int i = 0; i < ABI50_0_0RCTPLSize; i++) {
    [labels addObject:ABI50_0_0RCTPLLabelForTag((ABI50_0_0RCTPLTag)i)];
  }
  return labels;
}
...
Class ABI50_0_0RCTPerfMonitorCls(void)
{
#if ABI50_0_0RCT_DEV
  return ABI50_0_0RCTPerfMonitor.class;
#else
  return nil;
#endif
}
