...
#import <ABI50_0_0React/ABI50_0_0RCTWebSocketModule.h>

#import "ABI50_0_0RCTBlobCollector.h"
#import "ABI50_0_0RCTBlobPlugins.h"

static NSString *const kBlobURIScheme = @"blob";

@interface ABI50_0_0RCTBlobManager () <
    ABI50_0_0RCTNetworkingRequestHandler,
    ABI50_0_0RCTNetworkingResponseHandler,
    ABI50_0_0RCTWebSocketContentHandler,
    ABI50_0_0NativeBlobModuleSpec>

@end

@implementation ABI50_0_0RCTBlobManager {
...
ABI50_0_0RCT_EXPORT_MODULE(BlobModule)
...
- (void)initialize
{
  ...
  ABI50_0_0facebook::ABI50_0_0React::ABI50_0_0RCTBlobCollector::install(self);
}
...
{
  NSString *blobId = [ABI50_0_0RCTConvert NSString:blob[@"blobId"]];
  NSNumber *offset = [ABI50_0_0RCTConvert NSNumber:blob[@"offset"]];
  NSNumber *size = [ABI50_0_0RCTConvert NSNumber:blob[@"size"]];
...
ABI50_0_0RCT_EXPORT_METHOD(addNetworkingHandler)
{
  ABI50_0_0RCTNetworking *const networking = [_moduleRegistry moduleForName:"Networking"];
...
ABI50_0_0RCT_EXPORT_METHOD(addWebSocketHandler : (double)socketID)
{
  dispatch_async(((ABI50_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
    [[self->_moduleRegistry moduleForName:"WebSocketModule"] setContentHandler:self
                                                                   forSocketID:[NSNumber numberWithDouble:socketID]];
  });
}

ABI50_0_0RCT_EXPORT_METHOD(removeWebSocketHandler : (double)socketID)
{
  dispatch_async(((ABI50_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
...
ABI50_0_0RCT_EXPORT_METHOD(sendOverSocket : (NSDictionary *)blob socketID : (double)socketID)
{
  dispatch_async(((ABI50_0_0RCTWebSocketModule *)[_moduleRegistry moduleForName:"WebSocketModule"]).methodQueue, ^{
...
ABI50_0_0RCT_EXPORT_METHOD(createFromParts : (NSArray<NSDictionary<NSString *, id> *> *)parts withId : (NSString *)blobId)
{
  NSMutableData *data = [NSMutableData new];
  for (NSDictionary<NSString *, id> *part in parts) {
    NSString *type = [ABI50_0_0RCTConvert NSString:part[@"type"]];

    if ([type isEqualToString:@"blob"]) {
      NSData *partData = [self resolve:part[@"data"]];
      [data appendData:partData];
    } else if ([type isEqualToString:@"string"]) {
      NSData *partData = [[ABI50_0_0RCTConvert NSString:part[@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
      [data appendData:partData];
    } else {
...
ABI50_0_0RCT_EXPORT_METHOD(release : (NSString *)blobId)
{
  [self remove:blobId];
}

#pragma mark - ABI50_0_0RCTURLRequestHandler methods
...
#pragma mark - ABI50_0_0RCTNetworkingRequestHandler methods
...
{
  // @lint-ignore FBOBJCUNTYPEDCOLLECTION1
  NSDictionary *blob = [ABI50_0_0RCTConvert NSDictionary:data[@"blob"]];

  NSString *contentType = @"application/octet-stream";
  NSString *blobType = [ABI50_0_0RCTConvert NSString:ABI50_0_0RCTNilIfNull(blob[@"type"])];
...
- (id)handleNetworkingResponse:(NSURLResponse *)response data:(NSData *)data
{
  ...
  return @{
    ...
    @"offset" : @0,
    @"size" : @(data.length),
    @"name" : ABI50_0_0RCTNullIfNil([response suggestedFilename]),
    @"type" : ABI50_0_0RCTNullIfNil([response MIMEType]),
  };
}

#pragma mark - ABI50_0_0RCTWebSocketContentHandler methods
...
{
  return std::make_shared<ABI50_0_0facebook::ABI50_0_0React::NativeBlobModuleSpecJSI>(params);
}

@end

Class ABI50_0_0RCTBlobManagerCls(void)
{
  return ABI50_0_0RCTBlobManager.class;
}
