...
TEST(TextAttributesTest, testToDynamic) {
  ...
  ABI50_0_0EXPECT_EQ(result["opacity"], textAttributes.opacity);
  ABI50_0_0EXPECT_EQ(result["fontStyle"], toString(textAttributes.fontStyle.value()));
  ABI50_0_0EXPECT_EQ(result["fontWeight"], toString(textAttributes.fontWeight.value()));
}
