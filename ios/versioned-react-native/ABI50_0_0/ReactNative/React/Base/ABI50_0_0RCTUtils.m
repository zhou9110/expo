...
#import <ABI50_0_0React/ABI50_0_0RCTUtilsUIOverride.h>
#import "ABI50_0_0RCTAssert.h"
#import "ABI50_0_0RCTLog.h"

NSString *const ABI50_0_0RCTErrorUnspecified = @"EUNSPECIFIED";

// Returns the Path of Home directory
NSString *__nullable ABI50_0_0RCTHomePath(void);
...
NSString *__nullable ABI50_0_0RCTHomePathForURL(NSURL *__nullable URL);

// Determines if a given image URL refers to a image in Home directory (~)
BOOL ABI50_0_0RCTIsHomeAssetURL(NSURL *__nullable imageURL);

static NSString *__nullable _ABI50_0_0RCTJSONStringifyNoRetry(id __nullable jsonObject, NSError **error)
{
  if (!jsonObject) {
    return nil;
  }
  ...
  @try {
    ...
    if (error) {
      *error = [NSError errorWithDomain:ABI50_0_0RCTErrorDomain
                                   code:0
...
NSString *__nullable ABI50_0_0RCTJSONStringify(id __nullable jsonObject, NSError **error)
{
  if (error) {
    return _ABI50_0_0RCTJSONStringifyNoRetry(jsonObject, error);
  } else {
    NSError *localError;
    NSString *json = _ABI50_0_0RCTJSONStringifyNoRetry(jsonObject, &localError);
    if (localError) {
      ABI50_0_0RCTLogError(@"ABI50_0_0RCTJSONStringify() encountered the following error: %@", localError.localizedDescription);
      // Sanitize the data, then retry. This is slow, but it prevents uncaught
      // data issues from crashing in production
      return _ABI50_0_0RCTJSONStringifyNoRetry(ABI50_0_0RCTJSONClean(jsonObject), NULL);
    }
    return json;
  }
}

static id __nullable _ABI50_0_0RCTJSONParse(NSString *__nullable jsonString, BOOL mutable, NSError **error)
{
  static SEL JSONKitSelector = NULL;
  ...
  if (jsonString) {
    ...
    if (!jsonData) {
      ...
      if (jsonData) {
        ABI50_0_0RCTLogWarn(
            @"ABI50_0_0RCTJSONParse received the following string, which could "
             "not be losslessly converted to UTF8 data: '%@'",
            jsonString);
      } else {
        NSString *errorMessage = @"ABI50_0_0RCTJSONParse received invalid UTF8 data";
        if (error) {
          *error = ABI50_0_0RCTErrorWithMessage(errorMessage);
        } else {
          ABI50_0_0RCTLogError(@"%@", errorMessage);
        }
        return nil;
      }
    }
...
id __nullable ABI50_0_0RCTJSONParse(NSString *__nullable jsonString, NSError **error)
{
  return _ABI50_0_0RCTJSONParse(jsonString, NO, error);
}

id __nullable ABI50_0_0RCTJSONParseMutable(NSString *__nullable jsonString, NSError **error)
{
  return _ABI50_0_0RCTJSONParse(jsonString, YES, error);
}

id ABI50_0_0RCTJSONClean(id object)
{
  static dispatch_once_t onceToken;
  ...
  if ([validLeafTypes containsObject:[object classForCoder]]) {
    if ([object isKindOfClass:[NSNumber class]]) {
      return @(ABI50_0_0RCTZeroIfNaN([object doubleValue]));
    }
  ...
  if ([object isKindOfClass:[NSDictionary class]]) {
    ...
    [object enumerateKeysAndObjectsUsingBlock:^(NSString *key, id item, __unused BOOL *stop) {
      id value = ABI50_0_0RCTJSONClean(item);
      values[key] = value;
      copy |= value != item;
    }];
...
NSString *ABI50_0_0RCTMD5Hash(NSString *string)
{
...
BOOL ABI50_0_0RCTIsMainQueue(void)
{
...
void ABI50_0_0RCTExecuteOnMainQueue(dispatch_block_t block)
{
...
void ABI50_0_0RCTUnsafeExecuteOnMainQueueSync(dispatch_block_t block)
{
  if (ABI50_0_0RCTIsMainQueue()) {
    block();
  } else {
...
static void ABI50_0_0RCTUnsafeExecuteOnMainQueueOnceSync(dispatch_once_t *onceToken, dispatch_block_t block)
{
...
void ABI50_0_0RCTComputeScreenScale(void)
{
...
CGFloat ABI50_0_0RCTScreenScale(void)
{
  ABI50_0_0RCTUnsafeExecuteOnMainQueueOnceSync(&onceTokenScreenScale, ^{
    screenScale = [UIScreen mainScreen].scale;
  });
...
CGFloat ABI50_0_0RCTFontSizeMultiplier(void)
{
  ...
  return mapping[ABI50_0_0RCTSharedApplication().preferredContentSizeCategory].floatValue;
}

CGSize ABI50_0_0RCTScreenSize(void)
{
...
CGSize ABI50_0_0RCTViewportSize(void)
{
  UIWindow *window = ABI50_0_0RCTKeyWindow();
  return window ? window.bounds.size : ABI50_0_0RCTScreenSize();
}

CGFloat ABI50_0_0RCTRoundPixelValue(CGFloat value)
{
  CGFloat scale = ABI50_0_0RCTScreenScale();
  return round(value * scale) / scale;
}

CGFloat ABI50_0_0RCTCeilPixelValue(CGFloat value)
{
  CGFloat scale = ABI50_0_0RCTScreenScale();
  return ceil(value * scale) / scale;
}

CGFloat ABI50_0_0RCTFloorPixelValue(CGFloat value)
{
  CGFloat scale = ABI50_0_0RCTScreenScale();
  return floor(value * scale) / scale;
}

CGSize ABI50_0_0RCTSizeInPixels(CGSize pointSize, CGFloat scale)
{
  return (CGSize){
...
void ABI50_0_0RCTSwapClassMethods(Class cls, SEL original, SEL replacement)
{
...
void ABI50_0_0RCTSwapInstanceMethods(Class cls, SEL original, SEL replacement)
{
...
void ABI50_0_0RCTSwapInstanceMethodWithBlock(Class cls, SEL original, id replacementBlock, SEL replacementSelector)
{
  Method originalMethod = class_getInstanceMethod(cls, original);
  if (!originalMethod) {
    return;
...
BOOL ABI50_0_0RCTClassOverridesClassMethod(Class cls, SEL selector)
{
  return ABI50_0_0RCTClassOverridesInstanceMethod(object_getClass(cls), selector);
}

BOOL ABI50_0_0RCTClassOverridesInstanceMethod(Class cls, SEL selector)
{
...
NSDictionary<NSString *, id>
    *ABI50_0_0RCTMakeError(NSString *message, id __nullable toStringify, NSDictionary<NSString *, id> *__nullable extraData)
{
  if (toStringify) {
...
NSDictionary<NSString *, id> *
ABI50_0_0RCTMakeAndLogError(NSString *message, id __nullable toStringify, NSDictionary<NSString *, id> *__nullable extraData)
{
  NSDictionary<NSString *, id> *error = ABI50_0_0RCTMakeError(message, toStringify, extraData);
  ABI50_0_0RCTLogError(@"\nError: %@", error);
  return error;
}

NSDictionary<NSString *, id> *ABI50_0_0RCTJSErrorFromNSError(NSError *error)
{
  NSString *codeWithDomain =
      [NSString stringWithFormat:@"E%@%lld", error.domain.uppercaseString, (long long)error.code];
  return ABI50_0_0RCTJSErrorFromCodeMessageAndNSError(codeWithDomain, error.localizedDescription, error);
}

// TODO: Can we just replace ABI50_0_0RCTMakeError with this function instead?
NSDictionary<NSString *, id>
    *ABI50_0_0RCTJSErrorFromCodeMessageAndNSError(NSString *code, NSString *message, NSError *__nullable error)
{
  ...
  if (error) {
    ...
    errorInfo[@"domain"] = error.domain ?: ABI50_0_0RCTErrorDomain;
    if (error.userInfo) {
      ...
      if (userInfo != nil && userInfo[NSUnderlyingErrorKey] != nil) {
        ...
        userInfo[NSUnderlyingErrorKey] =
            ABI50_0_0RCTJSErrorFromCodeMessageAndNSError(underlyingCode, @"underlying error", underlyingError);
      }
    }
  } else {
    errorMessage = @"Unknown error from a native module";
    errorInfo[@"domain"] = ABI50_0_0RCTErrorDomain;
    userInfo = nil;
  }
  errorInfo[@"code"] = code ?: ABI50_0_0RCTErrorUnspecified;
  errorInfo[@"userInfo"] = ABI50_0_0RCTNullIfNil(userInfo);

  // Allow for explicit overriding of the error message
  errorMessage = message ?: errorMessage;

  return ABI50_0_0RCTMakeError(errorMessage, nil, errorInfo);
}

BOOL ABI50_0_0RCTRunningInTestEnvironment(void)
{
...
BOOL ABI50_0_0RCTRunningInAppExtension(void)
{
  return [[[[NSBundle mainBundle] bundlePath] pathExtension] isEqualToString:@"appex"];
}

UIApplication *__nullable ABI50_0_0RCTSharedApplication(void)
{
...
UIWindow *__nullable ABI50_0_0RCTKeyWindow(void)
{
  if (ABI50_0_0RCTRunningInAppExtension()) {
    return nil;
  }
...
UIViewController *__nullable ABI50_0_0RCTPresentedViewController(void)
{
  if ([ABI50_0_0RCTUtilsUIOverride hasPresentedViewController]) {
    return [ABI50_0_0RCTUtilsUIOverride presentedViewController];
  }

  UIViewController *controller = ABI50_0_0RCTKeyWindow().rootViewController;
...
BOOL ABI50_0_0RCTForceTouchAvailable(void)
{
  ...
  return forceSupported &&
      (ABI50_0_0RCTKeyWindow() ?: [UIView new]).traitCollection.forceTouchCapability == UIForceTouchCapabilityAvailable;
}

NSError *ABI50_0_0RCTErrorWithMessage(NSString *message)
{
  NSDictionary<NSString *, id> *errorInfo = @{NSLocalizedDescriptionKey : message};
  return [[NSError alloc] initWithDomain:ABI50_0_0RCTErrorDomain code:0 userInfo:errorInfo];
}

NSError *ABI50_0_0RCTErrorWithNSException(NSException *exception)
{
  NSString *message = [NSString stringWithFormat:@"NSException: %@; trace: %@.",
                                                 exception,
                                                 [[exception callStackSymbols] componentsJoinedByString:@";"]];
  NSDictionary<NSString *, id> *errorInfo =
      @{NSLocalizedDescriptionKey : message, ABI50_0_0RCTObjCStackTraceKey : [exception callStackSymbols]};
  return [[NSError alloc] initWithDomain:ABI50_0_0RCTErrorDomain code:0 userInfo:errorInfo];
}

double ABI50_0_0RCTZeroIfNaN(double value)
{
  return isnan(value) || isinf(value) ? 0 : value;
}

double ABI50_0_0RCTSanitizeNaNValue(double value, NSString *property)
{
  if (!isnan(value) && !isinf(value)) {
    return value;
  }

  ABI50_0_0RCTLogWarn(@"The value `%@` equals NaN or INF and will be replaced by `0`.", property);
  return 0;
}

NSURL *ABI50_0_0RCTDataURL(NSString *mimeType, NSData *data)
{
  return [NSURL
...
BOOL ABI50_0_0RCTIsGzippedData(NSData *__nullable); // exposed for unit testing purposes
BOOL ABI50_0_0RCTIsGzippedData(NSData *__nullable data)
{
  UInt8 *bytes = (UInt8 *)data.bytes;
  return (data.length >= 2 && bytes[0] == 0x1f && bytes[1] == 0x8b);
}

NSData *__nullable ABI50_0_0RCTGzipData(NSData *__nullable input, float level)
{
  if (input.length == 0 || ABI50_0_0RCTIsGzippedData(input)) {
    return input;
  }
  ...
  static const NSUInteger ABI50_0_0RCTGZipChunkSize = 16384;

  NSMutableData *output = nil;
  int compression = (level < 0.0f) ? Z_DEFAULT_COMPRESSION : (int)(roundf(level * 9));
  if (deflateInit2(&stream, compression, Z_DEFLATED, 31, 8, Z_DEFAULT_STRATEGY) == Z_OK) {
    output = [NSMutableData dataWithLength:ABI50_0_0RCTGZipChunkSize];
    while (stream.avail_out == 0) {
      if (stream.total_out >= output.length) {
        output.length += ABI50_0_0RCTGZipChunkSize;
      }
...
static NSString *ABI50_0_0RCTRelativePathForURL(NSString *basePath, NSURL *__nullable URL)
{
  if (!URL.fileURL) {
    // Not a file path
    return nil;
...
NSString *__nullable ABI50_0_0RCTLibraryPath(void)
{
...
NSString *__nullable ABI50_0_0RCTBundlePathForURL(NSURL *__nullable URL)
{
  return ABI50_0_0RCTRelativePathForURL([[NSBundle mainBundle] resourcePath], URL);
}

NSString *__nullable ABI50_0_0RCTLibraryPathForURL(NSURL *__nullable URL)
{
  return ABI50_0_0RCTRelativePathForURL(ABI50_0_0RCTLibraryPath(), URL);
}

NSString *__nullable ABI50_0_0RCTHomePathForURL(NSURL *__nullable URL)
{
  return ABI50_0_0RCTRelativePathForURL(ABI50_0_0RCTHomePath(), URL);
}

static BOOL ABI50_0_0RCTIsImageAssetsPath(NSString *path)
{
  NSString *extension = [path pathExtension];
  return [extension isEqualToString:@"png"] || [extension isEqualToString:@"jpg"];
}

BOOL ABI50_0_0RCTIsBundleAssetURL(NSURL *__nullable imageURL)
{
  return ABI50_0_0RCTIsImageAssetsPath(ABI50_0_0RCTBundlePathForURL(imageURL));
}

BOOL ABI50_0_0RCTIsLibraryAssetURL(NSURL *__nullable imageURL)
{
  return ABI50_0_0RCTIsImageAssetsPath(ABI50_0_0RCTLibraryPathForURL(imageURL));
}

BOOL ABI50_0_0RCTIsHomeAssetURL(NSURL *__nullable imageURL)
{
  return ABI50_0_0RCTIsImageAssetsPath(ABI50_0_0RCTHomePathForURL(imageURL));
}

BOOL ABI50_0_0RCTIsLocalAssetURL(NSURL *__nullable imageURL)
{
  return ABI50_0_0RCTIsBundleAssetURL(imageURL) || ABI50_0_0RCTIsHomeAssetURL(imageURL);
}
...
UIImage *__nullable ABI50_0_0RCTImageFromLocalBundleAssetURL(NSURL *imageURL)
{
  ...
  return ABI50_0_0RCTImageFromLocalAssetURL(bundleImageUrl);
}

UIImage *__nullable ABI50_0_0RCTImageFromLocalAssetURL(NSURL *imageURL)
{
  NSString *imageName = ABI50_0_0RCTBundlePathForURL(imageURL);
  ...
  if (!image && !bundle) {
    ...
    for (NSURL *frameworkURL in possibleFrameworks) {
      ...
      image = [UIImage imageNamed:imageName inBundle:bundle compatibleWithTraitCollection:nil];
      if (image) {
        ABI50_0_0RCTLogWarn(@"Image %@ not found in mainBundle, but found in %@", imageName, bundle);
        break;
      }
    }
  }
...
ABI50_0_0RCT_EXTERN NSString *__nullable ABI50_0_0RCTTempFilePath(NSString *extension, NSError **error)
{
  static NSError *setupError = nil;
  static NSString *directory;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    directory = [NSTemporaryDirectory() stringByAppendingPathComponent:@"ABI50_0_0ReactNative"];
      ...
      if (![fileManager createDirectoryAtPath:directory
                  withIntermediateDirectories:YES
                                   attributes:nil
                                        error:&localError]) {
        // This is bad
        ABI50_0_0RCTLogError(@"Failed to create temporary directory: %@", localError);
...
ABI50_0_0RCT_EXTERN void ABI50_0_0RCTGetRGBAColorComponents(CGColorRef color, CGFloat rgba[4])
{
  ...
  switch (model) {
    ...
    default: {
      ...
      // unsupported format
      ABI50_0_0RCTLogError(@"Unsupported color model: %i", model);
#endif
...
NSString *ABI50_0_0RCTColorToHexString(CGColorRef color)
{
  CGFloat rgba[4];
  ABI50_0_0RCTGetRGBAColorComponents(color, rgba);
...
NSString *ABI50_0_0RCTUIKitLocalizedString(NSString *string)
{
  NSBundle *UIKitBundle = [NSBundle bundleForClass:[UIApplication class]];
  return UIKitBundle ? [UIKitBundle localizedStringForKey:string value:string table:nil] : string;
}

NSString *ABI50_0_0RCTHumanReadableType(NSObject *obj)
{
...
NSString *__nullable ABI50_0_0RCTGetURLQueryParam(NSURL *__nullable URL, NSString *param)
{
  ABI50_0_0RCTAssertParam(param);
  if (!URL) {
    return nil;
  }
...
NSURL *__nullable ABI50_0_0RCTURLByReplacingQueryParam(NSURL *__nullable URL, NSString *param, NSString *__nullable value)
{
  ABI50_0_0RCTAssertParam(param);
  if (!URL) {
    return nil;
...
ABI50_0_0RCT_EXTERN NSString *ABI50_0_0RCTDropABI50_0_0ReactPrefixes(NSString *s)
{
  if ([s hasPrefix:@"RK"]) {
...
ABI50_0_0RCT_EXTERN BOOL ABI50_0_0RCTUIManagerTypeForTagIsFabric(NSNumber *ABI50_0_0ReactTag)
{
  // See https://github.com/facebook/ABI50_0_0React/pull/12587
  return [ABI50_0_0ReactTag integerValue] % 2 == 0;
}

ABI50_0_0RCT_EXTERN BOOL ABI50_0_0RCTValidateTypeOfViewCommandArgument(
    NSObject *obj,
    id expectedClass,
    ...
    const NSString *argPos)
{
  if (![obj isKindOfClass:expectedClass]) {
    NSString *kindOfClass = ABI50_0_0RCTHumanReadableType(obj);
...
BOOL ABI50_0_0RCTIsAppActive(void)
{
  return [ABI50_0_0RCTSharedApplication() applicationState] == UIApplicationStateActive;
}
