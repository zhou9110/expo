...
TEST(DynamicPropsUtilitiesTest, handleNestedObjects) {
  ...
  ABI50_0_0EXPECT_TRUE(result["style"].isObject());
  ABI50_0_0EXPECT_TRUE(result["style"]["backgroundColor"].isString());
  ABI50_0_0EXPECT_TRUE(result["style"]["color"].isString());
  ABI50_0_0EXPECT_TRUE(result["height"].isInt());

  ABI50_0_0EXPECT_EQ(result["style"]["backgroundColor"].asString(), "blue");
  ABI50_0_0EXPECT_EQ(result["style"]["color"], "black");
  ABI50_0_0EXPECT_EQ(result["height"], 100);
}
...
TEST(DynamicPropsUtilitiesTest, handleNull) {
  ...
  ABI50_0_0EXPECT_TRUE(result["height"].isNull());
}
