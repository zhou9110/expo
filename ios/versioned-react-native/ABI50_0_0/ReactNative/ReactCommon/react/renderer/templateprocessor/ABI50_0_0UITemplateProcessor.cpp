...
#include "ABI50_0_0UITemplateProcessor.h"
...
ShadowNode::Shared UITemplateProcessor::runCommand(
    ...
    const ComponentDescriptorRegistry& componentDescriptorRegistry,
    const NativeModuleRegistry& nativeModuleRegistry,
    const std::shared_ptr<const ABI50_0_0ReactNativeConfig>& ABI50_0_0ReactNativeConfig) {
  ...
  if (opcode == "createNode") {
    ...
    registers[registerNumber] = ABI50_0_0ReactNativeConfig->getBool(param);
...
ShadowNode::Shared UITemplateProcessor::buildShadowTree(
  ...
  LOG(ERROR) << "ABI50_0_0React ui template missing returnRoot command :(";
