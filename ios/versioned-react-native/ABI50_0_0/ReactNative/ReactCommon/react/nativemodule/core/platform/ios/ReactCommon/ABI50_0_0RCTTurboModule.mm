...
#import "ABI50_0_0RCTBlockGuard.h"
...
#import <ABI50_0_0ReactCommon/ABI50_0_0LongLivedObject.h>
...
using namespace ABI50_0_0facebook::ABI50_0_0React::TurboModuleConvertUtils;
...
static ABI50_0_0RCTResponseSenderBlock
...
convertJSIFunctionToCallback(jsi::Runtime &runtime, const jsi::Function &value, std::shared_ptr<CallInvoker> jsInvoker)
{
  ...
  ABI50_0_0RCTBlockGuard *blockGuard = [[ABI50_0_0RCTBlockGuard alloc] initWithCleanup:^() {
...
jsi::Value ObjCTurboModule::createPromise(jsi::Runtime &runtime, std::string methodName, PromiseInvocationBlock invoke)
{
  ...
  jsi::Function fn = jsi::Function::createFromHostFunction(
      ...
      [invokeCopy, jsInvoker = jsInvoker_, moduleName, methodName](
        ...
        ABI50_0_0RCTPromiseResolveBlock resolveBlock = ^(id result) {
          if (rejectWasCalled) {
            ABI50_0_0RCTLogError(@"%s: Tried to resolve a promise after it's already been rejected.", moduleMethod.c_str());
            return;
          }

          if (resolveWasCalled) {
            ABI50_0_0RCTLogError(@"%s: Tried to resolve a promise more than once.", moduleMethod.c_str());
            return;
          }
        ...
        ABI50_0_0RCTPromiseRejectBlock rejectBlock = ^(NSString *code, NSString *message, NSError *error) {
          if (resolveWasCalled) {
            ABI50_0_0RCTLogError(@"%s: Tried to reject a promise after it's already been resolved.", moduleMethod.c_str());
            return;
          }

          if (rejectWasCalled) {
            ABI50_0_0RCTLogError(@"%s: Tried to reject a promise more than once.", moduleMethod.c_str());
            return;
          }
          ...
          NSDictionary *jsError = ABI50_0_0RCTJSErrorFromCodeMessageAndNSError(code, message, error);
 ...
 * This serves as a backward-compatible support for ABI50_0_0RCTBridgeModule's methodQueue API.
 *
 * In the future:
...
{
  __block id result;
  __weak id<ABI50_0_0RCTBridgeModule> weakModule = instance_;
  const char *moduleName = name_.c_str();
  std::string methodNameStr{methodName};
  __block int32_t asyncCallCounter = 0;

  void (^block)() = ^{
    id<ABI50_0_0RCTBridgeModule> strongModule = weakModule;
    if (!strongModule) {
      return;
    }
  ...
  if (!methodArgumentTypeNames_) {
    ...
    if (methods) {
      for (unsigned int i = 0; i < numberOfMethods; i++) {
        ...
        ABI50_0_0RCTMethodInfo *(*getMethodInfo)(id, SEL) = (__typeof__(getMethodInfo))objc_msgSend;
        ABI50_0_0RCTMethodInfo *methodInfo = getMethodInfo(cls, s);

        NSArray<ABI50_0_0RCTMethodArgument *> *arguments;
        NSString *otherMethodName = ABI50_0_0RCTParseMethodSignature(methodInfo->objcName, &arguments);
    ...
    NSMutableArray *retainedObjectsForInvocation)
{
  ...
  if (objCArg) {
    ...
    /**
     * Convert objects using ABI50_0_0RCTConvert.
     */
    if (objCArgType == @encode(id)) {
      ...
      if (argumentType != nil) {
        ...
        if ([ABI50_0_0RCTConvert respondsToSelector:rctConvertSelector]) {
          // Message dispatch logic from old infra
          id (*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
          id convertedObjCArg = convert([ABI50_0_0RCTConvert class], rctConvertSelector, objCArg);
    ...
    /**
     * Convert objects using ABI50_0_0RCTCxxConvert to structs.
     */
    if ([objCArg isKindOfClass:[NSDictionary class]] && hasMethodArgConversionSelector(methodNameNSString, i)) {
      ...
      // https://github.com/facebook/ABI50_0_0React-native/commit/6783694158057662fd7b11fc123c339b2b21bfe6#diff-263fc157dfce55895cdc16495b55d190R350)
      ABI50_0_0RCTManagedPointer *(*convert)(id, SEL, id) = (__typeof__(convert))objc_msgSend;
      ABI50_0_0RCTManagedPointer *box = convert([ABI50_0_0RCTCxxConvert class], methodArgConversionSelector, objCArg);
...
{
  const char *moduleName = name_.c_str();
  const id<ABI50_0_0RCTBridgeModule> module = instance_;
    ...
    size_t count)
{
  ...
  if (returnType == PromiseKind) {
    returnValue = createPromise(
        runtime, methodNameStr, ^(ABI50_0_0RCTPromiseResolveBlock resolveBlock, ABI50_0_0RCTPromiseRejectBlock rejectBlock) {
          ABI50_0_0RCTPromiseResolveBlock resolveCopy = [resolveBlock copy];
          ABI50_0_0RCTPromiseRejectBlock rejectCopy = [rejectBlock copy];
