...
import ABI50_0_0React

/**
 Custom component data extending `ABI50_0_0RCTComponentData`. Its main purpose is to handle event-based props (callbacks),
 but it also simplifies capturing the view config so we can omit some reflections that ABI50_0_0React Native executes.
 */
@objc(ABI50_0_0EXComponentData)
public final class ComponentData: ABI50_0_0RCTComponentDataSwiftAdapter {
  /**
   ...
   */
  @objc
  public init(viewModule: ViewModuleWrapper, managerClass: ViewModuleWrapper.Type, bridge: ABI50_0_0RCTBridge) {
  ...
  // MARK: ABI50_0_0RCTComponentData

  /**
   Creates a setter for the specific prop. For non-event props we just let ABI50_0_0React Native do its job.
   Events are handled differently to conveniently use them in Swift.
   */
  public override func createPropBlock(_ propName: String, isShadowView: Bool) -> ABI50_0_0RCTPropBlockAlias {
  ...
  public override func setProps(_ props: [String: Any], forView view: ABI50_0_0RCTComponent) {
    ...
    // Let the base class `ABI50_0_0RCTComponentData` handle all remaining props.
  ...
  /**
   The base `ABI50_0_0RCTComponentData` class does some Objective-C dynamic calls in this function, but we don't
  ...
  public override func viewConfig() -> [String: Any] {
    ...
    if let viewManager = moduleHolder?.viewManager {
      ...
      for eventName in viewManager.eventNames {
        directEvents.append(ABI50_0_0RCTNormalizeInputEventName(eventName))
 ...
 */
private func createEventSetter(eventName: String, bridge: ABI50_0_0RCTBridge?) -> ABI50_0_0RCTPropBlockAlias {
  return { [weak bridge] (target: ABI50_0_0RCTComponent, value: Any) in
    installEventDispatcher(forEvent: eventName, onView: target) { [weak target] (body: [String: Any]) in
      if let target = target {
        let componentEvent = ABI50_0_0RCTComponentEvent(name: eventName, viewTag: target.abi50_0_0ReactTag, body: body)
