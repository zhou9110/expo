...
#import <ABI50_0_0React/ABI50_0_0RCTErrorInfo.h>
#import <ABI50_0_0React/ABI50_0_0RCTEventDispatcherProtocol.h>
#import <ABI50_0_0React/ABI50_0_0RCTJSStackFrame.h>
#import <ABI50_0_0React/ABI50_0_0RCTRedBoxExtraDataViewController.h>
...
@class ABI50_0_0RCTRedBoxWindow;

@interface UIButton (ABI50_0_0RCTRedBox)

@property (nonatomic) ABI50_0_0RCTRedBoxButtonPressHandler rct_handler;

- (void)rct_addBlock:(ABI50_0_0RCTRedBoxButtonPressHandler)handler forControlEvents:(UIControlEvents)controlEvents;

@end

@implementation UIButton (ABI50_0_0RCTRedBox)

- (ABI50_0_0RCTRedBoxButtonPressHandler)rct_handler
{
  return objc_getAssociatedObject(self, @selector(rct_handler));
}

- (void)setRct_handler:(ABI50_0_0RCTRedBoxButtonPressHandler)rct_handler
{
...
@protocol ABI50_0_0RCTRedBoxWindowActionDelegate <NSObject>

- (void)redBoxWindow:(ABI50_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI50_0_0RCTJSStackFrame *)stackFrame;
- (void)reloadFromRedBoxWindow:(ABI50_0_0RCTRedBoxWindow *)redBoxWindow;
- (void)loadExtraDataViewController;

@end

@interface ABI50_0_0RCTRedBoxWindow : NSObject <UITableViewDelegate, UITableViewDataSource>
@property (nonatomic, strong) UIViewController *rootViewController;
@property (nonatomic, weak) id<ABI50_0_0RCTRedBoxWindowActionDelegate> actionDelegate;
@end

@implementation ABI50_0_0RCTRedBoxWindow {
  UITableView *_stackTraceTableView;
  NSString *_lastErrorMessage;
  NSArray<ABI50_0_0RCTJSStackFrame *> *_lastStackTrace;
  int _lastErrorCookie;
}

- (instancetype)initWithFrame:(CGRect)frame
           customButtonTitles:(NSArray<NSString *> *)customButtonTitles
         customButtonHandlers:(NSArray<ABI50_0_0RCTRedBoxButtonPressHandler> *)customButtonHandlers
{
  if (self = [super init]) {
    _lastErrorCookie = -1;
...
- (UIButton *)redBoxButton:(NSString *)title
    accessibilityIdentifier:(NSString *)accessibilityIdentifier
                   selector:(SEL)selector
                      block:(ABI50_0_0RCTRedBoxButtonPressHandler)block
{
...
{
  return ABI50_0_0RCTSharedApplication().delegate.window.safeAreaInsets.bottom;
}
...
- (void)showErrorMessage:(NSString *)message
               withStack:(NSArray<ABI50_0_0RCTJSStackFrame *> *)stack
    ...
    if (!isRootViewControllerPresented) {
      [_stackTraceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]
                                  atScrollPosition:UITableViewScrollPositionTop
                                          animated:NO];
      [ABI50_0_0RCTKeyWindow().rootViewController presentViewController:self.rootViewController animated:YES completion:nil];
    }
  }
}
...
- (void)copyStack
{
  ...
  for (ABI50_0_0RCTJSStackFrame *stackFrame in _lastStackTrace) {
...
- (NSString *)formatFrameSource:(ABI50_0_0RCTJSStackFrame *)stackFrame
{
  NSString *fileName = ABI50_0_0RCTNilIfNull(stackFrame.file) ? [stackFrame.file lastPathComponent] : @"<unknown file>";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  ABI50_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[index];
...
- (UITableViewCell *)reuseCell:(UITableViewCell *)cell forStackFrame:(ABI50_0_0RCTJSStackFrame *)stackFrame
{
  if (!cell) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  if (indexPath.section == 1) {
    ...
    ABI50_0_0RCTJSStackFrame *stackFrame = _lastStackTrace[row];
...
{
  // NOTE: We could use ABI50_0_0RCTKeyCommands for this, but since
...
@interface ABI50_0_0RCTRedBox () <
    ABI50_0_0RCTInvalidating,
    ABI50_0_0RCTRedBoxWindowActionDelegate,
    ABI50_0_0RCTRedBoxExtraDataActionDelegate,
    ABI50_0_0NativeRedBoxSpec>
@end

@implementation ABI50_0_0RCTRedBox {
  ABI50_0_0RCTRedBoxWindow *_window;
  NSMutableArray<id<ABI50_0_0RCTErrorCustomizer>> *_errorCustomizers;
  ABI50_0_0RCTRedBoxExtraDataViewController *_extraDataViewController;
  NSMutableArray<NSString *> *_customButtonTitles;
  NSMutableArray<ABI50_0_0RCTRedBoxButtonPressHandler> *_customButtonHandlers;
}
...
- (ABI50_0_0RCTErrorInfo *)_customizeError:(ABI50_0_0RCTErrorInfo *)error
{
  ABI50_0_0RCTAssertMainQueue();
  if (!self->_errorCustomizers) {
    return error;
  }
  for (id<ABI50_0_0RCTErrorCustomizer> customizer in self->_errorCustomizers) {
    ABI50_0_0RCTErrorInfo *newInfo = [customizer customizeErrorInfo:error];
    if (newInfo) {
      error = newInfo;
    }
  }
...
- (void)showError:(NSError *)error
{
  [self showErrorMessage:error.localizedDescription
             withDetails:error.localizedFailureReason
                   stack:error.userInfo[ABI50_0_0RCTJSStackTraceKey]
             errorCookie:-1];
}
...
- (void)showErrorMessage:(NSString *)message
             withDetails:(NSString *)details
                   stack:(NSArray<ABI50_0_0RCTJSStackFrame *> *)stack
             errorCookie:(int)errorCookie
{
...
{
  NSArray<ABI50_0_0RCTJSStackFrame *> *stack = [ABI50_0_0RCTJSStackFrame stackFramesWithLines:rawStack];
...
- (void)showErrorMessage:(NSString *)message withStack:(NSArray<NSDictionary *> *)stack errorCookie:(int)errorCookie
{
  [self showErrorMessage:message
         withParsedStack:[ABI50_0_0RCTJSStackFrame stackFramesWithDictionaries:stack]
                isUpdate:NO
             ...
             errorCookie:(int)errorCookie
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (self->_extraDataViewController == nil) {
      self->_extraDataViewController = [ABI50_0_0RCTRedBoxExtraDataViewController new];
...
#pragma clang diagnostic pop
    ...
    if (!self->_window) {
      self->_window = [[ABI50_0_0RCTRedBoxWindow alloc] initWithFrame:[UIScreen mainScreen].bounds
    ...
    ABI50_0_0RCTErrorInfo *errorInfo = [[ABI50_0_0RCTErrorInfo alloc] initWithErrorMessage:message stack:stack];
...
ABI50_0_0RCT_EXPORT_METHOD(setExtraData : (NSDictionary *)extraData forIdentifier : (NSString *)identifier)
{
  [_extraDataViewController addExtraData:extraData forIdentifier:identifier];
}

ABI50_0_0RCT_EXPORT_METHOD(dismiss)
{
  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)redBoxWindow:(__unused ABI50_0_0RCTRedBoxWindow *)redBoxWindow openStackFrameInEditor:(ABI50_0_0RCTJSStackFrame *)stackFrame
{
  NSURL *const bundleURL = _overrideBundleURL ?: _bundleManager.bundleURL;
  if (![bundleURL.scheme hasPrefix:@"http"]) {
    ABI50_0_0RCTLogWarn(@"Cannot open stack frame in editor because you're not connected to the packager.");
    return;
  }

  NSData *stackFrameJSON = [ABI50_0_0RCTJSONStringify([stackFrame toDictionary], NULL) dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)reloadFromRedBoxWindow:(__unused ABI50_0_0RCTRedBoxWindow *)redBoxWindow
{
  if (_overrideReloadAction) {
    _overrideReloadAction();
  } else {
    ABI50_0_0RCTTriggerReloadCommandListeners(@"Redbox");
  }
  [self dismiss];
}
...
{
  return std::make_shared<ABI50_0_0facebook::ABI50_0_0React::NativeRedBoxSpecJSI>(params);
}

@end

@implementation ABI50_0_0RCTBridge (ABI50_0_0RCTRedBox)

- (ABI50_0_0RCTRedBox *)redBox
{
  return ABI50_0_0RCTRedBoxGetEnabled() ? [self moduleForClass:[ABI50_0_0RCTRedBox class]] : nil;
}

@end

@implementation ABI50_0_0RCTBridgeProxy (ABI50_0_0RCTRedBox)
...
@interface ABI50_0_0RCTRedBox () <ABI50_0_0NativeRedBoxSpec>
@end
...
Class ABI50_0_0RCTRedBoxCls(void)
{
  return ABI50_0_0RCTRedBox.class;
}
