//
//  ABI50_0_0AIRMapPolylineRenderer.h
//  mapDemo
//
...
@interface ABI50_0_0AIRMapPolylineRendererSegment : NSObject
...
@end
@implementation ABI50_0_0AIRMapPolylineRendererSegment
- (id)initWithPoint:(CGPoint)point color:(UIColor*)color
{
    self = [super init];
    if (self){
...
@implementation ABI50_0_0AIRMapPolylineRenderer {
...
- (NSArray*) createSegments
{
    ...
    ABI50_0_0AIRMapPolylineRendererSegment* segment = nil;
    for (NSUInteger i = 0, n = _polyline.pointCount; i < n; i++){
        CGPoint point = [self pointForIndex:i];
        ...
        if (segment == nil) {
            ...
            // Start new segment
            segment = [[ABI50_0_0AIRMapPolylineRendererSegment alloc] initWithPoint:point color:color];
            ...
            if (segment.endColor == nil) {
                ...
                segment = [[ABI50_0_0AIRMapPolylineRendererSegment alloc] initWithPoint:segment.endPoint color:segment.endColor];
                [segment addPoint:point color:color];
                [segments addObject:segment];
            }
...
- (void) drawWithZoomScale:(MKZoomScale)zoomScale inContext:(CGContextRef)context
{
    ...
    for (NSUInteger i = 0, n = segments.count; i < n; i++) {
        ABI50_0_0AIRMapPolylineRendererSegment* segment = segments[i];
