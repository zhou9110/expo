...
@implementation ABI50_0_0RCTLayoutAnimation

static UIViewAnimationCurve _currentKeyboardAnimationCurve;

static UIViewAnimationOptions UIViewAnimationOptionsFromABI50_0_0RCTAnimationType(ABI50_0_0RCTAnimationType type)
{
  switch (type) {
    case ABI50_0_0RCTAnimationTypeLinear:
      return UIViewAnimationOptionCurveLinear;
    case ABI50_0_0RCTAnimationTypeEaseIn:
      return UIViewAnimationOptionCurveEaseIn;
    case ABI50_0_0RCTAnimationTypeEaseOut:
      return UIViewAnimationOptionCurveEaseOut;
    case ABI50_0_0RCTAnimationTypeEaseInEaseOut:
      return UIViewAnimationOptionCurveEaseInOut;
    case ABI50_0_0RCTAnimationTypeKeyboard:
      // http://stackoverflow.com/questions/18870447/how-to-use-the-default-ios7-uianimation-curve
      return (UIViewAnimationOptions)(_currentKeyboardAnimationCurve << 16);
    default:
      ABI50_0_0RCTLogError(@"Unsupported animation type %lld", (long long)type);
      return UIViewAnimationOptionCurveEaseInOut;
  }
}
...
- (instancetype)initWithDuration:(NSTimeInterval)duration
                 ...
                 initialVelocity:(CGFloat)initialVelocity
                   animationType:(ABI50_0_0RCTAnimationType)animationType
{
  if (self = [super init]) {
    _duration = duration;
    _delay = delay;
...
- (instancetype)initWithDuration:(NSTimeInterval)duration config:(NSDictionary *)config
{
  ...
  if (self = [super init]) {
    _property = [ABI50_0_0RCTConvert NSString:config[@"property"]];

    _duration = [ABI50_0_0RCTConvert NSTimeInterval:config[@"duration"]] ?: duration;
    if (_duration > 0.0 && _duration < 0.01) {
      ABI50_0_0RCTLogError(@"ABI50_0_0RCTLayoutAnimationGroup expects timings to be in ms, not seconds.");
      _duration = _duration * 1000.0;
    }

    _delay = [ABI50_0_0RCTConvert NSTimeInterval:config[@"delay"]];
    if (_delay > 0.0 && _delay < 0.01) {
      ABI50_0_0RCTLogError(@"ABI50_0_0RCTLayoutAnimationGroup expects timings to be in ms, not seconds.");
      _delay = _delay * 1000.0;
    }

    _animationType = [ABI50_0_0RCTConvert ABI50_0_0RCTAnimationType:config[@"type"]];
    if (_animationType == ABI50_0_0RCTAnimationTypeSpring) {
      _springDamping = [ABI50_0_0RCTConvert CGFloat:config[@"springDamping"]];
      _initialVelocity = [ABI50_0_0RCTConvert CGFloat:config[@"initialVelocity"]];
    }
  }
...
- (void)performAnimations:(void (^)(void))animations withCompletionBlock:(void (^)(BOOL completed))completionBlock
{
  if (_animationType == ABI50_0_0RCTAnimationTypeSpring) {
    ...
    UIViewAnimationOptions options =
        UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionsFromABI50_0_0RCTAnimationType(_animationType);
...
- (BOOL)isEqual:(ABI50_0_0RCTLayoutAnimation *)animation
{
