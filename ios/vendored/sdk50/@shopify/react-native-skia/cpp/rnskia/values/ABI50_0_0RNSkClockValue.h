...
#include "ABI50_0_0RNSkReadonlyValue.h"
...
/**
 ...
 started.
 */
class ABI50_0_0RNSkClockValue : public ABI50_0_0RNSkReadonlyValue {
  enum ABI50_0_0RNSkClockState { NotStarted = 0, Running = 1, Stopped = 2 };

public:
  ABI50_0_0RNSkClockValue(std::shared_ptr<ABI50_0_0RNSkPlatformContext> platformContext,
                 size_t identifier, jsi::Runtime &runtime,
                 const jsi::Value *arguments, size_t count)
      : ABI50_0_0RNSkReadonlyValue(platformContext), _runtime(runtime),
        _identifier(identifier) {
    // Start by updating to zero (start value)
    update(_runtime, static_cast<double>(0));
  }

  virtual ~ABI50_0_0RNSkClockValue() { stopClock(); }
  ...
  JSI_EXPORT_FUNCTIONS(JSI_EXPORT_FUNC(ABI50_0_0RNSkReadonlyValue, addListener),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkClockValue, start),
                       JSI_EXPORT_FUNC(ABI50_0_0RNSkClockValue, stop))

  virtual void startClock() {
    if (_state == ABI50_0_0RNSkClockState::Running) {
      return;
    }

    auto now = std::chrono::high_resolution_clock::now();
    if (_state == ABI50_0_0RNSkClockState::NotStarted) {
      _start = now;
      _stop = now;
    }
    ...
    _state = ABI50_0_0RNSkClockState::Running;

    getContext()->beginDrawLoop(
        _identifier, [weakSelf = weak_from_this()](bool invalidated) {
          auto self = weakSelf.lock();
          if (self) {
            std::dynamic_pointer_cast<ABI50_0_0RNSkClockValue>(self)->notifyUpdate(
                invalidated);
          }
        });
  }
  ...
  virtual void stopClock() {
    if (_state == ABI50_0_0RNSkClockState::Running) {
      _state = ABI50_0_0RNSkClockState::Stopped;
...
protected:
  virtual void tick(jsi::Runtime &runtime, const jsi::Value &value) {
    ABI50_0_0RNSkClockValue::update(runtime, value);
  }
  ...
  void notifyUpdate(bool invalidated) {
    ...
    getContext()->runOnJavascriptThread(
        ...
        [weakSelf = weak_from_this()]() {
          ...
          if (self) {
            auto selfClockValue =
                std::dynamic_pointer_cast<ABI50_0_0RNSkClockValue>(self);
            if (selfClockValue->getState() == ABI50_0_0RNSkClockState::Running) {
              auto now = std::chrono::high_resolution_clock::now();
              auto deltaFromStart =
   ...
   */
  ABI50_0_0RNSkClockState getState() { return _state; }

  jsi::Runtime &_runtime;
  size_t _identifier;
  std::chrono::time_point<std::chrono::steady_clock> _start;
  std::chrono::time_point<std::chrono::steady_clock> _stop;
  std::atomic<ABI50_0_0RNSkClockState> _state = {ABI50_0_0RNSkClockState::NotStarted};
};
