...
#import <ABI50_0_0EXAV/ABI50_0_0EXVideoPlayerViewController.h>

static NSString *const ABI50_0_0EXVideoReadyForDisplayKeyPath = @"readyForDisplay";
static NSString *const ABI50_0_0EXVideoSourceURIKeyPath = @"uri";
static NSString *const ABI50_0_0EXVideoSourceHeadersKeyPath = @"headers";
static NSString *const ABI50_0_0EXVideoBoundsKeyPath = @"videoBounds";
static NSString *const ABI50_0_0EXAVFullScreenViewControllerClassName = @"AVFullScreenViewController";

@interface ABI50_0_0EXVideoView ()

@property (nonatomic, weak) ABI50_0_0EXAV *exAV;

@property (nonatomic, assign) BOOL playerHasLoaded;
@property (nonatomic, strong) ABI50_0_0EXAVPlayerData *data;
@property (nonatomic, strong) AVPlayerLayer *playerLayer;
@property (nonatomic, strong) ABI50_0_0EXVideoPlayerViewController *playerViewController;

@property (nonatomic, assign) BOOL fullscreenPlayerIsDismissing;
@property (nonatomic, strong) ABI50_0_0EXVideoPlayerViewController *fullscreenPlayerViewController;
@property (nonatomic, strong) ABI50_0_0EXPromiseResolveBlock requestedFullscreenChangeResolver;
@property (nonatomic, strong) ABI50_0_0EXPromiseRejectBlock requestedFullscreenChangeRejecter;
...
@implementation ABI50_0_0EXVideoView

#pragma mark - ABI50_0_0EXVideoView interface methods

- (instancetype)initWithModuleRegistry:(nullable ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  if ((self = [super init])) {
    _exAV = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI50_0_0EXAVInterface)];
...
- (void)_callFullscreenCallbackForUpdate:(ABI50_0_0EXVideoFullscreenUpdate)update
{
...
- (void)_tryUpdateDataStatus:(ABI50_0_0EXPromiseResolveBlock)resolve
...
- (void)_removeData {
  ...
  // Remove ABI50_0_0EXAVPlayerData on main thread to prevent race conditions
  // while KVO messages are dispatched on main thread and the player data is
  // de-allocating  
  [ABI50_0_0EXUtilities performSynchronouslyOnMainThread:block];
}
...
- (ABI50_0_0EXVideoPlayerViewController *)_createNewPlayerViewController
{
  if (_data == nil) {
    return nil;
  }
  ABI50_0_0EXVideoPlayerViewController *controller = [[ABI50_0_0EXVideoPlayerViewController alloc] init];
  ...
  [controller addObserver:self forKeyPath:ABI50_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
  return controller;
}
...
- (void)_usePlayerLayer
{
  if (_data) {
    ...
    [_playerLayer addObserver:self forKeyPath:ABI50_0_0EXVideoReadyForDisplayKeyPath options:NSKeyValueObservingOptionNew context:nil];
...
- (void)_removePlayerLayer
{
  if (_playerLayer) {
    ...
    [_playerLayer removeObserver:self forKeyPath:ABI50_0_0EXVideoReadyForDisplayKeyPath];
    _playerLayer = nil;
  }
}
...
- (void)_removeFullscreenPlayerViewController
{
  if (_fullscreenPlayerViewController) {
    [_fullscreenPlayerViewController removeObserver:self forKeyPath:ABI50_0_0EXVideoReadyForDisplayKeyPath];
    _fullscreenPlayerViewController = nil;
  }
}
...
- (void)_removePlayerViewController
{
  if (_playerViewController) {
    ...
    [_playerViewController removeObserver:self forKeyPath:ABI50_0_0EXVideoReadyForDisplayKeyPath];
    if (@available(iOS 12, *)) {
      // ABI50_0_0EXVideoBounds monitoring is only used as a fallback on iOS 11 or lower
    } else {
      [_playerViewController removeObserver:self forKeyPath:ABI50_0_0EXVideoBoundsKeyPath];
    }
    _playerViewController = nil;
  }
}
...
{
  if ((object == _playerLayer || object == _playerViewController || object == _fullscreenPlayerViewController) && [keyPath isEqualToString:ABI50_0_0EXVideoReadyForDisplayKeyPath]) {
  ...
  } else if (object == _playerViewController && [keyPath isEqualToString:ABI50_0_0EXVideoBoundsKeyPath]) {
    ...
    if (viewBounds.size.height != screen.size.height && viewBounds.size.width != screen.size.width && _fullscreenPlayerPresented && !_fullscreenPlayerViewController) {
      ...
      _fullscreenPlayerPresented = NO;
      [self _callFullscreenCallbackForUpdate:ABI50_0_0EXVideoFullscreenUpdatePlayerWillDismiss];
      [self _callFullscreenCallbackForUpdate:ABI50_0_0EXVideoFullscreenUpdatePlayerDidDismiss];
      ...
      [self _callFullscreenCallbackForUpdate:ABI50_0_0EXVideoFullscreenUpdatePlayerWillPresent];
      [self _callFullscreenCallbackForUpdate:ABI50_0_0EXVideoFullscreenUpdatePlayerDidPresent];
    } else {
      return;
    }
  } else {
         ...
         rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
  ...
  _data = [[ABI50_0_0EXAVPlayerData alloc] initWithEXAV:_exAV
                                    withSource:source
...
{
  return @{
    ABI50_0_0EXVideoSourceURIKeyPath: (_data != nil && _data.url != nil) ? _data.url.absoluteString : @"",
    ABI50_0_0EXVideoSourceHeadersKeyPath: _data.headers
  };
}
    ...
    if (self.useNativeControls) {
      ...
      if (!self.playerViewController && self.data) {
        ...
        if (@available(iOS 12, *)) {
          ...
          [self.playerViewController addObserver:self forKeyPath:ABI50_0_0EXVideoBoundsKeyPath options:NSKeyValueObservingOptionNew context:nil];
        }
        // Resize mode must be set before layer is added
        // to prevent video from being animated when `resizeMode` is `cover`
        [self _updateNativeResizeMode];
...
#pragma mark - ABI50_0_0React View Management
...
//  } else {
//    ABI50_0_0RCTLogError(@"video cannot have any subviews");
//  }
//}
...
#pragma mark - ABI50_0_0EXVideoPlayerViewControllerDelegate
...
{
  return _data == nil ? ABI50_0_0EXAVAudioSessionModeInactive : [_data getAudioSessionModeRequired];
}
