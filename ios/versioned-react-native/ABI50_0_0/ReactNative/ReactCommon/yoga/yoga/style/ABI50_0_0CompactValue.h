...
#include <ABI50_0_0yoga/bits/ABI50_0_0BitCast.h>
#include <ABI50_0_0yoga/numeric/ABI50_0_0Comparison.h>

static_assert(
    std::numeric_limits<float>::is_iec559,
    "ABI50_0_0facebook::yoga::detail::CompactValue only works with IEEE754 floats");
...
// This class stores ABI50_0_0YGValue in 32 bits.
...
class ABI50_0_0YG_EXPORT CompactValue {
 ...
 public:
  ...
  template <ABI50_0_0YGUnit Unit>
  static CompactValue of(float value) noexcept {
    if (value == 0.0f || (value < LOWER_BOUND && value > -LOWER_BOUND)) {
      constexpr auto zero =
          Unit == ABI50_0_0YGUnitPercent ? ZERO_BITS_PERCENT : ZERO_BITS_POINT;
      return {zero};
    }

    constexpr auto upperBound =
        Unit == ABI50_0_0YGUnitPercent ? UPPER_BOUND_PERCENT : UPPER_BOUND_POINT;
    if (value > upperBound || value < -upperBound) {
      value = copysignf(upperBound, value);
    }

    uint32_t unitBit = Unit == ABI50_0_0YGUnitPercent ? PERCENT_BIT : 0;
  ...
  CompactValue(const ABI50_0_0YGValue& x) noexcept : repr_(uint32_t{0}) {
    switch (x.unit) {
      ...
      case ABI50_0_0YGUnitPoint:
        *this = of<ABI50_0_0YGUnitPoint>(x.value);
        break;
      case ABI50_0_0YGUnitPercent:
        *this = of<ABI50_0_0YGUnitPercent>(x.value);
        break;
    }
  }

  operator ABI50_0_0YGValue() const noexcept {
    switch (repr_) {
      case AUTO_BITS:
        return ABI50_0_0YGValueAuto;
      case ZERO_BITS_POINT:
        return ABI50_0_0YGValue{0.0f, ABI50_0_0YGUnitPoint};
      case ZERO_BITS_PERCENT:
        return ABI50_0_0YGValue{0.0f, ABI50_0_0YGUnitPercent};
    }
    ...
    return ABI50_0_0YGValue{
        yoga::bit_cast<float>(data),
        repr_ & 0x40000000 ? ABI50_0_0YGUnitPercent : ABI50_0_0YGUnitPoint};
  }
...
template <>
CompactValue CompactValue::of<ABI50_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::of<ABI50_0_0YGUnitAuto>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI50_0_0YGUnitUndefined>(float) noexcept = delete;
template <>
CompactValue CompactValue::ofMaybe<ABI50_0_0YGUnitAuto>(float) noexcept = delete;
...
inline bool inexactEquals(CompactValue a, CompactValue b) {
  return inexactEquals((ABI50_0_0YGValue)a, (ABI50_0_0YGValue)b);
}
