...
@implementation ABI50_0_0RCTConvert (Transform)
...
+ (CATransform3D)CATransform3DFromMatrix:(id)json
{
  ...
  if (![json isKindOfClass:[NSArray class]]) {
    ABI50_0_0RCTLogConvertError(json, @"a CATransform3D. Expected array for transform matrix.");
    return transform;
  }
  if ([json count] != kMatrixArrayLength) {
    ABI50_0_0RCTLogConvertError(json, @"a CATransform3D. Expected 4x4 matrix array.");
    return transform;
  }
  for (NSUInteger i = 0; i < kMatrixArrayLength; i++) {
    ((CGFloat *)&transform)[i] = [ABI50_0_0RCTConvert CGFloat:json[i]];
  }
  return transform;
}
...
+ (CATransform3D)CATransform3D:(id)json
{
  ...
  if (![json isKindOfClass:[NSArray class]]) {
    ABI50_0_0RCTLogConvertError(json, @"a CATransform3D. Did you pass something other than an array?");
    return transform;
  }
  // legacy matrix support
  if ([(NSArray *)json count] == kMatrixArrayLength && [json[0] isKindOfClass:[NSNumber class]]) {
    ABI50_0_0RCTLogWarn(
        @"[ABI50_0_0RCTConvert CATransform3D:] has deprecated a matrix as input. Pass an array of configs (which can contain a matrix key) instead.");
  ...
  for (NSDictionary *transformConfig in (NSArray<NSDictionary *> *)json) {
    if (transformConfig.count != 1) {
      ABI50_0_0RCTLogConvertError(json, @"a CATransform3D. You must specify exactly one property per transform object.");
      return transform;
    }
    ...
    if ([property isEqualToString:@"matrix"]) {
      ...
      ABI50_0_0RCTLogInfo(@"Unsupported transform type for a CATransform3D: %@.", property);
    }
  }
  return transform;
}
