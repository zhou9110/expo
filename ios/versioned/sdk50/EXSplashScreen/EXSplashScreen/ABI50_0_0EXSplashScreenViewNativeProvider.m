...
@implementation ABI50_0_0EXSplashScreenViewNativeProvider
...
- (nonnull UIView *)createSplashScreenView
{
  ...
  @try {
    ...
    ABI50_0_0EXLogWarn([NSString stringWithFormat:@"'%@.storyboard' file is missing. Fallbacking to '%@.xib' file.", splashScreenFilename, splashScreenFilename]);
  }
  if (storyboard) {
    @try {
  ...
  @try {
    ...
    ABI50_0_0EXLogWarn([NSString stringWithFormat:@"'%@.xib' file is missing - 'expo-splash-screen' will not work as expected.", splashScreenFilename]);
  }
  if (views) {
