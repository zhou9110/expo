...
@interface ABI50_0_0EXTaskService ()

// Array of task requests that are being executed.
@property (nonatomic, strong) NSMutableArray<ABI50_0_0EXTaskExecutionRequest *> *requests;

// Table of registered tasks. Schema: { "<appId>": { "<taskName>": ABI50_0_0EXTask } }
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSMutableDictionary<NSString *, ABI50_0_0EXTask *> *> *tasks;

// Dictionary with app records of running background apps. Schema: { "<appId>": ABI50_0_0EXAppRecordInterface }
@property (nonatomic, strong) NSMutableDictionary<NSString *, id<ABI50_0_0UMAppRecordInterface>> *appRecords;

// MapTable with task managers of running (foregrounded) apps. Schema: { "<appId>": ABI50_0_0EXTaskManagerInterface }
@property (nonatomic, strong) NSMapTable<NSString *, id<ABI50_0_0EXTaskManagerInterface>> *taskManagers;

// Same as above but for headless (backgrounded) apps.
@property (nonatomic, strong) NSMapTable<NSString *, id<ABI50_0_0EXTaskManagerInterface>> *headlessTaskManagers;
...
@implementation ABI50_0_0EXTaskService

ABI50_0_0EX_REGISTER_SINGLETON_MODULE(TaskService)
...
# pragma mark - ABI50_0_0EXTaskServiceInterface
...
{
  id<ABI50_0_0EXTaskInterface> task = [self _getTaskWithName:taskName forAppId:appId];
  return task != nil;
}

/**
 *  Creates a new task, registers it and saves to the config stored in user defaults.
 *  It can throw an exception if given consumer class doesn't conform to ABI50_0_0EXTaskConsumerInterface protocol
                     ...
                     options:(NSDictionary *)options
{
  ...
  // Given consumer class doesn't conform to ABI50_0_0EXTaskConsumerInterface protocol
  if (![unversionedConsumerClass conformsToProtocol:@protocol(ABI50_0_0EXTaskConsumerInterface)]) {
    NSString *reason = @"Invalid `consumer` argument. It must be a class that conforms to ABI50_0_0EXTaskConsumerInterface protocol.";
...
- (void)unregisterTaskWithName:(NSString *)taskName
                 ...
                 consumerClass:(Class)consumerClass
{
  ABI50_0_0EXTask *task = (ABI50_0_0EXTask *)[self _getTaskWithName:taskName forAppId:appId];
...
- (void)unregisterAllTasksForAppId:(NSString *)appId
{
  ...
  if (appTasks) {
    ...
    for (ABI50_0_0EXTask *task in [appTasks allValues]) {
      if ([task.consumer respondsToSelector:@selector(didUnregister)]) {
        [task.consumer didUnregister];
      }
    }
...
{
  NSDictionary<NSString *, id<ABI50_0_0EXTaskInterface>> *tasks = [self _getTasksForAppId:appId];
  NSMutableArray *results = [NSMutableArray new];
  
  for (NSString *taskName in tasks) {
    id<ABI50_0_0EXTaskInterface> task = tasks[taskName];
     ...
     didFinishWithResponse:(NSDictionary *)response
{
  ...
  for (ABI50_0_0EXTaskExecutionRequest *request in [_requests copy]) {
...
- (void)setTaskManager:(id<ABI50_0_0EXTaskManagerInterface>)taskManager
              forAppId:(NSString *)appId
...
# pragma mark - ABI50_0_0EXTaskDelegate

- (void)executeTask:(nonnull id<ABI50_0_0EXTaskInterface>)task
           withData:(nullable NSDictionary *)data
          withError:(nullable NSError *)error
{
  id<ABI50_0_0EXTaskManagerInterface> taskManager = [self _taskManagerForAppId:task.appId];
  NSDictionary *executionInfo = [self _executionInfoForTask:task];
  NSDictionary *body = @{
    @"executionInfo": executionInfo,
    @"data": data ?: @{},
    @"error": ABI50_0_0EXNullIfNil([self _exportError:error]),
  };
  
  NSLog(@"ABI50_0_0EXTaskService: Executing task '%@' for app '%@'.", task.name, task.appId);
...
- (void)applicationDidFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
  ...
  ABI50_0_0EXTaskLaunchReason launchReason = [self _launchReasonForLaunchOptions:launchOptions];
 ...
 */
- (id<ABI50_0_0EXTaskInterface>)_getTaskWithName:(NSString *)taskName
                               forAppId:(NSString *)appId
{
 ...
 */
- (NSDictionary<NSString *, ABI50_0_0EXTask *> *)_getTasksForAppId:(NSString *)appId
{
  return _tasks[appId];
}
 ...
 */
- (ABI50_0_0EXTask *)_internalRegisterTaskWithName:(nonnull NSString *)taskName
                                  ...
                                  options:(nullable NSDictionary *)options
{
  ...
  ABI50_0_0EXTask *task = [[ABI50_0_0EXTask alloc] initWithName:taskName
                                        appId:appId
                                       appUrl:appUrl
 ...
 */
- (void)_addTaskToConfig:(nonnull id<ABI50_0_0EXTaskInterface>)task
{
...
- (void)_iterateTasksUsingBlock:(void(^)(id<ABI50_0_0EXTaskInterface> task))block
{
 ...
 *        "<taskName>": {
 *          "name": "task's name",
 *          "consumerClass": "name of consumer class, e.g. ABI50_0_0EXLocationTaskConsumer",
 *          "consumerVersion": 1,
 *          "options": {},
 *        },
 *      }
 ...
 */
- (nullable NSDictionary *)_dictionaryFromTask:(id<ABI50_0_0EXTaskInterface>)task
{
  return @{
    @"name": task.name,
    ...
    @"options": ABI50_0_0EXNullIfNil([task options]),
  };
}

- (void)_runTasksSupportingLaunchReason:(ABI50_0_0EXTaskLaunchReason)launchReason
                               userInfo:(nullable NSDictionary *)userInfo
                               callback:(void(^)(NSArray * _Nonnull results))callback
{
  __block ABI50_0_0EXTaskExecutionRequest *request;
  
  request = [[ABI50_0_0EXTaskExecutionRequest alloc] initWithCallback:^(NSArray * _Nonnull results) {
    if (callback != nil) {
      callback(results);
    }
  ...
  [self _iterateTasksUsingBlock:^(id<ABI50_0_0EXTaskInterface> task) {
...
- (void)_loadAppWithId:(nonnull NSString *)appId
                appUrl:(nonnull NSString *)appUrl
{
  id<ABI50_0_0UMAppLoaderInterface> appLoader = [[ABI50_0_0UMAppLoaderProvider sharedInstance] createAppLoader:@"react-native-experience"];
  
  if (appLoader != nil && appUrl != nil) {
    __block id<ABI50_0_0UMAppRecordInterface> appRecord;
    
    NSLog(@"ABI50_0_0EXTaskService: Loading headless app '%@' with url '%@'.", appId, appUrl);
    
    appRecord = [appLoader loadAppWithUrl:appUrl options:nil callback:^(BOOL success, NSError *error) {
      if (!success) {
        NSLog(@"ABI50_0_0EXTaskService: Loading app '%@' from url '%@' failed. Error description: %@", appId, appUrl, error.description);
        [self->_events removeObjectForKey:appId];
 ...
 */
- (id<ABI50_0_0EXTaskManagerInterface>)_taskManagerForAppId:(NSString *)appId
{
  id<ABI50_0_0EXTaskManagerInterface> taskManager = [_taskManagers objectForKey:appId];
...
- (void)_restoreTasks
{
  ...
  if (config) {
    ...
    NSLog(@"ABI50_0_0EXTaskService: Restoring tasks configuration: %@", config.description);
      ...
      for (NSString *taskName in tasksConfig) {
        ...
        if (consumerClass != nil) {
          ...
          if (currentConsumerVersion == previousConsumerVersion) {
            ...
            ABI50_0_0EXLogWarn(
                      @"ABI50_0_0EXTaskService: Task consumer '%@' has version '%d' that is not compatible with the saved version '%d'.",
                      consumerClassName,
          ...
          ABI50_0_0EXLogWarn(@"ABI50_0_0EXTaskService: Cannot restore task '%@' because consumer class doesn't exist.", taskName);
          [self _removeTaskFromConfig:taskName appId:appId];
        }
      }
    }
...
- (void)_addTask:(id<ABI50_0_0EXTaskInterface>)task toRequest:(ABI50_0_0EXTaskExecutionRequest *)request withInfo:(nullable NSDictionary *)info
{
...
- (NSDictionary *)_executionInfoForTask:(nonnull id<ABI50_0_0EXTaskInterface>)task
{
...
{
  id<ABI50_0_0UMAppRecordInterface> appRecord = _appRecords[appId];
...
- (ABI50_0_0EXTaskLaunchReason)_launchReasonForLaunchOptions:(nullable NSDictionary *)launchOptions
{
  if (launchOptions == nil) {
    return ABI50_0_0EXTaskLaunchReasonUser;
  }
  if (launchOptions[UIApplicationLaunchOptionsBluetoothCentralsKey]) {
    return ABI50_0_0EXTaskLaunchReasonBluetoothCentrals;
  }
  if (launchOptions[UIApplicationLaunchOptionsBluetoothPeripheralsKey]) {
    return ABI50_0_0EXTaskLaunchReasonBluetoothPeripherals;
  }
  if (launchOptions[UIApplicationLaunchOptionsLocationKey]) {
    return ABI50_0_0EXTaskLaunchReasonLocation;
  }
  if (launchOptions[UIApplicationLaunchOptionsNewsstandDownloadsKey]) {
    return ABI50_0_0EXTaskLaunchReasonNewsstandDownloads;
  }
  if (launchOptions[UIApplicationLaunchOptionsRemoteNotificationKey]) {
    return ABI50_0_0EXTaskLaunchReasonRemoteNotification;
  }
  return ABI50_0_0EXTaskLaunchReasonUnrecognized;
}
