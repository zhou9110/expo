...
#import "ABI50_0_0RCTScrollViewComponentView.h"
...
#import <ABI50_0_0React/ABI50_0_0RCTScrollEvent.h>

#import <ABI50_0_0React/renderer/components/scrollview/ABI50_0_0RCTComponentViewHelpers.h>
#import <ABI50_0_0React/renderer/components/scrollview/ABI50_0_0ScrollViewComponentDescriptor.h>
#import <ABI50_0_0React/renderer/components/scrollview/ABI50_0_0ScrollViewEventEmitter.h>
#import <ABI50_0_0React/renderer/components/scrollview/ABI50_0_0ScrollViewProps.h>
#import <ABI50_0_0React/renderer/components/scrollview/ABI50_0_0ScrollViewState.h>
#import <ABI50_0_0React/renderer/components/scrollview/ABI50_0_0conversions.h>

#import "ABI50_0_0RCTConversions.h"
#import "ABI50_0_0RCTCustomPullToRefreshViewProtocol.h"
#import "ABI50_0_0RCTEnhancedScrollView.h"
...
static UIScrollViewKeyboardDismissMode ABI50_0_0RCTUIKeyboardDismissModeFromProps(const ScrollViewProps &props)
{
...
static UIScrollViewIndicatorStyle ABI50_0_0RCTUIScrollViewIndicatorStyleFromProps(const ScrollViewProps &props)
{
...
// TODO: T116850910 [Fabric][iOS] Make Fabric not use legacy ABI50_0_0RCTEventDispatcher for native-driven AnimatedEvents
static void ABI50_0_0RCTSendScrollEventForNativeAnimations_DEPRECATED(UIScrollView *scrollView, NSInteger tag)
{
  static uint16_t coalescingKey = 0;
  ABI50_0_0RCTScrollEvent *scrollEvent = [[ABI50_0_0RCTScrollEvent alloc] initWithEventName:@"onScroll"
                                                                 ABI50_0_0ReactTag:[NSNumber numberWithInt:tag]
  ...
  if (bridge) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ABI50_0_0RCTNotifyEventDispatcherObserversOfEvent_DEPRECATED"
                                                        object:nil
                                                      userInfo:userInfo];
  }
}

@interface ABI50_0_0RCTScrollViewComponentView () <
    UIScrollViewDelegate,
    ABI50_0_0RCTScrollViewProtocol,
    ABI50_0_0RCTScrollableProtocol,
    ABI50_0_0RCTEnhancedScrollViewOverridingDelegate>

@end

@implementation ABI50_0_0RCTScrollViewComponentView {
...
+ (ABI50_0_0RCTScrollViewComponentView *_Nullable)findScrollViewComponentViewForView:(UIView *)view
{
  do {
    view = view.superview;
  } while (view != nil && ![view isKindOfClass:[ABI50_0_0RCTScrollViewComponentView class]]);
  return (ABI50_0_0RCTScrollViewComponentView *)view;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _scrollView = [[ABI50_0_0RCTEnhancedScrollView alloc] initWithFrame:self.bounds];
    _scrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    _scrollView.delaysContentTouches = NO;
    ((ABI50_0_0RCTEnhancedScrollView *)_scrollView).overridingDelegate = self;
...
- (ABI50_0_0RCTGenericDelegateSplitter<id<UIScrollViewDelegate>> *)scrollViewDelegateSplitter
{
  return ((ABI50_0_0RCTEnhancedScrollView *)_scrollView).delegateSplitter;
}

#pragma mark - ABI50_0_0RCTMountingTransactionObserving

- (void)mountingTransactionWillMount:(const ABI50_0_0facebook::ABI50_0_0React::MountingTransaction &)transaction
                withSurfaceTelemetry:(const ABI50_0_0facebook::ABI50_0_0React::SurfaceTelemetry &)surfaceTelemetry
{
...
#define REMAP_PROP(ABI50_0_0ReactName, localName, target)                      \
  if (oldScrollViewProps.ABI50_0_0ReactName != newScrollViewProps.ABI50_0_0ReactName) { \
    target.localName = newScrollViewProps.ABI50_0_0ReactName;                  \
  }

#define REMAP_VIEW_PROP(ABI50_0_0ReactName, localName) REMAP_PROP(ABI50_0_0ReactName, localName, self)
#define MAP_VIEW_PROP(name) REMAP_VIEW_PROP(name, name)
#define REMAP_SCROLL_VIEW_PROP(ABI50_0_0ReactName, localName) \
  REMAP_PROP(ABI50_0_0ReactName, localName, ((ABI50_0_0RCTEnhancedScrollView *)_scrollView))
#define MAP_SCROLL_VIEW_PROP(name) REMAP_SCROLL_VIEW_PROP(name, name)
  ...
  if (oldScrollViewProps.scrollIndicatorInsets != newScrollViewProps.scrollIndicatorInsets) {
    _scrollView.scrollIndicatorInsets = ABI50_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.scrollIndicatorInsets);
  }

  if (oldScrollViewProps.indicatorStyle != newScrollViewProps.indicatorStyle) {
    _scrollView.indicatorStyle = ABI50_0_0RCTUIScrollViewIndicatorStyleFromProps(newScrollViewProps);
  }
  ...
  if (oldScrollViewProps.contentInset != newScrollViewProps.contentInset) {
    _scrollView.contentInset = ABI50_0_0RCTUIEdgeInsetsFromEdgeInsets(newScrollViewProps.contentInset);
  }

  ABI50_0_0RCTEnhancedScrollView *scrollView = (ABI50_0_0RCTEnhancedScrollView *)_scrollView;
  if (oldScrollViewProps.contentOffset != newScrollViewProps.contentOffset) {
    _scrollView.contentOffset = ABI50_0_0RCTCGPointFromPoint(newScrollViewProps.contentOffset);
  }

  if (oldScrollViewProps.snapToAlignment != newScrollViewProps.snapToAlignment) {
    scrollView.snapToAlignment = ABI50_0_0RCTNSStringFromString(toString(newScrollViewProps.snapToAlignment));
  }
  ...
  if (oldScrollViewProps.keyboardDismissMode != newScrollViewProps.keyboardDismissMode) {
    scrollView.keyboardDismissMode = ABI50_0_0RCTUIKeyboardDismissModeFromProps(newScrollViewProps);
  }
...
- (void)updateState:(const State::Shared &)state oldState:(const State::Shared &)oldState
{
  ...
  auto contentOffset = ABI50_0_0RCTCGPointFromPoint(data.contentOffset);
  if (!oldState && !CGPointEqualToPoint(contentOffset, CGPointZero)) {
    _scrollView.contentOffset = contentOffset;
  }

  CGSize contentSize = ABI50_0_0RCTCGSizeFromSize(data.getContentSize());
  ...
  _containerView.frame = CGRect{ABI50_0_0RCTCGPointFromPoint(data.contentBoundingRect.origin), contentSize};
...
- (void)_preserveContentOffsetIfNeededWithBlock:(void (^)())block
{
  ...
  [((ABI50_0_0RCTEnhancedScrollView *)_scrollView) preserveContentOffsetWithBlock:block];
}
...
- (void)mountChildComponentView:(UIView<ABI50_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  ...
  if (![childComponentView conformsToProtocol:@protocol(ABI50_0_0RCTCustomPullToRefreshViewProtocol)]) {
...
- (BOOL)_shouldDisableScrollInteraction
{
  ...
  while (ancestorView) {
    if ([ancestorView respondsToSelector:@selector(isJSResponder)]) {
      BOOL isJSResponder = ((UIView<ABI50_0_0RCTComponentViewProtocol> *)ancestorView).isJSResponder;
      if (isJSResponder) {
        return YES;
      }
    }
...
- (ScrollViewMetrics)_scrollViewMetrics
{
  ...
  metrics.contentSize = ABI50_0_0RCTSizeFromCGSize(_scrollView.contentSize);
  metrics.contentOffset = ABI50_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
  metrics.contentInset = ABI50_0_0RCTEdgeInsetsFromUIEdgeInsets(_scrollView.contentInset);
  metrics.containerSize = ABI50_0_0RCTSizeFromCGSize(_scrollView.bounds.size);
...
- (void)_updateStateWithContentOffset
{
  ...
  auto contentOffset = ABI50_0_0RCTPointFromCGPoint(_scrollView.contentOffset);
...
- (void)prepareForRecycle
{
  ...
  _scrollView.contentOffset = ABI50_0_0RCTCGPointFromPoint(props.contentOffset);
...
{
  // Historically, `UIScrollView`s in ABI50_0_0React Native do not cancel touches
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  if ((_lastScrollEventDispatchTime == 0) || (now - _lastScrollEventDispatchTime > _scrollEventThrottle)) {
    ...
    ABI50_0_0RCTSendScrollEventForNativeAnimations_DEPRECATED(scrollView, self.tag);
  }
...
{
  ABI50_0_0RCTScrollViewHandleCommand(self, commandName, args);
}
...
#pragma mark - ABI50_0_0RCTScrollableProtocol
...
Class<ABI50_0_0RCTComponentViewProtocol> ABI50_0_0RCTScrollViewCls(void)
{
  return ABI50_0_0RCTScrollViewComponentView.class;
}
