...
NSRegularExpression *ABI50_0_0RNSVGDigitRegEx;

@implementation ABI50_0_0RCTConvert (ABI50_0_0RNSVG)

ABI50_0_0RCT_ENUM_CONVERTER(
    ABI50_0_0RNSVGCGFCRule,
    (@{
...
+ (ABI50_0_0RNSVGBrush *)ABI50_0_0RNSVGBrush:(id)json
{
  if ([json isKindOfClass:[NSNumber class]]) {
    return [[ABI50_0_0RNSVGSolidColorBrush alloc] initWithNumber:json];
  }
  if ([json isKindOfClass:[NSString class]]) {
    NSString *value = [self NSString:json];
    if (!ABI50_0_0RNSVGDigitRegEx) {
      ABI50_0_0RNSVGDigitRegEx = [NSRegularExpression regularExpressionWithPattern:@"[0-9.-]+"
                                                                  options:NSRegularExpressionCaseInsensitive
                                                                    error:nil];
    }
    NSArray<NSTextCheckingResult *> *_matches = [ABI50_0_0RNSVGDigitRegEx matchesInString:value
                                                                         options:0
    ...
    return [[ABI50_0_0RNSVGSolidColorBrush alloc] initWithArray:output];
  }
  ...
  switch (type) {
            ...
            // We should memoize colors but look ups may be just as expensive.
    {
      ...
      return [[ABI50_0_0RNSVGSolidColorBrush alloc] initWithArray:arr];
    }
    case 1: // brush
    {
    ...
    default:
      ABI50_0_0RCTLogError(@"Unknown brush type: %zd", (unsigned long)type);
      return nil;
  }
}
...
{
  return [[ABI50_0_0RNSVGPathParser alloc] initWithPathString:d];
}

+ (ABI50_0_0RNSVGLength *)ABI50_0_0RNSVGLength:(id)json
{
  if ([json isKindOfClass:[NSNumber class]]) {
    return [ABI50_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
  } else if ([json isKindOfClass:[NSString class]]) {
    NSString *stringValue = (NSString *)json;
    return [ABI50_0_0RNSVGLength lengthWithString:stringValue];
  } else {
    return [[ABI50_0_0RNSVGLength alloc] init];
  }
}
...
{
  if ([json isKindOfClass:[NSNumber class]]) {
    ABI50_0_0RNSVGLength *length = [ABI50_0_0RNSVGLength lengthWithNumber:(CGFloat)[json doubleValue]];
    return [NSArray arrayWithObject:length];
  } else if ([json isKindOfClass:[NSArray class]]) {
    NSArray *arrayValue = (NSArray *)json;
    NSMutableArray<ABI50_0_0RNSVGLength *> *lengths = [NSMutableArray arrayWithCapacity:[arrayValue count]];
    for (id obj in arrayValue) {
      [lengths addObject:[self ABI50_0_0RNSVGLength:obj]];
    }
    return lengths;
  } else if ([json isKindOfClass:[NSString class]]) {
    NSString *stringValue = (NSString *)json;
    ...
    NSMutableArray<ABI50_0_0RNSVGLength *> *svgLengthArray = [NSMutableArray array];

    for (NSString *string in array) {
      ABI50_0_0RNSVGLength *length = [ABI50_0_0RNSVGLength lengthWithString:string];
...
+ (CGRect)ABI50_0_0RNSVGCGRect:(id)json offset:(NSUInteger)offset
{
  ...
  if (arr.count < offset + 4) {
    ABI50_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", (ssize_t)(4 + offset), arr);
    return CGRectZero;
  }
  return (CGRect){
...
+ (ABI50_0_0RNSVGColor *)ABI50_0_0RNSVGColor:(id)json offset:(NSUInteger)offset
{
  ...
  if (arr.count == offset + 1) {
    return [self ABI50_0_0RNSVGColor:[arr objectAtIndex:offset]];
  }
  if (arr.count < offset + 4) {
    ABI50_0_0RCTLogError(@"Too few elements in array (expected at least %zd): %@", (ssize_t)(4 + offset), arr);
    return nil;
  }
  return [self ABI50_0_0RNSVGColor:[arr subarrayWithRange:(NSRange){offset, 4}]];
}
