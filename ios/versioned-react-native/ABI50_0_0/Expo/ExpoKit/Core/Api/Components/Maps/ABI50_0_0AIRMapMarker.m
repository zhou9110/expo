...
NSInteger const ABI50_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE = 999;

@implementation ABI50_0_0AIREmptyCalloutBackgroundView
@end

@implementation ABI50_0_0AIRMapMarker {
...
- (void)ABI50_0_0ReactSetFrame:(CGRect)frame
{
    ...
    if (isnan(center.x) || isnan(center.y) ||
        ...
        ABI50_0_0RCTLogError(@"Invalid layout for (%@)%@. position: %@. bounds: %@",
                self.ABI50_0_0ReactTag, self, NSStringFromCGPoint(center), NSStringFromCGRect(bounds));
        return;
    }
...
- (void)insertABI50_0_0ReactSubview:(id<ABI50_0_0RCTComponent>)subview atIndex:(NSInteger)atIndex {
    if ([subview isKindOfClass:[ABI50_0_0AIRMapCallout class]]) {
        self.calloutView = (ABI50_0_0AIRMapCallout *)subview;
    } else {
        [super insertABI50_0_0ReactSubview:(UIView *)subview atIndex:atIndex];
    }
}

- (void)removeABI50_0_0ReactSubview:(id<ABI50_0_0RCTComponent>)subview {
    if ([subview isKindOfClass:[ABI50_0_0AIRMapCallout class]] && self.calloutView == subview) {
        self.calloutView = nil;
    } else {
        [super removeABI50_0_0ReactSubview:(UIView *)subview];
    }
}
...
- (MKAnnotationView *)getAnnotationView
{
    if ([self shouldUsePinView]) {
        ...
        // If it has subviews, it means we are wanting to render a custom marker with arbitrary ABI50_0_0React views.
        // if it has a non-null image, it means we want to render a custom marker with the image.
        // In either case, we want to return the ABI50_0_0AIRMapMarker since it is both an MKAnnotation and an
...
- (void)fillCalloutView:(ABI50_0_0SMCalloutView *)calloutView
{
    ...
    if (self.calloutView) {
        ...
        if (self.calloutView.tooltip) {
            // if tooltip is true, then the user wants their ABI50_0_0React view to be the "tooltip" as wwell, so we set
            // the background view to something empty/transparent
            calloutView.backgroundView = [ABI50_0_0AIREmptyCalloutBackgroundView new];
        } else {
            // the default tooltip look is wanted, and the user is just filling the content with their ABI50_0_0React subviews.
            // as a result, we use the default "masked" background view.
            calloutView.backgroundView = [ABI50_0_0SMCalloutMaskedBackgroundView new];
        }

        // when this is set, the callout's content will be whatever ABI50_0_0React views the user has put as the callout's
        // children.
...
- (void)_handleTap:(UITapGestureRecognizer *)recognizer {
    ABI50_0_0AIRMapMarker *marker = self;
    if (!marker) return;
...
{
    return self.ABI50_0_0ReactSubviews.count == 0 && !self.imageSrc;
}
...
- (void)setZIndex:(NSInteger)zIndex
{
    ...
    _zIndex = _calloutIsOpen ? zIndex + ABI50_0_0AIR_CALLOUT_OPEN_ZINDEX_BASELINE : zIndex;
