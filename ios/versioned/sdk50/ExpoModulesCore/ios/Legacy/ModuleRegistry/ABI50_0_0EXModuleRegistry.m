...
@interface ABI50_0_0EXModuleRegistry ()

@property (nonatomic, weak) id<ABI50_0_0EXModuleRegistryDelegate> delegate;

@property NSMutableSet<id<ABI50_0_0EXInternalModule>> *internalModulesSet;
@property NSMapTable<Protocol *, id<ABI50_0_0EXInternalModule>> *internalModules;
@property NSMapTable<Protocol *, NSMutableArray<id<ABI50_0_0EXInternalModule>> *> *internalModulesPreResolution;
@property NSMapTable<Class, ABI50_0_0EXExportedModule *> *exportedModulesByClass;
@property NSMutableDictionary<const NSString *, ABI50_0_0EXExportedModule *> *exportedModules;
...
@implementation ABI50_0_0EXModuleRegistry
...
{
  if (self = [self init]) {
    for (id<ABI50_0_0EXInternalModule> internalModule in internalModules) {
      [self registerInternalModule:internalModule];
    }
    
    for (ABI50_0_0EXExportedModule *exportedModule in exportedModules) {
...
- (void)resolveInternalModulesConflicts
{
  ...
  for (Protocol *protocol in _internalModulesPreResolution) {
    NSArray<id<ABI50_0_0EXInternalModule>> *conflictingModules = [_internalModulesPreResolution objectForKey:protocol];

    id<ABI50_0_0EXInternalModule> resolvedModule;
...
- (void)initialize
{
  ...
  for (id<ABI50_0_0EXModuleRegistryConsumer> registryConsumer in _registryConsumers) {
...
- (void)registerInternalModule:(id<ABI50_0_0EXInternalModule>)internalModule
{
  for (Protocol *exportedInterface in [[internalModule class] exportedInterfaces]) {
    if (_internalModules) {
      id<ABI50_0_0EXInternalModule> resolvedModule = internalModule;
      if (_delegate && [_internalModules objectForKey:exportedInterface]) {
        id<ABI50_0_0EXInternalModule> oldModule = [_internalModules objectForKey:exportedInterface];
...
- (void)registerExportedModule:(ABI50_0_0EXExportedModule *)exportedModule
{
  ...
  if (_exportedModules[exportedModuleName]) {
    ABI50_0_0EXLogInfo(@"Module %@ overrides %@ as the module exported as %@.", exportedModule, _exportedModules[exportedModuleName], exportedModuleName);
  }
...
- (void)registerSingletonModule:(id)singletonModule
{
  if ([[singletonModule class] respondsToSelector:@selector(name)]) {
    ...
    ABI50_0_0EXLogWarn(@"One of the singleton modules does not respond to +(NSString *)name selector. This probably means you're either try to pass a strange object as a singleton module (it won't get registered in the module registry, sorry) or the ABI50_0_0EXSingletonModule interface and the ABI50_0_0EXModuleRegistry implementations versions are out of sync, which means things will probably not work as expected.");
  }
}

- (void)maybeAddRegistryConsumer:(id)maybeConsumer
{
  if ([maybeConsumer conformsToProtocol:@protocol(ABI50_0_0EXModuleRegistryConsumer)]) {
    [self addRegistryConsumer:(id<ABI50_0_0EXModuleRegistryConsumer>)maybeConsumer];
  }
}

- (void)addRegistryConsumer:(id<ABI50_0_0EXModuleRegistryConsumer>)registryConsumer
{
