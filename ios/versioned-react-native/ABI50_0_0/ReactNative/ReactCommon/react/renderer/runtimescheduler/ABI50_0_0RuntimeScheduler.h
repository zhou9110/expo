...
class RuntimeScheduler final {
 public:
  ...
  /*
   * Returns value of currently executed task. Designed to be called from ABI50_0_0React.
   *
   * Thread synchronization must be enforced externally.
   */
   ...
   * be called in the event pipeline after an event is dispatched to ABI50_0_0React.
   * ABI50_0_0React may schedule events with immediate priority which need to be handled
   * before the next event is sent to ABI50_0_0React.
   *
   * Thread synchronization must be enforced externally.
   */
