...
#import <ABI50_0_0React/ABI50_0_0RCTDevLoadingView.h>

#import <QuartzCore/QuartzCore.h>

#import <ABI50_0_0FBReactNativeSpec/ABI50_0_0FBReactNativeSpec.h>
#import <ABI50_0_0React/ABI50_0_0RCTAppearance.h>
...
#import <ABI50_0_0React/ABI50_0_0RCTDevLoadingViewSetEnabled.h>
...
@interface ABI50_0_0RCTDevLoadingView () <ABI50_0_0NativeDevLoadingViewSpec>
@end

#if ABI50_0_0RCT_DEV_MENU

@implementation ABI50_0_0RCTDevLoadingView {
  UIWindow *_window;
  UILabel *_label;
  NSDate *_showDate;
  BOOL _hiding;
...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(hide)
                                                 name:ABI50_0_0RCTJavaScriptDidFailToLoadNotification
                                               object:nil];
  }
  return self;
}
...
{
  ABI50_0_0RCTDevLoadingViewSetEnabled(enabled);
}
...
{
  if (!ABI50_0_0RCTDevLoadingViewGetEnabled() || _hiding) {
    return;
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!self->_window && !ABI50_0_0RCTRunningInTestEnvironment()) {
      CGSize screenSize = [UIScreen mainScreen].bounds.size;

      UIWindow *window = ABI50_0_0RCTSharedApplication().keyWindow;
      self->_window =
    ...
    UIWindowScene *scene = (UIWindowScene *)ABI50_0_0RCTSharedApplication().connectedScenes.anyObject;
...
ABI50_0_0RCT_EXPORT_METHOD(showMessage
                  : (NSString *)message withColor
                  : (NSNumber *__nonnull)color withBackgroundColor
                  : (NSNumber *__nonnull)backgroundColor)
{
  [self showMessage:message color:[ABI50_0_0RCTConvert UIColor:color] backgroundColor:[ABI50_0_0RCTConvert UIColor:backgroundColor]];
}

ABI50_0_0RCT_EXPORT_METHOD(hide)
{
  if (!ABI50_0_0RCTDevLoadingViewGetEnabled()) {
    return;
  }
...
- (void)showOfflineMessage
{
  ...
  NSString *message = [NSString stringWithFormat:@"Connect to %@ to develop JavaScript.", ABI50_0_0RCT_PACKAGER_NAME];
  [self showMessage:message color:color backgroundColor:backgroundColor];
}
...
- (BOOL)isDarkModeEnabled
{
  ...
  return [ABI50_0_0RCTColorSchemePreference(nil) isEqualToString:@"dark"];
}
- (void)showWithURL:(NSURL *)URL
{
  if (URL.fileURL) {
    ...
    [self showInitialMessageDelayed:^{
      NSString *message = [NSString stringWithFormat:@"Loading from %@\u2026", ABI50_0_0RCT_PACKAGER_NAME];
...
Class ABI50_0_0RCTDevLoadingViewCls(void)
{
  return ABI50_0_0RCTDevLoadingView.class;
}
