...
@implementation ABI50_0_0RNSVGPathParser {
  char prev_cmd;
  NSUInteger i;
  NSUInteger l;
  NSString *s;
...
- (CGPathRef)getPath
{
  ...
  while (i < l) {
    ...
    if (!has_prev_cmd && first_char != 'M' && first_char != 'm') {
      ...
      ABI50_0_0RCTLogError(@"UnexpectedData: %@", s);
      CGPathRelease(path);
      return nil;
    }
...
{
  // ABI50_0_0RCTLogInfo(@"move x: %f y: %f", x, y);
...
{
  // ABI50_0_0RCTLogInfo(@"line x: %f y: %f", x, y);
  [self setPenDown];
  _pivotX = _penX = x;
  _pivotY = _penY = y;
...
- (void)curveTo:(CGMutablePathRef)path
            ...
            c2y:(float)c2y
             ...
             ey:(float)ey
{
  // ABI50_0_0RCTLogInfo(@"curve c1x: %f c1y: %f c2x: %f c2y: %f ex: %f ey: %f", c1x, c1y, c2x, c2y, ex, ey);
  _pivotX = c2x;
  _pivotY = c2y;
...
{
  // ABI50_0_0RCTLogInfo(@"smoothcurve c1x: %f c1y: %f ex: %f ey: %f", c1x, c1y, ex, ey);
  float c2x = c1x;
  float c2y = c1y;
  c1x = (_penX * 2) - _pivotX;
  c1y = (_penY * 2) - _pivotY;
...
{
  // ABI50_0_0RCTLogInfo(@"quad c1x: %f c1y: %f c2x: %f c2y: %f", c1x, c1y, c2x, c2y);
  _pivotX = c1x;
  _pivotY = c1y;
  float ex = c2x;
  float ey = c2y;
...
{
  // ABI50_0_0RCTLogInfo(@"smoothquad c1x: %f c1y: %f", c1x, c1y);
  float c2x = c1x;
  float c2y = c1y;
  c1x = (_penX * 2) - _pivotX;
  c1y = (_penY * 2) - _pivotY;
...
- (void)arcTo:(CGMutablePathRef)path
    ...
    clockwise:(BOOL)clockwise
            ...
            y:(float)y
{
  // ABI50_0_0RCTLogInfo(@"arc rx: %f ry: %f rotation: %f outer: %i clockwise: %i x: %f y: %f", rx, ry, rotation, outer,
  // clockwise, x, y);
  float tX = _penX;
  float tY = _penY;
...
{
  if (i == l) {
    ABI50_0_0RCTLogError(@"UnexpectedEnd: %@", s);
  }
...
- (float)parse_number
{
  ...
  if (i == l) {
    ABI50_0_0RCTLogError(@"InvalidNumber: %@", s);
  }
