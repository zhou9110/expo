...
#import "ABI50_0_0RCTImageComponentView+ABI50_0_0RNSScreenStackHeaderConfig.h"
#else
...
#import "ABI50_0_0RNSUIBarButtonItem.h"
...
// of a given ABI50_0_0RCTImageView. See more comments in the code section processing SubviewTypeBackButton
@interface ABI50_0_0RCTImageView (Private)
- (UIImage *)image;
@end
#endif // !ABI50_0_0RCT_NEW_ARCH_ENABLED

@interface ABI50_0_0RCTImageLoader (Private)
- (id<ABI50_0_0RCTImageCache>)imageCache;
@end

@implementation NSString (ABI50_0_0RNSStringUtil)

+ (BOOL)ABI50_0_0RNSisBlank:(NSString *)string
{
  if (string == nil) {
    return YES;
  }
...
@implementation ABI50_0_0RNSScreenStackHeaderConfig {
  NSMutableArray<ABI50_0_0RNSScreenStackHeaderSubview *> *_ABI50_0_0ReactSubviews;
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigProps>();
    _props = defaultProps;
    _show = YES;
    _translucent = NO;
    [self initProps];
...
{
  for (ABI50_0_0RNSScreenStackHeaderSubview *subview in _ABI50_0_0ReactSubviews) {
    if (subview.type == ABI50_0_0RNSScreenStackHeaderSubviewTypeLeft || subview.type == ABI50_0_0RNSScreenStackHeaderSubviewTypeRight) {
      ...
      // we convert the point to ABI50_0_0RNSScreenStackView since it always contains the header inside it
      CGPoint convertedPoint = [_screenView.ABI50_0_0ReactSuperview convertPoint:point toView:headerComponent];
...
- (void)updateViewControllerIfNeeded
{
  ...
  BOOL isInFullScreenModal = nav == nil && _screenView.stackPresentation == ABI50_0_0RNSScreenStackPresentationFullScreenModal;
  ...
  if (vc != nil && (nextVC == vc || isInFullScreenModal || isPresentingVC)) {
    [ABI50_0_0RNSScreenStackHeaderConfig updateViewController:self.screenView.controller withConfig:self animated:YES];
...
- (void)layoutNavigationControllerView
{
  ...
  // frame of ABI50_0_0RNSScreen will not be changed and screen content will remain the same size.
...
+ (void)setAnimatedConfig:(UIViewController *)vc withConfig:(ABI50_0_0RNSScreenStackHeaderConfig *)config
{
...
#endif
  {
    ...
    if (config.titleFontFamily || config.titleFontSize || config.titleFontWeight || config.titleColor) {
      ...
      if (family || weight) {
        attrs[NSFontAttributeName] = [ABI50_0_0RCTFont updateFont:nil
                                              withFamily:family
                                                    size:size
                                                  weight:weight
                                                   style:nil
    ...
    if (@available(iOS 11.0, *)) {
      if (config.largeTitle &&
        ...
        if (largeFamily || largeWeight) {
          largeAttrs[NSFontAttributeName] = [ABI50_0_0RCTFont updateFont:nil
...
+ (UIImage *)loadBackButtonImageInViewController:(UIViewController *)vc withConfig:(ABI50_0_0RNSScreenStackHeaderConfig *)config
{
  BOOL hasBackButtonImage = NO;
  for (ABI50_0_0RNSScreenStackHeaderSubview *subview in config.ABI50_0_0ReactSubviews) {
    if (subview.type == ABI50_0_0RNSScreenStackHeaderSubviewTypeBackButton && subview.subviews.count > 0) {
      hasBackButtonImage = YES;
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
      ABI50_0_0RCTImageComponentView *imageView = subview.subviews[0];
#else
      ABI50_0_0RCTImageView *imageView = subview.subviews[0];
#endif // ABI50_0_0RCT_NEW_ARCH_ENABLED
      if (imageView.image == nil) {
        ...
        ABI50_0_0RCTImageSource *imageSource = [ABI50_0_0RNSScreenStackHeaderConfig imageSourceFromImageView:imageView];
        [imageView ABI50_0_0ReactSetFrame:CGRectMake(
      ...
      if (image == nil) {
        ...
        ABI50_0_0RCTImageSource *imageSource = [ABI50_0_0RNSScreenStackHeaderConfig imageSourceFromImageView:imageView];
        ABI50_0_0RCTImageLoader *imageLoader = [subview.bridge moduleForClass:[ABI50_0_0RCTImageLoader class]];
        ...
        image = [imageLoader.imageCache
            imageForUrl:imageSource.request.URL.absoluteString
             ...
             resizeMode:resizeModeFromCppEquiv(
                            std::static_pointer_cast<const ABI50_0_0React::ImageProps>(imageView.props)->resizeMode)];
#else
...
+ (void)willShowViewController:(UIViewController *)vc
                    ...
                    withConfig:(ABI50_0_0RNSScreenStackHeaderConfig *)config
{
  [self updateViewController:vc withConfig:config animated:animated];
  // As the header might have change in `updateViewController` we need to ensure that header height
  // returned by the `onHeaderHeightChange` event is correct.
  if ([vc isKindOfClass:[ABI50_0_0RNSScreen class]]) {
    [(ABI50_0_0RNSScreen *)vc calculateAndNotifyHeaderHeightChangeIsModal:NO];
  }
}
...
+ (UINavigationBarAppearance *)buildAppearance:(UIViewController *)vc
                                    withConfig:(ABI50_0_0RNSScreenStackHeaderConfig *)config API_AVAILABLE(ios(13.0))
{
                    ...
                    animated:(BOOL)animated
{
...
#if !TARGET_OS_TV
  const auto isBackTitleBlank = [NSString ABI50_0_0RNSisBlank:config.backTitle] == YES;
  NSString *resolvedBackTitle = isBackTitleBlank ? prevItem.title : config.backTitle;
  ABI50_0_0RNSUIBarButtonItem *backBarButtonItem = [[ABI50_0_0RNSUIBarButtonItem alloc] initWithTitle:resolvedBackTitle
  ...
  for (ABI50_0_0RNSScreenStackHeaderSubview *subview in config.ABI50_0_0ReactSubviews) {
    switch (subview.type) {
      case ABI50_0_0RNSScreenStackHeaderSubviewTypeLeft: {
#if !TARGET_OS_TV
      ...
      case ABI50_0_0RNSScreenStackHeaderSubviewTypeRight: {
        UIBarButtonItem *buttonItem = [[UIBarButtonItem alloc] initWithCustomView:subview];
        navitem.rightBarButtonItem = buttonItem;
        break;
      }
      case ABI50_0_0RNSScreenStackHeaderSubviewTypeCenter:
      case ABI50_0_0RNSScreenStackHeaderSubviewTypeTitle: {
        navitem.titleView = subview;
        break;
      }
      case ABI50_0_0RNSScreenStackHeaderSubviewTypeSearchBar: {
        ...
        if ([subview.subviews[0] isKindOfClass:[ABI50_0_0RNSSearchBar class]]) {
#if !TARGET_OS_TV
          if (@available(iOS 11.0, *)) {
            ABI50_0_0RNSSearchBar *searchBar = subview.subviews[0];
    ...
    __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_16_0
      ...
      case ABI50_0_0RNSScreenStackHeaderSubviewTypeBackButton: {
        break;
      }
    }
  }
    ...
    [vc.transitionCoordinator
        ...
        completion:^(id<UIViewControllerTransitionCoordinatorContext> _Nonnull context) {
          if ([context isCancelled]) {
            ...
            ABI50_0_0RNSScreenStackHeaderConfig *config = nil;
            for (UIView *subview in fromVC.view.ABI50_0_0ReactSubviews) {
              if ([subview isKindOfClass:[ABI50_0_0RNSScreenStackHeaderConfig class]]) {
                config = (ABI50_0_0RNSScreenStackHeaderConfig *)subview;
                break;
              }
            }
...
- (void)insertABI50_0_0ReactSubview:(ABI50_0_0RNSScreenStackHeaderSubview *)subview atIndex:(NSInteger)atIndex
{
  [_ABI50_0_0ReactSubviews insertObject:subview atIndex:atIndex];
  subview.ABI50_0_0ReactSuperview = self;
}

- (void)removeABI50_0_0ReactSubview:(ABI50_0_0RNSScreenStackHeaderSubview *)subview
{
...
{
  if (![childComponentView isKindOfClass:[ABI50_0_0RNSScreenStackHeaderSubview class]]) {
    ABI50_0_0RCTLogError(@"ScreenStackHeader only accepts children of type ScreenStackHeaderSubview");
    return;
  }
  ...
  //  [_ABI50_0_0ReactSubviews insertObject:(ABI50_0_0RNSScreenStackHeaderSubview *)childComponentView atIndex:index];
  [self insertABI50_0_0ReactSubview:(ABI50_0_0RNSScreenStackHeaderSubview *)childComponentView atIndex:index];
  [self updateViewControllerIfNeeded];
}

- (void)unmountChildComponentView:(UIView<ABI50_0_0RCTComponentViewProtocol> *)childComponentView index:(NSInteger)index
{
  [_ABI50_0_0ReactSubviews removeObject:(ABI50_0_0RNSScreenStackHeaderSubview *)childComponentView];
  [childComponentView removeFromSuperview];
}

static ABI50_0_0RCTResizeMode resizeModeFromCppEquiv(ABI50_0_0React::ImageResizeMode resizeMode)
{
  switch (resizeMode) {
    case ABI50_0_0React::ImageResizeMode::Cover:
      return ABI50_0_0RCTResizeModeCover;
    case ABI50_0_0React::ImageResizeMode::Contain:
      return ABI50_0_0RCTResizeModeContain;
    case ABI50_0_0React::ImageResizeMode::Stretch:
      return ABI50_0_0RCTResizeModeStretch;
    case ABI50_0_0React::ImageResizeMode::Center:
      return ABI50_0_0RCTResizeModeCenter;
    case ABI50_0_0React::ImageResizeMode::Repeat:
      return ABI50_0_0RCTResizeModeRepeat;
  }
}
 ...
 */
+ (ABI50_0_0RCTImageSource *)imageSourceFromImageView:(ABI50_0_0RCTImageComponentView *)view
{
  auto const imageProps = *std::static_pointer_cast<const ABI50_0_0React::ImageProps>(view.props);
  ABI50_0_0React::ImageSource cppImageSource = imageProps.sources.at(0);
  auto imageSize = CGSize{cppImageSource.size.width, cppImageSource.size.height};
  NSURLRequest *request =
      [NSURLRequest requestWithURL:[NSURL URLWithString:ABI50_0_0RCTNSStringFromStringNilIfEmpty(cppImageSource.uri)]];
  ABI50_0_0RCTImageSource *imageSource = [[ABI50_0_0RCTImageSource alloc] initWithURLRequest:request
                                                                      size:imageSize
...
{
  return ABI50_0_0React::concreteComponentDescriptorProvider<ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigComponentDescriptor>();
}
...
- (UISemanticContentAttribute)getDirectionPropValue:(ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigDirection)direction
{
  switch (direction) {
    case ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigDirection::Rtl:
      return UISemanticContentAttributeForceRightToLeft;
    case ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigDirection::Ltr:
...
{
  const auto &oldScreenProps = *std::static_pointer_cast<const ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigProps>(_props);
  const auto &newScreenProps = *std::static_pointer_cast<const ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigProps>(props);
  ...
  _title = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.title);
  if (newScreenProps.titleFontFamily != oldScreenProps.titleFontFamily) {
    _titleFontFamily = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.titleFontFamily);
  }
  _titleFontWeight = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.titleFontWeight);
  ...
  if (newScreenProps.largeTitleFontFamily != oldScreenProps.largeTitleFontFamily) {
    _largeTitleFontFamily = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.largeTitleFontFamily);
  }
  _largeTitleFontWeight = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.largeTitleFontWeight);
  ...
  _backTitle = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.backTitle);
  if (newScreenProps.backTitleFontFamily != oldScreenProps.backTitleFontFamily) {
    _backTitleFontFamily = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newScreenProps.backTitleFontFamily);
  }
  ...
  _titleColor = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.titleColor);
  _largeTitleColor = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.largeTitleColor);
  _color = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.color);
  _backgroundColor = ABI50_0_0RCTUIColorFromSharedColor(newScreenProps.backgroundColor);
  ...
  _props = std::static_pointer_cast<ABI50_0_0React::ABI50_0_0RNSScreenStackHeaderConfigProps const>(props);
 ...
 */
+ (ABI50_0_0RCTImageSource *)imageSourceFromImageView:(ABI50_0_0RCTImageView *)view
{
...
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
Class<ABI50_0_0RCTComponentViewProtocol> ABI50_0_0RNSScreenStackHeaderConfigCls(void)
{
  return ABI50_0_0RNSScreenStackHeaderConfig.class;
}
#endif

@implementation ABI50_0_0RNSScreenStackHeaderConfigManager

ABI50_0_0RCT_EXPORT_MODULE()

- (UIView *)view
{
  return [ABI50_0_0RNSScreenStackHeaderConfig new];
}

ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(title, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontFamily, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontSize, NSNumber)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(titleFontWeight, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(titleColor, UIColor)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(backTitle, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontFamily, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleFontSize, NSNumber)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(backgroundColor, UIColor)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(backTitleVisible, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(blurEffect, UIBlurEffectStyle)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(color, UIColor)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(direction, UISemanticContentAttribute)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitle, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontFamily, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontSize, NSNumber)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleFontWeight, NSString)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleColor, UIColor)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleBackgroundColor, UIColor)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(largeTitleHideShadow, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(hideBackButton, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(hideShadow, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(backButtonInCustomView, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(disableBackButtonMenu, BOOL)
// `hidden` is an UIView property, we need to use different name internally
ABI50_0_0RCT_REMAP_VIEW_PROPERTY(hidden, hide, BOOL)
ABI50_0_0RCT_EXPORT_VIEW_PROPERTY(translucent, BOOL)

@end

@implementation ABI50_0_0RCTConvert (ABI50_0_0RNSScreenStackHeader)
...
ABI50_0_0RCT_ENUM_CONVERTER(UIBlurEffectStyle, ([self blurEffectsForIOSVersion]), UIBlurEffectStyleExtraLight, integerValue)
