 ...
 *
 * The main intention of the class is to abstract ABI50_0_0React props parsing infra from
 ...
 private:
  ...
  static bool checkValueType(
    ...
    for (const auto& item : dynamic.items()) {
      ABI50_0_0React_native_assert(item.first.isString());
  ...
  static std::vector<T> castValue(
      const folly::dynamic& dynamic,
      std::vector<T>* type) {
    ABI50_0_0React_native_assert(dynamic.isArray());
  ...
  static std::unordered_map<std::string, T> castValue(
    ...
    ABI50_0_0React_native_assert(dynamic.isObject());
