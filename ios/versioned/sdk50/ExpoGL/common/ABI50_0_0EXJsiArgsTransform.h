...
#include "ABI50_0_0EXTypedArrayApi.h"
...
// if T = ABI50_0_0EXWebGLClass then return_type = ABI50_0_0EXGLObjectId else return_type = T
template <typename T>
using type_map = typename std::conditional<std::is_same_v<ABI50_0_0EXWebGLClass, T>, ABI50_0_0EXGLObjectId, T>::type;
...
template <>
inline ABI50_0_0EXGLObjectId unpackArg<ABI50_0_0EXWebGLClass>(jsi::Runtime &runtime, const jsi::Value *jsArgv) {
  if (!jsArgv->isObject() || !jsArgv->asObject(runtime).hasProperty(runtime, "id")) {
    return 0;
  }
  return static_cast<ABI50_0_0EXGLObjectId>(
...
// used in ABI50_0_0EXGLNativeMethods wrapped in ARGS macro
//
template <typename... T>
...
//   return nullptr;
// }
// used in ABI50_0_0EXGLNativeMethods wrapped in SIMPLE_NATIVE_METHOD macro
//
template <typename... T>
