...
#import <ABI50_0_0React/ABI50_0_0RCTNetworkTask.h>
...
#import "ABI50_0_0RCTNetworkPlugins.h"

typedef ABI50_0_0RCTURLRequestCancellationBlock (^ABI50_0_0RCTHTTPQueryResult)(NSError *error, NSDictionary<NSString *, id> *result);

NSString *const ABI50_0_0RCTNetworkingPHUploadHackScheme = @"ph-upload";

@interface ABI50_0_0RCTNetworking () <ABI50_0_0NativeNetworkingIOSSpec>

- (ABI50_0_0RCTURLRequestCancellationBlock)processDataForHTTPQuery:(NSDictionary<NSString *, id> *)data
                                                 callback:(ABI50_0_0RCTHTTPQueryResult)callback;
@end

/**
 * Helper to convert FormData payloads into multipart/formdata requests.
 */
@interface ABI50_0_0RCTHTTPFormDataHelper : NSObject

@property (nonatomic, weak) ABI50_0_0RCTNetworking *networker;

@end

@implementation ABI50_0_0RCTHTTPFormDataHelper {
  NSMutableArray<NSDictionary<NSString *, id> *> *_parts;
  NSMutableData *_multipartBody;
  ABI50_0_0RCTHTTPQueryResult _callback;
  NSString *_boundary;
}

static NSString *ABI50_0_0RCTGenerateFormBoundary()
{
...
- (ABI50_0_0RCTURLRequestCancellationBlock)process:(NSArray<NSDictionary *> *)formData callback:(ABI50_0_0RCTHTTPQueryResult)callback
{
  ABI50_0_0RCTAssertThread(_networker.methodQueue, @"process: must be called on method queue");
  ...
  _boundary = ABI50_0_0RCTGenerateFormBoundary();
  ...
  for (NSUInteger i = 0; i < _parts.count; i++) {
    ...
    if (uri && [[uri substringToIndex:@"ph:".length] caseInsensitiveCompare:@"ph:"] == NSOrderedSame) {
      uri = [ABI50_0_0RCTNetworkingPHUploadHackScheme stringByAppendingString:[uri substringFromIndex:@"ph".length]];
...
- (ABI50_0_0RCTURLRequestCancellationBlock)handleResult:(NSDictionary<NSString *, id> *)result error:(NSError *)error
{
  ABI50_0_0RCTAssertThread(_networker.methodQueue, @"handleResult: must be called on method queue");
 ...
 */
@implementation ABI50_0_0RCTNetworking {
  NSMutableDictionary<NSNumber *, ABI50_0_0RCTNetworkTask *> *_tasksByRequestID;
  std::mutex _handlersLock;
  NSArray<id<ABI50_0_0RCTURLRequestHandler>> *_handlers;
  NSArray<id<ABI50_0_0RCTURLRequestHandler>> * (^_handlersProvider)(ABI50_0_0RCTModuleRegistry *);
  NSMutableArray<id<ABI50_0_0RCTNetworkingRequestHandler>> *_requestHandlers;
  NSMutableArray<id<ABI50_0_0RCTNetworkingResponseHandler>> *_responseHandlers;
}
...
- (instancetype)initWithHandlersProvider:
    (NSArray<id<ABI50_0_0RCTURLRequestHandler>> * (^)(ABI50_0_0RCTModuleRegistry *moduleRegistry))getHandlers
{
...
- (id<ABI50_0_0RCTURLRequestHandler>)handlerForRequest:(NSURLRequest *)request
{
  if (!request.URL) {
    return nil;
  }
  NSArray<id<ABI50_0_0RCTURLRequestHandler>> *handlers = [self prioritizedHandlers];

  if (ABI50_0_0RCT_DEBUG) {
    // Check for handler conflicts
    float previousPriority = 0;
    id<ABI50_0_0RCTURLRequestHandler> previousHandler = nil;
    for (id<ABI50_0_0RCTURLRequestHandler> handler in handlers) {
        ...
        if (previousHandler) {
          if (priority == previousPriority) {
            ABI50_0_0RCTLogError(
                @"The ABI50_0_0RCTURLRequestHandlers %@ and %@ both reported that"
...
- (NSArray<id<ABI50_0_0RCTURLRequestHandler>> *)prioritizedHandlers
{
  std::lock_guard<std::mutex> lock(_handlersLock);
  if (_handlers) {
    return _handlers;
  ...
  NSArray<id<ABI50_0_0RCTURLRequestHandler>> *newHandlers = _handlersProvider
      ? _handlersProvider(self.moduleRegistry)
      : [self.bridge modulesConformingToProtocol:@protocol(ABI50_0_0RCTURLRequestHandler)];

  // Get handlers, sorted in reverse priority order (highest priority first)
  newHandlers = [newHandlers
      sortedArrayUsingComparator:^NSComparisonResult(id<ABI50_0_0RCTURLRequestHandler> a, id<ABI50_0_0RCTURLRequestHandler> b) {
...
- (ABI50_0_0RCTURLRequestCancellationBlock)buildRequest:(NSDictionary<NSString *, id> *)query
                               completionBlock:(void (^)(NSURLRequest *request))block
{
  ABI50_0_0RCTAssertThread(_methodQueue, @"buildRequest: must be called on method queue");

  NSURL *URL = [ABI50_0_0RCTConvert NSURL:query[@"url"]]; // this is marked as nullable in JS, but should not be null
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:URL];
  request.HTTPMethod = [ABI50_0_0RCTConvert NSString:ABI50_0_0RCTNilIfNull(query[@"method"])].uppercaseString ?: @"GET";
  request.HTTPShouldHandleCookies = [ABI50_0_0RCTConvert BOOL:query[@"withCredentials"]];
  ...
  NSDictionary *headers = [ABI50_0_0RCTConvert NSDictionary:query[@"headers"]];
  ...
  request.timeoutInterval = [ABI50_0_0RCTConvert NSTimeInterval:query[@"timeout"]];
  NSDictionary<NSString *, id> *data = [ABI50_0_0RCTConvert NSDictionary:ABI50_0_0RCTNilIfNull(query[@"data"])];
  ...
  return [self processDataForHTTPQuery:data
                              callback:^(NSError *error, NSDictionary<NSString *, id> *result) {
                                if (error) {
                                  ABI50_0_0RCTLogError(@"Error processing request body: %@", error);
                                  // Ideally we'd circle back to JS here and notify an error/abort on the request.
                                  return (ABI50_0_0RCTURLRequestCancellationBlock)nil;
                                }
                                ...
                                if ([request.allHTTPHeaderFields[@"Content-Encoding"] isEqualToString:@"gzip"]) {
                                  request.HTTPBody = ABI50_0_0RCTGzipData(request.HTTPBody, -1 /* default */);
 ...
 *
 */
- (ABI50_0_0RCTURLRequestCancellationBlock)
    processDataForHTTPQuery:(nullable NSDictionary<NSString *, id> *)query
                   callback:(ABI50_0_0RCTURLRequestCancellationBlock (^)(NSError *error, NSDictionary<NSString *, id> *result))
                                callback
{
  ABI50_0_0RCTAssertThread(_methodQueue, @"processDataForHTTPQuery: must be called on method queue");

  if (!query) {
    return callback(nil, nil);
  }
  for (id<ABI50_0_0RCTNetworkingRequestHandler> handler in _requestHandlers) {
  ...
  NSData *body = [ABI50_0_0RCTConvert NSData:query[@"string"]];
  if (body) {
    return callback(nil, @{@"body" : body});
  }
  NSString *base64String = [ABI50_0_0RCTConvert NSString:query[@"base64"]];
  if (base64String) {
    NSData *data = [[NSData alloc] initWithBase64EncodedString:base64String options:0];
    return callback(nil, @{@"body" : data});
  }
  NSURLRequest *request = [ABI50_0_0RCTConvert NSURLRequest:query[@"uri"]];
  if (request) {
    __block ABI50_0_0RCTURLRequestCancellationBlock cancellationBlock = nil;
    ABI50_0_0RCTNetworkTask *task =
        [self networkTaskWithRequest:request
                     completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
                       dispatch_async(self->_methodQueue, ^{
                         cancellationBlock = callback(
                             error, data ? @{@"body" : data, @"contentType" : ABI50_0_0RCTNullIfNil(response.MIMEType)} : nil);
                       });
                     }];

    [task start];

    __weak ABI50_0_0RCTNetworkTask *weakTask = task;
    return ^{
      [weakTask cancel];
  ...
  NSArray<NSDictionary *> *formData = [ABI50_0_0RCTConvert NSDictionaryArray:query[@"formData"]];
  if (formData) {
    ABI50_0_0RCTHTTPFormDataHelper *formDataHelper = [ABI50_0_0RCTHTTPFormDataHelper new];
...
- (void)sendData:(NSData *)data
    responseType:(NSString *)responseType
        response:(NSURLResponse *)response
         forTask:(ABI50_0_0RCTNetworkTask *)task
{
  ABI50_0_0RCTAssertThread(_methodQueue, @"sendData: must be called on method queue");

  id responseData = nil;
  for (id<ABI50_0_0RCTNetworkingResponseHandler> handler in _responseHandlers) {
  ...
  if (!responseData) {
    ...
    if ([responseType isEqualToString:@"text"]) {
      ...
      responseData = [ABI50_0_0RCTNetworking decodeTextData:data fromResponse:task.response withCarryData:nil];
      if (!responseData) {
        ABI50_0_0RCTLogWarn(@"Received data was not a string, or was not a recognised encoding.");
        return;
      }
    } else if ([responseType isEqualToString:@"base64"]) {
      responseData = [data base64EncodedStringWithOptions:0];
    } else {
      ABI50_0_0RCTLogWarn(@"Invalid responseType: %@", responseType);
      return;
    }
  }
...
- (void)sendRequest:(NSURLRequest *)request
    ...
    incrementalUpdates:(BOOL)incrementalUpdates
        responseSender:(ABI50_0_0RCTResponseSenderBlock)responseSender
{
  ABI50_0_0RCTAssertThread(_methodQueue, @"sendRequest: must be called on method queue");
  __weak __typeof(self) weakSelf = self;
  __block ABI50_0_0RCTNetworkTask *task;
  ABI50_0_0RCTURLRequestProgressBlock uploadProgressBlock = ^(int64_t progress, int64_t total) {
    NSArray *responseJSON = @[ task.requestID, @((double)progress), @((double)total) ];
    [weakSelf sendEventWithName:@"didSendNetworkData" body:responseJSON];
  };

  ABI50_0_0RCTURLRequestResponseBlock responseBlock = ^(NSURLResponse *response) {
  ...
  ABI50_0_0RCTURLRequestIncrementalDataBlock incrementalDataBlock = nil;
  ABI50_0_0RCTURLRequestProgressBlock downloadProgressBlock = nil;
  if (incrementalUpdates) {
    if ([responseType isEqualToString:@"text"]) {
      ...
      incrementalDataBlock = ^(NSData *data, int64_t progress, int64_t total) {
        ...
        NSString *responseString = [ABI50_0_0RCTNetworking decodeTextData:data
  ...
  ABI50_0_0RCTURLRequestCompletionBlock completionBlock = ^(NSURLResponse *response, NSData *data, NSError *error) {
    ...
    NSArray *responseJSON =
        @[ task.requestID, ABI50_0_0RCTNullIfNil(error.localizedDescription), error.code == kCFURLErrorTimedOut ? @YES : @NO ];
...
- (void)addRequestHandler:(id<ABI50_0_0RCTNetworkingRequestHandler>)handler
{
  if (!_requestHandlers) {
...
- (void)addResponseHandler:(id<ABI50_0_0RCTNetworkingResponseHandler>)handler
{
...
- (void)removeRequestHandler:(id<ABI50_0_0RCTNetworkingRequestHandler>)handler
{
  [_requestHandlers removeObject:handler];
}

- (void)removeResponseHandler:(id<ABI50_0_0RCTNetworkingResponseHandler>)handler
{
  [_responseHandlers removeObject:handler];
}

- (ABI50_0_0RCTNetworkTask *)networkTaskWithRequest:(NSURLRequest *)request
                           completionBlock:(ABI50_0_0RCTURLRequestCompletionBlock)completionBlock
{
  id<ABI50_0_0RCTURLRequestHandler> handler = [self handlerForRequest:request];
  if (!handler) {
    ABI50_0_0RCTLogError(@"No suitable URL request handler found for %@", request.URL);
    return nil;
  }

  ABI50_0_0RCTNetworkTask *task = [[ABI50_0_0RCTNetworkTask alloc] initWithRequest:request handler:handler callbackQueue:_methodQueue];
...
ABI50_0_0RCT_EXPORT_METHOD(sendRequest
                  : (ABI50_0_0JS::NativeNetworkingIOS::SpecSendRequestQuery &)query callback
                  : (ABI50_0_0RCTResponseSenderBlock)responseSender)
{
  ...
  [self buildRequest:queryDict
      completionBlock:^(NSURLRequest *request) {
        NSString *responseType = [ABI50_0_0RCTConvert NSString:queryDict[@"responseType"]];
        BOOL incrementalUpdates = [ABI50_0_0RCTConvert BOOL:queryDict[@"incrementalUpdates"]];
...
ABI50_0_0RCT_EXPORT_METHOD(abortRequest : (double)requestID)
{
...
ABI50_0_0RCT_EXPORT_METHOD(clearCookies : (ABI50_0_0RCTResponseSenderBlock)responseSender)
{
...
{
  return std::make_shared<ABI50_0_0facebook::ABI50_0_0React::NativeNetworkingIOSSpecJSI>(params);
}

@end

@implementation ABI50_0_0RCTBridge (ABI50_0_0RCTNetworking)

- (ABI50_0_0RCTNetworking *)networking
{
  return [self moduleForClass:[ABI50_0_0RCTNetworking class]];
}

@end

@implementation ABI50_0_0RCTBridgeProxy (ABI50_0_0RCTNetworking)
...
Class ABI50_0_0RCTNetworkingCls(void)
{
  return ABI50_0_0RCTNetworking.class;
}
