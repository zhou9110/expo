...
class ABI50_0_0RNSkCanvasProvider {
public:
  explicit ABI50_0_0RNSkCanvasProvider(std::function<void()> requestRedraw)
...
class ABI50_0_0RNSkRenderer {
public:
  explicit ABI50_0_0RNSkRenderer(std::function<void()> requestRedraw)
      : _requestRedraw(requestRedraw) {}

  /**
   Tries to render the current set of drawing operations. If we're busy we'll
   return false so that the calling ABI50_0_0RNSkBaseDrawView can request a new render
   ...
   */
  virtual bool
  tryRender(std::shared_ptr<ABI50_0_0RNSkCanvasProvider> canvasProvider) = 0;
   ...
   */
  virtual void
  renderImmediate(std::shared_ptr<ABI50_0_0RNSkCanvasProvider> canvasProvider) = 0;
...
class ABI50_0_0RNSkOffscreenCanvasProvider : public ABI50_0_0RNSkCanvasProvider {
public:
  ABI50_0_0RNSkOffscreenCanvasProvider(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context,
                              std::function<void()> requestRedraw, float width,
                              float height)
      : ABI50_0_0RNSkCanvasProvider(requestRedraw), _width(width), _height(height) {
...
enum ABI50_0_0RNSkDrawingMode { Default, Continuous };

using ABI50_0_0RNSkTouchInfo = struct {
...
class ABI50_0_0RNSkView : public std::enable_shared_from_this<ABI50_0_0RNSkView> {
public:
  /**
   * Constructor
   */
  ABI50_0_0RNSkView(std::shared_ptr<ABI50_0_0RNSkPlatformContext> context,
           std::shared_ptr<ABI50_0_0RNSkCanvasProvider> canvasProvider,
           std::shared_ptr<ABI50_0_0RNSkRenderer> renderer)
  ...
  virtual void setJsiProperties(
      std::unordered_map<std::string, ABI50_0_0RNJsi::JsiValueWrapper> &props) {
      ...
      if (prop.first == "onSize") {
        ...
        if (prop.second.isUndefinedOrNull()) {
          ...
          _onSize = nullptr;
                   ...
                   ABI50_0_0RNJsi::JsiWrapperValueType::HostObject) {
        ...
        // Save onSize
        _onSize =
            std::dynamic_pointer_cast<ABI50_0_0RNSkValue>(prop.second.getAsHostObject());
   ...
   */
  void setDrawingMode(ABI50_0_0RNSkDrawingMode mode) {
    _drawingMode = mode;
    requestRedraw();
  }
   ...
   */
  virtual void updateTouchState(std::vector<ABI50_0_0RNSkTouchInfo> &) {
    requestRedraw();
  }
  ...
  sk_sp<SkImage> makeImageSnapshot(SkRect *bounds) {
    ...
    auto provider = std::make_shared<ABI50_0_0RNSkOffscreenCanvasProvider>(
        getPlatformContext(), std::bind(&ABI50_0_0RNSkView::requestRedraw, this),
  ...
  std::shared_ptr<ABI50_0_0RNSkRenderer> getRenderer() { return _renderer; }

protected:
  std::shared_ptr<ABI50_0_0RNSkPlatformContext> getPlatformContext() {
    return _platformContext;
  }
  std::shared_ptr<ABI50_0_0RNSkCanvasProvider> getCanvasProvider() {
...
private:
  ...
  void drawLoopCallback(bool invalidated) {
    if (_redrawRequestCounter > 0 ||
        _drawingMode == ABI50_0_0RNSkDrawingMode::Continuous) {
      _redrawRequestCounter = 0;
  ...
  std::shared_ptr<ABI50_0_0RNSkCanvasProvider> _canvasProvider;
  std::shared_ptr<ABI50_0_0RNSkRenderer> _renderer;

  std::shared_ptr<ABI50_0_0RNSkValue> _onSize;
  std::function<void()> _onSizeUnsubscribe;
  ABI50_0_0RNSkDrawingMode _drawingMode = ABI50_0_0RNSkDrawingMode::Default;
  size_t _nativeId;
