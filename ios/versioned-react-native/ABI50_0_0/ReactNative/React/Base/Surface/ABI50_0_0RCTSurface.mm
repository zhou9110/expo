...
#import "ABI50_0_0RCTSurfaceDelegate.h"
#import "ABI50_0_0RCTSurfaceRootShadowView.h"
#import "ABI50_0_0RCTSurfaceRootShadowViewDelegate.h"
#import "ABI50_0_0RCTSurfaceRootView.h"
#import "ABI50_0_0RCTSurfaceView.h"
#import "ABI50_0_0RCTTouchHandler.h"
#import "ABI50_0_0RCTUIManager.h"
#import "ABI50_0_0RCTUIManagerObserverCoordinator.h"
#import "ABI50_0_0RCTUIManagerUtils.h"

@interface ABI50_0_0RCTSurface () <ABI50_0_0RCTSurfaceRootShadowViewDelegate, ABI50_0_0RCTUIManagerObserver>
@end

@implementation ABI50_0_0RCTSurface {
  // Immutable
  ...
  std::mutex _mutex;
  ABI50_0_0RCTBridge *_batchedBridge;
  ABI50_0_0RCTSurfaceStage _stage;
  NSDictionary *_properties;
  CGSize _minimumSize;
  CGSize _maximumSize;
  CGSize _intrinsicSize;
  ABI50_0_0RCTUIManagerMountingBlock _mountingBlock;

  // The Main thread only
  ABI50_0_0RCTSurfaceView *_Nullable _view;
  ABI50_0_0RCTTouchHandler *_Nullable _touchHandler;
...
{
  ABI50_0_0RCTErrorNewArchitectureValidation(ABI50_0_0RCTNotAllowedInFabricWithoutLegacy, @"ABI50_0_0RCTSurface", nil);
  ABI50_0_0RCTAssert(bridge.valid, @"Valid bridge is required to instantiate `ABI50_0_0RCTSurface`.");
  ...
  if (self = [super init]) {
    ...
    _rootViewTag = ABI50_0_0RCTAllocateRootViewTag();
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleBridgeWillLoadJavaScriptNotification:)
                                                 name:ABI50_0_0RCTJavaScriptWillStartLoadingNotification
                                               object:_bridge];
    ...
    _stage = ABI50_0_0RCTSurfaceStageSurfaceDidInitialize;

    if (!bridge.loading) {
      _stage = _stage | ABI50_0_0RCTSurfaceStageBridgeDidLoad;
    }
...
- (ABI50_0_0RCTSurfaceView *)view
{
  ABI50_0_0RCTAssertMainQueue();

  if (!_view) {
    _view = [[ABI50_0_0RCTSurfaceView alloc] initWithSurface:self];

    _touchHandler = [[ABI50_0_0RCTTouchHandler alloc] initWithBridge:self.bridge];
...
- (void)_mountRootViewIfNeeded
{
  ...
  ABI50_0_0RCTSurfaceView *view = self->_view;
  if (!view) {
    return;
  }

  ABI50_0_0RCTSurfaceRootView *rootView = (ABI50_0_0RCTSurfaceRootView *)[self._uiManager viewForABI50_0_0ReactTag:self->_rootViewTag];
  if (!rootView) {
    return;
  }

  ABI50_0_0RCTAssert(
      [rootView isKindOfClass:[ABI50_0_0RCTSurfaceRootView class]],
      @"Received root view is not an instance of `ABI50_0_0RCTSurfaceRootView`.");
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification *)notification
{
  ...
  _view = nil;
  _touchHandler = nil;
  [self _setStage:ABI50_0_0RCTSurfaceStageBridgeDidLoad];
}
...
- (void)handleBridgeDidLoadJavaScriptNotification:(NSNotification *)notification
{
  ...
  [self _setStage:ABI50_0_0RCTSurfaceStageModuleDidLoad];
...
- (void)_setStage:(ABI50_0_0RCTSurfaceStage)stage
{
  ABI50_0_0RCTSurfaceStage updatedStage;
  {
    std::lock_guard<std::mutex> lock(_mutex);
    ...
    updatedStage = (ABI50_0_0RCTSurfaceStage)(_stage | stage);
    _stage = updatedStage;
  }
...
- (void)_propagateStageChange:(ABI50_0_0RCTSurfaceStage)stage
{
  // Updating the `view`
  ...
  id<ABI50_0_0RCTSurfaceDelegate> delegate = self.delegate;
...
- (void)_run
{
  ...
  ABI50_0_0RCTLogInfo(@"Running surface %@ (%@)", _moduleName, applicationParameters);

  [self mountABI50_0_0ReactComponentWithBridge:batchedBridge moduleName:_moduleName params:applicationParameters];

  [self _setStage:ABI50_0_0RCTSurfaceStageSurfaceDidRun];
}

- (void)_stop
{
  [self unmountABI50_0_0ReactComponentWithBridge:self._batchedBridge rootViewTag:self->_rootViewTag];
}
...
- (void)_registerRootView
{
  ...
  ABI50_0_0RCTUIManager *uiManager = batchedBridge.uiManager;
  ...
  (ABI50_0_0RCTIsMainQueue() ? ABI50_0_0RCTUnsafeExecuteOnUIManagerQueueSync : ABI50_0_0RCTExecuteOnUIManagerQueue)(^{
    [uiManager registerRootViewTag:self->_rootViewTag];

    ABI50_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI50_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI50_0_0ReactTag:self->_rootViewTag];
    ABI50_0_0RCTAssert(
        [rootShadowView isKindOfClass:[ABI50_0_0RCTSurfaceRootShadowView class]],
        @"Received shadow view is not an instance of `ABI50_0_0RCTSurfaceRootShadowView`.");
...
{
  ABI50_0_0RCTUIManager *uiManager = self._uiManager;
  __block CGSize fittingSize;

  ABI50_0_0RCTUnsafeExecuteOnUIManagerQueueSync(^{
    ABI50_0_0RCTSurfaceRootShadowView *rootShadowView =
        (ABI50_0_0RCTSurfaceRootShadowView *)[uiManager shadowViewForABI50_0_0ReactTag:self->_rootViewTag];
...
{
  // `viewportOffset` is intentionally zero because `ABI50_0_0RCTSurface` ignores it.
  // However, it is needed in `ABI50_0_0RCTFabricSurface`.
...
- (BOOL)synchronouslyWaitForStage:(ABI50_0_0RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout
{
  if (ABI50_0_0RCTIsUIManagerQueue()) {
    ABI50_0_0RCTLogInfo(@"Synchronous waiting is not supported on UIManager queue.");
    return NO;
  }

  if (ABI50_0_0RCTIsMainQueue() && (stage & ABI50_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
  ...
  switch (stage) {
    case ABI50_0_0RCTSurfaceStageSurfaceDidInitialLayout:
      semaphore = _rootShadowViewDidStartLayingOutSemaphore;
      break;
    case ABI50_0_0RCTSurfaceStageSurfaceDidInitialRendering:
      semaphore = _rootShadowViewDidStartRenderingSemaphore;
      break;
    case ABI50_0_0RCTSurfaceStageSurfaceDidInitialMounting:
      semaphore = _uiManagerDidPerformMountingSemaphore;
      break;
    default:
      ABI50_0_0RCTAssert(
          NO,
          @"Only waiting for `ABI50_0_0RCTSurfaceStageSurfaceDidInitialRendering`, `ABI50_0_0RCTSurfaceStageSurfaceDidInitialLayout` and `ABI50_0_0RCTSurfaceStageSurfaceDidInitialMounting` stages are supported.");
  }
  ...
  if (ABI50_0_0RCTIsMainQueue() && (stage & ABI50_0_0RCTSurfaceStageSurfaceDidInitialMounting)) {
    ...
    ABI50_0_0RCTUIManagerMountingBlock mountingBlock;
    {
      std::lock_guard<std::mutex> lock(_mutex);
...
#pragma mark - ABI50_0_0RCTSurfaceRootShadowViewDelegate

- (void)rootShadowView:(__unused ABI50_0_0RCTRootShadowView *)rootShadowView didChangeIntrinsicSize:(CGSize)intrinsicSize
{
  self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused ABI50_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI50_0_0RCTSurfaceStageSurfaceDidInitialRendering];

  dispatch_semaphore_signal(_rootShadowViewDidStartRenderingSemaphore);
}

- (void)rootShadowViewDidStartLayingOut:(__unused ABI50_0_0RCTSurfaceRootShadowView *)rootShadowView
{
  [self _setStage:ABI50_0_0RCTSurfaceStageSurfaceDidInitialLayout];
...
- (BOOL)uiManager:(__unused ABI50_0_0RCTUIManager *)manager performMountingWithBlock:(ABI50_0_0RCTUIManagerMountingBlock)block
{
  if (atomic_load(&_waitingForMountingStageOnMainQueue) && (self.stage & ABI50_0_0RCTSurfaceStageSurfaceDidInitialLayout)) {
    // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
    atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused ABI50_0_0RCTUIManager *)manager
{
  if (self.stage & ABI50_0_0RCTSurfaceStageSurfaceDidInitialLayout) {
    [self _setStage:ABI50_0_0RCTSurfaceStageSurfaceDidInitialMounting];
...
#pragma mark - Mounting/Unmounting of ABI50_0_0React components

- (void)mountABI50_0_0ReactComponentWithBridge:(ABI50_0_0RCTBridge *)bridge
                           moduleName:(NSString *)moduleName
...
- (void)unmountABI50_0_0ReactComponentWithBridge:(ABI50_0_0RCTBridge *)bridge rootViewTag:(NSNumber *)rootViewTag
{
