...
#import <ABI50_0_0React/ABI50_0_0RCTAppState.h>
#import <ABI50_0_0React/ABI50_0_0RCTImageLoader.h>

#import <ABI50_0_0ExpoModulesCore/ABI50_0_0EXReactNativeAdapter.h>
...
#import <ABI50_0_0ExpoModulesCore/ABI50_0_0ExpoFabricViewObjC.h>
#endif

@interface ABI50_0_0EXReactNativeAdapter ()
...
@implementation ABI50_0_0EXReactNativeAdapter

ABI50_0_0EX_REGISTER_MODULE();

+ (NSString *)moduleName
{
  return @"ABI50_0_0EXReactNativeAdapter";
}

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI50_0_0EXAppLifecycleService), @protocol(ABI50_0_0EXUIManager), @protocol(ABI50_0_0EXJavaScriptContextProvider)];
}
...
{
  __weak ABI50_0_0EXReactNativeAdapter *weakSelf = self;
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    __strong ABI50_0_0EXReactNativeAdapter *strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI50_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
        block(viewRegistry);
      }];
    }
  });
...
{
  [self.bridge dispatchBlock:block queue:ABI50_0_0RCTJSThread];
}
...
# pragma mark - ABI50_0_0EXJavaScriptContextProvider
...
- (void)startObserving
{
  for (NSString *name in @[UIApplicationDidBecomeActiveNotification,
                           ...
                           ABI50_0_0RCTContentDidAppearNotification,
                           ABI50_0_0RCTBridgeWillReloadNotification]) {
...
{
  if ([notification.name isEqualToString:ABI50_0_0RCTContentDidAppearNotification]) {
    [self notifyAboutContentDidAppear];
  } else if ([notification.name isEqualToString:ABI50_0_0RCTBridgeWillReloadNotification]) {
    [self notifyAboutContentWillReload];
      ...
      _isForegrounded && (
       ...
       ABI50_0_0RCTSharedApplication().applicationState == UIApplicationStateBackground
      )
    ) {
    [self setAppStateToBackground];
  } else if (!_isForegrounded && ABI50_0_0RCTSharedApplication().applicationState == UIApplicationStateActive) {
...
- (void)executeUIBlock:(void (^)(UIView *view))block forView:(id)viewId
{
  ...
  dispatch_async(_bridge.uiManager.methodQueue, ^{
    ...
    if (strongSelf) {
      [strongSelf.bridge.uiManager addUIBlock:^(__unused ABI50_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
#if RN_FABRIC_ENABLED
        ABI50_0_0RCTSurfacePresenter *surfacePresenter = strongSelf.bridge.surfacePresenter;
        UIView<ABI50_0_0RCTComponentViewProtocol> *componentView = [surfacePresenter.mountingManager.componentViewRegistry findComponentViewWithTag:[viewId integerValue]];
