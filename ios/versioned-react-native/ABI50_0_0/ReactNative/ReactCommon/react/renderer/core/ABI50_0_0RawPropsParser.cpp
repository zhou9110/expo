...
#include "ABI50_0_0RawPropsParser.h"
...
const RawValue* RawPropsParser::at(
  ...
  if (UNLIKELY(!ready_)) {
    ...
    keys_.push_back(key);
    ABI50_0_0React_native_assert(size < std::numeric_limits<RawPropsValueIndex>::max());
...
void RawPropsParser::preparse(const RawProps& rawProps) const noexcept {
  ...
  switch (rawProps.mode_) {
    ...
    case RawProps::Mode::JSI: {
      ...
      ABI50_0_0React_native_assert(rawProps.value_.isObject());
