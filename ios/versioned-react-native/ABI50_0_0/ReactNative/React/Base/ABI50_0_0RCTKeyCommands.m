...
@interface ABI50_0_0RCTKeyCommand : NSObject <NSCopying>
...
@implementation ABI50_0_0RCTKeyCommand
...
- (BOOL)isEqual:(ABI50_0_0RCTKeyCommand *)object
{
  if (![object isKindOfClass:[ABI50_0_0RCTKeyCommand class]]) {
    return NO;
  }
...
@interface ABI50_0_0RCTKeyCommands ()

@property (nonatomic, strong) NSMutableSet<ABI50_0_0RCTKeyCommand *> *commands;

@end

@implementation ABI50_0_0RCTKeyCommands
...
- (void)handleKeyUIEventSwizzle:(UIEvent *)event
{
  ...
  BOOL interactionEnabled = !ABI50_0_0RCTSharedApplication().isIgnoringInteractionEvents;
  ...
  if (isKeyDown && modifiedInput.length > 0 && interactionEnabled) {
    ...
    if (!firstResponder) {
      [self ABI50_0_0RCT_handleKeyCommand:modifiedInput flags:modifierFlags];
    }
  }
};
...
- (void)ABI50_0_0RCT_handleKeyCommand:(NSString *)input flags:(UIKeyModifierFlags)modifierFlags
{
  for (ABI50_0_0RCTKeyCommand *command in [ABI50_0_0RCTKeyCommands sharedInstance].commands) {
...
{
  static ABI50_0_0RCTKeyCommands *sharedInstance;
                             ...
                             action:(void (^)(UIKeyCommand *))block
{
  ...
  ABI50_0_0RCTKeyCommand *keyCommand = [[ABI50_0_0RCTKeyCommand alloc] init:input flags:flags block:block];
...
- (void)unregisterKeyCommandWithInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI50_0_0RCTKeyCommand *command in _commands.allObjects) {
...
- (BOOL)isKeyCommandRegisteredForInput:(NSString *)input modifierFlags:(UIKeyModifierFlags)flags
{
  ...
  for (ABI50_0_0RCTKeyCommand *command in _commands) {
    if ([command matchesInput:input flags:flags]) {
      return YES;
    }
  }
