...
#import <ABI50_0_0EXStructuredHeaders/ABI50_0_0EXStructuredHeadersParser.h>

NS_ASSUME_NONNULL_BEGIN

typedef NS_ENUM(NSInteger, ABI50_0_0EXStructuredHeadersParserNumberType) {
  ABI50_0_0EXStructuredHeadersParserNumberTypeInteger,
  ABI50_0_0EXStructuredHeadersParserNumberTypeDecimal
};

static NSString * const ABI50_0_0EXStructuredHeadersParserErrorDomain = @"ABI50_0_0EXStructuredHeadersParser";

@interface ABI50_0_0EXStructuredHeadersParser ()

@property (nonatomic, strong) NSString *raw;
@property (nonatomic, assign) NSUInteger position;

@property (nonatomic, assign) ABI50_0_0EXStructuredHeadersParserFieldType fieldType;
@property (nonatomic, assign) BOOL shouldIgnoreParameters;

@end

@implementation ABI50_0_0EXStructuredHeadersParser

- (instancetype)initWithRawInput:(NSString *)raw fieldType:(ABI50_0_0EXStructuredHeadersParserFieldType)fieldType
{
  return [self initWithRawInput:raw fieldType:fieldType ignoringParameters:NO];
}

- (instancetype)initWithRawInput:(NSString *)raw fieldType:(ABI50_0_0EXStructuredHeadersParserFieldType)fieldType ignoringParameters:(BOOL)shouldIgnoreParameters
{
  if (self = [super init]) {
    _raw = raw;
    _position = 0;
...
- (nullable id)parseStructuredFieldsWithError:(NSError ** _Nullable)error
{
  ...
  id output;
  switch (_fieldType) {
    case ABI50_0_0EXStructuredHeadersParserFieldTypeList:
      output = [self _parseAListWithError:error];
      break;
    case ABI50_0_0EXStructuredHeadersParserFieldTypeDictionary:
      output = [self _parseADictionaryWithError:error];
      break;
    case ABI50_0_0EXStructuredHeadersParserFieldTypeItem:
...
{
  // 4.2.4
  ABI50_0_0EXStructuredHeadersParserNumberType type = ABI50_0_0EXStructuredHeadersParserNumberTypeInteger;
  NSInteger sign = 1;
  ...
  while ([self hasRemaining]) {
    ...
    } else if (type == ABI50_0_0EXStructuredHeadersParserNumberTypeInteger && nextChar == '.') {
      if (inputNumber.length > 12) {
        if (error) *error = [self errorWithMessage:@"Decimal cannot have more than 12 digits before the decimal point"];
        return nil;
      }
      [inputNumber appendFormat:@"%c", nextChar];
      type = ABI50_0_0EXStructuredHeadersParserNumberTypeDecimal;
    } else {
      [self backout];
      break;
    }

    if (type == ABI50_0_0EXStructuredHeadersParserNumberTypeInteger && inputNumber.length > 15) {
      if (error) *error = [self errorWithMessage:@"Integer cannot have more than 15 digits"];
      return nil;
    } else if (type == ABI50_0_0EXStructuredHeadersParserNumberTypeDecimal && inputNumber.length > 16) {
      if (error) *error = [self errorWithMessage:@"Decimal cannot have more than 16 characters"];
      return nil;
    }
  }

  if (type == ABI50_0_0EXStructuredHeadersParserNumberTypeInteger) {
...
{
  return [NSError errorWithDomain:ABI50_0_0EXStructuredHeadersParserErrorDomain code:1 userInfo:@{
