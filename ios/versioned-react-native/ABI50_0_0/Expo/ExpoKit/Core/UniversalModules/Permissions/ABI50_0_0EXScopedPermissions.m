...
@interface ABI50_0_0EXScopedPermissions ()

@property (nonatomic, strong) NSString *scopeKey;
@property (nonatomic, weak) id<ABI50_0_0EXPermissionsScopedModuleDelegate> permissionsService;
@property (nonatomic, weak) id<ABI50_0_0EXUtilitiesInterface> utils;
@property (nonatomic, weak) ABI50_0_0EXConstantsBinding *constantsBinding;

@end

@implementation ABI50_0_0EXScopedPermissions
...
- (void)setModuleRegistry:(ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  _utils = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI50_0_0EXUtilitiesInterface)];
  _permissionsService = [moduleRegistry getSingletonModuleForName:@"Permissions"];
}

# pragma mark - permission requesters / getters

// overriding ABI50_0_0EXPermission to inject scoped permission logic
...
- (NSString *)getScopedPermissionStatus:(NSString *)permissionType {
  if (!_permissionsService) {
    return [[self class] permissionStringForStatus:ABI50_0_0EXPermissionStatusGranted];
  }
...
- (BOOL)hasGrantedScopedPermission:(NSString *)permissionType
{
  ...
  return [_permissionsService getPermission:permissionType forExperience:_scopeKey] == ABI50_0_0EXPermissionStatusGranted;
}
  ...
  if (![globalPermissions[@"status"] isEqualToString:@"granted"]) {
    ...
    UIAlertAction *allowAction = [UIAlertAction actionWithTitle:@"Allow" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
      ...
      if (![self.permissionsService savePermission:permission ofType:permissionType forExperience:self.scopeKey]) {
        permission[@"status"] = [[self class] permissionStringForStatus:ABI50_0_0EXPermissionStatusDenied];
...
- (NSDictionary *)getScopedPermissionForType:(NSString *)permissionType withGlobalPermission:(NSDictionary *)globalPermission
{
  ...
  if ([_constantsBinding.appOwnership isEqualToString:@"expo"]
      ...
      && [ABI50_0_0EXPermissionsService statusForPermission:permission] == ABI50_0_0EXPermissionStatusGranted) {
