...
#import "ABI50_0_0NSTextStorage+FontScaling.h"

@implementation ABI50_0_0RCTTextShadowView {
...
- (instancetype)initWithBridge:(ABI50_0_0RCTBridge *)bridge
{
  if (self = [super init]) {
    ...
    ABI50_0_0YGNodeSetMeasureFunc(self.yogaNode, ABI50_0_0RCTTextShadowViewMeasure);
    ABI50_0_0YGNodeSetBaselineFunc(self.yogaNode, ABI50_0_0RCTTextShadowViewBaseline);
  }
...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  // because the background color is drawn on the ABI50_0_0RCTTextView itself, as well
  // as on the glphy background draw step. By setting this to nil, we allow
  // the ABI50_0_0RCTTextView backgroundColor to be used, without affecting nested Text
  // components.
...
{
  if (ABI50_0_0YGNodeIsDirty(self.yogaNode)) {
    return;
  }
  ...
  NSNumber *tag = self.ABI50_0_0ReactTag;
  NSMutableArray<NSNumber *> *descendantViewTags = [NSMutableArray new];
  [textStorage enumerateAttribute:ABI50_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                          inRange:NSMakeRange(0, textStorage.length)
                          options:0
                       usingBlock:^(ABI50_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                         if (!shadowView) {
                           return;
                         }

                         [descendantViewTags addObject:shadowView.ABI50_0_0ReactTag];
                       }];

  [_bridge.uiManager addUIBlock:^(ABI50_0_0RCTUIManager *uiManager, NSDictionary<NSNumber *, UIView *> *viewRegistry) {
    ABI50_0_0RCTTextView *textView = (ABI50_0_0RCTTextView *)viewRegistry[tag];
    if (!textView) {
      return;
    }
    ...
    [textStorage removeAttribute:ABI50_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
...
- (NSAttributedString *)attributedTextWithMeasuredAttachmentsThatFitSize:(CGSize)size
{
  ...
  [attributedText enumerateAttribute:ABI50_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                             inRange:NSMakeRange(0, attributedText.length)
                             options:0
                          usingBlock:^(ABI50_0_0RCTShadowView *shadowView, NSRange range, __unused BOOL *stop) {
                            if (!shadowView) {
...
- (NSTextStorage *)textStorageAndLayoutManagerThatFitsSize:(CGSize)size exclusiveOwnership:(BOOL)exclusiveOwnership
{
  ...
  if (_adjustsFontSizeToFit) {
    ...
    [textStorage abi50_0_0_rct_scaleFontSizeToFitSize:size
...
- (void)layoutSubviewsWithContext:(ABI50_0_0RCTLayoutContext)layoutContext
{
  ...
  [textStorage
      enumerateAttribute:ABI50_0_0RCTBaseTextShadowViewEmbeddedShadowViewAttributeName
                 inRange:characterRange
                 options:0
              usingBlock:^(ABI50_0_0RCTShadowView *shadowView, NSRange range, BOOL *stop) {
                if (!shadowView) {
                  return;
                }
                ...
                CGRect frame = {
                    {ABI50_0_0RCTRoundPixelValue(glyphRect.origin.x),
                     ABI50_0_0RCTRoundPixelValue(
                         glyphRect.origin.y + glyphRect.size.height - attachmentSize.height + font.descender)},
                    {ABI50_0_0RCTRoundPixelValue(attachmentSize.width), ABI50_0_0RCTRoundPixelValue(attachmentSize.height)}};
                ...
                ABI50_0_0RCTLayoutContext localLayoutContext = layoutContext;
                ...
                ABI50_0_0RCTLayoutMetrics localLayoutMetrics = shadowView.layoutMetrics;
                localLayoutMetrics.frame.origin =
                    frame.origin; // Reinforcing a proper frame origin for the Shadow View.
                if (viewIsTruncated) {
                  localLayoutMetrics.displayType = ABI50_0_0RCTDisplayTypeNone;
                }
...
static ABI50_0_0YGSize ABI50_0_0RCTTextShadowViewMeasure(
    ...
    ABI50_0_0YGMeasureMode heightMode)
{
  CGSize maximumSize = (CGSize){
      widthMode == ABI50_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI50_0_0RCTCoreGraphicsFloatFromYogaFloat(width),
      heightMode == ABI50_0_0YGMeasureModeUndefined ? CGFLOAT_MAX : ABI50_0_0RCTCoreGraphicsFloatFromYogaFloat(height),
  };

  ABI50_0_0RCTTextShadowView *shadowTextView = (__bridge ABI50_0_0RCTTextShadowView *)ABI50_0_0YGNodeGetContext(node);
  ...
  size = (CGSize){
      MIN(ABI50_0_0RCTCeilPixelValue(size.width), maximumSize.width), MIN(ABI50_0_0RCTCeilPixelValue(size.height), maximumSize.height)};
  ...
  return (ABI50_0_0YGSize){
      ABI50_0_0RCTYogaFloatFromCoreGraphicsFloat(size.width + epsilon),
      ABI50_0_0RCTYogaFloatFromCoreGraphicsFloat(size.height + epsilon)};
}

static float ABI50_0_0RCTTextShadowViewBaseline(ABI50_0_0YGNodeConstRef node, const float width, const float height)
{
  ABI50_0_0RCTTextShadowView *shadowTextView = (__bridge ABI50_0_0RCTTextShadowView *)ABI50_0_0YGNodeGetContext(node);

  CGSize size = (CGSize){ABI50_0_0RCTCoreGraphicsFloatFromYogaFloat(width), ABI50_0_0RCTCoreGraphicsFloatFromYogaFloat(height)};

  CGFloat lastBaseline = [shadowTextView lastBaselineForSize:size];

  return ABI50_0_0RCTYogaFloatFromCoreGraphicsFloat(lastBaseline);
}
