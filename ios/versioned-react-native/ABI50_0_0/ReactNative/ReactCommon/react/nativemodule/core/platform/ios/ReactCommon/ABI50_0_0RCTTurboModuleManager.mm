...
#import "ABI50_0_0RCTTurboModuleManager.h"
#import "ABI50_0_0RCTInteropTurboModule.h"
...
#import <ABI50_0_0ReactCommon/ABI50_0_0TurboCxxModule.h>
#import <ABI50_0_0ReactCommon/ABI50_0_0TurboModulePerfLogger.h>
...
/**
 * A global variable whose address we use to associate method queues to id<ABI50_0_0RCTBridgeModule> objects.
 */
...
class ModuleHolder {
 private:
  ...
  id<ABI50_0_0RCTBridgeModule> module_;
 ...
 public:
  ...
  void setModule(id<ABI50_0_0RCTBridgeModule> module)
  {
    module_ = module;
  }

  id<ABI50_0_0RCTBridgeModule> getModule() const
  {
    return module_;
  }
...
class ModuleNativeMethodCallInvoker : public NativeMethodCallInvoker {
 ...
 public:
  ...
  {
    if (methodQueue_ == ABI50_0_0RCTJSThread) {
      work();
      return;
    }
...
{
  return [cls conformsToProtocol:@protocol(ABI50_0_0RCTTurboModule)];
}
...
// Fallback lookup since ABI50_0_0RCT class prefix is sometimes stripped in the existing NativeModule system.
...
static Class getFallbackClassFromName(const char *name)
{
  ...
  if (!moduleClass) {
    moduleClass = NSClassFromString([NSString stringWithFormat:@"ABI50_0_0RCT%s", name]);
  }
  return moduleClass;
}

@implementation ABI50_0_0RCTTurboModuleManager {
  std::shared_ptr<CallInvoker> _jsInvoker;
  __weak id<ABI50_0_0RCTTurboModuleManagerDelegate> _delegate;
                     ...
                     jsInvoker:(std::shared_ptr<CallInvoker>)jsInvoker
{
  if (self = [super init]) {
    ...
    if (ABI50_0_0RCTTurboModuleInteropEnabled()) {
      NSMutableDictionary<NSString *, id<ABI50_0_0RCTBridgeModule>> *legacyInitializedModules = [NSMutableDictionary new];

      if ([_delegate respondsToSelector:@selector(extraModulesForBridge:)]) {
        for (id<ABI50_0_0RCTBridgeModule> module in [_delegate extraModulesForBridge:nil]) {
          if (!isTurboModuleInstance(module)) {
            [legacyInitializedModules setObject:module forKey:ABI50_0_0RCTBridgeModuleNameForClass([module class])];
          }
        }
      }
      ...
      for (Class moduleClass in ABI50_0_0RCTGetModuleClasses()) {
        if (!isTurboModuleClass(moduleClass)) {
          [legacyEagerlyRegisteredModuleClasses setObject:moduleClass forKey:ABI50_0_0RCTBridgeModuleNameForClass(moduleClass)];
        }
      }
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(bridgeDidInvalidateModules:)
                                                 name:ABI50_0_0RCTBridgeDidInvalidateModulesNotification
                                               object:nil];
  }
  return self;
}
...
- (std::shared_ptr<TurboModule>)provideTurboModule:(const char *)moduleName
{
   ...
   */
  id<ABI50_0_0RCTBridgeModule> module =
      !ABI50_0_0RCTTurboModuleInteropEnabled() || [self _isTurboModule:moduleName] ? [self _provideObjCModule:moduleName] : nil;
  ...
  if (methodQueue == nil) {
    ABI50_0_0RCTLogError(@"TurboModule \"%@\" was not associated with a method queue.", moduleClass);
  }
  ...
  /**
   * Have ABI50_0_0RCTCxxBridge decorate native CallInvoker, so that it's aware of TurboModule async method calls.
   ...
   * wraps CxxModule.
   */
  if ([moduleClass isSubclassOfClass:ABI50_0_0RCTCxxModule.class]) {
    ...
    auto turboModule = std::make_shared<TurboCxxModule>([((ABI50_0_0RCTCxxModule *)module) createModule], _jsInvoker);
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
   ...
   *
   * Use respondsToSelector: below to infer conformance to @protocol(ABI50_0_0RCTTurboModule). Using conformsToProtocol: is
   * expensive.
   */
  if ([module respondsToSelector:@selector(getTurboModule:)]) {
    ObjCTurboModule::InitParams params = {
        ...
        .isSyncModule = methodQueue == ABI50_0_0RCTJSThread,
    };

    auto turboModule = [(id<ABI50_0_0RCTTurboModule>)module getTurboModule:params];
    if (turboModule == nullptr) {
      ABI50_0_0RCTLogError(@"TurboModule \"%@\"'s getTurboModule: method returned nil.", moduleClass);
    }
    _turboModuleCache.insert({moduleName, turboModule});
    return turboModule;
  }
...
- (std::shared_ptr<TurboModule>)provideLegacyModule:(const char *)moduleName
{
  ...
  id<ABI50_0_0RCTBridgeModule> module = [self _isLegacyModule:moduleName] ? [self _provideObjCModule:moduleName] : nil;

  TurboModulePerfLogger::moduleJSRequireEndingStart(moduleName);

  // If we request that a TurboModule be created, its respective ObjC class must exist
  // If the class doesn't exist, then provideABI50_0_0RCTBridgeModule returns nil
  if (!module) {
    return nullptr;
  }
  ...
  dispatch_queue_t methodQueue = (dispatch_queue_t)objc_getAssociatedObject(module, &kAssociatedMethodQueueKey);
  if (methodQueue == nil) {
    ABI50_0_0RCTLogError(@"Legacy NativeModule \"%@\" was not associated with a method queue.", moduleClass);
  }
...
{
  if (ABI50_0_0RCTTurboModuleInteropForAllTurboModulesEnabled()) {
    return NO;
  }

  Class moduleClass = [self _getModuleClassFromName:moduleName];
  return moduleClass != nil && (isTurboModuleClass(moduleClass) && ![moduleClass isSubclassOfClass:ABI50_0_0RCTCxxModule.class]);
}
 ...
 * (for now).
 */
- (id<ABI50_0_0RCTBridgeModule>)_provideObjCModule:(const char *)moduleName
{
  if (strncmp("ABI50_0_0RCT", moduleName, 3) == 0) {
  ...
  id<ABI50_0_0RCTBridgeModule> module = [self _provideObjCModule:moduleName moduleHolder:moduleHolder shouldPerfLog:YES];
                            ...
                            shouldPerfLog:(BOOL)shouldPerfLog
{
  ...
  if (shouldCreateModule) {
    ...
    __block id<ABI50_0_0RCTBridgeModule> module = nil;
    ...
    if ([self _shouldCreateObjCModule:moduleClass]) {
      ...
      if ([self _requiresMainQueueSetup:moduleClass]) {
        ABI50_0_0RCTUnsafeExecuteOnMainQueueSync(work);
      } else {
        work();
      }
    }
...
{
  if (ABI50_0_0RCTTurboModuleInteropEnabled()) {
    return [moduleClass conformsToProtocol:@protocol(ABI50_0_0RCTBridgeModule)];
  }

  return [moduleClass conformsToProtocol:@protocol(ABI50_0_0RCTTurboModule)];
}
 ...
 * _provideObjCModule:.
 */
- (id<ABI50_0_0RCTBridgeModule>)_createAndSetUpObjCModule:(Class)moduleClass
                                      moduleName:(const char *)moduleName
                                        moduleId:(int32_t)moduleId
{
  id<ABI50_0_0RCTBridgeModule> module = nil;
   ...
   * implementation, and a `- (ABI50_0_0RCTBridge *) bridge { ... }` method won't be
  ...
  if ([module respondsToSelector:@selector(bridge)] && (_bridge || _bridgeProxy)) {
    ...
    @try {
      /**
       * ABI50_0_0RCTBridgeModule declares the bridge property as readonly.
       * Therefore, when authors of NativeModules synthesize the bridge
       * via @synthesize bridge = bridge;, the ObjC runtime generates
       * only a - (ABI50_0_0RCTBridge *) bridge: { ... } method. No setter is
      ...
      ABI50_0_0RCTLogError(
          ...
          ABI50_0_0RCTBridgeModuleNameForClass([module class]));
    }
  }
  ...
  if (moduleHasMethodQueueGetter) {
    methodQueue = [(id<ABI50_0_0RCTBridgeModule>)module methodQueue];
  }

  /**
   * Note: ABI50_0_0RCTJSThread, which is a valid method queue, is defined as (id)kCFNull. It should rightfully not enter the
   * following if condition's block.
   */
  if (!methodQueue) {
    NSString *methodQueueName = [NSString stringWithFormat:@"com.facebook.ABI50_0_0React.%sQueue", moduleName];
  ...
  /**
   * If the NativeModule conforms to ABI50_0_0RCTInitializing, invoke its initialize method.
   */
  if ([module respondsToSelector:@selector(initialize)]) {
    [(id<ABI50_0_0RCTInitializing>)module initialize];
  }

  /**
   * Attach method queue to id<ABI50_0_0RCTBridgeModule> object.
   * This is necessary because the id<ABI50_0_0RCTBridgeModule> object can be eagerly created/initialized before the method
   * queue is required. The method queue is required for an id<ABI50_0_0RCTBridgeModule> for JS -> Native calls. So, we need it
   * before we create the id<ABI50_0_0RCTBridgeModule>'s TurboModule jsi::HostObject in provideTurboModule:.
   */
  objc_setAssociatedObject(module, &kAssociatedMethodQueueKey, methodQueue, OBJC_ASSOCIATION_RETAIN);

  /**
   * NativeModules that implement the ABI50_0_0RCTFrameUpdateObserver protocol
   * require registration with ABI50_0_0RCTDisplayLink.
   *
   * TODO(T55504345): Investigate whether we can improve this after TM
   * rollout.
   */
  if (_bridge) {
    ABI50_0_0RCTModuleData *data = [[ABI50_0_0RCTModuleData alloc] initWithModuleInstance:(id<ABI50_0_0RCTBridgeModule>)module
                                                                 bridge:_bridge
                                                         moduleRegistry:_bridge.moduleRegistry
                                                viewRegistry_DEPRECATED:nil
                                                          bundleManager:nil
                                                      callableJSModules:nil];
    [_bridge registerModuleForFrameUpdates:(id<ABI50_0_0RCTBridgeModule>)module withModuleData:data];
  }
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:ABI50_0_0RCTDidInitializeModuleNotification
                    object:_bridge
                  userInfo:@{@"module" : module, @"bridge" : ABI50_0_0RCTNullIfNil([_bridge parentBridge])}];
...
- (Class)_getModuleClassFromName:(const char *)moduleName
{
  ...
  // fallback on modules registered throught ABI50_0_0RCT_EXPORT_MODULE with custom names
  NSString *objcModuleName = [NSString stringWithUTF8String:moduleName];
  NSArray<Class> *modules = ABI50_0_0RCTGetModuleClasses();
...
- (id<ABI50_0_0RCTBridgeModule>)_getModuleInstanceFromClass:(Class)moduleClass
{
  NSString *moduleNameStr = ABI50_0_0RCTBridgeModuleNameForClass(moduleClass);
  if (_legacyEagerlyInitializedModules && _legacyEagerlyInitializedModules[moduleNameStr]) {
  ...
  {
    std::lock_guard<std::mutex> delegateGuard(_turboModuleManagerDelegateMutex);
    module = (id<ABI50_0_0RCTBridgeModule>)[_delegate getModuleInstanceFromClass:moduleClass];
  }
  ...
  if (requiresMainQueueSetup) {
    ABI50_0_0RCTLogWarn(
        @"Module %@ requires main queue setup since it overrides `%s` but doesn't implement "
         "`requiresMainQueueSetup`. In a future release ABI50_0_0React Native will default to initializing all NativeModules "
...
- (void)installJSBindings:(ABI50_0_0facebook::jsi::Runtime &)runtime
{
   ...
   */
  auto turboModuleProvider = [self](const std::string &name) -> std::shared_ptr<ABI50_0_0React::TurboModule> {
    auto moduleName = name.c_str();

    TurboModulePerfLogger::moduleJSRequireBeginningStart(moduleName);
    auto moduleWasNotInitialized = ![self moduleIsInitialized:moduleName];
    if (moduleWasNotInitialized) {
      [self->_bridge.performanceLogger markStartForTag:ABI50_0_0RCTPLTurboModuleSetup];
    }
    ...
    if (moduleWasNotInitialized && [self moduleIsInitialized:moduleName]) {
      [self->_bridge.performanceLogger markStopForTag:ABI50_0_0RCTPLTurboModuleSetup];
    }
  ...
  if (ABI50_0_0RCTTurboModuleInteropEnabled()) {
    auto legacyModuleProvider = [self](const std::string &name) -> std::shared_ptr<ABI50_0_0React::TurboModule> {
      auto moduleName = name.c_str();
...
#pragma mark ABI50_0_0RCTTurboModuleRegistry
...
- (id)moduleForName:(const char *)moduleName warnOnLookupFailure:(BOOL)warnOnLookupFailure
{
  ...
  id<ABI50_0_0RCTBridgeModule> module = [self _provideObjCModule:moduleName];

  if (warnOnLookupFailure && !module) {
    ABI50_0_0RCTLogError(@"Unable to find module for %@", [NSString stringWithUTF8String:moduleName]);
  }
...
{
  // Backward-compatibility: ABI50_0_0RCTInvalidating handling.
  ...
  for (auto &pair : _moduleHolders) {
    ...
    /**
     * We could start tearing down ABI50_0_0ReactNative before a NativeModule is fully initialized. In this case, we should wait
     ...
     */
    id<ABI50_0_0RCTBridgeModule> module = [self _provideObjCModule:moduleName.c_str()
    ...
    if ([module respondsToSelector:@selector(invalidate)]) {
      ...
      dispatch_block_t invalidateModule = ^{
        [((id<ABI50_0_0RCTInvalidating>)module) invalidate];
      ...
      if (_bridge) {
        ...
        // Bridgeless mode
        if (methodQueue == ABI50_0_0RCTJSThread) {
          invalidateModule();
        } else {
  ...
  if (dispatch_group_wait(moduleInvalidationGroup, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC))) {
    ABI50_0_0RCTLogError(@"TurboModuleManager: Timed out waiting for modules to be invalidated");
  }
