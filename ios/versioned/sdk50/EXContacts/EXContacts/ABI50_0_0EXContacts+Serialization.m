...
@implementation ABI50_0_0EXContacts (Serialization)
...
+ (nullable NSDictionary *)birthdayForContact:(NSDateComponents *)birthday
{
  ...
  if (birthday.calendar) {
    [birthdayObject setObject:[ABI50_0_0EXContacts calendarFormatToString:birthday.calendar.calendarIdentifier] forKey:@"format"];
  }
...
+ (nullable NSArray *)datesForContact:(CNContact * _Nonnull)person
{
  ...
  if (person.dates) {
    ...
    for (CNLabeledValue<NSDateComponents *> *container in person.dates) {
      ...
      if (val.calendar) {
        date[@"format"] = [ABI50_0_0EXContacts calendarFormatToString:val.calendar.calendarIdentifier];
      }
      if (container.label) {
...
{
  label = [ABI50_0_0EXContacts decodeLabel:label];
...
+ (nullable NSMutableArray<CNLabeledValue *> *)decodeDates:(nullable NSArray *)input
{
  ...
  NSMutableArray *output = [[NSMutableArray alloc] init];
  for (NSDictionary *item in input) {
    NSString *label = [ABI50_0_0EXContacts decodeDateLabel:item[@"label"]];
...
+ (nullable NSMutableArray<CNLabeledValue *> *)decodeUrlAddresses:(nullable NSArray *)input
{
  ...
  NSMutableArray *output = [[NSMutableArray alloc] init];
  for (NSDictionary *item in input) {
    NSString *label = [ABI50_0_0EXContacts decodeUrlAddressLabel:item[@"label"]];
    [output addObject:[[CNLabeledValue alloc]
                       initWithLabel:label
                       value:item[@"url"]]];
  }
...
+ (nullable NSMutableArray<CNLabeledValue *> *)decodeInstantMessageAddresses:(nullable NSArray *)input
{
  ...
  NSMutableArray *output = [[NSMutableArray alloc] init];
  for (NSDictionary *item in input) {
    NSString *label = [ABI50_0_0EXContacts decodeLabel:item[@"label"]];
...
+ (nullable NSMutableArray<CNLabeledValue *> *)decodeEmailAddresses:(nullable NSArray *)input
{
  ...
  NSMutableArray *output = [[NSMutableArray alloc] init];
  for (NSDictionary *item in input) {
    NSString *label = [ABI50_0_0EXContacts decodeEmailLabel:item[@"label"]];
...
+ (nullable NSMutableArray<CNLabeledValue *> *)decodePhoneNumbers:(NSArray *)input
{
  ...
  NSMutableArray *output = [[NSMutableArray alloc] init];
  for (NSDictionary *item in input) {
    NSString *label = [ABI50_0_0EXContacts decodePhoneLabel:item[@"label"]];
...
+ (NSDateComponents *)decodeBirthday:(NSDictionary *)input contact:(CNContact *)contact
{
  if (input) {
    ...
    if (input[@"format"]) {
      //      NSCalendar *adjustedCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:[ABI50_0_0EXContacts calendarFormatFromString:input[@"format"]]];
