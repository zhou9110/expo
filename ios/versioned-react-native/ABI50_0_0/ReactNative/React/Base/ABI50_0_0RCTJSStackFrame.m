...
#import "ABI50_0_0RCTJSStackFrame.h"
 ...
 */
static NSRegularExpression *ABI50_0_0RCTJSStackFrameRegex(void)
{
...
@implementation ABI50_0_0RCTJSStackFrame
...
{
  return @{
    @"methodName" : ABI50_0_0RCTNullIfNil(self.methodName),
    @"file" : ABI50_0_0RCTNullIfNil(self.file),
...
{
  NSTextCheckingResult *match = [ABI50_0_0RCTJSStackFrameRegex() firstMatchInString:line
                                                                   options:0
...
{
  return [[self alloc] initWithMethodName:ABI50_0_0RCTNilIfNull(dict[@"methodName"])
                                     file:dict[@"file"]
                               lineNumber:[ABI50_0_0RCTNilIfNull(dict[@"lineNumber"]) integerValue]
                                   column:[ABI50_0_0RCTNilIfNull(dict[@"column"]) integerValue]
                                 collapse:[ABI50_0_0RCTNilIfNull(dict[@"collapse"]) boolValue]];
}

+ (NSArray<ABI50_0_0RCTJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
    ABI50_0_0RCTJSStackFrame *frame = [self stackFrameWithLine:line];
    if (frame) {
...
+ (NSArray<ABI50_0_0RCTJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
  NSMutableArray *stack = [NSMutableArray new];
  for (NSDictionary *dict in dicts) {
    ABI50_0_0RCTJSStackFrame *frame = [self stackFrameWithDictionary:dict];
    if (frame) {
      [stack addObject:frame];
    }
  }
