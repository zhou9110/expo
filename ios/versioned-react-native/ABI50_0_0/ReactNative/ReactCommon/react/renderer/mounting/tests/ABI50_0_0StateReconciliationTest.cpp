...
#include <ABI50_0_0React/renderer/mounting/ABI50_0_0ShadowTree.h>
...
TEST_P(StateReconciliationTest, testStateReconciliation) {
  ...
  ABI50_0_0EXPECT_EQ(state1->getMostRecentState(), state1);
  ...
  ABI50_0_0EXPECT_EQ(state1->getMostRecentState(), state2);
  ABI50_0_0EXPECT_EQ(state2->getMostRecentState(), state2);
  ...
  ABI50_0_0EXPECT_EQ(findDescendantNode(shadowTree, family)->getState(), state3);

  ABI50_0_0EXPECT_EQ(state1->getMostRecentState(), state3);
  ABI50_0_0EXPECT_EQ(state2->getMostRecentState(), state3);
  ABI50_0_0EXPECT_EQ(state3->getMostRecentState(), state3);
  ...
  if (!GetParam()) {
    ...
    ABI50_0_0EXPECT_EQ(findDescendantNode(shadowTree, family)->getState(), state2);
  }
}
...
TEST_P(StateReconciliationTest, testCloneslessStateReconciliationDoesntClone) {
  ...
  ABI50_0_0EXPECT_EQ(findDescendantNode(*rootShadowNode1, family)->getState(), state1);
  ...
  ABI50_0_0EXPECT_EQ(findDescendantNode(*rootShadowNode2, family)->getState(), state2);
  ABI50_0_0EXPECT_EQ(state1->getMostRecentState(), state1);
  ...
  auto rootShadowNodeClonedFromABI50_0_0React =
