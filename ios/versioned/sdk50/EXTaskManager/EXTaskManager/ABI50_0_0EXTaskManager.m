...
NSString * const ABI50_0_0EXTaskManagerEventName = @"TaskManager.executeTask";

@interface ABI50_0_0EXTaskManager ()
...
@property (nonatomic, weak) id<ABI50_0_0EXConstantsInterface> constantsService;
@property (nonatomic, weak) id<ABI50_0_0EXTaskServiceInterface> taskService;

@end

@implementation ABI50_0_0EXTaskManager

ABI50_0_0EX_EXPORT_MODULE(ExpoTaskManager);

+ (const NSArray<Protocol *> *)exportedInterfaces
{
  return @[@protocol(ABI50_0_0EXTaskManagerInterface)];
}
...
// TODO: Remove when adding bare ABI50_0_0React Native support
...
- (void)setModuleRegistry:(ABI50_0_0EXModuleRegistry *)moduleRegistry
{
  ...
  _constantsService = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI50_0_0EXConstantsInterface)];
...
{
  return @{
           @"EVENT_NAME": ABI50_0_0EXTaskManagerEventName,
           };
}
...
{
  return @[ABI50_0_0EXTaskManagerEventName];
}
...
- (void)startObserving
{
  if (_eventsQueue && _eventsQueue.count > 0) {
    ...
    for (NSDictionary *eventBody in _eventsQueue) {
      [_eventEmitter sendEventWithName:ABI50_0_0EXTaskManagerEventName body:eventBody];
    }
  }
  _eventsQueue = nil;
}
...
ABI50_0_0EX_EXPORT_METHOD_AS(isAvailableAsync,
                    isAvailable:(ABI50_0_0EXPromiseResolveBlock)resolve
                    rejecter:(ABI50_0_0EXPromiseRejectBlock)reject)
{
  resolve(@(_taskService != nil));
}

ABI50_0_0EX_EXPORT_METHOD_AS(notifyTaskFinishedAsync,
...
ABI50_0_0EX_EXPORT_METHOD_AS(isTaskRegisteredAsync,
...
ABI50_0_0EX_EXPORT_METHOD_AS(getRegisteredTasksAsync,
                    getRegisteredTasks:(ABI50_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI50_0_0EXPromiseRejectBlock)reject)
{
  resolve([_taskService getRegisteredTasksForAppId:_appId]);
}

ABI50_0_0EX_EXPORT_METHOD_AS(getTaskOptionsAsync,
                    getConfigurationForTaskName:(nonnull NSString *)taskName
                    resolve:(ABI50_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI50_0_0EXPromiseRejectBlock)reject)
{
  resolve(ABI50_0_0EXNullIfNil([_taskService getOptionsForTaskName:taskName forAppId:_appId]));
}
...
ABI50_0_0EX_EXPORT_METHOD_AS(unregisterAllTasksAsync,
                    unregisterAllTasks:(ABI50_0_0EXPromiseResolveBlock)resolve
                    reject:(ABI50_0_0EXPromiseRejectBlock)reject)
{
...
# pragma mark - ABI50_0_0EXTaskManagerInterface
...
{
  return [ABI50_0_0EXTaskService hasBackgroundModeEnabled:backgroundMode];
}
...
- (void)executeWithBody:(NSDictionary *)body
{
  if (!_eventsQueue) {
    ...
    [_eventEmitter sendEventWithName:ABI50_0_0EXTaskManagerEventName body:body];
  } else {
