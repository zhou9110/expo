...
@interface ABI50_0_0EXLegacyRemoteNotificationPermissionRequester ()
...
@property (nonatomic, weak) id<ABI50_0_0EXPermissionsRequester> userNotificationPermissionRequester;
@property (nonatomic, weak) dispatch_queue_t methodQueue;
@property (nonatomic, weak) id<ABI50_0_0EXRemoteNotificationPermissionProgressPublisher> permissionProgressPublisher;

@end

@implementation ABI50_0_0EXLegacyRemoteNotificationPermissionRequester
...
- (instancetype)initWithUserNotificationPermissionRequester:(id<ABI50_0_0EXPermissionsRequester>)userNotificationPermissionRequester
                                        permissionPublisher:(id<ABI50_0_0EXRemoteNotificationPermissionProgressPublisher>)permissionProgressPublisher
...
{
  __block ABI50_0_0EXPermissionStatus status;
  [ABI50_0_0EXUtilities performSynchronouslyOnMainThread:^{
    status = (ABI50_0_0UMSharedApplication().isRegisteredForRemoteNotifications) ?
...
- (void)requestPermissionsWithResolver:(ABI50_0_0EXPromiseResolveBlock)resolve rejecter:(ABI50_0_0EXPromiseRejectBlock)reject
{
  ...
  [ABI50_0_0EXUtilities performSynchronouslyOnMainThread:^{
    isRegisteredForRemoteNotifications = ABI50_0_0UMSharedApplication().isRegisteredForRemoteNotifications;
  }];
  ...
  if (isRegisteredForRemoteNotifications) {
    ...
    [_userNotificationPermissionRequester requestPermissionsWithResolver:^(NSDictionary *permission){
      ...
      ABI50_0_0EXPermissionStatus localNotificationsStatus = [[permission objectForKey:@"status"] intValue];
      // We may assume that `ABI50_0_0EXLocalNotificationRequester`'s permission request will always finish
      ...
      if (localNotificationsStatus == ABI50_0_0EXPermissionStatusDenied) {
        [self _clearObserver];
        [self _maybeConsumeResolverWithCurrentPermissions];
      } else {
        self.remoteNotificationsRegistrationIsPending = YES;
        dispatch_async(dispatch_get_main_queue(), ^{
          [ABI50_0_0UMSharedApplication() registerForRemoteNotifications];
        });
      }
