...
#import "ABI50_0_0EXNativeModuleIntrospection.h"
...
@implementation ABI50_0_0EXNativeModuleIntrospection

@synthesize bridge = _bridge;

ABI50_0_0RCT_EXPORT_MODULE(ExpoNativeModuleIntrospection)

ABI50_0_0RCT_REMAP_METHOD(getNativeModuleNamesAsync,
                 nativeModuleNamesWithResolver:(ABI50_0_0RCTPromiseResolveBlock)resolve
                 rejecter:(ABI50_0_0RCTPromiseRejectBlock)reject) {
  NSError *error;
  NSDictionary<NSString *, ABI50_0_0RCTModuleData *> *moduleDataByName = [self _moduleDataFromBridge:&error];
  if (!moduleDataByName) {
...
ABI50_0_0RCT_REMAP_METHOD(introspectNativeModuleAsync,
  ...
  ABI50_0_0RCTModuleData *moduleData = moduleDataByName[name];
  if (!moduleData) {
    resolve(nil);
    return;
  }
  
  NSArray<id<ABI50_0_0RCTBridgeMethod>> *moduleMethods = moduleData.methods;
  ...
  for (id<ABI50_0_0RCTBridgeMethod> method in moduleMethods) {
    NSString *methodName = [NSString stringWithCString:method.JSMethodName encoding:NSASCIIStringEncoding];
    NSString *functionType = [NSString stringWithCString:ABI50_0_0RCTFunctionDescriptorFromType(method.functionType)
...
- (NSDictionary<NSString *, ABI50_0_0RCTModuleData *> *)_moduleDataFromBridge:(NSError **)error {
  ABI50_0_0RCTBridge *bridge = _bridge;
  if (![NSStringFromClass(bridge.class) isEqualToString:@"ABI50_0_0RCTCxxBridge"]) {
    if (error) {
  ...
  NSDictionary<NSString *, ABI50_0_0RCTModuleData *> *moduleDataByName;
  @try {
