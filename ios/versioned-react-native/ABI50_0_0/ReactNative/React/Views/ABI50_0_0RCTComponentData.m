...
#import "ABI50_0_0RCTComponentEvent.h"
...
typedef void (^ABI50_0_0RCTPropBlock)(id<ABI50_0_0RCTComponent> view, id json);
typedef NSMutableDictionary<NSString *, ABI50_0_0RCTPropBlock> ABI50_0_0RCTPropBlockDictionary;
...
@implementation ABI50_0_0RCTComponentData {
  id<ABI50_0_0RCTComponent> _defaultView; // Only needed for ABI50_0_0RCT_CUSTOM_VIEW_PROPERTY
  ABI50_0_0RCTPropBlockDictionary *_viewPropBlocks;
  ABI50_0_0RCTPropBlockDictionary *_shadowPropBlocks;
  __weak ABI50_0_0RCTBridge *_bridge;
  __weak id<ABI50_0_0RCTEventDispatcherProtocol> _eventDispatcher;
}
...
- (instancetype)initWithManagerClass:(Class)managerClass
                     ...
                     eventDispatcher:(id<ABI50_0_0RCTEventDispatcherProtocol>)eventDispatcher
{
  if ((self = [super init])) {
    _bridge = bridge;
    ...
    _name = ABI50_0_0RCTViewManagerModuleNameForClass(managerClass);
  }
  return self;
}

- (ABI50_0_0RCTViewManager *)manager
{
  if (!_manager && _bridge) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:ABI50_0_0RCTDidInitializeModuleNotification
                                                        object:nil
...
{
  ABI50_0_0RCTShadowView *shadowView = [self.manager shadowView];
  shadowView.ABI50_0_0ReactTag = tag;
  shadowView.viewName = _name;
  return shadowView;
}

- (void)callCustomSetter:(SEL)setter onView:(id<ABI50_0_0RCTComponent>)view withProp:(id)json isShadowView:(BOOL)isShadowView
{
  json = ABI50_0_0RCTNilIfNull(json);
  if (!isShadowView) {
...
static ABI50_0_0RCTPropBlock createEventSetter(
    NSString *propName,
    SEL setter,
    InterceptorBlock eventInterceptor,
    id<ABI50_0_0RCTEventDispatcherProtocol> eventDispatcher)
{
  __weak id<ABI50_0_0RCTEventDispatcherProtocol> weakEventDispatcher = eventDispatcher;
  return ^(id target, id json) {
    void (^eventHandler)(NSDictionary *event) = nil;
    if ([ABI50_0_0RCTConvert BOOL:json]) {
      __weak id<ABI50_0_0RCTComponent> weakTarget = target;
      eventHandler = ^(NSDictionary *event) {
        // The component no longer exists, we shouldn't send the event
        id<ABI50_0_0RCTComponent> strongTarget = weakTarget;
        if (!strongTarget) {
          return;
        }

        if (eventInterceptor) {
          eventInterceptor(propName, event, strongTarget.ABI50_0_0ReactTag);
        } else {
          ABI50_0_0RCTComponentEvent *componentEvent = [[ABI50_0_0RCTComponentEvent alloc] initWithName:propName
                                                                              viewTag:strongTarget.ABI50_0_0ReactTag
                                                                                 body:event];
...
static ABI50_0_0RCTPropBlock createNSInvocationSetter(NSMethodSignature *typeSignature, SEL type, SEL getter, SEL setter)
{
...
- (ABI50_0_0RCTPropBlock)createPropBlock:(NSString *)name isShadowView:(BOOL)isShadowView
{
  // Get type
  SEL type = NULL;
  ...
  if ([keyPath isEqualToString:@"__custom__"]) {
    ...
    __weak ABI50_0_0RCTComponentData *weakSelf = self;
    return ^(id<ABI50_0_0RCTComponent> view, id json) {
    ...
    if (type == NSSelectorFromString(@"ABI50_0_0RCTBubblingEventBlock:") ||
        type == NSSelectorFromString(@"ABI50_0_0RCTDirectEventBlock:") ||
        type == NSSelectorFromString(@"ABI50_0_0RCTCapturingEventBlock:")) {
      // Special case for event handlers
      setterBlock =
          createEventSetter(name, setter, self.eventInterceptor, _bridge ? _bridge.eventDispatcher : _eventDispatcher);
    } else {
      // Ordinary property handlers
      NSMethodSignature *typeSignature = [[ABI50_0_0RCTConvert class] methodSignatureForSelector:type];
      if (!typeSignature) {
        ABI50_0_0RCTLogError(@"No +[ABI50_0_0RCTConvert %@] function found.", NSStringFromSelector(type));
        return ^(__unused id<ABI50_0_0RCTComponent> view, __unused id json) {
        };
      }
      switch (typeSignature.methodReturnType[0]) {
#define ABI50_0_0RCT_CASE(_value, _type)                                       \
  case _value: {                                                      \
    ...
    setterBlock = ^(id target, id json) {                             \
      if (json) {                                                     \
        ...
        set(target, setter, convert([ABI50_0_0RCTConvert class], type, json)); \
    ...
    break;                                                            \
        ...
        ABI50_0_0RCT_CASE(_C_SEL, SEL)
        ABI50_0_0RCT_CASE(_C_CHARPTR, const char *)
        ABI50_0_0RCT_CASE(_C_CHR, char)
        ABI50_0_0RCT_CASE(_C_UCHR, unsigned char)
        ABI50_0_0RCT_CASE(_C_SHT, short)
        ABI50_0_0RCT_CASE(_C_USHT, unsigned short)
        ABI50_0_0RCT_CASE(_C_INT, int)
        ABI50_0_0RCT_CASE(_C_UINT, unsigned int)
        ABI50_0_0RCT_CASE(_C_LNG, long)
        ABI50_0_0RCT_CASE(_C_ULNG, unsigned long)
        ABI50_0_0RCT_CASE(_C_LNG_LNG, long long)
        ABI50_0_0RCT_CASE(_C_ULNG_LNG, unsigned long long)
        ABI50_0_0RCT_CASE(_C_FLT, float)
        ABI50_0_0RCT_CASE(_C_DBL, double)
        ABI50_0_0RCT_CASE(_C_BOOL, BOOL)
        ABI50_0_0RCT_CASE(_C_PTR, void *)
        ABI50_0_0RCT_CASE(_C_ID, id)
    ...
    return ^(__unused id view, __unused id json) {
      ...
      setterBlock(target, ABI50_0_0RCTNilIfNull(json));
    };
  }
}

- (ABI50_0_0RCTPropBlock)propBlockForKey:(NSString *)name isShadowView:(BOOL)isShadowView
{
  ABI50_0_0RCTPropBlockDictionary *propBlocks = isShadowView ? _shadowPropBlocks : _viewPropBlocks;
  ABI50_0_0RCTPropBlock propBlock = propBlocks[name];
  if (!propBlock) {
    ...
    ABI50_0_0RCTPropBlock unwrappedBlock = propBlock;
    __weak __typeof(self) weakSelf = self;
    propBlock = ^(id<ABI50_0_0RCTComponent> view, id json) {
      NSString *logPrefix = [NSString
          stringWithFormat:@"Error setting property '%@' of %@ with tag #%@: ", name, weakSelf.name, view.ABI50_0_0ReactTag];
      ABI50_0_0RCTPerformBlockWithLogPrefix(
          ^{
                                                         ...
                                                         methodCount:(unsigned int)methodCount
{
  ...
  for (int i = 0; i < methodCount; i++) {
    ...
    ABI50_0_0RCTMethodInfo *methodInfo = ((ABI50_0_0RCTMethodInfo * (*)(id, SEL)) objc_msgSend)(managerClass, selector);
    ABI50_0_0RCTModuleMethod *moduleMethod = [[ABI50_0_0RCTModuleMethod alloc] initWithExportedMethod:methodInfo
                                                                        moduleClass:managerClass];
    NSString *methodName = @(moduleMethod.JSMethodName);
    commands[methodName] = @(commandCount);
    commandCount += 1;
  }
  // View manager do not export getConstants with ABI50_0_0RCT_EXPORT_METHOD, so we inject it into "Commands" manually.
  if (commandCount > 0) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  if (ABI50_0_0RCTClassOverridesInstanceMethod(managerClass, @selector(customBubblingEventTypes))) {
    NSArray<NSString *> *events = [[managerClass new] customBubblingEventTypes];
    for (NSString *event in events) {
      [bubblingEvents addObject:ABI50_0_0RCTNormalizeInputEventName(event)];
    }
  }
#pragma clang diagnostic pop
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (ABI50_0_0RCT_DEBUG && propTypes[name] && ![propTypes[name] isEqualToString:type]) {
      ABI50_0_0RCTLogError(
          @"Property '%@' of component '%@' redefined from '%@' "
           "to '%@'",
          name,
          ABI50_0_0RCTViewManagerModuleNameForClass(managerClass),
          propTypes[name],
          type);
    }

    if ([type isEqualToString:@"ABI50_0_0RCTBubblingEventBlock"]) {
      [bubblingEvents addObject:ABI50_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI50_0_0RCTCapturingEventBlock"]) {
      [capturingEvents addObject:ABI50_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else if ([type isEqualToString:@"ABI50_0_0RCTDirectEventBlock"]) {
      [directEvents addObject:ABI50_0_0RCTNormalizeInputEventName(name)];
      propTypes[name] = @"BOOL";
    } else {
      propTypes[name] = type;
    }
  ...
  NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithDictionary:@{
    ...
    @"baseModuleName" : superClass == [NSObject class] ? (id)kCFNull : ABI50_0_0RCTViewManagerModuleNameForClass(superClass),
  }];
...
- (NSDictionary<NSString *, id> *)viewConfig
{
  ...
  [self manager];
  return [ABI50_0_0RCTComponentData viewConfigForViewMangerClass:_managerClass];
}

NSString *ABI50_0_0RCTViewManagerModuleNameForClass(Class managerClass)
{
  // Hackety hack, this partially re-implements ABI50_0_0RCTBridgeModuleNameForClass
  // We want to get rid of ABI50_0_0RCT and RK prefixes, but a lot of JS code still references
  // view names by prefix. So, while ABI50_0_0RCTBridgeModuleNameForClass now drops these
  ...
  name = ABI50_0_0EX_REMOVE_VERSION(name);
  ...
  ABI50_0_0RCTAssert(name.length, @"Invalid moduleName '%@'", name);
