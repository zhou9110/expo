 ...
 */
#import "ABI50_0_0RNSVGTSpan.h"
#import "ABI50_0_0RNSVGFontData.h"
...
#import "ABI50_0_0RNSVGTopAlignedLabel.h"
#import "ABI50_0_0RNSVGUIKit.h"

static NSCharacterSet *ABI50_0_0RNSVGTSpan_separators = nil;
static CGFloat ABI50_0_0RNSVGTSpan_radToDeg = 180 / (CGFloat)M_PI;
...
@implementation ABI50_0_0RNSVGTSpan {
  CGFloat startOffset;
  ABI50_0_0RNSVGTextPath *textPath;
  ...
  CGFloat firstX;
  CGFloat firstY;
  ABI50_0_0RNSVGPathMeasure *measure;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const ABI50_0_0RNSVGTSpanProps>();
    _props = defaultProps;
  }
  return self;
}
...
{
  return concreteComponentDescriptorProvider<ABI50_0_0RNSVGTSpanComponentDescriptor>();
}

- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  const auto &newProps = static_cast<const ABI50_0_0RNSVGTSpanProps &>(*props);

  self.content = ABI50_0_0RCTNSStringFromStringNilIfEmpty(newProps.content);

  setCommonTextProps(newProps, self);
  _props = std::static_pointer_cast<ABI50_0_0RNSVGTSpanProps const>(props);
}
...
- (void)prepareForRecycle
{
  ...
  measure = [[ABI50_0_0RNSVGPathMeasure alloc] init];
  ABI50_0_0RNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
}
#endif // ABI50_0_0RCT_NEW_ARCH_ENABLED
...
- (id)init
{
  ...
  if (ABI50_0_0RNSVGTSpan_separators == nil) {
    ABI50_0_0RNSVGTSpan_separators = [NSCharacterSet whitespaceCharacterSet];
  }
...
{
  if (self.content) {
    ABI50_0_0RNSVGGlyphContext *gc = [self.textRoot getGlyphContext];
    if (self.inlineSize != nil && self.inlineSize.value != 0) {
      if (self.fill) {
        if (self.fill.class == ABI50_0_0RNSVGBrush.class) {
      ...
      if (self.stroke) {
        if (self.stroke.class == ABI50_0_0RNSVGBrush.class) {
          CGColorRef color = [self.tintColor CGColor];
          [self drawWrappedText:context gc:gc rect:rect color:color];
        } else {
      ...
      if (self.path) {
        ...
        for (NSUInteger i = 0; i < count; i++) {
          ABI50_0_0RNSVGPlatformView *label = [emoji objectAtIndex:i];
...
- (NSMutableDictionary *)getAttributes:(ABI50_0_0RNSVGFontData *)fontdata
{
  ...
  bool allowOptionalLigatures = letterSpacing == 0 && fontdata->fontVariantLigatures == ABI50_0_0RNSVGFontVariantLigaturesNormal;
...
static ABI50_0_0RNSVGTopAlignedLabel *label;
- (void)drawWrappedText:(CGContextRef)context gc:(ABI50_0_0RNSVGGlyphContext *)gc rect:(CGRect)rect color:(CGColorRef)color
{
  ...
  ABI50_0_0RNSVGFontData *fontdata = [gc getFont];
  ...
  enum ABI50_0_0RNSVGTextAnchor textAnchor = fontdata->textAnchor;
  NSTextAlignment align;
  switch (textAnchor) {
    case ABI50_0_0RNSVGTextAnchorStart:
      align = NSTextAlignmentLeft;
      break;

    case ABI50_0_0RNSVGTextAnchorMiddle:
      align = NSTextAlignmentCenter;
      break;

    case ABI50_0_0RNSVGTextAnchorEnd:
  ...
  if (!label) {
    label = [[ABI50_0_0RNSVGTopAlignedLabel alloc] init];
  }
  label.attributedText = (__bridge NSAttributedString *_Nullable)(attrString);
  label.lineBreakMode = NSLineBreakByWordWrapping;
  label.backgroundColor = ABI50_0_0RNSVGColor.clearColor;
...
#endif // TARGET_OS_OSX
  label.font = font;
  label.textColor = [ABI50_0_0RNSVGColor colorWithCGColor:color];

  CGFloat fontSize = [gc getFontSize];
  CGFloat height = CGRectGetHeight(rect);
  CGFloat width = [ABI50_0_0RNSVGPropHelper fromRelative:self.inlineSize relative:[gc getWidth] fontSize:fontSize];
...
- (CGFloat)getSubtreeTextChunksTotalAdvance
{
  ...
  if (!str) {
    for (ABI50_0_0RNSVGView *node in self.subviews) {
      if ([node isKindOfClass:[ABI50_0_0RNSVGText class]]) {
        ABI50_0_0RNSVGText *text = (ABI50_0_0RNSVGText *)node;
...
- (CGPathRef)getLinePath:(NSString *)str context:(CGContextRef)context
{
  ...
  ABI50_0_0RNSVGGlyphContext *gc = [self.textRoot getGlyphContext];
  ABI50_0_0RNSVGFontData *font = [gc getFont];
   ...
   */
  bool allowOptionalLigatures = letterSpacing == 0 && font->fontVariantLigatures == ABI50_0_0RNSVGFontVariantLigaturesNormal;
   ...
   */
  enum ABI50_0_0RNSVGTextAnchor textAnchor = font->textAnchor;
  ABI50_0_0RNSVGText *anchorRoot = [self getTextAnchorRoot];
  CGFloat textMeasure = [anchorRoot getSubtreeTextChunksTotalAdvance];
  CGFloat offset = [ABI50_0_0RNSVGTSpan getTextAnchorOffset:textAnchor width:textMeasure];
  ...
  if (hasTextPath) {
    // sharpMidLine = ABI50_0_0RNSVGTextPathMidLineFromString([textPath midLine]) == ABI50_0_0RNSVGTextPathMidLineSharp;
    /*
     Name
     side
     Value
     ...
     */
    side = ABI50_0_0RNSVGTextPathSideFromString([textPath side]) == ABI50_0_0RNSVGTextPathSideRight ? -1 : 1;
    /*
     Name
     startOffset
     Value
     ...
     the path is reached.
     */
    CGFloat absoluteStartOffset = [ABI50_0_0RNSVGPropHelper fromRelative:textPath.startOffset
                                                       relative:measure.pathLength
                                                       fontSize:fontSize];
    offset += absoluteStartOffset;
    if (measure.isClosed) {
      CGFloat halfPathDistance = measure.pathLength / 2;
      startOfRendering = absoluteStartOffset + (textAnchor == ABI50_0_0RNSVGTextAnchorMiddle ? -halfPathDistance : 0);
      endOfRendering = startOfRendering + measure.pathLength;
    }
    /*
     ABI50_0_0RNSVGTextPathSpacing spacing = textPath.getSpacing();
     if (spacing == ABI50_0_0RNSVGTextPathSpacing.auto) {
     // Hmm, what to do here?
  ...
  ABI50_0_0RNSVGLength *mTextLength = [self textLength];
  enum ABI50_0_0RNSVGTextLengthAdjust mLengthAdjust = ABI50_0_0RNSVGTextLengthAdjustFromString([self lengthAdjust]);
  if (mTextLength != nil) {
    CGFloat author = [ABI50_0_0RNSVGPropHelper fromRelative:mTextLength relative:[gc getWidth] fontSize:fontSize];
    if (author < 0) {
    ...
    switch (mLengthAdjust) {
      default:
      case ABI50_0_0RNSVGTextLengthAdjustSpacing:
        // TODO account for ligatures
        letterSpacing += (author - textMeasure) / (n - 1);
        break;
      case ABI50_0_0RNSVGTextLengthAdjustSpacingAndGlyphs:
  ...
  */
  ...
  enum ABI50_0_0RNSVGAlignmentBaseline baseline = ABI50_0_0RNSVGAlignmentBaselineFromString(self.alignmentBaseline);
  if (baseline != ABI50_0_0RNSVGAlignmentBaselineBaseline) {
    ...
    switch (baseline) {
      ...
      default:
      case ABI50_0_0RNSVGAlignmentBaselineBaseline:
        // Use the dominant baseline choice of the parent.
        // Match the box’s corresponding baseline to that of its parent.
        baselineShift = 0;
        break;

      case ABI50_0_0RNSVGAlignmentBaselineTextBottom:
      case ABI50_0_0RNSVGAlignmentBaselineAfterEdge:
      case ABI50_0_0RNSVGAlignmentBaselineTextAfterEdge:
      ...
      case ABI50_0_0RNSVGAlignmentBaselineCentral:
      ...
      case ABI50_0_0RNSVGAlignmentBaselineMathematical:
      ...
      case ABI50_0_0RNSVGAlignmentBaselineHanging:
        baselineShift = (CGFloat)0.8 * ascenderHeight;
        break;

      case ABI50_0_0RNSVGAlignmentBaselineTextTop:
      case ABI50_0_0RNSVGAlignmentBaselineBeforeEdge:
      case ABI50_0_0RNSVGAlignmentBaselineTextBeforeEdge:
      ...
      case ABI50_0_0RNSVGAlignmentBaselineBottom:
        // Align the top of the aligned subtree with the top of the line box.
        baselineShift = bottom;
        break;

      case ABI50_0_0RNSVGAlignmentBaselineCenter:
        // Align the center of the aligned subtree with the center of the line box.
        baselineShift = totalHeight / 2;
        break;

      case ABI50_0_0RNSVGAlignmentBaselineTop:
    ...
    switch (baseline) {
      ...
      default:
        if (fontData != nil && [baselineShiftString isEqualToString:@"sub"]) {
          ...
          baselineShift -= [ABI50_0_0RNSVGPropHelper fromRelativeWithNSString:baselineShiftString
                                                            relative:fontSize
                                                            fontSize:fontSize];
        }
        break;
  ...
  for (CFIndex ri = 0; ri < runEnd; ri++) {
    ...
    for (CFIndex g = 0; g < runGlyphCount; g++) {
      ...
      bool isWordSeparator = [ABI50_0_0RNSVGTSpan_separators characterIsMember:currentChar];
      ...
      CGFloat r = [gc nextRotation] / ABI50_0_0RNSVGTSpan_radToDeg;
      ...
      if (width == 0) { // Render unicode emoji
        ABI50_0_0RNSVGTextView *label = [[ABI50_0_0RNSVGTextView alloc] init];
...
+ (CGFloat)getTextAnchorOffset:(ABI50_0_0RNSVGTextAnchor)textAnchor width:(CGFloat)width
{
  switch (textAnchor) {
    case ABI50_0_0RNSVGTextAnchorStart:
      return 0;
...
{
  textPath = nil;
  ABI50_0_0RNSVGText *parent = (ABI50_0_0RNSVGText *)[self superview];
  CGPathRef path = nil;
  while (parent) {
    if ([parent class] == [ABI50_0_0RNSVGTextPath class]) {
      textPath = (ABI50_0_0RNSVGTextPath *)parent;
      ABI50_0_0RNSVGNode *definedTemplate = [self.svgView getDefinedTemplate:textPath.href];
      path = [definedTemplate getPath:context];
      [measure extractPathData:path];
      break;
    } else if (![parent isKindOfClass:[ABI50_0_0RNSVGText class]]) {
      break;
    }
    parent = (ABI50_0_0RNSVGText *)[parent superview];
  }
  if (!path) {
    [measure reset];
  }
...
#ifdef ABI50_0_0RCT_NEW_ARCH_ENABLED
Class<ABI50_0_0RCTComponentViewProtocol> ABI50_0_0RNSVGTSpanCls(void)
{
  return ABI50_0_0RNSVGTSpan.class;
}
#endif // ABI50_0_0RCT_NEW_ARCH_ENABLED
